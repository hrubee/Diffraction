...
#
# Run the primary Diffract install flow on a local machine, capture logs,
...
LOG_DIR="${TMPDIR:-/tmp}/diffract-smoke"
RUNTIME=""
...
Options:
  ...
  --allow-existing-state      Allow running even if Diffract/Diffract state already exists
  --keep-logs                 Preserve log files after success
  --remove-diffract          Allow uninstall.sh to remove diffract
...
ensure_clean_start() {
  ...
  if [ -d "$HOME/.diffract" ] || [ -d "$HOME/.config/diffract" ] || [ -d "$HOME/.config/diffract" ]; then
    fail "Existing Diffract/Diffract state detected. Re-run with --allow-existing-state if you really want to test on this machine."
  fi

  if command -v diffract > /dev/null 2>&1; then
    if diffract sandbox list 2>/dev/null | grep -Eq '[[:alnum:]]'; then
      fail "Existing Diffract sandboxes detected. Re-run with --allow-existing-state only if you are prepared for uninstall.sh to remove them."
    fi
  fi
}
...
feed_install_answers() {
  ...
  (
    ...
    while :; do
      if [ -f "$install_log" ] && grep -q "Diffract gateway launched inside sandbox" "$install_log"; then
        break
      fi
      sleep 1
    done
...
verify_cleanup() {
  ...
  if [ -d "$HOME/.diffract" ] || [ -d "$HOME/.config/diffract" ]; then
    warn "Diffract state directories still exist under HOME."
    leftovers=1
  fi

  if command -v diffract > /dev/null 2>&1; then
    local sandbox_output
    sandbox_output="$(diffract sandbox list 2>/dev/null || true)"
    if printf '%s' "$sandbox_output" | grep -Eq '[[:alnum:]]'; then
      warn "Diffract still reports sandbox entries after uninstall."
      leftovers=1
    fi
  fi
  ...
  if command -v docker > /dev/null 2>&1 && docker info > /dev/null 2>&1; then
    ...
    related_containers="$(
      docker ps -a --format '{{.Image}} {{.Names}}' 2>/dev/null \
        | awk 'BEGIN { IGNORECASE=1 } /diffract-cluster|diffract|diffract|diffract/ { print }'
    )"
...
main() {
  ...
  ANSWERS_PIPE="$(mktemp -u "${TMPDIR:-/tmp}/diffract-smoke-answers-XXXXXX")"
  mkfifo "$ANSWERS_PIPE"
  trap cleanup EXIT
