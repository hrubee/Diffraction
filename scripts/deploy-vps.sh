...
DIFFRACT_SKIP_AUTH="${DIFFRACT_SKIP_AUTH:-1}"

export REPO_DIR DIFFRACT_DOMAIN DIFFRACT_UI_INSTALL_DIR DIFFRACT_BASE_PATH DIFFRACT_SKIP_AUTH
...
info "Building From Scrtch UI (basePath=${DIFFRACT_BASE_PATH}, skip_auth=${DIFFRACT_SKIP_AUTH})..."
(cd "$DIFFRACT_UI_INSTALL_DIR" && \
  NEXT_PUBLIC_BASE_PATH="$DIFFRACT_BASE_PATH" \
  NEXT_PUBLIC_DIFFRACT_SKIP_AUTH="$DIFFRACT_SKIP_AUTH" \
