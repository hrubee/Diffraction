...
kubectl -n diffract get statefulset/diffract >/dev/null 2>&1 || exit 1
kubectl -n diffract wait --for=jsonpath='{.status.readyReplicas}'=1 statefulset/diffract --timeout=1s >/dev/null 2>&1 || exit 1

# ---------------------------------------------------------------------------
# Verify the sandbox supervisor binary exists on the node filesystem.
# Sandbox pods mount /opt/diffract/bin as a read-only hostPath volume and
# exec /opt/diffract/bin/diffract-sandbox as their entrypoint. If the binary
...
# ---------------------------------------------------------------------------
if [ ! -x /opt/diffract/bin/diffract-sandbox ]; then
    echo "HEALTHCHECK_MISSING_SUPERVISOR: /opt/diffract/bin/diffract-sandbox not found" >&2
    exit 1
fi

# Verify TLS secrets exist (created by diffract-bootstrap before the StatefulSet starts)
# Skip when TLS is disabled — secrets are not required.
if [ "${DISABLE_TLS:-}" != "true" ]; then
    kubectl -n diffract get secret diffract-server-tls >/dev/null 2>&1 || exit 1
    kubectl -n diffract get secret diffract-client-tls >/dev/null 2>&1 || exit 1
fi
