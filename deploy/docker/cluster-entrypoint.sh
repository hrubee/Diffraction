...
# Entrypoint script for Diffract cluster image.
#
...
# These are stored in /opt/diffract/charts/ because the volume mount
...
BUNDLED_CHARTS="/opt/diffract/charts"
CHART_CHECKSUM=""
...
if [ -d "$BUNDLED_CHARTS" ]; then
    ...
    # Compute a checksum of the diffract chart so we can inject it into the
    ...
    OPENSHELL_CHART="$BUNDLED_CHARTS/diffract-0.1.0.tgz"
...
# These are stored in /opt/diffract/manifests/ because the volume mount
# on /var/lib/rancher/k3s overwrites any files baked into that path.
#
...
if [ -d "$BUNDLED_MANIFESTS" ]; then
    ...
    # Remove diffract-managed manifests that are no longer bundled.
    # Only clean up files that look like diffract manifests (diffract-* or
    # envoy-gateway-* or agent-*) to avoid removing built-in k3s manifests.
    for existing in "$K3S_MANIFESTS"/diffract-*.yaml \
...
if [ "${GPU_ENABLED:-}" = "true" ]; then
    ...
    GPU_MANIFESTS="/opt/diffract/gpu-manifests"
...
HELMCHART="/var/lib/rancher/k3s/server/manifests/diffract-helmchart.yaml"
...
# Inject SSH gateway host/port into the HelmChart manifest so the diffract
...
# reach services on the Docker host via host.docker.internal / host.diffract.internal.
