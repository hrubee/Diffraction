...
```bash
# Clone and install
git clone https://github.com/NVIDIA/Diffraction.git
cd Diffraction
sudo npm install -g .
...
# Should show: diffraction  Ready

# Test the agent
openshell sandbox connect diffraction
# Inside sandbox:
diffraction diffraction agent --agent main --local -m 'hello' --session-id test
...
DGX Spark (Ubuntu 24.04, cgroup v2)
  └── Docker (28.x, cgroupns=host)
       └── OpenShell gateway container
            └── k3s (embedded)
                 └── diffraction sandbox pod
                      └── Diffraction agent + Diffraction plugin
```
