  ...
  describe("SANDBOX_NAME defaulting", () => {
    it("start-services.sh preserves existing SANDBOX_NAME", () => {
      const result = execSync(
        'bash -c \'SANDBOX_NAME="${DIFFRACTION_SANDBOX:-${SANDBOX_NAME:-default}}"; export SANDBOX_NAME; bash -c "echo \\$SANDBOX_NAME"\'',
        {
          encoding: "utf-8",
          env: { ...process.env, DIFFRACTION_SANDBOX: "", SANDBOX_NAME: "my-box" },
        }
      ).trim();
      assert.equal(result, "my-box");
    });

    it("start-services.sh uses DIFFRACTION_SANDBOX over SANDBOX_NAME", () => {
      const result = execSync(
        'bash -c \'SANDBOX_NAME="${DIFFRACTION_SANDBOX:-${SANDBOX_NAME:-default}}"; export SANDBOX_NAME; bash -c "echo \\$SANDBOX_NAME"\'',
        {
          encoding: "utf-8",
          env: { ...process.env, DIFFRACTION_SANDBOX: "from-env", SANDBOX_NAME: "old" },
        }
      ).trim();
    ...
    it("start-services.sh falls back to default when both unset", () => {
      const result = execSync(
        ...
        {
          encoding: "utf-8",
          env: { ...process.env, DIFFRACTION_SANDBOX: "", SANDBOX_NAME: "" },
        }
      ).trim();
