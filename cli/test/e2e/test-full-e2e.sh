...
# Node.js, openshell, Diffraction, and onboard setup automatically.
#
# Prerequisites:
#   - Docker running
...
#   DIFFRACTION_NON_INTERACTIVE=1   — required (enables non-interactive install + onboard)
#   DIFFRACTION_SANDBOX_NAME        — sandbox name (default: e2e-nightly)
#   DIFFRACTION_RECREATE_SANDBOX=1  — recreate sandbox if it exists from a previous run
#   NVIDIA_API_KEY               — required for NVIDIA Cloud API inference
#
# Usage:
#   DIFFRACTION_NON_INTERACTIVE=1 NVIDIA_API_KEY=nvapi-... bash test/e2e/test-full-e2e.sh
#
# See: https://github.com/NVIDIA/Diffraction/issues/71
...
SANDBOX_NAME="${DIFFRACTION_SANDBOX_NAME:-e2e-nightly}"
...
if command -v diffraction > /dev/null 2>&1; then
  diffraction "$SANDBOX_NAME" destroy 2>/dev/null || true
fi
if command -v openshell > /dev/null 2>&1; then
  openshell sandbox delete "$SANDBOX_NAME" 2>/dev/null || true
  openshell gateway destroy -g diffraction 2>/dev/null || true
fi
...
if [ "${DIFFRACTION_NON_INTERACTIVE:-}" != "1" ]; then
  fail "DIFFRACTION_NON_INTERACTIVE=1 is required"
  exit 1
fi

# ══════════════════════════════════════════════════════════════════
# Phase 2: Install diffraction (non-interactive mode)
# ══════════════════════════════════════════════════════════════════
section "Phase 2: Install diffraction (non-interactive mode)"

cd "$REPO" || { fail "Could not cd to repo root: $REPO"; exit 1; }

info "Running install.sh --non-interactive..."
info "This installs Node.js, openshell, Diffraction, and runs onboard."
info "Expected duration: 5-10 minutes on first run."

INSTALL_LOG="/tmp/diffraction-e2e-install.log"
...
# Verify diffraction is on PATH
if command -v diffraction > /dev/null 2>&1; then
  pass "diffraction installed at $(command -v diffraction)"
else
  fail "diffraction not found on PATH after install"
  exit 1
fi
...
diffraction --help > /dev/null 2>&1 \
  && pass "diffraction --help exits 0" \
  || fail "diffraction --help failed"
...
# 3a: diffraction list
if list_output=$(diffraction list 2>&1); then
  echo "$list_output" | grep -Fq -- "$SANDBOX_NAME" \
    && pass "diffraction list contains '${SANDBOX_NAME}'" \
    || fail "diffraction list does not contain '${SANDBOX_NAME}'"
else
  fail "diffraction list failed: ${list_output:0:200}"
fi

# 3b: diffraction status
status_output=$(diffraction "$SANDBOX_NAME" status 2>&1)
if [ $? -eq 0 ]; then
  pass "diffraction ${SANDBOX_NAME} status exits 0"
else
  fail "diffraction ${SANDBOX_NAME} status failed: ${status_output:0:200}"
fi
...
# ══════════════════════════════════════════════════════════════════
# Phase 5: Diffraction CLI operations
# ══════════════════════════════════════════════════════════════════
section "Phase 5: Diffraction CLI operations"
...
# Diffraction tests only that its onboard correctly *configured* the policies
# (Phase 3d above), not that OpenShell *enforces* them.

# ── Test 5a: diffraction logs ──
info "Testing sandbox log retrieval..."
logs_output=$(diffraction "$SANDBOX_NAME" logs 2>&1) || true
if [ -n "$logs_output" ]; then
  pass "diffraction logs: produced output ($(echo "$logs_output" | wc -l | tr -d ' ') lines)"
else
  fail "diffraction logs: no output"
fi
...
diffraction "$SANDBOX_NAME" destroy 2>&1 | tail -3 || true
openshell gateway destroy -g diffraction 2>/dev/null || true

list_after=$(diffraction list 2>&1)
