#!/bin/bash
# Double onboard: verify that consecutive `diffraction onboard` runs recover
...
REGISTRY="$HOME/.diffraction/sandboxes.json"
...
# Use diffraction destroy (not just openshell sandbox delete) to also clean
# the diffraction registry at ~/.diffraction/sandboxes.json.  Stale registry
...
if command -v diffraction > /dev/null 2>&1; then
  diffraction "$SANDBOX_A" destroy 2>/dev/null || true
  diffraction "$SANDBOX_B" destroy 2>/dev/null || true
fi
...
if command -v diffraction > /dev/null 2>&1; then
  pass "diffraction CLI installed"
else
  fail "diffraction CLI not found — cannot continue"
  exit 1
fi
...
info "Running diffraction onboard — expect exit 1 (no API key)..."

# Write to temp file to avoid openshell FD inheritance blocking $()
ONBOARD_LOG="$(mktemp)"
DIFFRACTION_NON_INTERACTIVE=1 \
  DIFFRACTION_SANDBOX_NAME="$SANDBOX_A" \
  DIFFRACTION_POLICY_MODE=skip \
  diffraction onboard --non-interactive > "$ONBOARD_LOG" 2>&1
exit1=$?
...
openshell gateway info -g diffraction 2>/dev/null | grep -q "diffraction" \
...
info "Running diffraction onboard with DIFFRACTION_RECREATE_SANDBOX=1..."

ONBOARD_LOG="$(mktemp)"
DIFFRACTION_NON_INTERACTIVE=1 \
  DIFFRACTION_SANDBOX_NAME="$SANDBOX_A" \
  DIFFRACTION_RECREATE_SANDBOX=1 \
  DIFFRACTION_POLICY_MODE=skip \
  diffraction onboard --non-interactive > "$ONBOARD_LOG" 2>&1
exit2=$?
...
echo "$output2" | grep -q "Cleaning up previous Diffraction session" \
...
info "Running diffraction onboard with new sandbox name..."

ONBOARD_LOG="$(mktemp)"
DIFFRACTION_NON_INTERACTIVE=1 \
  DIFFRACTION_SANDBOX_NAME="$SANDBOX_B" \
  DIFFRACTION_POLICY_MODE=skip \
  diffraction onboard --non-interactive > "$ONBOARD_LOG" 2>&1
exit3=$?
...
echo "$output3" | grep -q "Cleaning up previous Diffraction session" \
