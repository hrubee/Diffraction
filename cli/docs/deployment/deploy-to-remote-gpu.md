---
title:
  page: "Deploy Diffraction to a Remote GPU Instance with Brev"
  nav: "Deploy to Remote GPU"
description: "Provision a remote GPU VM with Diffraction using Brev deployment."
keywords: ["deploy diffraction remote gpu", "diffraction brev cloud deployment"]
topics: ["generative_ai", "ai_agents"]
tags: ["diffraction", "openshell", "deployment", "gpu", "diffraction"]
content:
  type: how_to
...
# Deploy Diffraction to a Remote GPU Instance

Run Diffraction on a remote GPU instance through [Brev](https://brev.nvidia.com).
...
- Diffraction installed locally. Follow the [Quickstart](../get-started/quickstart.md) install steps.
...
Create a Brev instance and run the Diffraction setup:
...
3. Runs the diffraction setup to create the gateway, register providers, and launch the sandbox.
...
```console
$ ssh <instance-name> 'cd /home/ubuntu/diffraction && set -a && . .env && set +a && openshell term'
```
...
```console
$ diffraction agent --agent main --local -m "Hello from the remote sandbox" --session-id test
```

## GPU Configuration

The deploy script uses the `DIFFRACTION_GPU` environment variable to select the GPU type.
The default value is `a2-highgpu-1g:nvidia-tesla-a100:1`.
Set this variable before running `diffraction deploy` to use a different GPU configuration:

```console
$ export DIFFRACTION_GPU="a2-highgpu-1g:nvidia-tesla-a100:2"
$ diffraction deploy <instance-name>
```
