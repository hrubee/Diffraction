---
title:
  page: "Diffraction Network Policies — Baseline Rules and Operator Approval"
  nav: "Network Policies"
description: "Baseline network policy, filesystem rules, and operator approval flow."
keywords: ["diffraction network policy", "sandbox egress control operator approval"]
topics: ["generative_ai", "ai_agents"]
tags: ["diffraction", "openshell", "sandboxing", "network_policy", "security"]
content:
  type: reference
...
Diffraction runs with a strict-by-default network policy.
...
The baseline policy is defined in `diffraction-blueprint/policies/diffraction-sandbox.yaml`.
...
* - `nvidia`
  ...
  - `/usr/local/bin/claude`, `/usr/local/bin/diffraction`
  - All methods
...
* - `diffraction_docs`
  - `docs.diffraction.ai:443`
  - `/usr/local/bin/diffraction`
  - GET only

* - `npm_registry`
  - `registry.npmjs.org:443`
  - `/usr/local/bin/diffraction`, `/usr/local/bin/npm`
  - GET only
...
Edit `diffraction-blueprint/policies/diffraction-sandbox.yaml` and re-run the onboard wizard:
