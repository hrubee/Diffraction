---
title:
  page: "Diffraction Troubleshooting Guide"
  nav: "Troubleshooting"
description: "Diagnose and resolve common Diffraction installation, onboarding, and runtime issues."
keywords: ["diffraction troubleshooting", "diffraction debug sandbox issues"]
topics: ["generative_ai", "ai_agents"]
tags: ["diffraction", "openshell", "troubleshooting", "diffraction"]
content:
  type: reference
...
This page covers common issues you may encounter when installing, onboarding, or running Diffraction, along with their resolution steps.

:::{admonition} Get Help
:class: tip

If your issue is not listed here, join the [Diffraction Discord channel](https://discord.gg/XFpfPv9Uvx) to ask questions and get help from the community. You can also [file an issue on GitHub](https://github.com/NVIDIA/Diffraction/issues/new).
:::

## Installation

### `diffraction` not found after install

If you use nvm or fnm to manage Node.js, the installer may not update your current shell's PATH.
The `diffraction` binary is installed but the shell session does not know where to find it.
...
The Diffraction gateway uses port `18789` by default.
...
Run `diffraction onboard` to retry.
...
Run `diffraction onboard` to recreate the sandbox from the same blueprint and policy definitions.

### Status shows "not running" inside the sandbox

This is expected behavior.
When running `diffraction diffraction status` inside an active sandbox, host-side sandbox state and inference configuration are not inspectable.
