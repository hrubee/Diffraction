---
title:
  page: "Customize the Diffraction Sandbox Network Policy"
  nav: "Customize Network Policy"
description: "Add, remove, or modify allowed endpoints in the sandbox policy."
keywords: ["customize diffraction network policy", "sandbox egress policy configuration"]
topics: ["generative_ai", "ai_agents"]
tags: ["diffraction", "openshell", "network_policy", "security", "diffraction"]
content:
  type: how_to
...
Diffraction supports both static policy changes that persist across restarts and dynamic updates to a running sandbox.

## Prerequisites

- A running Diffraction sandbox for dynamic changes, or the Diffraction source repository for static changes.
...
Open `diffraction-blueprint/policies/diffraction-sandbox.yaml` and add or modify endpoint entries.
...
Follow the same format as the baseline policy in `diffraction-blueprint/policies/diffraction-sandbox.yaml`.
