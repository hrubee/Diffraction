...
/**
 * Diffraction — Diffraction Plugin for OpenShell
 *
 * Uses the real Diffraction plugin API. Types defined locally are minimal stubs
 * that match the Diffraction SDK interfaces available at runtime via
 * `diffraction/plugin-sdk`. We define them here because the SDK package is only
 * available inside the Diffraction host process and cannot be imported at build
 * time.
 */
...
// ---------------------------------------------------------------------------
// Diffraction Plugin SDK compatible types (mirrors diffraction/plugin-sdk)
// ---------------------------------------------------------------------------

/** Subset of DiffractionConfig that we actually read. */
...
export interface PluginService {
  id: string;
  start: (ctx: { config: DiffractionConfig; logger: PluginLogger }) => void | Promise<void>;
  stop?: (ctx: { config: DiffractionConfig; logger: PluginLogger }) => void | Promise<void>;
}

/**
 * The API object injected into the plugin's register function by the Diffraction
 * host. Only the methods we actually call are listed here.
 */
export interface DiffractionPluginApi {
  id: string;
  name: string;
  version?: string;
  config: DiffractionConfig;
...
// ---------------------------------------------------------------------------
// Plugin-specific config (read from pluginConfig in diffraction.plugin.json)
// ---------------------------------------------------------------------------
...
function registeredProviderForConfig(
  ...
  return {
    ...
    aliases: ["inference-local", "diffraction"],
...
const DEFAULT_PLUGIN_CONFIG: DiffractionConfig = {
  blueprintVersion: "latest",
  blueprintRegistry: "ghcr.io/nvidia/diffraction-blueprint",
  sandboxName: "diffraction",
  inferenceProvider: "nvidia",
};

export function getPluginConfig(api: DiffractionPluginApi): DiffractionConfig {
...
export default function register(api: DiffractionPluginApi): void {
  // 1. Register /diffraction slash command (chat interface)
  api.registerCommand({
    name: "diffraction",
    description: "Diffraction sandbox management (status, eject).",
    acceptsArgs: true,
    handler: (ctx) => handleSlashCommand(ctx, api),
  });

  // 2. Register `diffraction diffraction` CLI subcommands (commander.js)
  api.registerCli(
    (cliCtx) => {
      registerCliCommands(cliCtx, api);
    },
    { commands: ["diffraction"] },
  );
  ...
  api.logger.info("");
  api.logger.info("  ┌─────────────────────────────────────────────────────┐");
  api.logger.info("  │  Diffraction registered                                │");
  api.logger.info("  │                                                     │");
  ...
  api.logger.info("  │  Commands:  diffraction diffraction <command>             │");
  api.logger.info("  └─────────────────────────────────────────────────────┘");
  api.logger.info("");
}
