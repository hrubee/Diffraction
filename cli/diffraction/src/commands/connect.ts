...
export async function cliConnect(opts: ConnectOptions): Promise<void> {
  ...
  logger.info(`Connecting to Diffraction sandbox: ${sandboxName}`);
  logger.info("You will be inside the sandbox. Run 'diffraction' commands normally.");
  ...
  if (exitCode !== 0 && exitCode !== null) {
    ...
    logger.info("Run 'diffraction diffraction status' to check available sandboxes.");
  }
}
