...
function showConfig(config: DiffractionOnboardConfig, logger: PluginLogger): void {
...
export async function cliOnboard(opts: OnboardOptions): Promise<void> {
  ...
  logger.info("Diffraction Onboarding");
  logger.info("-------------------");
  ...
  const summaryConfig: DiffractionOnboardConfig = {
    endpointType,
    endpointUrl,
    ncpPartner,
    model,
  ...
  logger.info("  diffraction diffraction launch     # Bootstrap sandbox");
  logger.info("  diffraction diffraction status     # Check configuration");
}
