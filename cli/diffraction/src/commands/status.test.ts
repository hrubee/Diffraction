...
import type { DiffractionState } from "../blueprint/state.js";
...
function blankState(): DiffractionState {
  return {
    lastRunId: null,
    lastAction: null,
...
function populatedState(): DiffractionState {
  return {
    lastRunId: "run-a1b2c3d4",
    lastAction: "migrate",
    blueprintVersion: "0.1.0",
    sandboxName: "diffraction",
    migrationSnapshot: "/root/.diffraction/snapshots/pre-migrate.tar.gz",
    hostBackupPath: "/root/.diffraction/backups/host-backup",
    createdAt: "2026-03-15T10:30:00.000Z",
    updatedAt: "2026-03-15T10:32:45.000Z",
  };
}

const defaultConfig: DiffractionConfig = {
...
describe("cliStatus", () => {
  ...
  describe("host — sandbox running, inference configured", () => {
    ...
    it("shows running sandbox with uptime in text output", async () => {
      ...
      expect(output).toContain("Name:    diffraction");
      expect(output).not.toContain("inside sandbox");
    });
  ...
  describe("inside sandbox — core bug fix", () => {
    beforeEach(() => {
      vi.mocked(existsSync).mockImplementation((p: string | URL | Buffer) => {
        const path = String(p);
        return path === "/sandbox/.diffraction" || path === "/sandbox/.diffraction";
      });
    });
  ...
  describe("inside sandbox — with prior plugin state", () => {
    ...
    it("shows rollback section when migrationSnapshot exists", async () => {
      ...
      expect(output).toContain("Snapshot:  /root/.diffraction/snapshots/pre-migrate.tar.gz");
      expect(output).toContain("diffraction diffraction eject");
    });

    it("JSON includes full diffraction state alongside insideSandbox: true", async () => {
      const { lines, logger } = captureLogger();
      ...
      const data = JSON.parse(lines.join(""));
      expect(data.insideSandbox).toBe(true);
      expect(data.diffraction.lastAction).toBe("migrate");
      expect(data.diffraction.blueprintVersion).toBe("0.1.0");
      expect(data.diffraction.lastRunId).toBe("run-a1b2c3d4");
      expect(data.diffraction.migrationSnapshot).toBe(
        "/root/.diffraction/snapshots/pre-migrate.tar.gz",
      );
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("defaults sandbox name to 'diffraction' when state has none", async () => {
      mockExec({
      ...
      expect(exec).toHaveBeenCalledWith(
        expect.stringContaining("diffraction"),
        expect.anything(),
        expect.anything(),
      );
    });

    it("only detects sandbox via /sandbox/.diffraction", async () => {
      vi.mocked(existsSync).mockImplementation((p: string | URL | Buffer) => {
        return String(p) === "/sandbox/.diffraction";
      });
