...
export { detectHostDiffraction, type HostDiffractionState } from "./migration-state.js";

const SANDBOX_ARCHIVE_DIR = "/sandbox/.diffraction/migration/archives";
...
export async function cliMigrate(opts: MigrateOptions): Promise<void> {
  ...
  logger.info("Diffraction migrate: moving host Diffraction into OpenShell sandbox");

  logger.info("Detecting host Diffraction installation...");
  const hostState = detectHostDiffraction();

  if (!hostState.exists || !hostState.stateDir) {
    logger.error("No Diffraction installation found for the current host environment.");
    logger.info("Use 'diffraction diffraction launch' for a fresh install.");
    return;
  }
  ...
  if (hostState.errors.length > 0) {
    ...
    logger.error("Refusing to migrate until all external Diffraction roots can be resolved.");
    return;
  }
  ...
  if (dryRun) {
    ...
    if (hostState.externalRoots.length > 0) {
      logger.info("  3. Capture external Diffraction roots and rewrite config paths for the sandbox:");
      for (const root of hostState.externalRoots) {
  ...
  try {
    logger.info("Packaging Diffraction state for sandbox import...");
  ...
  logger.info("");
  logger.info("Migration complete. Diffraction is now running inside OpenShell.");
  logger.info(`Sandbox: ${pluginConfig.sandboxName}`);
  logger.info("");
  logger.info("Next steps:");
  logger.info("  diffraction diffraction connect    # Enter the sandbox");
  logger.info("  diffraction diffraction status     # Verify everything is healthy");
  ...
  if (skipBackup) {
    ...
    logger.info("  diffraction diffraction eject");
  }
}
...
function syncSnapshotBundleIntoSandbox(bundle: SnapshotBundle, sandboxName: string): void {
  ...
  syncArchive(sandboxName, "state.tar", stateArchivePath(bundle), "/sandbox/.diffraction");
  for (const root of bundle.manifest.externalRoots) {
    syncArchive(
      sandboxName,
      `${root.id}.tar`,
...
function verifySandboxMigration(bundle: SnapshotBundle, sandboxName: string): void {
  ...
  const verification = {
    stateDir: "/sandbox/.diffraction",
    configPath: "/sandbox/.diffraction/diffraction.json",
