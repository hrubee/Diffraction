...
const SANDBOX_MIGRATION_DIR = "/sandbox/.diffraction/migration";
...
export interface HostDiffractionState {
  exists: boolean;
  homeDir: string | null;
...
type DiffractionConfigDocument = Record<string, unknown>;
...
function resolveHostHome(env: NodeJS.ProcessEnv = process.env): string {
  ...
  const explicitHome = env.DIFFRACTION_HOME?.trim();
  if (explicitHome) {
    if (explicitHome === "~") {
      return fallbackHome;
    }
...
function resolveStateDir(env: NodeJS.ProcessEnv = process.env): string {
  const override = env.DIFFRACTION_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env);
  }
  return path.join(resolveHostHome(env), ".diffraction");
}

function resolveConfigPath(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.DIFFRACTION_CONFIG_PATH?.trim();
  if (override) {
    return resolveUserPath(override, env);
  }
  return path.join(stateDir, "diffraction.json");
}

function loadConfigDocument(configPath: string): DiffractionConfigDocument | null {
  ...
  return parsed as DiffractionConfigDocument;
}
...
function defaultWorkspacePath(env: NodeJS.ProcessEnv = process.env): string {
  ...
  const profile = env.DIFFRACTION_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".diffraction", `workspace-${profile}`);
  }
  return path.join(home, ".diffraction", "workspace");
}

function collectExternalRoots(
  config: DiffractionConfigDocument | null,
  stateDir: string,
...
export function detectHostDiffraction(env: NodeJS.ProcessEnv = process.env): HostDiffractionState {
  ...
  const errors: string[] = [];
  const warnings: string[] = [];
  let config: DiffractionConfigDocument | null = null;

  if (!stateExists) {
    errors.push(`Resolved Diffraction state directory does not exist: ${stateDir}`);
  }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    errors.push(`Failed to parse Diffraction config at ${configPath}: ${msg}`);
  }
...
function resolveConfigSourcePath(manifest: SnapshotManifest, snapshotDir: string): string {
  if (manifest.hasExternalConfig) {
    return path.join(snapshotDir, "config", "diffraction.json");
  }
  return path.join(snapshotDir, "diffraction", "diffraction.json");
}
...
function prepareSandboxState(snapshotDir: string, manifest: SnapshotManifest): string {
  const preparedStateDir = path.join(snapshotDir, "sandbox-bundle", "diffraction");
  rmSync(preparedStateDir, { recursive: true, force: true });
  mkdirSync(path.dirname(preparedStateDir), { recursive: true });
  copyDirectory(path.join(snapshotDir, "diffraction"), preparedStateDir);
  ...
  writeFileSync(path.join(preparedStateDir, "diffraction.json"), JSON.stringify(config, null, 2));
  return preparedStateDir;
}

export function createSnapshotBundle(
  hostState: HostDiffractionState,
  logger: PluginLogger,
  options: { persist: boolean },
): SnapshotBundle | null {
  if (!hostState.stateDir || !hostState.homeDir) {
    logger.error("Cannot snapshot host Diffraction state: no state directory was resolved.");
    return null;
  }
  ...
  try {
    ...
    const snapshotStateDir = path.join(parentDir, "diffraction");
    ...
    if (hostState.configPath && hostState.hasExternalConfig) {
      ...
      copyFileSync(hostState.configPath, path.join(configSnapshotDir, "diffraction.json"));
    }
...
export function restoreSnapshotToHost(snapshotDir: string, logger: PluginLogger): boolean {
  ...
  const snapshotStateDir = path.join(snapshotDir, "diffraction");
  ...
  try {
    if (existsSync(manifest.stateDir)) {
      const archiveName = `${manifest.stateDir}.diffraction-archived-${String(Date.now())}`;
    ...
    if (manifest.hasExternalConfig && manifest.configPath) {
      const configSnapshotPath = path.join(snapshotDir, "config", "diffraction.json");
    ...
    logger.info("Host Diffraction state restored.");
    return true;
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
