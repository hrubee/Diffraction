...
function isInsideSandbox(): boolean {
  return existsSync("/sandbox/.diffraction") || existsSync("/sandbox/.diffraction");
}
...
export async function cliStatus(opts: StatusOptions): Promise<void> {
  ...
  const sandboxName = state.sandboxName ?? "diffraction";
  ...
  logger.info("Diffraction Status");
  logger.info("===============");
  logger.info("");
  ...
  if (state.migrationSnapshot) {
    ...
    logger.info("  Run 'diffraction diffraction eject' to restore host installation.");
  }
}
