...
const CONFIG_DIR = join(process.env.HOME ?? "/tmp", ".diffraction");

export type EndpointType = "build" | "ncp" | "nim-local" | "vllm" | "ollama" | "custom";

export interface DiffractionOnboardConfig {
...
export function describeOnboardEndpoint(config: DiffractionOnboardConfig): string {
...
export function describeOnboardProvider(config: DiffractionOnboardConfig): string {
...
export function loadOnboardConfig(): DiffractionOnboardConfig | null {
  ensureConfigDir();
  const path = configPath();
  if (!existsSync(path)) {
    return null;
  }
  return JSON.parse(readFileSync(path, "utf-8")) as DiffractionOnboardConfig;
}

export function saveOnboardConfig(config: DiffractionOnboardConfig): void {
  ensureConfigDir();
