...
import type { DiffractionConfig } from "../index.js";
...
const CACHE_DIR = join(process.env.HOME ?? "/tmp", ".diffraction", "blueprints");
...
function parseManifestHeader(raw: string): BlueprintManifest {
  ...
  return {
    ...
    minDiffractionVersion: get("min_diffraction_version"),
    profiles: profiles ? profiles.split(",").map((p) => p.trim()) : ["default"],
    digest: get("digest"),
  };
}

export async function resolveBlueprint(config: DiffractionConfig): Promise<ResolvedBlueprint> {
