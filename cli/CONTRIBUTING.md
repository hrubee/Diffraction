# Contributing to NVIDIA Diffraction

Thank you for your interest in contributing to NVIDIA Diffraction. This guide covers how to set up your development environment, run tests, and submit changes.
...
```bash
# Install root dependencies (Diffraction + CLI entry point)
npm install

# Install and build the TypeScript plugin
cd diffraction && npm install && npm run build && cd ..

# Install Python deps for the blueprint
cd diffraction-blueprint && uv sync && cd ..
```

## Building

The TypeScript plugin lives in `diffraction/` and compiles with `tsc`:
...
| `cd diffraction && npm test` | Run plugin unit tests (Vitest) |
...
| Path | Purpose |
|------|---------|
| `diffraction/` | TypeScript plugin (Commander CLI, Diffraction extension) |
| `diffraction-blueprint/` | Python blueprint for sandbox orchestration |
| `bin/` | CLI entry point (`diffraction.js`) |
...
```
feat(cli): add --profile flag to diffraction onboard
