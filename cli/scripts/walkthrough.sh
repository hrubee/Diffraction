...
#
# Diffraction walkthrough — sandboxed agent approval flow.
#
# This sets up a split-screen workflow:
#   LEFT:  Diffraction agent (chat)
...
#
# Prerequisites:
#   - Diffraction setup complete (./scripts/setup.sh)
...
#
#   Terminal 2 (Agent):
#     openshell sandbox connect diffraction
#     export NVIDIA_API_KEY=nvapi-...
#     diffraction
#     diffraction agent --agent main --local --session-id live
...
echo ""
echo "  ┌─────────────────────────────────────────────────────┐"
echo "  │  Diffraction Walkthrough                            │"
echo "  │                                                     │"
echo "  │  LEFT pane:   OpenShell TUI (monitor + approve)     │"
echo "  │  RIGHT pane:  Diffraction agent (chat)                 │"
echo "  │                                                     │"
...
if ! command -v tmux > /dev/null 2>&1; then
  ...
  echo "    openshell sandbox connect diffraction"
  echo "    export NVIDIA_API_KEY=$NVIDIA_API_KEY"
  echo "    diffraction"
  echo "    diffraction agent --agent main --local --session-id live"
  exit 0
fi

SESSION="diffraction-walkthrough"
...
tmux split-window -h -t "$SESSION" \
  "openshell sandbox connect diffraction -- bash -c 'export NVIDIA_API_KEY=$NVIDIA_API_KEY && diffraction diffraction agent --agent main --local --session-id live'"
