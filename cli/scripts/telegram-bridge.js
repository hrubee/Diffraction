...
/**
 * Telegram → Diffraction bridge.
 *
 * Messages from Telegram are forwarded to the Diffraction agent running
 ...
 *   SANDBOX_NAME        — sandbox name (default: diffraction)
...
const SANDBOX = process.env.SANDBOX_NAME || "diffraction";
...
function runAgentInSandbox(message, sessionId) {
  return new Promise((resolve) => {
    ...
    const confPath = `/tmp/diffraction-tg-ssh-${sessionId}.conf`;
    require("fs").writeFileSync(confPath, sshConfig);

    const escaped = message.replace(/'/g, "'\\''");
    const cmd = `export NVIDIA_API_KEY='${API_KEY}' && diffraction diffraction agent --agent main --local -m '${escaped}' --session-id 'tg-${sessionId}'`;
    ...
    proc.on("close", (code) => {
      ...
      const responseLines = lines.filter(
        (l) =>
          !l.startsWith("Setting up Diffraction") &&
          !l.startsWith("[plugins]") &&
          !l.startsWith("(node:") &&
          !l.includes("Diffraction ready") &&
          !l.includes("Diffraction registered") &&
          !l.includes("diffraction agent") &&
          !l.includes("┌─") &&
          !l.includes("│ ") &&
          !l.includes("└─") &&
          l.trim() !== "",
    ...
    if (res.ok && res.result?.length > 0) {
      for (const update of res.result) {
        ...
        if (msg.text === "/start") {
          await sendMessage(
            chatId,
            "🦀 *Diffraction* — powered by Nemotron 3 Super 120B\n\n" +
              "Send me a message and I'll run it through the Diffraction agent " +
...
async function main() {
  ...
  console.log("");
  console.log("  ┌─────────────────────────────────────────────────────┐");
  console.log("  │  Diffraction Telegram Bridge                          │");
  console.log("  │                                                     │");
  ...
  console.log("  │                                                     │");
  console.log("  │  Messages are forwarded to the Diffraction agent      │");
