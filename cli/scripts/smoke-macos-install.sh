...
#
# Run the primary Diffraction install flow on a local machine, capture logs,
...
LOG_DIR="${TMPDIR:-/tmp}/diffraction-smoke"
RUNTIME=""
...
Options:
  ...
  --allow-existing-state      Allow running even if Diffraction/OpenShell state already exists
...
ensure_clean_start() {
  ...
  if [ -d "$HOME/.diffraction" ] || [ -d "$HOME/.config/diffraction" ] || [ -d "$HOME/.config/openshell" ]; then
    fail "Existing Diffraction/OpenShell state detected. Re-run with --allow-existing-state if you really want to test on this machine."
  fi
...
feed_install_answers() {
  ...
  (
    ...
    while :; do
      if [ -f "$install_log" ] && grep -q "Diffraction gateway launched inside sandbox" "$install_log"; then
        break
      fi
      sleep 1
    done
...
verify_cleanup() {
  ...
  if [ -d "$HOME/.diffraction" ] || [ -d "$HOME/.config/diffraction" ]; then
    warn "Diffraction state directories still exist under HOME."
    leftovers=1
  fi
  ...
  if command -v docker > /dev/null 2>&1 && docker info > /dev/null 2>&1; then
    ...
    related_containers="$(
      docker ps -a --format '{{.Image}} {{.Names}}' 2>/dev/null \
        | awk 'BEGIN { IGNORECASE=1 } /openshell-cluster|openshell|diffraction|diffraction/ { print }'
    )"
...
main() {
  ...
  ANSWERS_PIPE="$(mktemp -u "${TMPDIR:-/tmp}/diffraction-smoke-answers-XXXXXX")"
  mkfifo "$ANSWERS_PIPE"
  trap cleanup EXIT
