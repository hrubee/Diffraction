...
#
# Diffraction installer — installs Node.js, Ollama (if GPU present), and Diffraction.
...
RUNTIME_REQUIREMENT_MSG="Diffraction requires Node.js >=${MIN_NODE_MAJOR} and npm >=${MIN_NPM_MAJOR} (recommended Node.js ${RECOMMENDED_NODE_MAJOR})."
...
refresh_path() {
  ...
  if [[ -d "$DIFFRACTION_SHIM_DIR" && ":$PATH:" != *":$DIFFRACTION_SHIM_DIR:"* ]]; then
    export PATH="$DIFFRACTION_SHIM_DIR:$PATH"
  fi
}
...
ensure_diffraction_shim() {
  ...
  if [[ ":$ORIGINAL_PATH:" == *":$npm_bin:"* ]] || [[ ":$ORIGINAL_PATH:" == *":$DIFFRACTION_SHIM_DIR:"* ]]; then
    return 0
  fi
...
install_diffraction() {
  if [[ -f "./package.json" ]] && grep -q '"name": "diffraction"' ./package.json 2>/dev/null; then
    ...
    info "Installing Diffraction from GitHub…"
    # Revert once https://github.com/NVIDIA/Diffraction/issues/71 is complete and the package is published
    npm install -g git+https://github.com/NVIDIA/Diffraction.git
  fi
...
run_onboard() {
  ...
  if [ "${NON_INTERACTIVE:-}" = "1" ]; then
    ...
    error "Interactive onboarding requires a TTY. Re-run in a terminal or set DIFFRACTION_NON_INTERACTIVE=1."
  fi
}
...
main() {
  ...
  # Also honor env var
  NON_INTERACTIVE="${NON_INTERACTIVE:-${DIFFRACTION_NON_INTERACTIVE:-}}"
  export DIFFRACTION_NON_INTERACTIVE="${NON_INTERACTIVE}"

  info "=== Diffraction Installer ==="
