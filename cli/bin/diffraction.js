...
const REMOTE_UNINSTALL_URL = "https://raw.githubusercontent.com/NVIDIA/Diffraction/refs/heads/main/uninstall.sh";
...
async function onboard(args) {
  ...
  if (unknownArgs.length > 0) {
    ...
    console.error("  Usage: diffraction onboard [--non-interactive]");
    process.exit(1);
  }
...
async function setup() {
  console.log("");
  console.log("  ⚠  `diffraction setup` is deprecated. Use `diffraction onboard` instead.");
...
async function deploy(instanceName) {
  if (!instanceName) {
    console.error("  Usage: diffraction deploy <instance-name>");
    console.error("");
    console.error("  Examples:");
    console.error("    diffraction deploy my-gpu-box");
    console.error("    diffraction deploy diffraction-prod");
    console.error("    diffraction deploy diffraction-test");
    process.exit(1);
  }
  await ensureApiKey();
  ...
  console.log("");
  console.log(`  Deploying Diffraction to Brev instance: ${name}`);
  console.log("");
  ...
  console.log("  Syncing Diffraction to VM...");
  run(`ssh -o StrictHostKeyChecking=no -o LogLevel=ERROR ${name} 'mkdir -p /home/ubuntu/diffraction'`);
  run(`rsync -az --delete --exclude node_modules --exclude .git --exclude src -e "ssh -o StrictHostKeyChecking=no -o LogLevel=ERROR" "${ROOT}/scripts" "${ROOT}/Dockerfile" "${ROOT}/diffraction" "${ROOT}/diffraction-blueprint" "${ROOT}/bin" "${ROOT}/package.json" ${name}:/home/ubuntu/diffraction/`);
  ...
  const envTmp = path.join(os.tmpdir(), `diffraction-env-${Date.now()}`);
  fs.writeFileSync(envTmp, envLines.join("\n") + "\n", { mode: 0o600 });
  run(`scp -q -o StrictHostKeyChecking=no -o LogLevel=ERROR "${envTmp}" ${name}:/home/ubuntu/diffraction/.env`);
  fs.unlinkSync(envTmp);

  console.log("  Running setup...");
  runInteractive(`ssh -t -o StrictHostKeyChecking=no -o LogLevel=ERROR ${name} 'cd /home/ubuntu/diffraction && set -a && . .env && set +a && bash scripts/brev-setup.sh'`);

  if (tgToken) {
    console.log("  Starting services...");
    run(`ssh -o StrictHostKeyChecking=no -o LogLevel=ERROR ${name} 'cd /home/ubuntu/diffraction && set -a && . .env && set +a && bash scripts/start-services.sh'`);
  }

  console.log("");
  console.log("  Connecting to sandbox...");
  console.log("");
  runInteractive(`ssh -t -o StrictHostKeyChecking=no -o LogLevel=ERROR ${name} 'cd /home/ubuntu/diffraction && set -a && . .env && set +a && openshell sandbox connect diffraction'`);
}
...
function listSandboxes() {
  ...
  if (sandboxes.length === 0) {
    console.log("");
    console.log("  No sandboxes registered. Run `diffraction onboard` to get started.");
    console.log("");
    return;
  }
...
function help() {
  console.log(`
  diffraction — Diffraction CLI

  Getting Started:
    diffraction onboard                 Interactive setup wizard (recommended)
    diffraction setup                   Legacy setup (deprecated, use onboard)
    diffraction setup-spark             Set up on DGX Spark (fixes cgroup v2 + Docker)

  Sandbox Management:
    diffraction list                    List all sandboxes
    diffraction <name> connect          Connect to a sandbox
    diffraction <name> status           Show sandbox status and health
    diffraction <name> logs [--follow]  View sandbox logs
    diffraction <name> destroy          Stop NIM + delete sandbox

  Policy Presets:
    diffraction <name> policy-add       Add a policy preset to a sandbox
    diffraction <name> policy-list      List presets (● = applied)

  Deploy:
    diffraction deploy <instance>       Deploy to a Brev VM and start services

  Services:
    diffraction start                   Start services (Telegram, tunnel)
    diffraction stop                    Stop all services
    diffraction status                  Show sandbox list and service status
    diffraction uninstall [flags]       Run uninstall.sh (local first, curl fallback)
  ...
  in ~/.diffraction/credentials.json (mode 600).
`);
}
...
(async () => {
  ...
  // Sandbox-scoped commands: diffraction <name> <action>
  ...
  if (allNames.length > 0) {
    ...
    console.error(`  Try: diffraction <sandbox-name> connect`);
    console.error("");
  }

  console.error(`  Run 'diffraction help' for usage.`);
  process.exit(1);
})();
