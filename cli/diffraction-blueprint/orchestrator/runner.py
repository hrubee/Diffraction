...
"""
Diffraction Blueprint Runner

Orchestrates Diffraction sandbox lifecycle inside OpenShell.
...
def load_blueprint() -> dict[str, Any]:
    blueprint_path = Path(os.environ.get("DIFFRACTION_BLUEPRINT_PATH", "."))
...
def action_plan(
    ...
    *,
    ...
    plan: dict[str, Any] = {
        ...
        "profile": profile,
        "sandbox": {
            "image": sandbox_cfg.get("image", "diffraction"),
            "name": sandbox_cfg.get("name", "diffraction"),
...
def action_apply(
    ...
    sandbox_name: str = sandbox_cfg.get("name", "diffraction")
    sandbox_image: str = sandbox_cfg.get("image", "diffraction")
    forward_ports: list[int] = sandbox_cfg.get("forward_ports", [18789])

    # Step 1: Create sandbox
    progress(20, "Creating Diffraction sandbox")
    create_args = [
        "openshell",
        "sandbox",
        "create",
    ...
    state_dir = Path.home() / ".diffraction" / "state" / "runs" / rid
...
def action_status(rid: str | None = None) -> None:
    ...
    emit_run_id()
    state_dir = Path.home() / ".diffraction" / "state" / "runs"
...
def action_rollback(rid: str) -> None:
    ...
    if plan_file.exists():
        ...
        sandbox_name = plan.get("sandbox_name", "diffraction")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Diffraction Blueprint Runner")
