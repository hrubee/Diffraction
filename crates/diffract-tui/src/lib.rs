...
use diffract_core::proto::open_shell_client::DiffractClient;
...
/// Launch the Diffract TUI.
///
/// `channel` must be a connected gRPC channel to the Diffract gateway.
...
pub async fn run(
    ...
    let client = DiffractClient::new(channel);
    let mut app = App::new(
        client,
...
fn refresh_gateway_list(app: &mut App) {
    if let Ok(gateways) = diffract_bootstrap::list_gateways() {
...
async fn handle_gateway_switch(app: &mut App) {
    ...
    match connect_to_gateway(&name, &endpoint).await {
        Ok(channel) => {
            app.client = DiffractClient::new(channel);
...
fn gateway_mtls_dir(name: &str) -> Option<PathBuf> {
    let config_dir = diffract_core::paths::xdg_config_dir().ok()?;
    Some(
        config_dir
            .join("diffract")
            .join("gateways")
...
fn spawn_log_stream(app: &mut App, tx: mpsc::UnboundedSender<Event>) {
    ...
    let handle = tokio::spawn(async move {
        ...
        let req = diffract_core::proto::GetSandboxLogsRequest {
        ...
        let req = diffract_core::proto::WatchSandboxRequest {
            id: sandbox_id,
            follow_status: false,
            follow_logs: true,
            follow_events: false,
...
fn proto_to_log_line(log: diffract_core::proto::SandboxLogLine) -> LogLine {
...
async fn handle_sandbox_delete(app: &mut App) {
    ...
    if let Ok(stopped) = diffract_core::forward::stop_forwards_for_sandbox(&sandbox_name) {
        for port in &stopped {
            tracing::info!("stopped forward of port {port} for sandbox {sandbox_name}");
        }
    }

    let req = diffract_core::proto::DeleteSandboxRequest { name: sandbox_name };
...
async fn fetch_sandbox_detail(app: &mut App) {
    ...
    let req = diffract_core::proto::GetSandboxRequest {
    ...
    if let Some(id) = sandbox_id {
        let policy_req = diffract_core::proto::GetSandboxConfigRequest { sandbox_id: id };
        ...
        .await
        {
            Ok(Ok(resp)) => {
                ...
                app.sandbox_policy_is_global =
                    inner.policy_source == diffract_core::proto::PolicySource::Global as i32;
...
///
/// This replicates the `diffract sandbox connect` flow but uses `Command::status()`
...
async fn handle_shell_connect(
    ...
    let session = {
        let req = diffract_core::proto::CreateSshSessionRequest {
            sandbox_id: sandbox_id.clone(),
        };
...
// SSH utility functions are shared via diffract_core::forward.
use diffract_core::forward::{resolve_ssh_gateway, shell_escape};

/// Convert a `SandboxPolicy` proto into styled ratatui lines for the policy viewer.
fn render_policy_lines(
    policy: &diffract_core::proto::SandboxPolicy,
    theme: &theme::Theme,
...
fn spawn_create_sandbox(app: &mut App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        ...
        let template = if has_custom_image {
            Some(diffract_core::proto::SandboxTemplate {
                image,
                ..Default::default()
            })
        } else {
        ...
        let policy = if has_custom_image {
            Some(diffract_policy::restrictive_default_policy())
        } else {
            None
        };

        let req = diffract_core::proto::CreateSandboxRequest {
            name,
            spec: Some(diffract_core::proto::SandboxSpec {
...
async fn start_port_forwards(
    client: &mut DiffractClient<Channel>,
    endpoint: &str,
    gateway_name: &str,
    sandbox_name: &str,
    sandbox_id: &str,
    specs: &[diffract_core::forward::ForwardSpec],
) {
    // Create SSH session.
    let session = {
        let req = diffract_core::proto::CreateSshSessionRequest {
    ...
    for spec in specs {
        ...
        match result {
            Ok(Ok(true)) => {
                if let Some(pid) = diffract_core::forward::find_ssh_forward_pid(&sid, port_val) {
                    let _ = diffract_core::forward::write_forward_pid(
...
fn spawn_create_provider(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        ...
        for attempt in 0..5u32 {
            ...
            let req = diffract_core::proto::CreateProviderRequest {
                provider: Some(diffract_core::proto::Provider {
                    id: String::new(),
...
fn spawn_get_provider(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        let req = diffract_core::proto::GetProviderRequest { name };
...
fn spawn_update_provider(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        ...
        let req = diffract_core::proto::UpdateProviderRequest {
            provider: Some(diffract_core::proto::Provider {
                id: String::new(),
                name: name.clone(),
                r#type: ptype,
...
fn spawn_delete_provider(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        let req = diffract_core::proto::DeleteProviderRequest { name };
...
fn spawn_draft_approve(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        let req = diffract_core::proto::ApproveDraftChunkRequest { name, chunk_id };
...
fn spawn_draft_reject(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        let req = diffract_core::proto::RejectDraftChunkRequest {
            name,
            chunk_id,
            reason: String::new(),
        };
...
fn spawn_draft_approve_all(
    app: &App,
    snapshot: Vec<diffract_core::proto::PolicyChunk>,
    ...
    tokio::spawn(async move {
        ...
        for chunk in &snapshot {
            let req = diffract_core::proto::ApproveDraftChunkRequest {
                name: name.clone(),
...
async fn refresh_providers(app: &mut App) {
    let req = diffract_core::proto::ListProvidersRequest {
        limit: 100,
        offset: 0,
    };
...
async fn refresh_global_settings(app: &mut App) {
    let req = diffract_core::proto::GetGatewayConfigRequest {};
    let result =
    ...
    let policy_req = diffract_core::proto::ListSandboxPoliciesRequest {
        name: String::new(),
        limit: 1,
        offset: 0,
        global: true,
    ...
    .await
    {
        ...
        if let Some(latest) = revisions.first() {
            let status =
                diffract_core::proto::PolicyStatus::try_from(latest.status).unwrap_or_default();
            app.global_policy_active = status == diffract_core::proto::PolicyStatus::Loaded;
...
fn spawn_set_global_setting(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        ...
        use diffract_core::proto::{SettingValue, UpdateConfigRequest, setting_value};

        let value = match kind {
            diffract_core::settings::SettingValueKind::Bool => {
                match diffract_core::settings::parse_bool_like(&raw) {
            ...
            diffract_core::settings::SettingValueKind::Int => match raw.parse::<i64>() {
            ...
            diffract_core::settings::SettingValueKind::String => {
...
fn spawn_delete_global_setting(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        use diffract_core::proto::UpdateConfigRequest;
...
async fn refresh_health(app: &mut App) {
    let req = diffract_core::proto::HealthRequest {};
...
async fn refresh_sandboxes(app: &mut App) {
    let req = diffract_core::proto::ListSandboxesRequest {
        limit: 100,
        offset: 0,
    };
    ...
    match result {
        ...
        Ok(Ok(resp)) => {
            ...
            let forwards = diffract_core::forward::list_forwards().unwrap_or_default();
            app.sandbox_notes = sandboxes
                .iter()
                .map(|s| diffract_core::forward::build_sandbox_notes(&s.name, &forwards))
                .collect();
...
async fn refresh_sandbox_policy(app: &mut App) {
    ...
    let policy_req = diffract_core::proto::GetSandboxConfigRequest { sandbox_id };
...
async fn refresh_draft_chunks(app: &mut App) {
    ...
    let req = diffract_core::proto::GetDraftPolicyRequest {
        name: sandbox_name,
