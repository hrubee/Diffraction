...
use diffract_bootstrap::{load_last_sandbox, save_last_sandbox};
use diffract_cli::run;
use diffract_cli::tls::TlsOptions;
use diffract_core::proto::open_shell_server::{Diffract, DiffractServer};
...
impl Diffract for TestDiffract {
    ...
    async fn revoke_ssh_session(
        &self,
        _request: tonic::Request<diffract_core::proto::RevokeSshSessionRequest>,
    ) -> Result<Response<diffract_core::proto::RevokeSshSessionResponse>, Status> {
        Ok(Response::new(
...
async fn run_server() -> TestServer {
    ...
    let diffract = TestDiffract::default();
    let svc_diffract = diffract.clone();

    tokio::spawn(async move {
        Server::builder()
            .tls_config(tls_config)
            .unwrap()
            .add_service(DiffractServer::new(svc_diffract))
...
async fn sandbox_get_sends_correct_name() {
    ...
    let recorded = ts.diffract.state.last_get_name.lock().await.clone();
    assert_eq!(
        recorded.as_deref(),
        Some("my-sandbox"),
