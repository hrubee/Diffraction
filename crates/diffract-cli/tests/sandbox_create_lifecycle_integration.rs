...
use diffract_bootstrap::load_last_sandbox;
...
async fn deleted_names(server: &TestServer) -> Vec<Vec<String>> {
    server.diffract.state.deleted_names.lock().await.clone()
}

fn test_tls(server: &TestServer) -> TlsOptions {
    server.tls.with_gateway_name("diffract")
}
...
async fn sandbox_create_keeps_command_sessions_by_default() {
    ...
    assert_eq!(
        load_last_sandbox("diffract").as_deref(),
        Some("default-command"),
...
async fn sandbox_create_keeps_sandbox_with_forwarding() {
    ...
    run::sandbox_create(
        ...
        &[],
        None,
        Some(diffract_core::forward::ForwardSpec::new(8080)),
        &["echo".to_string(), "OK".to_string()],
        Some(false),
        Some(false),
        Some(false),
