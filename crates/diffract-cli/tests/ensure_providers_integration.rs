...
async fn explicit_provider_name_passes_through_when_it_exists() {
    ...
    ts.diffract.seed_provider("nvidia", "nvidia").await;

    let mut client = diffract_cli::tls::grpc_client(&ts.endpoint, &ts.tls)
        .await
        .expect("grpc client");
    ...
    let providers = ts.diffract.state.providers.lock().await;
