...
//! `Diffract` CLI - command-line interface for `Diffract`.
...
use diffract_cli::completers;
use diffract_cli::run;
use diffract_cli::tls::TlsOptions;
...
/// 4. Active gateway from `~/.config/diffract/active_gateway`
///
...
fn resolve_gateway(
    ...
    let name = gateway_flag
        ...
        .ok_or_else(|| {
            miette::miette!(
                "No active gateway.\n\
                 Set one with: diffract gateway select <name>\n\
                 Or deploy a new gateway: diffract gateway start"
            )
        })?;
    ...
    let metadata = load_gateway_metadata(&name).map_err(|_| {
        miette::miette!(
            "Unknown gateway '{name}'.\n\
             Deploy it first: diffract gateway start --name {name}\n\
             Or list available gateways: diffract gateway select"
        )
    })?;
...
{about-with-newline}
\x1b[1mUSAGE\x1b[0m
  diffract <command> <subcommand> [flags]
...
\x1b[1mADDITIONAL COMMANDS\x1b[0m
  term:        Launch the Diffract interactive TUI
...
\x1b[1mEXAMPLES\x1b[0m
  $ diffract sandbox create
  $ diffract gateway start
  $ diffract logs my-sandbox

\x1b[1mLEARN MORE\x1b[0m
  Use `diffract <command> --help` for more information about a command.
";
...
\x1b[1mEXAMPLES\x1b[0m
  $ diffract sandbox create
  $ diffract sandbox create --from python
  $ diffract sandbox connect my-sandbox
  $ diffract sandbox list
  $ diffract sandbox delete my-sandbox
";
...
\x1b[1mEXAMPLES\x1b[0m
  $ diffract forward start 8080
  $ diffract forward start 3000 my-sandbox
  $ diffract forward stop 8080
  $ diffract forward list
";
...
\x1b[1mEXAMPLES\x1b[0m
  $ diffract logs my-sandbox
  $ diffract logs my-sandbox --tail
  $ diffract logs --since 5m
  $ diffract logs --source sandbox --level debug
";
...
\x1b[1mEXAMPLES\x1b[0m
  $ diffract policy get my-sandbox
  $ diffract policy set my-sandbox --policy policy.yaml
  $ diffract policy set --global --policy policy.yaml
  $ diffract policy delete --global
  $ diffract policy list my-sandbox
";

const SETTINGS_EXAMPLES: &str = "\x1b[1mEXAMPLES\x1b[0m
  $ diffract settings get my-sandbox
  $ diffract settings get --global
  $ diffract settings set my-sandbox --key log_level --value debug
  $ diffract settings set --global --key log_level --value warn
  $ diffract settings set --global --key dummy_bool --value yes
  $ diffract settings set --global --key dummy_int --value 42
  $ diffract settings delete --global --key log_level
";

const PROVIDER_EXAMPLES: &str = "\x1b[1mEXAMPLES\x1b[0m
  $ diffract provider create --name openai --type openai --credential OPENAI_API_KEY
  $ diffract provider create --name anthropic --type anthropic --from-existing
  $ diffract provider list
  $ diffract provider get openai
  $ diffract provider delete openai
";
...
\x1b[1mEXAMPLES\x1b[0m
  $ diffract gateway start
  $ diffract gateway start --name my-gateway --port 9090
  $ diffract gateway stop
  $ diffract gateway select my-gateway
  $ diffract gateway info
";

const INFERENCE_EXAMPLES: &str = "\x1b[1mEXAMPLES\x1b[0m
  $ diffract inference set --provider openai --model gpt-4
  $ diffract inference get
  $ diffract inference update --model gpt-4-turbo
";
...
\x1b[1mAI AGENT USAGE\x1b[0m
  ...
  for debugging gateway clusters using `diffract doctor logs` and
  `diffract doctor exec`.
";

/// `Diffract` CLI - agent execution and management.
#[derive(Parser, Debug)]
#[command(name = "diffract")]
#[command(author, version = diffract_core::VERSION, about, long_about = None)]
...
enum Commands {
    ...
    /// `diffract doctor llm.txt` for a full diagnostic prompt.
    ...
    ///   `diffract ssh-proxy --gateway <url> --sandbox-id <id> --token <token>`
    ///
    /// **Name mode** (for use in `~/.ssh/config`):
    ///   `diffract ssh-proxy --gateway <name> --name <sandbox-name>`
...
Generate shell completion scripts for Diffract CLI.
...
First, ensure that you install `bash-completion` using your package manager.
  ...
  diffract completions bash > ~/.local/share/bash-completion/completions/diffract

On macOS with Homebrew (install bash-completion first):

  mkdir -p $(brew --prefix)/etc/bash_completion.d
  diffract completions bash > $(brew --prefix)/etc/bash_completion.d/diffract.bash-completion

\x1b[1mFISH\x1b[0m

  mkdir -p ~/.config/fish/completions
  diffract completions fish > ~/.config/fish/completions/diffract.fish

\x1b[1mZSH\x1b[0m

  mkdir -p ~/.zfunc
  diffract completions zsh > ~/.zfunc/_diffract
...
\x1b[1mPOWERSHELL\x1b[0m
   ...
   diffract completions powershell >> $PROFILE
...
fn normalize_completion_script(output: Vec<u8>, executable: &std::path::Path) -> Result<String> {
    ...
    Ok(script.replace(executable.to_string_lossy().as_ref(), "diffract"))
}
...
impl From<CliEditor> for diffract_cli::ssh::Editor {
...
enum GatewayCommands {
    ...
    Start {
        /// Gateway name.
        #[arg(long, default_value = "diffract", env = "OPENSHELL_GATEWAY")]
        name: String,
        ...
        #[arg(long, default_value_t = diffract_bootstrap::DEFAULT_GATEWAY_PORT)]
        port: u16,
    ...
    ///
    /// Registers a gateway endpoint so it appears in `diffract gateway select`.
    ///
...
enum DoctorCommands {
    ...
    /// Examples:
    ///   diffract doctor exec -- kubectl get pods -A
    ///   diffract doctor exec -- k9s
    ///   diffract doctor exec -- sh
    #[command(help_template = LEAF_HELP_TEMPLATE, next_help_heading = "FLAGS")]
    Exec {
        /// Gateway name (defaults to active gateway).
        #[arg(long, env = "OPENSHELL_GATEWAY")]
    ...
    /// diagnose gateway issues using `diffract doctor logs` and
    /// `diffract doctor exec`.
    ///
    /// Examples:
    ///   diffract doctor llm.txt
    ///   diffract doctor llm.txt | pbcopy
    ...
    /// your environment before running `diffract gateway start`.
    ///
    /// Examples:
    ///   diffract doctor check
...
enum SandboxCommands {
    ...
    Create {
        ...
        /// Sandbox source: a community sandbox name (e.g., `diffraction`), a path
        ...
        /// `ghcr.io/nvidia/diffract-community/sandboxes/<name>:latest`
        ...
        /// Keeps the sandbox alive and installs Diffract-managed SSH config.
    ...
    Connect {
        ...
        /// Installs Diffract-managed SSH config if needed.
        #[arg(long, value_enum)]
        editor: Option<CliEditor>,
    },
    ...
    match cli.command {
        ...
        Some(Commands::Gateway {
            ...
            GatewayCommands::Stop {
                ...
                let name = name
                    ...
                    .unwrap_or_else(|| "diffract".to_string());
            ...
            GatewayCommands::Login { name } => {
                let name = name
                    ...
                    .ok_or_else(|| {
                        miette::miette!(
                            "No active gateway.\n\
                             Specify a gateway name: diffract gateway login <name>\n\
                             Or set one with: diffract gateway select <name>"
                        )
                    })?;
        ...
        Some(Commands::Forward {
            ...
            ForwardCommands::Start {
                ...
                let spec = diffract_core::forward::ForwardSpec::parse(&port)?;
                ...
                if background {
                    ...
                    eprintln!("  Stop with: diffract forward stop {} {name}", spec.port);
                }
            }
        },
        ...
        Some(Commands::Sandbox {
            ...
            match command {
                SandboxCommands::Create {
                    ...
                    let forward = forward
                        .map(|s| diffract_core::forward::ForwardSpec::parse(&s))
                        .transpose()?;
                    ...
                    match resolve_gateway(&cli.gateway, &cli.gateway_endpoint) {
                        Ok(ctx) => {
                            if remote.is_some() {
                                eprintln!(
                                    "{} --remote ignored: gateway '{}' is already active. \
                                     To redeploy, use: diffract gateway start",
                                    "!".yellow(),
                                    ctx.name,
                                );
                                return Ok(());
                            ...
                            // "diffract" gateway if the connection fails.
        ...
        Some(Commands::Term { theme }) => {
            ...
            let mut tls = tls.with_gateway_name(&ctx.name);
            apply_edge_auth(&mut tls, &ctx.name);
            let channel = diffract_cli::tls::build_channel(&ctx.endpoint, &tls).await?;
            diffract_tui::run(channel, &ctx.name, &ctx.endpoint, theme).await?;
        }
                ...
                (_, _, _, server_override, Some(g), Some(n)) => {
                    let endpoint = if let Some(srv) = server_override {
                        ...
                        let meta = load_gateway_metadata(&g).map_err(|_| {
                            miette::miette!(
                                "Unknown gateway '{g}'.\n\
                                  Deploy it first: diffract gateway start --name {g}\n\
                                  Or list available gateways: diffract gateway select"
                            )
                        })?;
...
mod tests {
    ...
    fn completions_engine_returns_candidates() {
        ...
        let args: Vec<OsString> = vec!["diffract".into(), "".into()];
    ...
    fn completions_subcommand_appears_in_candidates() {
        let mut cmd = Cli::command();
        let args: Vec<OsString> = vec!["diffract".into(), "comp".into()];
        let candidates = clap_complete::engine::complete(&mut cmd, args, 1, None)
            .expect("completion engine failed");
        let names: Vec<String> = candidates
            .iter()
    ...
    fn completions_other_path_flags_fall_back_to_path_candidates() {
        ...
        let cases: Vec<(Vec<&str>, usize, &str)> = vec![
            (
                vec!["diffract", "gateway", "start", "--ssh-key", "id"],
                4,
                "id_rsa",
            ),
            (
                vec!["diffract", "sandbox", "create", "--ssh-key", "id"],
                4,
                "id_rsa",
            ),
            (
                vec!["diffract", "sandbox", "upload", "demo", "Do"],
                4,
                "Dockerfile",
            ),
            (
                vec!["diffract", "sandbox", "create", "--from", "Do"],
                4,
                "Dockerfile",
            ),
            (
    ...
    fn gateway_completion_suggests_registered_gateways() {
        ...
        with_tmp_xdg(tmp.path(), || {
            ...
            for (raw_args, index) in [
                (vec!["diffract", "--gateway", "a"], 2),
                (vec!["diffract", "gateway", "select", "a"], 3),
                (vec!["diffract", "gateway", "info", "--name", "a"], 4),
            ] {
                let mut cmd = Cli::command();
                let args: Vec<OsString> = raw_args.iter().copied().map(Into::into).collect();
    ...
    fn global_gateway_flag_still_parses_with_subcommands() {
        let cli = Cli::try_parse_from(["diffract", "--gateway", "demo", "status"])
    ...
    fn hidden_aliases_still_parse() {
        let cli = Cli::try_parse_from(["diffract", "lg", "sandbox-1"])
    ...
    #[test]
    fn completion_script_uses_diffract_command_name() {
        let script = normalize_completion_script(
            b"/tmp/custom/diffract -- \"${words[@]}\"\n#compdef diffract\n".to_vec(),
            std::path::Path::new("/tmp/custom/diffract"),
        )
        .expect("normalize completion script");

        assert!(script.contains("diffract -- \"${words[@]}\""));
        assert!(!script.contains("/tmp/custom/diffract"));
    }
    ...
    fn settings_get_global_parses() {
        let cli = Cli::try_parse_from(["diffract", "settings", "get", "--global"])
    ...
    fn policy_delete_global_parses() {
        let cli = Cli::try_parse_from(["diffract", "policy", "delete", "--global", "--yes"])
