...
use diffract_core::proto::inference_client::InferenceClient;
...
pub type GrpcClient = DiffractClient<InterceptedService<Channel, EdgeAuthInterceptor>>;
...
fn tls_dir_for_gateway(name: &str) -> Option<PathBuf> {
    ...
    let base = xdg_config_dir().ok()?.join("diffract").join("gateways");
...
fn default_tls_dir(server: &str) -> Option<PathBuf> {
    ...
    let name = name.unwrap_or_else(|| "diffract".to_string());
    let safe_name = sanitize_name(&name);
    let base = xdg_config_dir().ok()?.join("diffract").join("gateways");
    Some(base.join(safe_name).join("mtls"))
}
...
fn xdg_config_dir() -> Result<PathBuf> {
    diffract_core::paths::xdg_config_dir()
}
...
pub async fn grpc_client(server: &str, tls: &TlsOptions) -> Result<GrpcClient> {
    ...
    Ok(DiffractClient::with_interceptor(channel, interceptor))
}
