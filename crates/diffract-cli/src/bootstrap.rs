...
const DEFAULT_GATEWAY_NAME: &str = "diffract";
...
pub async fn run_bootstrap(
    ...
    eprintln!();
    eprintln!("  The Gateway provides a secure control plane for Diffract. It streamlines");
    ...
    eprintln!(
        ...
        "diffract status".bold(),
        "diffract gateway stop".bold(),
    );
    eprintln!();
    ...
    let mut options = diffract_bootstrap::DeployOptions::new(&gateway_name).with_recreate(true);
    if let Some(dest) = remote {
        let mut remote_opts = diffract_bootstrap::RemoteOptions::new(dest);
    ...
    if let Err(err) = diffract_bootstrap::save_active_gateway(&gateway_name) {
...
mod tests {
    ...
    fn missing_default_tls_files_triggers_bootstrap() {
        let err = report(
            "failed to read TLS CA from /home/user/.config/diffract/clusters/diffract/mtls/ca.crt: No such file or directory",
        );
        assert!(should_attempt_bootstrap(&err, &TlsOptions::default()));
    }
