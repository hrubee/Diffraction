...
use diffract_core::forward::{
    find_ssh_forward_pid, resolve_ssh_gateway, shell_escape, write_forward_pid,
};
use diffract_core::proto::{CreateSshSessionRequest, GetSandboxRequest};
...
async fn ssh_session_config(
    ...
    let exe = std::env::current_exe()
        .into_diagnostic()
        .wrap_err("failed to resolve Diffract executable")?;
...
pub async fn sandbox_forward(
    server: &str,
    name: &str,
    spec: &diffract_core::forward::ForwardSpec,
    background: bool,
    tls: &TlsOptions,
) -> Result<()> {
...
fn host_alias(name: &str) -> String {
    format!("diffract-{name}")
}

fn render_ssh_config(gateway: &str, name: &str) -> String {
    let exe = std::env::current_exe().expect("failed to resolve Diffract executable");
...
fn diffract_ssh_config_path() -> Result<PathBuf> {
    Ok(diffract_core::paths::xdg_config_dir()?
        .join("diffract")
        .join("ssh_config"))
}
...
fn ensure_diffract_include(main_config: &Path, managed_config: &Path) -> Result<()> {
...
pub fn install_ssh_config(gateway: &str, name: &str) -> Result<PathBuf> {
    let managed_config = diffract_ssh_config_path()?;
    let main_config = user_ssh_config_path()?;
    ensure_diffract_include(&main_config, &managed_config)?;

    if let Some(parent) = managed_config.parent() {
        diffract_core::paths::create_dir_restricted(parent)?;
    }
    ...
    fs::write(&managed_config, updated)
        .into_diagnostic()
        .wrap_err("failed to write Diffract SSH config")?;
    Ok(managed_config)
}
...
/// (`~/.config/diffract/gateways/<name>/mtls/`).
...
mod tests {
    ...
    fn upsert_host_block_appends_when_missing() {
        ...
        let block = "Host diffract-demo\n    User sandbox\n";
        let output = upsert_host_block(input, "diffract-demo", block);
        assert!(output.contains("Host existing"));
        assert!(output.contains("Host diffract-demo"));
        assert_eq!(output.matches("Host diffract-demo").count(), 1);
    }

    #[test]
    fn upsert_host_block_replaces_existing_without_duplicates() {
        let input = "Host diffract-demo\n    User old\n\nHost other\n    HostName other.example\n";
        let block = "Host diffract-demo\n    User sandbox\n    LogLevel ERROR\n";
        let output = upsert_host_block(input, "diffract-demo", block);
    ...
    fn install_ssh_config_adds_include_once_and_updates_managed_file() {
        ...
        let managed_path = install_ssh_config("diffract", "demo").unwrap();
        install_ssh_config("diffract", "demo").unwrap();
        ...
        assert_eq!(managed_contents.matches("Host diffract-demo").count(), 1);
    ...
    fn launch_editor_returns_friendly_error_when_binary_missing() {
        let err = launch_editor_command(
            "diffract-test-missing-binary",
            "Test Editor",
            "ssh-remote+diffract-demo",
        )
        .unwrap_err();
        let text = format!("{err}");
        assert!(text.contains("diffract-test-missing-binary is not installed or not on PATH"));
    }
    ...
    fn foreground_forward_started_message_custom_bind_addr() {
        let spec = diffract_core::forward::ForwardSpec::parse("0.0.0.0:3000").unwrap();
        let message = foreground_forward_started_message("demo", &spec);
