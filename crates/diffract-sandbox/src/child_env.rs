...
mod tests {
    ...
    fn apply_tls_env_sets_node_and_bundle_paths() {
        ...
        let ca_cert_path = Path::new("/etc/diffract-tls/diffract-ca.pem");
        let combined_bundle_path = Path::new("/etc/diffract-tls/ca-bundle.pem");
        ...
        assert!(stdout.contains("NODE_EXTRA_CA_CERTS=/etc/diffract-tls/diffract-ca.pem"));
        assert!(stdout.contains("SSL_CERT_FILE=/etc/diffract-tls/ca-bundle.pem"));
    }
}
