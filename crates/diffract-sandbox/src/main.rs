...
//! Diffract Sandbox - process sandbox and monitor.
...
use diffract_sandbox::run_sandbox;

/// Diffract Sandbox - process isolation and monitoring.
#[derive(Parser, Debug)]
#[command(name = "diffract-sandbox")]
#[command(version = diffract_core::VERSION)]
...
struct Args {
    ...
    /// Sandbox ID for fetching policy via gRPC from Diffract server.
    /// Requires --diffract-endpoint to be set.
    ...
    /// Diffract server gRPC endpoint for fetching policy.
...
async fn main() -> Result<()> {
    ...
    let log_push_state = if let (Some(sandbox_id), Some(endpoint)) =
        (&args.sandbox_id, &args.diffract_endpoint)
    {
        let (tx, handle) =
            diffract_sandbox::log_push::spawn_log_push_task(endpoint.clone(), sandbox_id.clone());
        let layer = diffract_sandbox::log_push::LogPushLayer::new(sandbox_id.clone(), tx);
        Some((layer, handle))
    } else {
        None
    };
