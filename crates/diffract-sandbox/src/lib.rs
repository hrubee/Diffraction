...
pub async fn run_sandbox(
    ...
    diffract_endpoint: Option<String>,
    ...
    let diffract_endpoint_for_proxy = diffract_endpoint.clone();
    let sandbox_name_for_agg = sandbox.clone();
    let (policy, opa_engine) = load_policy(
        sandbox_id.clone(),
        sandbox,
        diffract_endpoint.clone(),
        policy_rules,
        policy_data,
    )
    .await?;
    ...
    let provider_env = if let (Some(id), Some(endpoint)) = (&sandbox_id, &diffract_endpoint) {
    ...
    let (tls_state, ca_file_paths) = if matches!(policy.network.mode, NetworkMode::Proxy) {
        match SandboxCa::generate() {
            Ok(ca) => {
                let tls_dir = std::path::Path::new("/etc/diffract-tls");
                match write_ca_files(&ca, tls_dir) {
                    Ok(paths) => {
                        // /etc/diffract-tls is subsumed by the /etc baseline
    ...
    let (_proxy, denial_rx, bypass_denial_tx) = if matches!(policy.network.mode, NetworkMode::Proxy)
    {
        ...
        let inference_ctx = build_inference_context(
            sandbox_id.as_deref(),
            diffract_endpoint_for_proxy.as_deref(),
    ...
    // Zombie reaper — diffract-sandbox may run as PID 1 in containers and
    ...
    if let (Some(id), Some(endpoint), Some(engine)) =
        (&sandbox_id, &diffract_endpoint, &opa_engine)
    {
...
async fn build_inference_context(
    ...
    use diffract_router::Router;
    use diffract_router::config::RouterConfig;

    let source = infer_route_source(sandbox_id, diffract_endpoint, inference_routes);
    ...
    let routes = match source {
        ...
        InferenceRouteSource::Cluster => {
            let (Some(_id), Some(endpoint)) = (sandbox_id, diffract_endpoint) else {
                return Ok(None);
            };
    ...
    if matches!(source, InferenceRouteSource::Cluster)
        && let (Some(_id), Some(endpoint)) = (sandbox_id, diffract_endpoint)
    {
        spawn_route_refresh(
            ctx.route_cache(),
...
fn partition_routes(
    routes: Vec<diffract_router::config::ResolvedRoute>,
) -> (
    Vec<diffract_router::config::ResolvedRoute>,
    Vec<diffract_router::config::ResolvedRoute>,
) {
...
pub(crate) fn bundle_to_resolved_routes(
    bundle: &diffract_core::proto::GetInferenceBundleResponse,
) -> Vec<diffract_router::config::ResolvedRoute> {
    bundle
        .routes
        .iter()
        .map(|r| {
            let (auth, default_headers) =
                diffract_core::inference::auth_for_provider_type(&r.provider_type);
            diffract_router::config::ResolvedRoute {
                name: r.name.clone(),
...
pub(crate) fn spawn_route_refresh(
    user_cache: Arc<tokio::sync::RwLock<Vec<diffract_router::config::ResolvedRoute>>>,
    system_cache: Arc<tokio::sync::RwLock<Vec<diffract_router::config::ResolvedRoute>>>,
    endpoint: String,
    interval_secs: u64,
...
fn enrich_proto_baseline_paths(proto: &mut diffract_core::proto::SandboxPolicy) -> bool {
    ...
    let fs = proto
        .filesystem
        .get_or_insert_with(|| diffract_core::proto::FilesystemPolicy {
...
/// 2. If `sandbox_id` and `diffract_endpoint` are provided, fetch via gRPC
...
async fn load_policy(
    ...
    if let (Some(id), Some(endpoint)) = (&sandbox_id, &diffract_endpoint) {
        info!(
            sandbox_id = %id,
            endpoint = %endpoint,
    ...
    Err(miette::miette!(
        "Sandbox policy required. Provide one of:\n\
         ...
         - --sandbox-id and --diffract-endpoint (or OPENSHELL_SANDBOX_ID and OPENSHELL_ENDPOINT env vars)"
    ))
}
...
fn discover_policy_from_disk_or_default() -> diffract_core::proto::SandboxPolicy {
    let primary = std::path::Path::new(diffract_policy::CONTAINER_POLICY_PATH);
    if primary.exists() {
        return discover_policy_from_path(primary);
    }
    let legacy = std::path::Path::new(diffract_policy::LEGACY_CONTAINER_POLICY_PATH);
    if legacy.exists() {
        info!(
...
fn discover_policy_from_path(path: &std::path::Path) -> diffract_core::proto::SandboxPolicy {
    use diffract_policy::{
...
async fn flush_proposals_to_gateway(
    ...
    use crate::grpc_client::CachedDiffractClient;
    use diffract_core::proto::{DenialSummary, L7RequestSample};

    let client = CachedDiffractClient::connect(endpoint).await?;
...
async fn run_policy_poll_loop(
    ...
    use crate::grpc_client::CachedDiffractClient;
    use diffract_core::proto::PolicySource;
...
fn log_setting_changes(
    old: &std::collections::HashMap<String, diffract_core::proto::EffectiveSetting>,
    new: &std::collections::HashMap<String, diffract_core::proto::EffectiveSetting>,
) {
...
fn format_setting_value(es: &diffract_core::proto::EffectiveSetting) -> String {
...
mod tests {
    ...
    fn bundle_to_resolved_routes_converts_all_fields() {
        let bundle = diffract_core::proto::GetInferenceBundleResponse {
            routes: vec![
                diffract_core::proto::ResolvedRoute {
        ...
        assert_eq!(
            routes[0].auth,
            diffract_core::inference::AuthHeader::Bearer
        );
        assert_eq!(
            routes[0].protocols,
    ...
    fn bundle_to_resolved_routes_preserves_name_field() {
        let bundle = diffract_core::proto::GetInferenceBundleResponse {
            routes: vec![diffract_core::proto::ResolvedRoute {
    ...
    fn routes_segregated_by_name() {
        let routes = vec![
            diffract_router::config::ResolvedRoute {
                ...
                protocols: vec!["openai_chat_completions".to_string()],
                auth: diffract_core::inference::AuthHeader::Bearer,
                default_headers: vec![],
            },
            diffract_router::config::ResolvedRoute {
                name: "sandbox-system".to_string(),
                ...
                auth: diffract_core::inference::AuthHeader::Custom("x-api-key"),
                default_headers: vec![],
            },
        ];
...
"#,
    ...
    fn discover_policy_restrictive_default_blocks_network() {
        ...
        let proto = diffract_policy::restrictive_default_policy();
    ...
    async fn route_cache_preserves_content_when_not_written() {
        ...
        let routes = vec![diffract_router::config::ResolvedRoute {
            name: "inference.local".to_string(),
