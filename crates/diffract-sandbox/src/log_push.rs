...
//! Push sandbox tracing events to the Diffract server via gRPC.
//!
...
use diffract_core::proto::{PushSandboxLogsRequest, SandboxLogLine};
...
/// Tracing layer that pushes log events to the Diffract server.
///
...
async fn run_push_loop(
    ...
    loop {
        ...
        // --- Connect ---
        let client = match CachedDiffractClient::connect(&endpoint).await {
            Ok(c) => {
                if attempt > 1 {
                    eprintln!("diffract: log push reconnected (attempt {attempt})");
                }
                backoff = INITIAL_BACKOFF;
                c
            }
            Err(e) => {
                eprintln!("diffract: log push connect failed: {e}");
        ...
        tokio::spawn({
            ...
            async move {
                if let Err(e) = nav_client.push_sandbox_logs(stream).await {
                    eprintln!("diffract: log push RPC failed: {e}");
                }
        ...
        if stream_broken {
            eprintln!("diffract: log push stream lost, reconnecting...");
            backoff = INITIAL_BACKOFF;
