...
//! route bundles from Diffract server.
...
use diffract_core::proto::{
    ...
    inference_client::InferenceClient, open_shell_client::DiffractClient,
};
use tonic::transport::{Certificate, Channel, ClientTlsConfig, Endpoint, Identity};
use tracing::debug;

/// Create a channel to the Diffract server.
///
...
async fn connect_channel(endpoint: &str) -> Result<Channel> {
    ...
    ep.connect()
        .await
        .into_diagnostic()
        .wrap_err("failed to connect to Diffract server")
}

/// Connect to the Diffract server (mTLS or plaintext based on endpoint scheme).
async fn connect(endpoint: &str) -> Result<DiffractClient<Channel>> {
    let channel = connect_channel(endpoint).await?;
    Ok(DiffractClient::new(channel))
}

/// Fetch sandbox policy from Diffract server via gRPC.
///
...
pub async fn fetch_policy(endpoint: &str, sandbox_id: &str) -> Result<Option<ProtoSandboxPolicy>> {
    debug!(endpoint = %endpoint, sandbox_id = %sandbox_id, "Connecting to Diffract server");
...
/// gateway stores the effective policy users see via `diffract sandbox get`.
...
/// Fetch provider environment variables for a sandbox from Diffract server via gRPC.
///
...
/// A reusable gRPC client for the Diffract service.
///
...
#[derive(Clone)]
pub struct CachedDiffractClient {
    client: DiffractClient<Channel>,
}

/// Settings poll result returned by [`CachedDiffractClient::poll_settings`].
pub struct SettingsPollResult {
    ...
    pub settings: std::collections::HashMap<String, diffract_core::proto::EffectiveSetting>,
    /// When `policy_source` is `Global`, the version of the global policy revision.
    pub global_policy_version: u32,
}

impl CachedDiffractClient {
    pub async fn connect(endpoint: &str) -> Result<Self> {
        debug!(endpoint = %endpoint, "Connecting diffract gRPC client for policy polling");
        let channel = connect_channel(endpoint).await?;
    ...
    pub fn raw_client(&self) -> DiffractClient<Channel> {
        self.client.clone()
    }
    ...
    pub async fn submit_policy_analysis(
        ...
        proposed_chunks: Vec<diffract_core::proto::PolicyChunk>,
        analysis_mode: &str,
    ) -> Result<()> {
        self.client
            .clone()
