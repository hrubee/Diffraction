...
//!
//! All `Diffract` crates should use [`xdg_config_dir`] from this module instead
...
/// The top-level `Diffract` config directory: `$XDG_CONFIG_HOME/diffract/`.
pub fn diffract_config_dir() -> Result<PathBuf> {
    Ok(xdg_config_dir()?.join("diffract"))
}
...
mod tests {
    ...
    fn xdg_config_dir_respects_env() {
        ...
        // we rely on the integration tests in diffract-bootstrap for full
        // round-trip testing.
    ...
    #[test]
    fn diffract_config_dir_appends_diffract() {
        let dir = diffract_config_dir().unwrap();
        assert!(
            dir.ends_with("diffract"),
            "expected path ending with 'diffract', got: {dir:?}"
        );
    }
