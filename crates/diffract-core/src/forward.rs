...
//!
//! Used by both the CLI (`diffract-cli`) and the TUI (`diffract-tui`) to
...
pub fn forward_pid_dir() -> Result<PathBuf> {
    Ok(xdg_config_dir()?.join("diffract").join("forwards"))
}
...
/// hint:
///
/// - If an existing diffract forward owns the port, suggest the stop command.
...
pub fn check_port_available(spec: &ForwardSpec) -> Result<()> {
    ...
    // Port is occupied.  Check if it belongs to a tracked diffract forward.
        ...
        && let Some(fwd) = forwards.iter().find(|f| f.port == port && f.alive)
    {
        return Err(miette::miette!(
            "Port {port} is already forwarded to sandbox '{}'.\n\
             Stop it with: diffract forward stop {port} {}",
            fwd.sandbox,
            fwd.sandbox,
        ));
    }
