...
///
/// Defined in `diffract-core` so both `diffract-router` (which applies it)
/// and `diffract-server` / `diffract-sandbox` (which resolve it from
...
///
/// This is separate from [`diffract_providers::ProviderPlugin`] which handles
