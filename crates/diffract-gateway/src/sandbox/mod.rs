...
const SANDBOX_ID_LABEL: &str = "diffract.ai/sandbox-id";
const SANDBOX_MANAGED_LABEL: &str = "diffract.ai/managed-by";
const SANDBOX_MANAGED_VALUE: &str = "diffract";
...
pub struct SandboxClient {
    ...
    /// `host.docker.internal` and `host.diffract.internal` to this IP.
    ...
    pub async fn validate_gpu_support(&self) -> Result<(), tonic::Status> {
        ...
        if !runtime_class_exists {
            return Err(tonic::Status::failed_precondition(
                "GPU sandbox requested, but the active gateway is not GPU-enabled. To start a gateway with GPU support run: `diffract gateway start --gpu`",
            ));
        }
        ...
        if !has_gpu_capacity {
            return Err(tonic::Status::failed_precondition(
                "GPU sandbox requested, but the active gateway has no allocatable GPUs. Please refer to documentation and use `diffract doctor` commands to inspect GPU support and gateway configuration.",
            ));
        }
...
const SUPERVISOR_MOUNT_PATH: &str = "/opt/diffract/bin";

/// Name of the volume used to side-load the supervisor binary.
const SUPERVISOR_VOLUME_NAME: &str = "diffract-supervisor-bin";
...
const SUPERVISOR_HOST_PATH: &str = "/opt/diffract/bin";
...
fn apply_supervisor_sideload(pod_template: &mut serde_json::Value) {
    ...
    if let Some(container) = containers.get_mut(index).and_then(|v| v.as_object_mut()) {
        ...
        container.insert(
            "command".to_string(),
            serde_json::json!([format!("{}/diffract-sandbox", SUPERVISOR_MOUNT_PATH)]),
        );
...
fn sandbox_template_to_k8s(
    ...
    // Build environment variables - start with Diffract-required vars
    ...
    if !client_tls_secret_name.is_empty() {
        container.insert(
            ...
            serde_json::json!([{
                "name": "diffract-client-tls",
                "mountPath": "/etc/diffract-tls/client",
                "readOnly": true
            }]),
        );
    }
    ...
    if !host_gateway_ip.is_empty() {
        spec.insert(
            ...
            serde_json::json!([{
                "ip": host_gateway_ip,
                "hostnames": ["host.docker.internal", "host.diffract.internal"]
            }]),
        );
    }
...
fn apply_required_env(
    ...
    if tls_enabled {
        upsert_env(env, "OPENSHELL_TLS_CA", "/etc/diffract-tls/client/ca.crt");
        upsert_env(
            env,
            "OPENSHELL_TLS_CERT",
            "/etc/diffract-tls/client/tls.crt",
        );
        upsert_env(
            env,
            "OPENSHELL_TLS_KEY",
            "/etc/diffract-tls/client/tls.key",
        );
    }
}
...
fn rewrite_user_facing_conditions(status: &mut Option<SandboxStatus>, spec: Option<&SandboxSpec>) {
    ...
    if let Some(status) = status {
        for condition in &mut status.conditions {
            ...
            {
                condition.message = "GPU sandbox could not be scheduled on the active gateway. Another GPU sandbox may already be using the available GPU, or the gateway may not currently be able to satisfy GPU placement. Please refer to documentation and use `diffract doctor` commands to inspect GPU support and gateway configuration.".to_string();
            }
        }
    }
}
...
mod tests {
    ...
    fn rewrite_user_facing_conditions_rewrites_gpu_unschedulable_message() {
        ...
        assert_eq!(
            message,
            "GPU sandbox could not be scheduled on the active gateway. Another GPU sandbox may already be using the available GPU, or the gateway may not currently be able to satisfy GPU placement. Please refer to documentation and use `diffract doctor` commands to inspect GPU support and gateway configuration."
        );
    }
    ...
    fn supervisor_sideload_injects_hostpath_volume_and_mount() {
        ...
        assert_eq!(
            ...
            format!("{}/diffract-sandbox", SUPERVISOR_MOUNT_PATH)
        );
    ...
    fn tls_env_vars_match_volume_mount_path() {
        ...
        const TLS_MOUNT_PATH: &str = "/etc/diffract-tls/client";
    ...
    fn host_aliases_injected_when_gateway_ip_set() {
        ...
        assert!(hostnames.contains(&serde_json::json!("host.diffract.internal")));
    }
    ...
    fn tls_secret_volume_uses_restrictive_default_mode() {
        ...
        let tls_vol = volumes
            .iter()
            .find(|v| v["name"] == "diffract-client-tls")
