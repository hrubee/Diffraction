...
//! `Diffract` Server - gRPC/HTTP server with protocol multiplexing.
...
use diffract_gateway::{run_server, tracing_bus::TracingLogBus};

/// `Diffract` Server - gRPC and HTTP server with protocol multiplexing.
#[derive(Parser, Debug)]
#[command(name = "diffract-server")]
#[command(version = diffract_core::VERSION)]
#[command(about = "Diffract gRPC/HTTP server", long_about = None)]
struct Args {
    ...
    /// gRPC endpoint for sandboxes to callback to `Diffract`.
...
async fn main() -> Result<()> {
    ...
    let mut config = diffract_core::Config::new(tls)
    ...
    info!(bind = %config.bind_address, "Starting Diffract server");
