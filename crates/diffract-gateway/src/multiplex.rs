...
use diffract_core::proto::{
    inference_server::InferenceServer, open_shell_server::DiffractServer,
};
...
use crate::{DiffractService, ServerState, http_router, inference::InferenceService};
...
impl MultiplexService {
    ...
    {
        let diffract = DiffractServer::new(DiffractService::new(self.state.clone()))
            .max_decoding_message_size(MAX_GRPC_DECODE_SIZE);
        let inference = InferenceServer::new(InferenceService::new(self.state.clone()))
            .max_decoding_message_size(MAX_GRPC_DECODE_SIZE);
        let grpc_service = GrpcRouter::new(diffract, inference);
...
/// Combined gRPC service that routes between `Diffract` and Inference services
...
impl<N, I> GrpcRouter<N, I> {
    fn new(diffract: N, inference: I) -> Self {
        Self {
            diffract,
            inference,
        }
...
const INFERENCE_PATH_PREFIX: &str = "/diffract.inference.v1.Inference/";
    ...
    B: Send + 'static,
{
    ...
    fn call(&mut self, req: Request<B>) -> Self::Future {
        ...
        if is_inference {
            ...
            let mut svc = self.diffract.clone();
            Box::pin(async move { svc.ready().await?.call(req).await })
        }
    }
}
