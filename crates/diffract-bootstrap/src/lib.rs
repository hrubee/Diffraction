...
use crate::runtime::{
    clean_stale_nodes, exec_capture_with_exit, fetch_recent_logs, diffract_workload_exists,
    restart_diffract_deployment, wait_for_gateway_ready,
};
    ...
    F: FnMut(String) + Send + 'static,
{
        ...
        if recreate {
            ...
            return Err(miette::miette!(
                "Gateway '{name}' already exists (container_running={}).\n\
                 ...
                 diffract gateway destroy --name {name}",
    ...
    // See: https://github.com/NVIDIA/Diffract/issues/463
    let deploy_result: Result<GatewayMetadata> = async {
        ...
        let workload_existed_before_pki = diffract_workload_exists(&target_docker, &name).await?;
        let (pki_bundle, rotated) = reconcile_pki(&target_docker, &name, &extra_sans, &log).await?;

        if rotated && workload_existed_before_pki {
            // If an diffract workload is already running, it must be restarted so
            ...
            restart_diffract_deployment(&target_docker, &name).await?;
        }
...
/// `diffract-cluster-{name}`, and writes them to the provided writer.
///
...
/// Create the three TLS K8s secrets required by the `Diffract` server and sandbox pods.
///
/// Secrets are created via `kubectl` exec'd inside the cluster container:
/// - `diffract-server-tls` (kubernetes.io/tls): server cert + key
/// - `diffract-server-client-ca` (Opaque): CA cert for verifying client certs
/// - `diffract-client-tls` (Opaque): client cert + key + CA cert (shared by CLI & sandboxes)
async fn create_k8s_tls_secrets(
    ...
    // 1. diffract-server-tls (kubernetes.io/tls)
    ...
    apply_secret(server_tls_manifest.to_string())
        .await
        .wrap_err("failed to create diffract-server-tls secret")?;

    // 2. diffract-server-client-ca (Opaque)
    ...
    apply_secret(client_ca_manifest.to_string())
        .await
        .wrap_err("failed to create diffract-server-client-ca secret")?;

    // 3. diffract-client-tls (Opaque) — shared by CLI and sandbox pods
    ...
    apply_secret(client_tls_manifest.to_string())
        .await
        .wrap_err("failed to create diffract-client-tls secret")?;
    ...
    F: Fn(String) + Sync,
{
    ...
    log("[progress] Waiting for diffract namespace".to_string());
    wait_for_namespace(docker, &cname, kubeconfig, "diffract").await?;
...
async fn load_existing_pki_bundle(
    ...
    let read_secret_key = |secret: &str, key: &str| {
        ...
        async move {
            ...
            let cmd = format!(
                "KUBECONFIG={kubeconfig} kubectl get secret {secret} -n diffract -o jsonpath='{jsonpath}' 2>/dev/null"
            );
            let (output, exit_code) = exec_capture_with_exit(
                &docker,
                &container_name,
...
///
/// The Helm controller creates the `diffract` namespace when it processes
