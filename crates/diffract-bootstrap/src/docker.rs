...
const REGISTRY_NAMESPACE_DEFAULT: &str = "diffract";
    ...
    if !socket_exists && docker_host.is_none() {
        ...
        hints.push(
            "Install and start a Docker-compatible runtime. See the support matrix \
             in the Diffract docs for tested configurations."
                .to_string(),
        );
...
/// Find the running diffract gateway container by image name.
///
/// Lists all running containers and returns the name of the one whose image
/// contains `diffract/cluster`. When `port` is provided, only containers
...
pub async fn find_gateway_container(docker: &Docker, port: Option<u16>) -> Result<String> {
    ...
    let is_gateway_image = |c: &bollard::models::ContainerSummary| {
        c.image
            .as_deref()
            .is_some_and(|img| img.contains("diffract/cluster"))
    };
    ...
    match matches.len() {
        0 => {
            let hint = if let Some(p) = port {
                format!(
                    "No diffract gateway container found listening on port {p}.\n\
                     Is the gateway running? Check with: docker ps"
                )
            } else {
                "No diffract gateway container found.\n\
                 Is the gateway running? Check with: docker ps"
                    .to_string()
            };
        ...
        _ => Err(miette::miette!(
            "Found multiple diffract gateway containers: {}\n\
...
pub async fn ensure_container(
    ...
    let mut host_config = HostConfig {
        ...
        extra_hosts: Some(vec![
            ...
            "host.diffract.internal:host-gateway".to_string(),
        ]),
        ..Default::default()
    };
