...
pub const DEFAULT_IMAGE_REPO_BASE: &str = "ghcr.io/nvidia/diffract";

/// Default full gateway image path on GHCR (without tag).
pub const DEFAULT_GATEWAY_IMAGE: &str = "ghcr.io/nvidia/diffract/cluster";
...
/// `diffract/cluster:dev`) so that all downstream container creation logic works
/// without changes.
///
...
pub async fn pull_remote_image(
    ...
    // e.g., tag "ghcr.io/nvidia/diffract/cluster:latest" as "diffract/cluster:dev"
