...
pub use diffract_core::inference::AuthHeader;
...
impl RouteConfig {
    ...
    fn resolve(&self) -> Result<ResolvedRoute, RouterError> {
        let protocols = diffract_core::inference::normalize_protocols(&self.protocols);
...
///
/// Delegates to [`diffract_core::inference::auth_for_provider_type`] which
...
fn auth_from_provider_type(provider_type: Option<&str>) -> (AuthHeader, Vec<(String, String)>) {
    diffract_core::inference::auth_for_provider_type(provider_type.unwrap_or(""))
}
