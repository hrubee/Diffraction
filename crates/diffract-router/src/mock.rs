...
pub fn mock_response(route: &ResolvedRoute, source_protocol: &str) -> ProxyResponse {
    ...
    ProxyResponse {
        ...
        headers: vec![
            ...
            ("x-diffract-mock".to_string(), "true".to_string()),
        ],
        body: body_bytes,
    }
}
...
fn openai_chat_completion_body(model: &str) -> Vec<u8> {
    serde_json::to_vec(&serde_json::json!({
        ...
        "choices": [{
            ...
            "message": {
                "role": "assistant",
                "content": "Hello from diffract mock backend"
            },
            "finish_reason": "stop"
        }],
        "usage": {
...
fn openai_completion_body(model: &str) -> Vec<u8> {
    serde_json::to_vec(&serde_json::json!({
        ...
        "choices": [{
            "index": 0,
            "text": "Hello from diffract mock backend",
            "finish_reason": "stop"
        }],
        "usage": {
            "prompt_tokens": 1,
...
fn generic_body(model: &str) -> Vec<u8> {
    serde_json::to_vec(&serde_json::json!({
        "mock": true,
        "model": model,
        "message": "Hello from diffract mock backend"
    }))
    .expect("static JSON must serialize")
}
...
mod tests {
    ...
    fn mock_openai_chat_completion() {
        ...
        assert_eq!(
            ...
            "Hello from diffract mock backend"
        );
    }
    ...
    fn mock_response_includes_marker_header() {
        ...
        assert!(
            resp.headers
                .iter()
                .any(|(k, v)| k == "x-diffract-mock" && v == "true")
        );
    }
}
