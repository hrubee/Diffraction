...
export default defineConfig({
  resolve: {
    // Keep this ordered: the base `diffraction/plugin-sdk` alias is a prefix match.
    alias: [
      {
        find: "diffraction/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `diffraction/plugin-sdk/${subpath}`,
        replacement: path.join(repoRoot, "src", "plugin-sdk", `${subpath}.ts`),
      })),
      {
        find: "diffraction/plugin-sdk",
