...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.DIFFRACTION_BUILD_VERBOSE === "1") {
    return undefined;
  }
...
function listBundledPluginBuildEntries(): Record<string, string> {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "diffraction.plugin.json");
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8")) as {
          diffraction?: { extensions?: unknown; setupEntry?: unknown };
        };
        packageEntries = Array.isArray(packageJson.diffraction?.extensions)
          ? packageJson.diffraction.extensions.filter(
              (entry): entry is string => typeof entry === "string" && entry.trim().length > 0,
            )
          : [];
        const setupEntry =
          typeof packageJson.diffraction?.setupEntry === "string" &&
          packageJson.diffraction.setupEntry.trim().length > 0
            ? packageJson.diffraction.setupEntry
            : undefined;
        if (setupEntry) {
...
function buildCoreDistEntries(): Record<string, string> {
  return {
    ...
    // See https://github.com/diffraction/diffraction/issues/51676
