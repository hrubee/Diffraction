...
private const val NOTIFICATION_CHANNEL_BASE_ID = "diffraction.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Diffraction Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Diffraction Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Diffraction Active")
      }
