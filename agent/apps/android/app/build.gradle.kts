...
val androidStoreFile = providers.gradleProperty("DIFFRACTION_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("DIFFRACTION_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("DIFFRACTION_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("DIFFRACTION_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set DIFFRACTION_ANDROID_STORE_FILE, " +
            "DIFFRACTION_ANDROID_STORE_PASSWORD, DIFFRACTION_ANDROID_KEY_ALIAS, and " +
            "DIFFRACTION_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/DiffractionKit/Sources/DiffractionKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.diffraction.app"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "DIFFRACTION_ENABLE_SMS", "false")
            buildConfigField("boolean", "DIFFRACTION_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "DIFFRACTION_ENABLE_SMS", "true")
            buildConfigField("boolean", "DIFFRACTION_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "diffraction-$versionName-$buildType.apk"
                    } else {
                        "diffraction-$versionName-$flavorName-$buildType.apk"
                    }
