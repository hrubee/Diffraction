...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.diffraction.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Diffraction", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Diffraction gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Diffraction.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "DiffractionShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Diffraction is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Diffraction Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("diffraction-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
