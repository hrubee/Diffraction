...
final class MotionService: MotionServicing {
    func activities(params: DiffractionMotionActivityParams) async throws -> DiffractionMotionActivityPayload {
        ...
        let mapped: [DiffractionMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
            manager.queryActivityStarting(from: start, to: end, to: OperationQueue()) { activity, error in
                if let error {
                    ...
                    let entries = sliced.map { entry in
                        DiffractionMotionActivityEntry(
        ...
        return DiffractionMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: DiffractionPedometerParams) async throws -> DiffractionPedometerPayload {
        ...
        let payload: DiffractionPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = DiffractionPedometerPayload(
