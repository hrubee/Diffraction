...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [DiffractionCapability.canvas.rawValue, DiffractionCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(DiffractionCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(DiffractionCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = DiffractionLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(DiffractionCapability.location.rawValue) }

        caps.append(DiffractionCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(DiffractionCapability.watch.rawValue)
        }
        caps.append(DiffractionCapability.photos.rawValue)
        caps.append(DiffractionCapability.contacts.rawValue)
        caps.append(DiffractionCapability.calendar.rawValue)
        caps.append(DiffractionCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(DiffractionCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            DiffractionScreenCommand.record.rawValue,
            DiffractionSystemCommand.notify.rawValue,
            DiffractionChatCommand.push.rawValue,
        ...
        if caps.contains(DiffractionCapability.camera.rawValue) {
            commands.append(DiffractionCameraCommand.list.rawValue)
            commands.append(DiffractionCameraCommand.snap.rawValue)
            commands.append(DiffractionCameraCommand.clip.rawValue)
        }
        if caps.contains(DiffractionCapability.location.rawValue) {
            commands.append(DiffractionLocationCommand.get.rawValue)
        }
        if caps.contains(DiffractionCapability.device.rawValue) {
            commands.append(DiffractionDeviceCommand.status.rawValue)
            commands.append(DiffractionDeviceCommand.info.rawValue)
        }
        if caps.contains(DiffractionCapability.watch.rawValue) {
            commands.append(DiffractionWatchCommand.status.rawValue)
            commands.append(DiffractionWatchCommand.notify.rawValue)
        }
        if caps.contains(DiffractionCapability.photos.rawValue) {
            commands.append(DiffractionPhotosCommand.latest.rawValue)
        }
        if caps.contains(DiffractionCapability.contacts.rawValue) {
            commands.append(DiffractionContactsCommand.search.rawValue)
            commands.append(DiffractionContactsCommand.add.rawValue)
        }
        if caps.contains(DiffractionCapability.calendar.rawValue) {
            commands.append(DiffractionCalendarCommand.events.rawValue)
            commands.append(DiffractionCalendarCommand.add.rawValue)
        }
        if caps.contains(DiffractionCapability.reminders.rawValue) {
            commands.append(DiffractionRemindersCommand.list.rawValue)
            commands.append(DiffractionRemindersCommand.add.rawValue)
        }
        if caps.contains(DiffractionCapability.motion.rawValue) {
            commands.append(DiffractionMotionCommand.activity.rawValue)
            commands.append(DiffractionMotionCommand.pedometer.rawValue)
        }
