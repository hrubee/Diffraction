...
final class RemindersService: RemindersServicing {
    func list(params: DiffractionRemindersListParams) async throws -> DiffractionRemindersListPayload {
        ...
        let payload: [DiffractionReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return DiffractionReminderPayload(
        ...
        return DiffractionRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: DiffractionRemindersAddParams) async throws -> DiffractionRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = DiffractionReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return DiffractionRemindersAddPayload(reminder: payload)
    }
