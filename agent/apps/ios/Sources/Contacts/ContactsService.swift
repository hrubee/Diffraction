...
final class ContactsService: ContactsServicing {
    ...
    func search(params: DiffractionContactsSearchParams) async throws -> DiffractionContactsSearchPayload {
        ...
        return DiffractionContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: DiffractionContactsAddParams) async throws -> DiffractionContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return DiffractionContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return DiffractionContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> DiffractionContactPayload {
        DiffractionContactPayload(
