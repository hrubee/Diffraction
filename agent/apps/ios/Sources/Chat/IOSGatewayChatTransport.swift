...
struct IOSGatewayChatTransport: DiffractionChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.diffraction", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> DiffractionChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(DiffractionChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> DiffractionChatHistoryPayload {
        ...
        return try JSONDecoder().decode(DiffractionChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [DiffractionChatAttachmentPayload]) async throws -> DiffractionChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [DiffractionChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(DiffractionChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(DiffractionGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DiffractionGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: DiffractionChatEventPayload.self)
                        {
