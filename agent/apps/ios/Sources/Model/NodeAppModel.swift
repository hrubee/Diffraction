...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.diffraction.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.diffraction.ios", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "ai.diffraction.ios", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "ai.diffraction.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.diffraction.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = DiffractionCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = DiffractionCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = DiffractionCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = DiffractionCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = DiffractionCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: DiffractionLocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(DiffractionGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: DiffractionNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: DiffractionNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DiffractionNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(DiffractionLocationGetParams.self, from: req.paramsJSON)) ??
            DiffractionLocationGetParams()
        ...
        let payload = DiffractionLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DiffractionCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(DiffractionCanvasPresentParams.self, from: req.paramsJSON)) ??
                DiffractionCanvasPresentParams()
        ...
        case DiffractionCanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case DiffractionCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(DiffractionCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case DiffractionCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(DiffractionCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case DiffractionCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(DiffractionCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case DiffractionCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.diffractionA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing diffractionA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

        case DiffractionCanvasA2UICommand.push.rawValue, DiffractionCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [DiffractionKit.AnyCodable]
            if command == DiffractionCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(DiffractionCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try DiffractionCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(DiffractionCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try DiffractionCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.diffractionA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DiffractionCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case DiffractionCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(DiffractionCameraSnapParams.self, from: req.paramsJSON)) ??
                DiffractionCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case DiffractionCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(DiffractionCameraClipParams.self, from: req.paramsJSON)) ??
                DiffractionCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(DiffractionScreenRecordParams.self, from: req.paramsJSON)) ??
            DiffractionScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(DiffractionSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DiffractionNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DiffractionNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DiffractionNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(DiffractionChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: DiffractionNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = DiffractionChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DiffractionDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DiffractionDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(DiffractionPhotosLatestParams.self, from: req.paramsJSON)) ??
            DiffractionPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DiffractionContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(DiffractionContactsSearchParams.self, from: req.paramsJSON)) ??
                DiffractionContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DiffractionContactsCommand.add.rawValue:
            let params = try Self.decodeParams(DiffractionContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DiffractionCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(DiffractionCalendarEventsParams.self, from: req.paramsJSON)) ??
                DiffractionCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DiffractionCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(DiffractionCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DiffractionRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(DiffractionRemindersListParams.self, from: req.paramsJSON)) ??
                DiffractionRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DiffractionRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(DiffractionRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DiffractionMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(DiffractionMotionActivityParams.self, from: req.paramsJSON)) ??
                DiffractionMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DiffractionMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(DiffractionPedometerParams.self, from: req.paramsJSON)) ??
                DiffractionPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DiffractionTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DiffractionTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DiffractionTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DiffractionTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([DiffractionLocationCommand.get.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleLocationInvoke(req)
        }

        register([
            DiffractionCanvasCommand.present.rawValue,
            DiffractionCanvasCommand.hide.rawValue,
            DiffractionCanvasCommand.navigate.rawValue,
            DiffractionCanvasCommand.evalJS.rawValue,
            DiffractionCanvasCommand.snapshot.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            DiffractionCanvasA2UICommand.reset.rawValue,
            DiffractionCanvasA2UICommand.push.rawValue,
            DiffractionCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([DiffractionScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([DiffractionSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([DiffractionChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            DiffractionWatchCommand.status.rawValue,
            DiffractionWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([DiffractionPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            DiffractionRemindersCommand.list.rawValue,
            DiffractionRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            DiffractionTalkCommand.pttStart.rawValue,
            DiffractionTalkCommand.pttStop.rawValue,
            DiffractionTalkCommand.pttCancel.rawValue,
            DiffractionTalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case DiffractionWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = DiffractionWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case DiffractionWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(DiffractionWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = DiffractionWatchNotifyPayload(
    ...
    func locationMode() -> DiffractionLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return DiffractionLocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to Diffraction."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to Diffraction."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "diffraction-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "Diffraction Share Self-Test",
            url: URL(string: "https://diffraction.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.diffractionPushKind(userInfo)
        let receivedMessage =
    ...
    private static func diffractionPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["diffraction"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["diffraction"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
