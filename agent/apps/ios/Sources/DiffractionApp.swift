...
@MainActor
final class DiffractionAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.diffraction.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.diffraction.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.diffraction.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "diffraction.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "diffraction.watch.promptId"
    static let sessionKeyKey = "diffraction.watch.sessionKey"
    static let actionPrimaryIDKey = "diffraction.watch.action.primary.id"
    static let actionPrimaryLabelKey = "diffraction.watch.action.primary.label"
    static let actionSecondaryIDKey = "diffraction.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "diffraction.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "diffraction.watch.action.primary"
    static let actionSecondaryIdentifier = "diffraction.watch.action.secondary"
    static let actionIdentifierPrefix = "diffraction.watch.action."
    static let actionIDKeyPrefix = "diffraction.watch.action.id."
    static let actionLabelKeyPrefix = "diffraction.watch.action.label."
    static let categoryPrefix = "diffraction.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: DiffractionWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> DiffractionWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return DiffractionWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Diffraction" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [DiffractionWatchAction]) -> [UNNotificationAction] {
...
@main
struct DiffractionApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(DiffractionAppDelegate.self) private var appDelegate
...
extension DiffractionApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("Diffraction: installing uncaught exception handler")
