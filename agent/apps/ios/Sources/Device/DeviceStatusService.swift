...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> DiffractionDeviceStatusPayload {
        ...
        return DiffractionDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> DiffractionDeviceInfoPayload {
        ...
        return DiffractionDeviceInfoPayload(
    ...
    private func batteryStatus() -> DiffractionBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: DiffractionBatteryState = switch device.batteryState {
        ...
        return DiffractionBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> DiffractionThermalStatusPayload {
        let state: DiffractionThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return DiffractionThermalStatusPayload(state: state)
    }

    private func storageStatus() -> DiffractionStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return DiffractionStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
