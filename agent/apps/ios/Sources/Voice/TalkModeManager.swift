...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__DIFFRACTION_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<DiffractionTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.diffraction", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = DiffractionTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> DiffractionTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return DiffractionTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> DiffractionTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> DiffractionTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return DiffractionTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> DiffractionTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = DiffractionTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: DiffractionTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: DiffractionAgentEventPayload.self
            ) else {
                continue
            }
