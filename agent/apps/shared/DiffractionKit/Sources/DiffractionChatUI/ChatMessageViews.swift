...
struct ChatMessageBubble: View {
    let message: DiffractionChatMessage
    let style: DiffractionChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? DiffractionChatTheme.userText : DiffractionChatTheme.assistantText
    ...
    private var inlineAttachments: [DiffractionChatMessageContent] {
    ...
    private var toolCalls: [DiffractionChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [DiffractionChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? DiffractionChatTheme.userBubble
        }
        if self.style == .onboarding {
            return DiffractionChatTheme.onboardingAssistantBubble
        }
        return DiffractionChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return DiffractionChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: DiffractionChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? DiffractionChatTheme.userText : DiffractionChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: DiffractionChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(DiffractionChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [DiffractionChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: DiffractionChatTheme.assistantText)
            }
        }
    }
}
