...
private let chatUILogger = Logger(subsystem: "ai.diffraction", category: "DiffractionChatUI")

@MainActor
@Observable
public final class DiffractionChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [DiffractionChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [DiffractionChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [DiffractionPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [DiffractionChatPendingToolCall] = []
    public private(set) var sessions: [DiffractionChatSessionEntry] = []
    private let transport: any DiffractionChatTransport
    private var sessionDefaults: DiffractionChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: DiffractionChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any DiffractionChatTransport,
    ...
    public var sessionChoices: [DiffractionChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [DiffractionChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: DiffractionPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [DiffractionChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: DiffractionChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: DiffractionChatMessage) -> DiffractionChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> DiffractionChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return DiffractionChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: DiffractionChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: DiffractionChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: DiffractionChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [DiffractionChatMessage],
        incoming: [DiffractionChatMessage]) -> [DiffractionChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [DiffractionChatMessage]) -> [DiffractionChatMessage] {
        var result: [DiffractionChatMessage] = []
    ...
    private static func dedupeKey(for message: DiffractionChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [DiffractionChatMessageContent] = [
            DiffractionChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> DiffractionChatAttachmentPayload in
            DiffractionChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> DiffractionChatSessionEntry {
        DiffractionChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = DiffractionChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: DiffractionChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: DiffractionChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: DiffractionAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = DiffractionChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> DiffractionPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
