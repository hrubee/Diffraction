...
@MainActor
public struct DiffractionChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: DiffractionChatViewModel
    ...
    public init(
        viewModel: DiffractionChatViewModel,
    ...
    public var body: some View {
        ZStack {
            if self.style == .standard {
                DiffractionChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    private var visibleMessages: [DiffractionChatMessage] {
        let base: [DiffractionChatMessage]
    ...
    private func mergeToolResults(in messages: [DiffractionChatMessage]) -> [DiffractionChatMessage] {
        ...
        for message in messages {
            ...
            let merged = DiffractionChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: DiffractionChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: DiffractionChatMessage) -> Bool {
    ...
    private func primaryText(in message: DiffractionChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: DiffractionChatMessage) -> Bool {
    ...
    private func toolCalls(in message: DiffractionChatMessage) -> [DiffractionChatMessageContent] {
    ...
    private func inlineToolResults(in message: DiffractionChatMessage) -> [DiffractionChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: DiffractionChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: DiffractionChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(DiffractionChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
