...
import Foundation
import Testing
@testable import DiffractionChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> DiffractionChatHistoryPayload
{
    DiffractionChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> DiffractionChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> DiffractionChatSessionEntry
{
    DiffractionChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> DiffractionChatModelChoice {
    DiffractionChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [DiffractionChatHistoryPayload],
    sessionsResponses: [DiffractionChatSessionsListResponse] = [],
    modelResponses: [[DiffractionChatModelChoice]] = [],
    ...
    -> (TestChatTransport, DiffractionChatViewModel)
{
...
private func sendUserMessage(_ vm: DiffractionChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, DiffractionChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [DiffractionChatHistoryPayload]
    private let sessionsResponses: [DiffractionChatSessionsListResponse]
    private let modelResponses: [[DiffractionChatModelChoice]]
    private let resetSessionHook: (@Sendable (String) async throws -> Void)?
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<DiffractionChatTransportEvent>
    private let continuation: AsyncStream<DiffractionChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<DiffractionChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> DiffractionChatHistoryPayload {
        ...
        return self.historyResponses.last ?? DiffractionChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [DiffractionChatAttachmentPayload]) async throws -> DiffractionChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return DiffractionChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> DiffractionChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? DiffractionChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: DiffractionChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = DiffractionChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = DiffractionChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: DiffractionChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = DiffractionChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: DiffractionChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(DiffractionChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == DiffractionChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = DiffractionChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: DiffractionChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = DiffractionChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = DiffractionChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = DiffractionChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { DiffractionChatViewModel(sessionKey: "main", transport: transport) }
