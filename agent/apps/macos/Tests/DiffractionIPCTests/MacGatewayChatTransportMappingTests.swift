...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: DiffractionProtocol.AnyCodable(["ok": DiffractionProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: DiffractionProtocol.AnyCodable(["ok": DiffractionProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = DiffractionProtocol.AnyCodable([
            "runId": DiffractionProtocol.AnyCodable("run-1"),
            "sessionKey": DiffractionProtocol.AnyCodable("main"),
            "state": DiffractionProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: DiffractionProtocol.AnyCodable(["a": DiffractionProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
