...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func `gateway entrypoint prefers dist over bin`() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/diffraction.js")
    ...
    @Test func `log locator picks newest log file`() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/diffraction", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("diffraction-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("diffraction-new-\(UUID().uuidString).log")
