...
struct VoiceWakeRuntimeTests {
    @Test func `trims after trigger keeps post speech`() {
        let triggers = ["claude", "diffraction"]
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["diffraction"]
        let text = "hey diffraction"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey diffraction do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["diffraction"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "diffraction", start: 0.2, duration: 0.1, range: transcript.range(of: "diffraction")),
