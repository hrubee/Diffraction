...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: DiffractionProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: DiffractionProtocol.AnyCodable] = [
            "presence": .init([DiffractionProtocol.AnyCodable(entry)]),
        ]
        let payload = DiffractionProtocol.AnyCodable(payloadMap)
