...
enum DiffractionLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = DiffractionOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = DiffractionFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        DiffractionLogging.bootstrapIfNeeded()
        let label = DiffractionLogging.makeLabel(subsystem: subsystem, category: category)
...
struct DiffractionOSLogHandler: AppLogLevelBackedHandler {
...
struct DiffractionFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = DiffractionLogging.parseLabel(self.label)
