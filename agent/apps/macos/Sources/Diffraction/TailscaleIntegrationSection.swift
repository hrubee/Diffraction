...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.diffraction/diffraction.json. Prefer DIFFRACTION_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.diffraction/diffraction.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.diffraction/diffraction.json. Restart the gateway to apply."
        }
