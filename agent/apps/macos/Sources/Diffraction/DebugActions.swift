...
enum DebugActions {
    private static let verboseDefaultsKey = "diffraction.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "diffraction.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = DiffractionPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Diffraction", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = DiffractionPaths.configURL
        guard
