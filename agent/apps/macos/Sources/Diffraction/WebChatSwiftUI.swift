...
private let webChatSwiftLogger = Logger(subsystem: "ai.diffraction", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "diffraction.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: DiffractionChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> DiffractionChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(DiffractionChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            DiffractionChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return DiffractionChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> DiffractionChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                DiffractionGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    DiffractionChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    DiffractionAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: DiffractionProtocol.ModelChoice) -> DiffractionChatModelChoice {
        DiffractionChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<DiffractionChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any DiffractionChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = DiffractionChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: DiffractionChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<DiffractionChatView>) -> NSViewController
    {
