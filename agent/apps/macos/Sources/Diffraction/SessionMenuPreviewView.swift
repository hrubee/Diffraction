...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.diffraction", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> DiffractionSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: DiffractionSessionPreviewEntry,
    ...
    private static func cache(payload: DiffractionSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: DiffractionChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [DiffractionKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [DiffractionKit.AnyCodable]) -> [DiffractionChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(DiffractionChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: DiffractionChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: DiffractionChatMessage) -> Bool {
    ...
    private static func toolNames(for message: DiffractionChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: DiffractionChatMessage) -> String? {
