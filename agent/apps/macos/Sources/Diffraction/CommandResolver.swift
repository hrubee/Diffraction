...
enum CommandResolver {
    private static let projectRootDefaultsKey = "diffraction.gatewayProjectRootPath"
    private static let helperName = "diffraction"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let diffractionEntry = root.appendingPathComponent("diffraction.mjs").path
        if FileManager().isReadableFile(atPath: diffractionEntry) { return diffractionEntry }
        let binEntry = root.appendingPathComponent("bin/diffraction.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/diffraction")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let diffractionPaths = self.diffractionManagedPaths(home: home)
        if !diffractionPaths.isEmpty {
            extras.insert(contentsOf: diffractionPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + diffractionPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func diffractionManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".diffraction"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func diffractionExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectDiffractionExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("diffraction.mjs").path,
            root.appendingPathComponent("bin/diffraction.js").path,
        ]
    ...
    static func hasAnyDiffractionInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.diffractionExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func diffractionNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let diffractionPath = self.projectDiffractionExecutable(projectRoot: root) {
            return [diffractionPath, subcommand] + extraArgs
        }
        if let diffractionPath = self.diffractionExecutable(searchPaths: searchPaths) {
            return [diffractionPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "diffraction", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            diffraction entrypoint missing (looked for dist/index.js or diffraction.mjs); run pnpm build.
            """
    ...
    static func diffractionCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real diffraction CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/diffraction"
        ...
        \(cliSection)
        \(projectSection)
        if command -v diffraction >/dev/null 2>&1; then
          CLI="$(command -v diffraction)"
          diffraction \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/diffraction.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/diffraction.mjs"
            node "$PRJ/diffraction.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/diffraction.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/diffraction.js"
            node "$PRJ/bin/diffraction.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent diffraction"
          pnpm --silent diffraction \(quotedArgs);
        else
          echo "diffraction CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? DiffractionConfigFile.loadDict()
