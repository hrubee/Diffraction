    ...
    func welcomePage() -> some View {
        self.onboardingPage {
            VStack(spacing: 22) {
                ...
                Text("Diffraction is a powerful personal AI assistant that can connect to WhatsApp or Telegram.")
                    .font(.body)
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Text(
                                "The connected AI agent (e.g. Claude) can trigger powerful actions on your Mac, " +
                                    ...
                                    "Only enable Diffraction if you understand the risks and trust the prompts and " +
    ...
    func connectionPage() -> some View {
        self.onboardingPage {
            Text("Choose your Gateway")
                .font(.largeTitle.weight(.semibold))
            Text(
                "Diffraction uses a single Gateway that stays running. Pick this Mac, " +
                    ...
                    if self.state.remoteTransport == .ssh {
                        ...
                        GridRow {
                            Text("Project root")
                                .font(.callout.weight(.semibold))
                                .frame(width: labelWidth, alignment: .leading)
                            TextField("/home/you/Projects/diffraction", text: self.$state.remoteProjectRoot)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: fieldWidth)
                        }
                        GridRow {
                            Text("CLI path")
                                ...
                                .frame(width: labelWidth, alignment: .leading)
                            TextField(
                                "/Applications/Diffraction.app/.../diffraction",
    ...
    private func remoteTokenField() -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if self.state.remoteTokenUnsupported {
                Text(
                    "The current gateway.remote.token value is not plain text. Diffraction for macOS cannot use it directly; enter a plaintext token here to replace it.")
                    .font(.caption)
    ...
    func permissionsPage() -> some View {
        self.onboardingPage {
            Text("Grant permissions")
                .font(.largeTitle.weight(.semibold))
            Text("These macOS permissions let Diffraction automate apps and capture context on this Mac.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func cliPage() -> some View {
        self.onboardingPage {
            Text("Install the CLI")
                .font(.largeTitle.weight(.semibold))
            Text("Required for local mode: installs `diffraction` so launchd can run the gateway.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func workspacePage() -> some View {
        self.onboardingPage {
            Text("Agent workspace")
                .font(.largeTitle.weight(.semibold))
            Text(
                "Diffraction runs the agent from a dedicated workspace so it can load `AGENTS.md` " +
                ...
                if self.state.connectionMode == .remote {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Workspace folder")
                            .font(.headline)
                        TextField(
                            AgentWorkspace.displayPath(for: DiffractionConfigFile.defaultWorkspaceURL()),
                                ...
                                Task {
                                    ...
                                    if saved {
                                        self.workspaceStatus =
                                            "Saved to ~/.diffraction/diffraction.json (agents.defaults.workspace)"
                                    }
                                }
                            }
                            .buttonStyle(.bordered)
    ...
    func onboardingChatPage() -> some View {
        VStack(spacing: 16) {
            ...
            self.onboardingGlassCard(padding: 8) {
                DiffractionChatView(viewModel: self.onboardingChatModel, style: .onboarding)
            ...
            self.onboardingCard {
                ...
                if self.state.connectionMode == .remote {
                    self.featureRow(
                        ...
                        subtitle: """
                        On your gateway host: install/update the `diffraction` package and make sure credentials exist
                        (typically `~/.diffraction/credentials/oauth.json`). Then connect again if needed.
                        """,
                        systemImage: "network")
                    Divider()
                        .padding(.vertical, 6)
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the Diffraction menu bar icon for quick chat and status.",
