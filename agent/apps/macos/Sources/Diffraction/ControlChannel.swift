...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: DiffractionProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.diffraction", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: DiffractionKit.AnyCodable]()) {
                $0[$1.key] = DiffractionKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: DiffractionProtocol.AnyCodable?) -> [String: DiffractionProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: DiffractionProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: DiffractionKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: DiffractionProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("diffraction.control.heartbeat")
    static let controlAgentEvent = Notification.Name("diffraction.control.agent")
}
