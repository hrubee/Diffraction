...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "ai.diffraction", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "ai.diffraction", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [DiffractionChatMessage] = messages.compactMap { item in
