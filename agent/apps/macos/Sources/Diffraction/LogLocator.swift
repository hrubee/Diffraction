...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["DIFFRACTION_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/diffraction")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("diffraction-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("diffraction-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/diffraction/ (rolling or stdout), or nil if none exist.
