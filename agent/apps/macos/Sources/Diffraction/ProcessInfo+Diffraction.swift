...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.diffraction.mac`) even if the shipped
    ...
    {
        if environment["DIFFRACTION_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "diffraction.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "diffraction.nixMode") { return true }
