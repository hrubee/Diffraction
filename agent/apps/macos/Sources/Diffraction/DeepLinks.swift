...
private let deepLinkLogger = Logger(subsystem: "ai.diffraction", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Diffraction is paused", message: "Unpause Diffraction to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Diffraction agent?", message: body) else { return }
        }
