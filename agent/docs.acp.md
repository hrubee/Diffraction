# Diffraction ACP Bridge

This document describes how the Diffraction ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`diffraction acp` exposes an ACP agent over stdio and forwards prompts to a running
Diffraction Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Diffraction Gateway
...
- ACP client spawns `diffraction acp` and speaks ACP messages over stdio.
...
`diffraction acp` resolves the Gateway URL and auth from CLI flags or config:
