...
describe("irc setup wizard", () => {
  it("configures host and nick via setup prompts", async () => {
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("diffraction-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#diffraction", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#diffraction", "#ops"]);
  });
