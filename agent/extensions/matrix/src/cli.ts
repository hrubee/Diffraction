...
import { isDiffractionManagedMatrixDevice } from "./matrix/device-health.js";
import {
...
function formatMatrixCliCommand(command: string, accountId?: string): string {
  ...
  return `diffraction matrix ${command}${suffix}`;
}
...
type MatrixCliAccountAddResult = {
  ...
  deviceHealth: {
    ...
    staleDiffractionDeviceIds: string[];
    error?: string;
  };
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleDiffractionDeviceIds: addedDevices
        .filter((device) => !device.current && isDiffractionManagedMatrixDevice(device.displayName))
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.diffraction.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleDiffractionDeviceIds.length > 0) {
              console.log(
                `Matrix device hygiene warning: stale Diffraction devices detected (${result.deviceHealth.staleDiffractionDeviceIds.join(", ")}). Run 'diffraction matrix devices prune-stale --account ${result.accountId}'.`,
              );
            }
            ...
            const bindHint = `diffraction agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale Diffraction-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
      ...
      await runMatrixCliCommand({
        ...
        onText: (result, verbose) => {
          printAccountLabel(accountId);
          console.log(
            `Deleted stale Diffraction devices: ${result.deletedDeviceIds.length ? result.deletedDeviceIds.join(", ") : "none"}`,
          );
