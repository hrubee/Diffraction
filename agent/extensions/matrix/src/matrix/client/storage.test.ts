...
const createBackupArchiveMock = vi.hoisted(() =>
  vi.fn(async (_params: unknown) => ({
    ...
    archiveRoot: "2026-03-17-diffraction-backup",
...
describe("matrix client storage paths", () => {
  ...
  function setupStateDir(
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-matrix-storage-"));
    const stateDir = path.join(homeDir, ".diffraction");
  ...
  function createMigrationEnv(stateDir: string): NodeJS.ProcessEnv {
    return {
      ...
      DIFFRACTION_HOME: path.dirname(stateDir),
      DIFFRACTION_STATE_DIR: stateDir,
      DIFFRACTION_TEST_FAST: "1",
