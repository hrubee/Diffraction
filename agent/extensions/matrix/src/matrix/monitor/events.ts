...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Diffraction runtime is using the same account. " +
    `Check 'diffraction matrix verify status --verbose --account ${accountId}' and 'diffraction matrix devices list --account ${accountId}'.`
  );
}
