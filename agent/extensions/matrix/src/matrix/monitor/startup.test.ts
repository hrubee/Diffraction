...
const hoisted = vi.hoisted(() => ({
  ...
  summarizeMatrixDeviceHealth: vi.fn(() => ({
    staleDiffractionDevices: [] as Array<{ deviceId: string }>,
  })),
...
describe("runMatrixStartupMaintenance", () => {
  beforeEach(() => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockClear().mockReturnValue({ staleDiffractionDevices: [] });
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    hoisted.summarizeMatrixDeviceHealth.mockReturnValue({
      staleDiffractionDevices: [{ deviceId: "DEV123" }],
    });
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale Diffraction devices detected for @bot:example.org: DEV123. Run 'diffraction matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
      "matrix: device not verified — run 'diffraction matrix verify device <key>' to enable E2EE",
    );
    expect(params.logger.info).toHaveBeenCalledWith(
