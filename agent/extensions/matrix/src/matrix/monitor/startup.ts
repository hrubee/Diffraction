...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleDiffractionDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Diffraction devices detected for ${params.auth.userId}: ${deviceHealth.staleDiffractionDevices.map((device) => device.deviceId).join(", ")}. Run 'diffraction matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
