import { resolveDefaultModelForAgent } from "diffraction/plugin-sdk/agent-runtime";
import type { DiffractionConfig } from "diffraction/plugin-sdk/config-runtime";
import { resetInboundDedupe } from "diffraction/plugin-sdk/reply-runtime";
import type { MsgContext } from "diffraction/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "diffraction/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "diffraction/plugin-sdk/reply-runtime";
import type { MockFn } from "diffraction/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("diffraction/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("diffraction/plugin-sdk/config-runtime").resolveStorePath;
type TelegramBotRuntimeForTest = NonNullable<
  Parameters<typeof import("./bot.js").setTelegramBotRuntimeForTest>[0]
>;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("diffraction/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/diffraction-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("diffraction/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("diffraction/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("diffraction/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("diffraction/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("diffraction/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
const replySpyHoisted = vi.hoisted(() => ({
  replySpy: vi.fn(async (_ctx: MsgContext, opts?: GetReplyOptions) => {
    ...
    (
      ...
      configOverride?: DiffractionConfig,
...
function createModelsProviderDataFromConfig(cfg: DiffractionConfig): {
...
vi.doMock("diffraction/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("diffraction/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("diffraction/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("diffraction/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("diffraction/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: DiffractionConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: DiffractionConfig) => {
