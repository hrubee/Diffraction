...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): DiffractionConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as DiffractionConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): DiffractionConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as DiffractionConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: DiffractionConfig, accountId = "media") {
    await handleTelegramAction(
