...
import type { ChannelSetupDmPolicy, ChannelSetupWizard } from "diffraction/plugin-sdk/setup";
...
function shouldShowTelegramDmAccessWarning(cfg: DiffractionConfig, accountId: string): boolean {
...
function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    "  " + formatCliCommand(`diffraction config set ${configBase}.dmPolicy "allowlist"`),
    "  " + formatCliCommand(`diffraction config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`),
