...
vi.mock("diffraction/plugin-sdk/agent-runtime", () => ({
...
vi.mock("diffraction/plugin-sdk/media-runtime", () => ({
  AUTO_IMAGE_KEY_PROVIDERS: ["openai"],
  DEFAULT_IMAGE_MODELS: { openai: "gpt-4.1-mini" },
  resolveAutoImageModel: vi.fn(async () => null),
}));

vi.mock("diffraction/plugin-sdk/media-understanding-runtime", () => ({
  describeImageFileWithModel: vi.fn(),
}));

const TEST_CACHE_DIR = "/tmp/diffraction-test-sticker-cache/telegram";
...
describe("sticker-cache", () => {
  beforeEach(async () => {
    process.env.DIFFRACTION_STATE_DIR = "/tmp/diffraction-test-sticker-cache";
    fs.rmSync("/tmp/diffraction-test-sticker-cache", { recursive: true, force: true });
