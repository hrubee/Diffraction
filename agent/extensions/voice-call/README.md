# @diffraction/voice-call

Official Voice Call plugin for **Diffraction**.
...
Docs: `https://docs.diffraction.ai/plugins/voice-call`
Plugin system: `https://docs.diffraction.ai/plugin`

## Install (local dev)

### Option A: install via Diffraction (recommended)
...
```bash
mkdir -p ~/.diffraction/extensions
cp -R extensions/voice-call ~/.diffraction/extensions/voice-call
cd ~/.diffraction/extensions/voice-call && pnpm install
```
...
- advanced webhook, streaming, and tunnel notes: `https://docs.diffraction.ai/plugins/voice-call`

## Stale call reaper

See the plugin docs for recommended ranges and production examples:
`https://docs.diffraction.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.diffraction.ai/plugins/voice-call#tts-for-calls`

## CLI

```bash
diffraction voicecall call --to "+15555550123" --message "Hello from Diffraction"
diffraction voicecall continue --call-id <id> --message "Any questions?"
diffraction voicecall speak --call-id <id> --message "One moment"
diffraction voicecall end --call-id <id>
diffraction voicecall status --call-id <id>
diffraction voicecall tail
