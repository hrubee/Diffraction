...
function createAgentRuntime(payloads: Array<Record<string, unknown>>) {
  ...
  const runtime = {
    ...
    resolveAgentDir: () => "/tmp/diffraction/agents/main",
    resolveAgentWorkspaceDir: () => "/tmp/diffraction/workspace/main",
    ...
    runEmbeddedPiAgent,
    session: {
      resolveStorePath: () => "/tmp/diffraction/sessions.json",
      loadSessionStore: () => ({}),
      saveSessionStore: async () => {},
      resolveSessionFilePath: () => "/tmp/diffraction/sessions/session.jsonl",
    },
