...
import { normalizeSecretInput } from "diffraction/plugin-sdk/provider-auth";
...
type WebSearchConfig = NonNullable<DiffractionConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<DiffractionConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: DiffractionConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: DiffractionConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: DiffractionConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: DiffractionConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: DiffractionConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: DiffractionConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: DiffractionConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: DiffractionConfig, override?: number): number {
