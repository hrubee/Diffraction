...
describe("openshell remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("diffraction-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("diffraction-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("diffraction-openshell-remote-agent-");
