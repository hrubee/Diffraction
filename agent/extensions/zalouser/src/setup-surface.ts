...
function ensureZalouserPluginEnabled(cfg: DiffractionConfig): DiffractionConfig {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("diffraction directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
  ...
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ...
        : resolveDefaultZalouserAccountId(cfg as DiffractionConfig);
    return await promptZalouserAllowFrom({
      cfg: cfg as DiffractionConfig,
      prompter,
      accountId: id,
    });
  },
...
export const zalouserSetupWizard: ChannelSetupWizard = {
  ...
  groupAccess: {
    ...
    setPolicy: ({ cfg, accountId, policy }) =>
      setZalouserGroupPolicy(cfg as DiffractionConfig, accountId, policy),
    resolveAllowlist: async ({ cfg, accountId, entries, prompter }) => {
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("diffraction directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
      ...
      const updatedAccount = resolveZalouserAccountSync({ cfg: cfg as DiffractionConfig, accountId });
      try {
    ...
    applyAllowlist: ({ cfg, accountId, resolved }) =>
      setZalouserGroupAllowlist(cfg as DiffractionConfig, accountId, resolved as string[]),
  },
