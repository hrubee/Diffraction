...
import type { DiffractionPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/diffraction-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): DiffractionPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("diffraction.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("diffraction.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("diffraction.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("diffraction.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("diffraction.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("diffraction.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("diffraction.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("diffraction.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("diffraction.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("diffraction.webhook.processed");
    expect(spanNames).toContain("diffraction.message.processed");
    expect(spanNames).toContain("diffraction.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["diffraction.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("diffraction.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "diffraction.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["diffraction.reason"]).toBe("string");
    expect(String(attrs?.["diffraction.reason"])).not.toContain(
