...
vi.mock("diffraction/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("diffraction/plugin-sdk/media-runtime")>(
    "diffraction/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `diffraction-inbound-media-${crypto.randomUUID()}`);
