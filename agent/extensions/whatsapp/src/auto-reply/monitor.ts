import { resolveInboundDebounceMs } from "diffraction/plugin-sdk/channel-inbound";
import { formatCliCommand } from "diffraction/plugin-sdk/cli-runtime";
import { waitForever } from "diffraction/plugin-sdk/cli-runtime";
...
import { formatDurationPrecise } from "diffraction/plugin-sdk/infra-runtime";
import { enqueueSystemEvent } from "diffraction/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "diffraction/plugin-sdk/reply-history";
import { getReplyFromConfig } from "diffraction/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "diffraction/plugin-sdk/routing";
import { logVerbose } from "diffraction/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "diffraction/plugin-sdk/runtime-env";
import { getChildLogger } from "diffraction/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "diffraction/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("diffraction channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("diffraction channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
