...
import { resolvePreferredDiffractionTmpDir } from "../../../src/infra/tmp-diffraction-dir.js";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredDiffractionTmpDir(), "diffraction-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub DIFFRACTION_STATE_DIR.
    // Also keep it outside the Diffraction temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["DIFFRACTION_STATE_DIR"]);
    process.env.DIFFRACTION_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "diffraction-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredDiffractionTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Diffraction state workspace and sandbox roots", async () => {
  ...
  it("rejects default Diffraction state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
