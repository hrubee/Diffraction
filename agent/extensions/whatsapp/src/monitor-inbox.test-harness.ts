...
import { resetLogger, setLoggerOverride } from "diffraction/plugin-sdk/runtime-env";
...
vi.mock("diffraction/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/media-runtime")>();
  return {
    ...actual,
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "diffraction-auth-"));
    } else {
      authDir = undefined;
    }
  });
