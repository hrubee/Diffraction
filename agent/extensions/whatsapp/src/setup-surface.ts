...
import type { ChannelSetupWizard } from "diffraction/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "diffraction/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  ...
  patch: Partial<NonNullable<NonNullable<DiffractionConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: DiffractionConfig, dmPolicy: DmPolicy): DiffractionConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: DiffractionConfig, allowFrom?: string[]): DiffractionConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: DiffractionConfig, selfChatMode: boolean): DiffractionConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: DiffractionConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Diffraction can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Diffraction" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("diffraction channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
