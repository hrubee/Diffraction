...
const CONFIG_KEY = Symbol.for("diffraction:testConfigMock");
...
vi.mock("diffraction/plugin-sdk/state-paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/state-paths")>();
  return {
    ...actual,
    resolveOAuthDir: () => "/tmp/diffraction-oauth",
  };
});

vi.mock("@whiskeysockets/baileys", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("diffraction:lastSocket")] =
    created.lastSocket;
  return created.mod;
});
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("diffraction:lastSocket")];
  if (typeof getter === "function") {
