...
const resolveWhatsAppAuthDirMock = vi.hoisted(() =>
  vi.fn(() => ({
    authDir: "/tmp/diffraction-whatsapp-test",
  })),
);
...
describe("whatsapp setup wizard", () => {
  beforeEach(() => {
    ...
    resolveWhatsAppAuthDirMock.mockReturnValue({ authDir: "/tmp/diffraction-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("diffraction channels login"),
      "WhatsApp",
    );
  });
