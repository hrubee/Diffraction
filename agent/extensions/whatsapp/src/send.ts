...
import { generateSecureUuid } from "diffraction/plugin-sdk/infra-runtime";
...
import { getChildLogger } from "diffraction/plugin-sdk/text-runtime";
...
import { markdownToWhatsApp } from "diffraction/plugin-sdk/text-runtime";
import { toWhatsappJid } from "diffraction/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: DiffractionConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
