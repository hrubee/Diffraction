...
import { VERSION } from "diffraction/plugin-sdk/cli-runtime";
import { danger, success } from "diffraction/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "diffraction/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "diffraction/plugin-sdk/text-runtime";
...
export async function createWaSocket(
  ...
  const sock = makeWASocket({
    ...
    browser: ["diffraction", "cli", VERSION],
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === DisconnectReason.loggedOut) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("diffraction channels login")}`,
              ),
            );
          }
        }
