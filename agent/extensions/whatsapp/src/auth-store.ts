...
import { info, success } from "diffraction/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "diffraction/plugin-sdk/state-paths";
import type { WebChannel } from "diffraction/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "diffraction/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("diffraction channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
