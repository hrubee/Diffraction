...
vi.mock("diffraction/plugin-sdk/routing", async (importOriginal) => {
  const actual = await importOriginal<typeof import("diffraction/plugin-sdk/routing")>();
  return {
    ...actual,
...
export function resetSlackSlashMocks() {
  ...
  mocks.resolveStorePathMock.mockReset().mockReturnValue("/tmp/diffraction-sessions.json");
}
