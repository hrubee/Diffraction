import type { SlackSlashCommandConfig } from "diffraction/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "diffraction");
  const name = normalizedName || "diffraction";
  return {
