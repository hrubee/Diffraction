...
// Prefix for Diffraction-generated action IDs to scope our handler
const DIFFRACTION_ACTION_PREFIX = "diffraction:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${DIFFRACTION_ACTION_PREFIX}`);

  // Handle Diffraction modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
