import { readStoreAllowFromForDmPolicy } from "diffraction/plugin-sdk/security-runtime";
import {
  allowListMatches,
  normalizeAllowList,
...
function getPairingAllowFromCacheTtlMs(): number {
  const raw = process.env.DIFFRACTION_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS?.trim();
  if (!raw) {
