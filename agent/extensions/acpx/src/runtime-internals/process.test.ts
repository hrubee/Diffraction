...
import { createWindowsCmdShimFixture } from "diffraction/plugin-sdk/testing";
...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "diffraction-acpx-process-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("spawnAndCollect", () => {
  ...
  async function collectSpawnedEnvSnapshot(options?: {
    ...
    const result = await spawnAndCollect({
      ...
      args: [
        "-e",
        `process.stdout.write(JSON.stringify({openai:process.env.${openAiEnvKey},github:process.env.${githubEnvKey},hf:process.env.${hfEnvKey},diffraction:process.env.DIFFRACTION_API_KEY,shell:process.env.DIFFRACTION_SHELL}))`,
      ],
      cwd: process.cwd(),
  ...
  it("strips shared provider auth env vars from spawned acpx children", async () => {
    ...
    expect(parsed.diffraction).toBe("keep-me");
