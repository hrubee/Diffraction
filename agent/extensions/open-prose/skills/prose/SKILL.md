...
metadata: { "diffraction": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Diffraction `sessions_spawn`
- **File I/O** == Diffraction `read`/`write`
- **Remote fetch** == Diffraction `web_fetch` (or `exec` with curl when POST is required)
