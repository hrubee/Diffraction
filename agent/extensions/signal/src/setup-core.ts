...
export const signalDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: DiffractionConfig) => cfg.channels?.signal?.dmPolicy ?? "pairing",
...
export const signalCompletionNote = {
  ...
  lines: [
    'Link device with: signal-cli link -n "Diffraction"',
    "Scan QR in Signal -> Linked Devices",
    `Then run: ${formatCliCommand("diffraction gateway call channels.status --params '{\"probe\":true}'")}`,
    `Docs: ${formatDocsLink("/signal", "signal")}`,
  ],
};
