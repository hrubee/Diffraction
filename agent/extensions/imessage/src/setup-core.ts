...
export const imessageDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: DiffractionConfig) => cfg.channels?.imessage?.dmPolicy ?? "pairing",
  setPolicy: (cfg: DiffractionConfig, policy) =>
    setChannelDmPolicyWithAllowFrom({
      cfg,
      channel,
      dmPolicy: policy,
...
function resolveIMessageCliPath(params: { cfg: DiffractionConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    ...
    "Ensure Diffraction has Full Disk Access to Messages DB.",
