...
import type { DiffractionPluginApi, DiffractionPluginToolContext } from "../runtime-api.js";
import {
...
function fakeApi(overrides: Partial<DiffractionPluginApi> = {}): DiffractionPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<DiffractionPluginToolContext> = {}): DiffractionPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: DiffractionPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
