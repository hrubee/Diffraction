...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredDiffractionTmpDir: vi.fn(() => path.join(os.tmpdir(), "diffraction-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredDiffractionTmpDir: pluginApiMocks.resolvePreferredDiffractionTmpDir,
...
function createApi(params?: {
  runtime?: DiffractionPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as DiffractionPluginApi["runtime"],
    registerCommand: params?.registerCommand,
  }) as DiffractionPluginApi;
}
...
function registerPairCommand(params?: {
  ...
  let command: DiffractionPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as DiffractionPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredDiffractionTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredDiffractionTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Diffraction iOS app:");
    expect(text).toContain("![Diffraction pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Diffraction iOS app:");
