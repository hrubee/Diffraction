...
async function writeQrPngTempFile(data: string): Promise<string> {
  const pngBase64 = await renderQrPngBase64(data);
  const tmpRoot = resolvePreferredDiffractionTmpDir();
...
type QrChannelSender = {
  resolveSend: (api: DiffractionPluginApi) => QrSendFn | undefined;
  createOpts: (params: {
...
function resolveGatewayPort(cfg: DiffractionPluginApi["config"]): number {
  const envPort = parsePositiveInteger(process.env.DIFFRACTION_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveScheme(
  cfg: DiffractionPluginApi["config"],
...
function resolveAuthLabel(cfg: DiffractionPluginApi["config"]): ResolveAuthLabelResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    pickFirstDefined([process.env.DIFFRACTION_GATEWAY_TOKEN, cfg.gateway?.auth?.token]) ?? undefined;
  const password =
    pickFirstDefined([process.env.DIFFRACTION_GATEWAY_PASSWORD, cfg.gateway?.auth?.password]) ??
    undefined;
...
async function resolveGatewayUrl(api: DiffractionPluginApi): Promise<ResolveUrlResult> {
...
export default definePluginEntry({
  id: "device-pair",
  name: "Device Pair",
  description: "QR/bootstrap pairing helpers for Diffraction devices",
      ...
      handler: async (ctx) => {
        ...
        if (action === "qr") {
          ...
          if (target && canSendQrPngToChannel(channel)) {
            ...
            try {
              ...
              const sent = await sendQrPngToSupportedChannel({
                ...
                ctx,
                target,
                caption: ["Scan this QR code with the Diffraction iOS app:", "", ...infoLines].join(
                  "\n",
                ),
                qrFilePath,
              });
          ...
          if (channel === "webchat") {
            ...
            return {
              text: [
                "Scan this QR code with the Diffraction iOS app:",
                "",
                ...
                "",
                `![Diffraction pairing QR](${qrDataUrl})`,
              ].join("\n"),
            };
          }
