...
`diffraction.invoke --each`) without adding new Diffraction code per workflow.
...
This extension depends on Diffraction internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Diffraction extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.diffraction/extensions` as a standalone plugin directory.
