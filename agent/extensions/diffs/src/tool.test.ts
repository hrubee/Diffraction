...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("diffraction-diffs-tool-"));
  });
...
function createApi(): DiffractionPluginApi {
  return createTestPluginApi({
    ...
    runtime: {} as DiffractionPluginApi["runtime"],
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: DiffractionPluginToolContext | undefined = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",
