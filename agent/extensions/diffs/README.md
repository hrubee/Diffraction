...
Read-only diff viewer plugin for **Diffraction** agents.
...
Set plugin-wide defaults in `~/.diffraction/diffraction.json`:
...
Before:
Diffraction supports plugins.

After:
Diffraction supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/diffraction-diffs`).
