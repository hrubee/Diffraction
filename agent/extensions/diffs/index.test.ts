...
import type { DiffractionPluginApi, DiffractionPluginToolContext } from "./api.js";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<DiffractionPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: DiffractionPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<DiffractionPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as DiffractionPluginApi);
