...
export function createDiffractFsBridge(params: {
  sandbox: SandboxContext;
  backend: DiffractSandboxBackend;
}): SandboxFsBridge {
  return new DiffractFsBridge(params.sandbox, params.backend);
}

class DiffractFsBridge implements SandboxFsBridge {
  constructor(
    private readonly sandbox: SandboxContext,
    private readonly backend: DiffractSandboxBackend,
  ) {}
  ...
  async writeFile(params: {
    ...
    const tempPath = path.join(
      parentDir,
      `.diffraction-diffract-write-${path.basename(hostPath)}-${process.pid}-${Date.now()}`,
    );
  ...
  private requireHostPath(target: ResolvedMountPath): string {
    if (!target.hostPath) {
      throw new Error(
        `Diffract mirror bridge requires a local host path: ${target.containerPath}`,
      );
    }
