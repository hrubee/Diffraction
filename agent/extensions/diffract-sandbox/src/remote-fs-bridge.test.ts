...
import type { DiffractSandboxBackend } from "./backend.js";
import { createDiffractRemoteFsBridge } from "./remote-fs-bridge.js";
...
function createBackendMock(roots: { workspace: string; agent: string }): DiffractSandboxBackend {
  return {
    id: "diffract",
    runtimeId: "diffract-test",
    runtimeLabel: "diffract-test",
    workdir: "/sandbox",
    env: {},
    mode: "remote",
  ...
  } as unknown as DiffractSandboxBackend;
}
...
describe("diffract remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("diffraction-diffract-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("diffraction-diffract-remote-workspace-");
    const remoteAgentDir = await makeTempDir("diffraction-diffract-remote-agent-");
    ...
    const bridge = createDiffractRemoteFsBridge({ sandbox, backend });
