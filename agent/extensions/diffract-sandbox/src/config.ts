...
import type { DiffractPluginConfigSchema } from "diffraction/plugin-sdk/core";

export type DiffractPluginConfig = {
  mode?: string;
  command?: string;
  gateway?: string;
...
export type ResolvedDiffractPluginConfig = {
  mode: "mirror" | "remote";
  command: string;
  gateway?: string;
  gatewayEndpoint?: string;
...
function normalizeRemotePath(value: string | undefined, fallback: string): string {
  ...
  if (!normalized.startsWith("/")) {
    throw new Error(`Diffract remote path must be absolute: ${candidate}`);
  }
  return normalized;
}

export function createDiffractPluginConfigSchema(): DiffractPluginConfigSchema {
...
export function resolveDiffractPluginConfig(value: unknown): ResolvedDiffractPluginConfig {
  const parsed = createDiffractPluginConfigSchema().safeParse?.(value);
  if (!parsed || !parsed.success) {
    ...
    throw new Error(`Invalid diffract plugin config: ${message}`);
  }
  const raw = parsed.data ?? {};
  const cfg = (raw ?? {}) as DiffractPluginConfig;
  const mode = cfg.mode ?? DEFAULT_MODE;
  if (mode !== "mirror" && mode !== "remote") {
    throw new Error(`Invalid diffract plugin config: mode must be one of mirror, remote`);
  }
  return {
    mode,
