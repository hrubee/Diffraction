...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-diffract-fs-"));
  tempDirs.push(dir);
  return dir;
}
...
function createBackendMock(): DiffractSandboxBackend {
  return {
    id: "diffract",
...
describe("diffract fs bridge", () => {
  it("writes locally and syncs the file to the remote workspace", async () => {
    ...
    const bridge = createDiffractFsBridge({ sandbox, backend });
