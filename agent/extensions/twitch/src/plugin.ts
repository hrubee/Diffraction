/**
 * Twitch channel plugin for Diffraction.
 *
...
} from "diffraction/plugin-sdk/channel-pairing";
...
import type { DiffractionConfig } from "../api.js";
 ...
 * for Diffraction. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: DiffractionConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: DiffractionConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        isConfigured: (_account: unknown, cfg: DiffractionConfig): boolean =>
