...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: DIFFRACTION_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: DiffractionConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var DIFFRACTION_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
function resolveTwitchGroupPolicy(cfg: DiffractionConfig): "open" | "allowlist" | "disabled" {
  const account = getAccountConfig(cfg, DEFAULT_ACCOUNT_ID);
...
const twitchDmPolicy: ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg) => {
    const account = getAccountConfig(cfg as DiffractionConfig, DEFAULT_ACCOUNT_ID);
    if (account?.allowedRoles?.includes("all")) {
      return "open";
    }
  ...
  setPolicy: (cfg, policy) => {
    ...
    return setTwitchAccessControl(cfg as DiffractionConfig, allowedRoles, true);
  },
  promptAllowFrom: async ({ cfg, prompter }) => {
    ...
    return setTwitchAccount(cfg as DiffractionConfig, {
      ...(account ?? undefined),
      allowFrom,
    });
  },
...
const twitchGroupAccess: NonNullable<ChannelSetupWizard["groupAccess"]> = {
  ...
  currentPolicy: ({ cfg }) => resolveTwitchGroupPolicy(cfg as DiffractionConfig),
  currentEntries: ({ cfg }) => {
    const account = getAccountConfig(cfg as DiffractionConfig, DEFAULT_ACCOUNT_ID);
  ...
  setPolicy: ({ cfg, policy }) => setTwitchGroupPolicy(cfg as DiffractionConfig, policy),
  resolveAllowlist: async () => [],
  applyAllowlist: ({ cfg }) => cfg as DiffractionConfig,
};
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.DIFFRACTION_TWITCH_ACCESS_TOKEN?.trim();
    ...
    const cfgWithAccount = setTwitchAccount(cfg as DiffractionConfig, {
      username,
      accessToken: token,
      clientId,
      channel: channelName,
