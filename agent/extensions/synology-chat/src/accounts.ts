...
import { resolveDangerousNameMatchingEnabled } from "diffraction/plugin-sdk/config-runtime";
import type { SynologyChatChannelConfig, ResolvedSynologyChatAccount } from "./types.js";

/** Extract the channel config from the full Diffraction config object. */
function getChannelConfig(cfg: DiffractionConfig): SynologyChatChannelConfig | undefined {
 ...
 */
export function listAccountIds(cfg: DiffractionConfig): string[] {
...
export function resolveAccount(
  ...
  const envBotName = process.env.DIFFRACTION_BOT_NAME ?? "Diffraction";
