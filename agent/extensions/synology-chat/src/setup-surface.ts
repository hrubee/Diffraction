...
function getChannelConfig(cfg: DiffractionConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: DiffractionConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: DiffractionConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: DiffractionConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Diffraction uses to send replies back to Chat.",
      ],
