import type { AnyAgentTool, DiffractionPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: DiffractionPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<DiffractionPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as DiffractionPluginApi,
    resolveTool,
  };
}
