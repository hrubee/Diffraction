...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.DIFFRACTION_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-nostr-"));
  process.env.DIFFRACTION_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.DIFFRACTION_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".diffraction");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.DIFFRACTION_STATE_DIR = previous;
    }
