...
import type { DiffractionPluginCommandDefinition } from "../../test/helpers/extensions/plugin-command.js";
...
function createHarness(config: Record<string, unknown>) {
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: DiffractionPluginCommandDefinition) => {
      command = definition;
    }),
  };
