...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["diffractionPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["diffraction", "pollId"]) ??
    readNestedString(value, ["diffraction", "poll", "id"]) ??
    readNestedString(value, ["data", "diffractionPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "diffraction", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          diffractionPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "diffraction poll vote",
          displayText: "Vote recorded",
          value: { diffractionPollId: pollId, pollId },
        },
      },
    ],
  };
