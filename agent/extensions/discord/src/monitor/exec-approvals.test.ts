...
const STORE_PATH = path.join(os.tmpdir(), "diffraction-exec-approvals-test.json");
...
beforeEach(() => {
  ...
  mockResolveGatewayConnectionAuth.mockReset().mockImplementation(
    async (params: {
      ...
      const envToken = params.env.DIFFRACTION_GATEWAY_TOKEN;
      const envPassword = params.env.DIFFRACTION_GATEWAY_PASSWORD;
...
describe("DiscordExecApprovalHandler gateway auth", () => {
  ...
  it("prefers DIFFRACTION_GATEWAY_TOKEN when config token is missing", async () => {
    vi.stubEnv("DIFFRACTION_GATEWAY_TOKEN", "env-gateway-token");
    const handler = new DiscordExecApprovalHandler({
      token: "discord-bot-token",
      accountId: "default",
      config: { enabled: true, approvers: ["123"] },
...
describe("DiscordExecApprovalHandler gateway auth resolution", () => {
  ...
  it("passes env URL overrides to shared gateway auth resolver", async () => {
    const previousGatewayUrl = process.env.DIFFRACTION_GATEWAY_URL;
    try {
      process.env.DIFFRACTION_GATEWAY_URL = "wss://gateway-from-env.example/ws";
      const handler = new DiscordExecApprovalHandler({
        token: "test-token",
        accountId: "default",
        config: { enabled: true, approvers: ["123"] },
      ...
      if (typeof previousGatewayUrl === "string") {
        process.env.DIFFRACTION_GATEWAY_URL = previousGatewayUrl;
      } else {
