...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): DiffractionConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
vi.mock("diffraction/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("diffraction/plugin-sdk/plugin-runtime")>(
