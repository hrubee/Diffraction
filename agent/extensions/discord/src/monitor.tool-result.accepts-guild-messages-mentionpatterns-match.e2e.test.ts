  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/diffraction",
      },
    },
    session: { store: "/tmp/diffraction-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bdiffraction\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "diffraction: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "Diffraction" },
          },
        },
        eventPatch: {
