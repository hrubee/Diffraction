...
import { createScopedDmSecurityResolver } from "diffraction/plugin-sdk/channel-config-helpers";
import { createPairingPrefixStripper } from "diffraction/plugin-sdk/channel-pairing";
import { createOpenProviderConfiguredRouteWarningCollector } from "diffraction/plugin-sdk/channel-policy";
import { resolveTargetsWithOptionalToken } from "diffraction/plugin-sdk/channel-targets";
...
function hasDiscordExecApprovalDmRoute(cfg: DiffractionConfig): boolean {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Diffraction adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
