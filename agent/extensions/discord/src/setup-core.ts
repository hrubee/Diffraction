import { DEFAULT_ACCOUNT_ID } from "diffraction/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "diffraction/plugin-sdk/config-runtime";
import type { DiffractionConfig } from "diffraction/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "diffraction/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "diffraction/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "diffraction/plugin-sdk/setup-runtime";
import { formatDocsLink } from "diffraction/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: DiffractionConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: DiffractionConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: DiffractionConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: DiffractionConfig; accountId: string }) =>
    ...
    disable: (cfg: DiffractionConfig) => setSetupChannelEnabled(cfg, channel, false),
