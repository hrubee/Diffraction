...
- 替代旧版 `gateway` 命令。CLI 入口点：`diffraction gateway`。
...
- 配置热重载监视 `~/.diffraction/diffraction.json`（或 `DIFFRACTION_CONFIG_PATH`）。
...
- 默认在 `canvasHost.port`（默认 `18793`）上启动 Canvas 文件服务器，从 `~/.diffraction/workspace/canvas` 提供 `http://<gateway-host>:18793/__diffraction__/canvas/`。使用 `canvasHost.enabled=false` 或 `DIFFRACTION_SKIP_CANVAS_HOST=1` 禁用。
...
- 默认需要 Gateway 网关认证：设置 `gateway.auth.token`（或 `DIFFRACTION_GATEWAY_TOKEN`）或 `gateway.auth.password`。客户端必须发送 `connect.params.auth.token/password`，除非使用 Tailscale Serve 身份。
- 向导现在默认生成令牌，即使在 loopback 上也是如此。
- 端口优先级：`--port` > `DIFFRACTION_GATEWAY_PORT` > `gateway.port` > 默认 `18789`。
...
- macOS：`bot.molt.<profile>`（旧版 `com.diffraction.*` 可能仍然存在）
- Linux：`diffraction-gateway-<profile>.service`
- Windows：`Diffraction Gateway (<profile>)`

安装元数据嵌入在服务配置中：

- `DIFFRACTION_SERVICE_MARKER=diffraction`
- `DIFFRACTION_SERVICE_KIND=gateway`
- `DIFFRACTION_SERVICE_VERSION=<version>`
...
diffraction --dev health
```

默认值（可通过 env/flags/config 覆盖）：

- `DIFFRACTION_STATE_DIR=~/.diffraction-dev`
- `DIFFRACTION_CONFIG_PATH=~/.diffraction-dev/diffraction.json`
- `DIFFRACTION_GATEWAY_PORT=19001`（Gateway 网关 WS + HTTP）
- 浏览器控制服务端口 = `19003`（派生：`gateway.port+2`，仅 loopback）
- `canvasHost.port=19005`（派生：`gateway.port+4`）
- 当你在 `--dev` 下运行 `setup`/`onboard` 时，`agents.defaults.workspace` 默认变为 `~/.diffraction/workspace-dev`。

派生端口（经验法则）：

- 基础端口 = `gateway.port`（或 `DIFFRACTION_GATEWAY_PORT` / `--port`）
- 浏览器控制服务端口 = 基础 + 2（仅 loopback）
- `canvasHost.port = 基础 + 4`（或 `DIFFRACTION_CANVAS_HOST_PORT` / 配置覆盖）
- 浏览器配置文件 CDP 端口从 `browser.controlPort + 9 .. + 108` 自动分配（按配置文件持久化）。

每个实例的检查清单：

- 唯一的 `gateway.port`
- 唯一的 `DIFFRACTION_CONFIG_PATH`
- 唯一的 `DIFFRACTION_STATE_DIR`
...
```bash
diffraction --profile main gateway install
diffraction --profile rescue gateway install
```
...
- `health` — 完整健康快照（与 `diffraction health --json` 形状相同）。
- `status` — 简短摘要。
...
- LaunchAgents 是按用户的，需要已登录的会话；对于无头设置，使用自定义 LaunchDaemon（未随附）。
  - `diffraction gateway install` 写入 `~/Library/LaunchAgents/bot.molt.gateway.plist`
    （或 `bot.molt.<profile>.plist`；旧版 `com.diffraction.*` 会被清理）。
  - `diffraction doctor` 审计 LaunchAgent 配置，可以将其更新为当前默认值。
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 Diffraction 配置文件服务。
  我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](/gateway/multiple-gateways)。
  - 清理：`diffraction gateway uninstall`（当前服务）和 `diffraction doctor`（旧版迁移）。
- `gateway install` 在已安装时是无操作的；使用 `diffraction gateway install --force` 重新安装（配置文件/env/路径更改）。

捆绑的 mac 应用：

- Diffraction.app 可以捆绑基于 Node 的 Gateway 网关中继并安装标记为
  `bot.molt.gateway`（或 `bot.molt.<profile>`；旧版 `com.diffraction.*` 标签仍能干净卸载）的按用户 LaunchAgent。
- 要干净地停止它，使用 `diffraction gateway stop`（或 `launchctl bootout gui/$UID/bot.molt.gateway`）。
- 要重启，使用 `diffraction gateway restart`（或 `launchctl kickstart -k gui/$UID/bot.molt.gateway`）。
  - `launchctl` 仅在 LaunchAgent 已安装时有效；否则先使用 `diffraction gateway install`。
  - 运行命名配置文件时，将标签替换为 `bot.molt.<profile>`。

## 监管（systemd 用户单元）

Diffraction 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
建议单用户机器使用用户服务（更简单的 env，按用户配置）。
对于多用户或常驻服务器使用**系统服务**（无需 lingering，
共享监管）。

`diffraction gateway install` 写入用户单元。`diffraction doctor` 审计
单元并可以将其更新以匹配当前推荐的默认值。

创建 `~/.config/systemd/user/diffraction-gateway[-<profile>].service`：

```
[Unit]
Description=Diffraction Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/diffraction gateway --port 18789
Restart=always
RestartSec=5
Environment=DIFFRACTION_GATEWAY_TOKEN=
...
创建 `/etc/systemd/system/diffraction-gateway[-<profile>].service`（复制上面的单元，
...
- `diffraction gateway health|status` — 通过 Gateway 网关 WS 请求 health/status。
- `diffraction message send --target <num> --message "hi" [--media ...]` — 通过 Gateway 网关发送（对 WhatsApp 是幂等的）。
- `diffraction agent --message "hi" --to <num>` — 运行智能体轮次（默认等待最终结果）。
- `diffraction gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `diffraction gateway stop|restart` — 停止/重启受监管的 Gateway 网关服务（launchd/systemd）。
- Gateway 网关辅助子命令假设 `--url` 上有运行中的 Gateway 网关；它们不再自动生成一个。

## 迁移指南

- 淘汰 `diffraction gateway` 和旧版 TCP 控制端口的使用。
