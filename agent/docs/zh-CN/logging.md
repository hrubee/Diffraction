...
你可以在 `~/.diffraction/diffraction.json` 中覆盖此设置：
...
所有日志配置都在 `~/.diffraction/diffraction.json` 的 `logging` 下。
...
- Diffraction 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `diffraction plugins enable diagnostics-otel` 启用插件。
...
- `diffraction.tokens`（计数器，属性：`diffraction.token`、`diffraction.channel`、`diffraction.provider`、`diffraction.model`）
- `diffraction.cost.usd`（计数器，属性：`diffraction.channel`、`diffraction.provider`、`diffraction.model`）
- `diffraction.run.duration_ms`（直方图，属性：`diffraction.channel`、`diffraction.provider`、`diffraction.model`）
- `diffraction.context.tokens`（直方图，属性：`diffraction.context`、`diffraction.channel`、`diffraction.provider`、`diffraction.model`）

消息流：

- `diffraction.webhook.received`（计数器，属性：`diffraction.channel`、`diffraction.webhook`）
- `diffraction.webhook.error`（计数器，属性：`diffraction.channel`、`diffraction.webhook`）
- `diffraction.webhook.duration_ms`（直方图，属性：`diffraction.channel`、`diffraction.webhook`）
- `diffraction.message.queued`（计数器，属性：`diffraction.channel`、`diffraction.source`）
- `diffraction.message.processed`（计数器，属性：`diffraction.channel`、`diffraction.outcome`）
- `diffraction.message.duration_ms`（直方图，属性：`diffraction.channel`、`diffraction.outcome`）

队列 + 会话：

- `diffraction.queue.lane.enqueue`（计数器，属性：`diffraction.lane`）
- `diffraction.queue.lane.dequeue`（计数器，属性：`diffraction.lane`）
- `diffraction.queue.depth`（直方图，属性：`diffraction.lane` 或 `diffraction.channel=heartbeat`）
- `diffraction.queue.wait_ms`（直方图，属性：`diffraction.lane`）
- `diffraction.session.state`（计数器，属性：`diffraction.state`、`diffraction.reason`）
- `diffraction.session.stuck`（计数器，属性：`diffraction.state`）
- `diffraction.session.stuck_age_ms`（直方图，属性：`diffraction.state`）
- `diffraction.run.attempt`（计数器，属性：`diffraction.attempt`）
...
- `diffraction.webhook.error`
  - `diffraction.channel`、`diffraction.webhook`、`diffraction.chatId`、`diffraction.error`
- `diffraction.message.processed`
  - `diffraction.channel`、`diffraction.outcome`、`diffraction.chatId`、`diffraction.messageId`、`diffraction.sessionKey`、`diffraction.sessionId`、`diffraction.reason`
- `diffraction.session.stuck`
  - `diffraction.state`、`diffraction.ageMs`、`diffraction.queueDepth`、`diffraction.sessionKey`、`diffraction.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `diffraction doctor`。
