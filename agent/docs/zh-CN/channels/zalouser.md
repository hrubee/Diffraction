---
read_when:
  - 为 Diffraction 设置 Zalo Personal
...
- 通过 CLI 安装：`diffraction plugins install @diffraction/zalouser`
- 或从源码检出安装：`diffraction plugins install ./extensions/zalouser`
- 详情：[插件](/tools/plugin)
...
- 配置向导可以提示输入群组允许列表。
- 启动时，Diffraction 将允许列表中的群组/用户名称解析为 ID 并记录映射；未解析的条目保持原样。
...
- 重新登录：`diffraction channels logout --channel zalouser && diffraction channels login --channel zalouser`
