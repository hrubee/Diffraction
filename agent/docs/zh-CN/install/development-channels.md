...
当你使用 `--channel` **显式**切换渠道时，Diffraction 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/diffraction`，可通过 `DIFFRACTION_GIT_DIR` 覆盖），
...
当你使用 `diffraction update` 切换渠道时，Diffraction 还会同步插件来源：
