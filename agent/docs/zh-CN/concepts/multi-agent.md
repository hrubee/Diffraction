...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.diffraction/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.diffraction/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.diffraction`（或 `DIFFRACTION_STATE_DIR`）
- 工作区：`~/.diffraction/workspace`（或 `~/.diffraction/workspace-<agentId>`）
- 智能体目录：`~/.diffraction/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.diffraction/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，Diffraction 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.diffraction/workspace`（或当设置了 `DIFFRACTION_PROFILE` 时为 `~/.diffraction/workspace-<profile>`）。
- 状态默认为 `~/.diffraction/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.diffraction/credentials/whatsapp/personal
          // authDir: "~/.diffraction/credentials/whatsapp/personal",
        },
        biz: {
          // 可选覆盖。默认：~/.diffraction/credentials/whatsapp/biz
