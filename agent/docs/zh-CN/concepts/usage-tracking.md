...
- 聊天中的 `/usage cost`：从 Diffraction 会话日志汇总的本地费用摘要。
- CLI：`diffraction status --usage` 打印完整的按提供商分类的详细信息。
- CLI：`diffraction channels list` 在提供商配置旁打印相同的使用量快照（使用 `--no-usage` 跳过）。
