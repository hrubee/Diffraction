...
- `~/Library/Application Support/Diffraction/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `diffraction-canvas://<session>/<path>`

示例：

- `diffraction-canvas://main/` → `<canvasRoot>/main/index.html`
- `diffraction-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `diffraction-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
diffraction nodes canvas present --node <id>
diffraction nodes canvas navigate --node <id> --url "/"
diffraction nodes canvas eval --node <id> --js "document.title"
diffraction nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__diffraction__/a2ui/
```
...
diffraction nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
diffraction nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "diffraction://agent?message=Review%20this%20design";
```
