...
Diffraction 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`diffraction gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`diffraction onboard --install-daemon`
- 直接安装：`diffraction gateway install`
- 配置流程：`diffraction configure` → 选择 **Gateway 服务**
- 修复/迁移：`diffraction doctor`（会提供安装或修复服务的选项）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.diffraction.gateway` 或 `ai.diffraction.<profile>`；旧版为 `com.diffraction.*`）
- Linux/WSL2：systemd 用户服务（`diffraction-gateway[-<profile>].service`）
