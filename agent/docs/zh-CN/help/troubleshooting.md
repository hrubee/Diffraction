...
你也可以设置 `DIFFRACTION_VERBOSE=1` 代替标志。
...
### `docs.diffraction.ai` 显示 SSL 错误（Comcast/Xfinity）

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 阻止 `docs.diffraction.ai`。
禁用 Advanced Security 或将 `docs.diffraction.ai` 添加到允许列表，然后重试。
...
- 检查允许列表：`diffraction config get agents.defaults.models`
...
如果可以的话，包含来自 `diffraction logs --follow` 的相关日志尾部。
