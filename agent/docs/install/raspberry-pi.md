---
summary: "Host Diffraction on a Raspberry Pi for always-on self-hosting"
read_when:
...
Run a persistent, always-on Diffraction Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well.
...
**Service will not start** -- Check logs with `journalctl -u diffraction --no-pager -n 100` and run `diffraction doctor --non-interactive`.
