---
summary: "Deploy Diffraction on Render with Infrastructure-as-Code"
read_when:
  - Deploying Diffraction to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Diffraction on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/diffraction/diffraction)
...
repository configures everything needed to run Diffraction:
...
services:
  - type: web
    ...
    envVars:
      - key: DIFFRACTION_GATEWAY_PORT
        value: "8080"
      - key: DIFFRACTION_STATE_DIR
        value: /data/.diffraction
      - key: DIFFRACTION_WORKSPACE_DIR
        value: /data/workspace
      - key: DIFFRACTION_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: diffraction-data
      mountPath: /data
      sizeGB: 1
```
...
Connect using the `DIFFRACTION_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original Diffraction repository, Render will not auto-deploy your Diffraction. To update it, run a manual Blueprint sync from the dashboard.
...
For Diffraction, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive you can restore on any Diffraction host. See [Backup](/cli/backup) for details.
...
- Missing `DIFFRACTION_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `DIFFRACTION_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `diffraction backup create` in the Render shell.
