---
summary: "Host Diffraction on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Diffraction on Oracle Cloud
  - Looking for free VPS hosting for Diffraction
...
Run a persistent Diffraction Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@diffraction`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=diffraction --reset` to re-authenticate.

**Gateway will not start** -- Run `diffraction doctor --non-interactive` and check logs with `journalctl --user -u diffraction-gateway -n 50`.
...
- [Updating](/install/updating) -- keep Diffraction up to date
