---
summary: "Deploy Diffraction Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Diffraction on a Kubernetes cluster
  - You want to test Diffraction in a Kubernetes environment
title: "Kubernetes"
---

# Diffraction on Kubernetes

A minimal starting point for running Diffraction on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Diffraction is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/diffraction 18789:18789 -n diffraction
...
```bash
kubectl get secret diffraction-secrets -n diffraction -o jsonpath='{.data.DIFFRACTION_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: diffraction (configurable via DIFFRACTION_NAMESPACE)
├── Deployment/diffraction        # Single pod, init container + gateway
├── Service/diffraction           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/diffraction-config  # diffraction.json + AGENTS.md
└── Secret/diffraction-secrets    # Gateway token + API keys
```
...
Edit `diffraction.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret diffraction-secrets -n diffraction \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/diffraction -n diffraction
```

### Custom namespace

```bash
DIFFRACTION_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/diffraction/diffraction:latest # or pin to a specific version from https://github.com/diffraction/diffraction/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # diffraction.json + AGENTS.md
