---
summary: "Optional Docker-based setup and onboarding for Diffraction"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Diffraction on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export DIFFRACTION_IMAGE="ghcr.io/diffraction/diffraction:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/diffraction/diffraction/pkgs/container/diffraction).
...
<Note>
Run `docker compose` from the repo root. If you enabled `DIFFRACTION_EXTRA_MOUNTS`
or `DIFFRACTION_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `DIFFRACTION_IMAGE`               | Use a remote image instead of building locally                   |
| `DIFFRACTION_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `DIFFRACTION_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `DIFFRACTION_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `DIFFRACTION_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `DIFFRACTION_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `DIFFRACTION_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `DIFFRACTION_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `DIFFRACTION_CONFIG_DIR` to `/home/node/.diffraction` and
`DIFFRACTION_WORKSPACE_DIR` to `/home/node/.diffraction/workspace`, so those paths
...
and rolling file logs under `/tmp/diffraction/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/diffraction/diffraction/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/diffraction/diffraction/blob/main/scripts/shell-helpers/README.md).

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export DIFFRACTION_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export DIFFRACTION_SANDBOX=1
    export DIFFRACTION_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm diffraction-cli gateway probe
    docker compose run -T --rm diffraction-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `diffraction-cli` uses `network_mode: "service:diffraction-gateway"` so CLI
    ...
    `no-new-privileges` on `diffraction-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.diffraction`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export DIFFRACTION_HOME_VOLUME="diffraction_home"`
    2. **Bake system deps**: `export DIFFRACTION_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `DIFFRACTION_HOME_VOLUME` or `DIFFRACTION_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/diffraction/diffraction/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Diffraction runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm diffraction-cli config set gateway.mode local
    docker compose run --rm diffraction-cli config set gateway.bind lan
    docker compose run --rm diffraction-cli devices list --url ws://127.0.0.1:18789
    ```
