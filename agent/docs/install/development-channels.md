...
Diffraction ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/diffraction`, override with
  `DIFFRACTION_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
diffraction update --tag 2026.3.22
...
diffraction update --tag diffraction@2026.3.22
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `diffraction update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Diffraction prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `diffraction update` would do without making changes:

```bash
diffraction update --dry-run
diffraction update --channel beta --dry-run
diffraction update --tag 2026.3.22 --dry-run
diffraction update --dry-run --json
```
...
When you switch channels with `diffraction update`, Diffraction also syncs plugin
sources:
