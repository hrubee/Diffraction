---
summary: "Move (migrate) an Diffraction install from one machine to another"
read_when:
  - You are moving Diffraction to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Diffraction to a New Machine

This guide moves an Diffraction gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.diffraction/` by default) and your **workspace**, you preserve:

- **Config** -- `diffraction.json` and all gateway settings
...
<Tip>
Run `diffraction status` on the old machine to confirm your state directory path.
Custom profiles use `~/.diffraction-<profile>/` or a path set via `DIFFRACTION_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    If you use multiple profiles (e.g. `~/.diffraction-work`), archive each separately.

  </Step>

  <Step title="Install Diffraction on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.diffraction/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `DIFFRACTION_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `diffraction doctor`.
  </Accordion>

  <Accordion title="Copying only diffraction.json">
  ...
  <Accordion title="Remote mode">
    ...
    Migrate the gateway host itself, not your local laptop. See [FAQ](/help/faq#where-does-diffraction-store-its-data).
  </Accordion>
...
- [ ] `diffraction status` shows the gateway running
