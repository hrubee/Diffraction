...
CLI onboarding is the **recommended** way to set up Diffraction on macOS,
...
`diffraction dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`diffraction configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.diffraction/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `diffraction doctor` first.
</Note>
...
Use `diffraction agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.diffraction/workspace-<agentId>`.
...
- CLI command reference: [`diffraction onboard`](/cli/onboard)
