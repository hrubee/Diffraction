...
read_when:
  ...
  - You are defining plugin config schemas or package.json diffraction metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`diffraction.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `diffraction` field that tells the plugin system what
...
```json
{
  "name": "@myorg/diffraction-my-channel",
  "version": "1.0.0",
  "type": "module",
  "diffraction": {
    "extensions": ["./index.ts"],
...
```json
{
  "name": "@myorg/diffraction-my-provider",
  "version": "1.0.0",
  "type": "module",
  "diffraction": {
    "extensions": ["./index.ts"],
...
When enabled, Diffraction loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `diffraction.plugin.json` in the package root.
Diffraction uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Diffraction",
  "configSchema": {
    "type": "object",
...
Diffraction loads when it only needs setup surfaces (onboarding, config repair,
...
**When Diffraction uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `diffraction/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that Diffraction validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "diffraction/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `diffraction onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "diffraction/plugin-sdk/channel-setup";
...
helpers from `diffraction/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`diffraction/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `diffraction/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "diffraction/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/diffraction-my-channel",
...
```bash
diffraction plugins install @myorg/diffraction-my-plugin
```

Diffraction tries ClawHub first and falls back to npm automatically. You can also
force a specific source:

```bash
diffraction plugins install clawhub:@myorg/diffraction-my-plugin   # ClawHub only
diffraction plugins install npm:@myorg/diffraction-my-plugin       # npm only
```
...
```bash
diffraction plugins search <query>
diffraction plugins install <package-name>
```

<Info>
  For npm-sourced installs, `diffraction plugins install` runs
