...
read_when:
  ...
  - You want a reference for all registration methods on DiffractionPluginApi
...
import { definePluginEntry } from "diffraction/plugin-sdk";
```
...
The `register(api)` callback receives an `DiffractionPluginApi` object with these
methods:
...
| `api.config`             | `DiffractionConfig`          | Current config snapshot                                   |
...
<Warning>
  Never import your own plugin through `diffraction/plugin-sdk/<your-plugin>`
