...
read_when:
  - You see the DIFFRACTION_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the DIFFRACTION_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Diffraction plugin
---

# Plugin SDK Migration

Diffraction has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`diffraction/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`diffraction/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`diffraction/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "diffraction/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "diffraction/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "diffraction/extension-api";
...
```bash
DIFFRACTION_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 diffraction gateway run
DIFFRACTION_SUPPRESS_EXTENSION_API_WARNING=1 diffraction gateway run
```
