...
read_when:
  - You want to place an outbound voice call from Diffraction
...
Voice calls for Diffraction via a plugin. Supports outbound notifications and
...
- Use `diffraction voicecall ...` or the `voice_call` tool
...
```bash
diffraction plugins install ./extensions/voice-call
...
diffraction voicecall start --to "+15555550123"   # alias for call
...
diffraction voicecall latency                     # summarize turn latency from logs
