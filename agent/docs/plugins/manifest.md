...
read_when:
  - You are building an Diffraction plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (diffraction.plugin.json)

This page is for the **native Diffraction plugin manifest** only.
...
Diffraction auto-detects those bundle layouts too, but they are not validated
against the `diffraction.plugin.json` schema described here.

For compatible bundles, Diffraction currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Diffraction runtime expectations.

Every native Diffraction plugin **must** ship a `diffraction.plugin.json` file in the
**plugin root**. Diffraction uses this manifest to validate configuration
...
`diffraction.plugin.json` is the metadata Diffraction reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Diffraction can inspect without loading plugin code.                                      |
...
Diffraction reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, Diffraction falls back to `choiceId`.                                        |
...
| File                   | Use it for                                                                                                         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `diffraction.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs           |
| `package.json`         | npm metadata, dependency installation, and the `diffraction` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Diffraction must know it before loading plugin code, put it in `diffraction.plugin.json`
...
- The manifest is **required for native Diffraction plugins**, including local filesystem loads.
