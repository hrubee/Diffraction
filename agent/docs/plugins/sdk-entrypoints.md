...
**Import:** `diffraction/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `DiffractionPluginConfigSchema \| () => DiffractionPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: DiffractionPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `diffraction.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `diffraction/plugin-sdk/core`
...
| `registerFull` | `(api: DiffractionPluginApi) => void`                               | No       | —                   |
...
Diffraction loads this instead of the full entry when a channel is disabled,
...
Diffraction classifies loaded plugins by their registration behavior:
...
Use `diffraction plugins inspect <id>` to see a plugin's shape.
