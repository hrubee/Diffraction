...
summary: "Step-by-step guide to building a messaging channel plugin for Diffraction"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Diffraction to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects Diffraction to a
...
Channel plugins do not need their own send/edit/react tools. Diffraction keeps one
...
<Steps>
  <Step title="Package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/diffraction-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "diffraction": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Diffraction to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Add a setup entry">
    ...
    Diffraction loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    Diffraction. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Diffraction.
...
├── package.json              # diffraction.channel metadata
├── diffraction.plugin.json      # Manifest with config schema
