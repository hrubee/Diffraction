---
summary: "Install and use Codex, Claude, and Cursor bundles as Diffraction plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Diffraction maps bundle content into native features
...
Diffraction can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Diffraction maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Diffraction plugins. Native plugins run
...
of requiring authors to rewrite them as native Diffraction plugins, Diffraction
...
## What Diffraction maps from bundles

Not every bundle feature runs in Diffraction today. Here is what works and what
...
| ------------- | ---------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Diffraction skills                                                    | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                           | Claude, Cursor |
| Hook packs    | Diffraction-style `HOOK.md` + `handler.ts` layouts                                                      | Codex          |
...
These are recognized and shown in diagnostics, but Diffraction does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Diffraction best when they use skill roots and Diffraction-style
...
Diffraction checks for native plugin format first:

1. `diffraction.plugin.json` or valid `package.json` with `diffraction.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Diffraction uses the native path. This prevents
...
- Diffraction does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `diffraction plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Diffraction does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Diffraction hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
