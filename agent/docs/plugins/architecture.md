...
read_when:
  - Building or debugging native Diffraction plugins
...
This page covers the internal architecture of the Diffraction plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Diffraction. Every
native Diffraction plugin registers against one or more capability types:
...
Diffraction classifies every loaded plugin into a shape based on its actual
...
Use `diffraction plugins inspect <id>` to see a plugin's shape and capability
...
When you run `diffraction doctor` or `diffraction plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `diffraction status --all` and `diffraction plugins doctor`.

## Architecture overview

Diffraction's plugin system has four layers:

1. **Manifest + discovery**
   Diffraction finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `diffraction.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Diffraction plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Diffraction reads the registry to expose tools, channels, provider
...
That split lets Diffraction validate config, explain missing/disabled plugins, and
...
normal chat actions. Diffraction keeps one shared `message` tool in core, and
...
Diffraction treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Diffraction-facing
  surfaces
...
So if Diffraction adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Diffraction has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { DiffractionPluginDefinition } from "diffraction/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "diffraction/plugin-sdk";

const plugin: DiffractionPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Diffraction already treats image/audio/video understanding as one shared
...
If Diffraction adds a new domain later, such as video generation, use the same
...
`DiffractionPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Diffraction can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Diffraction knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `DiffractionPluginApi` or
  `api.runtime`
...
Native Diffraction plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because Diffraction currently treats them
...
name: `@diffraction/<id>` by default, or an approved typed suffix such as
...
Diffraction exports capabilities, not implementation convenience.
...
At startup, Diffraction does roughly this:
...
The manifest is the control-plane source of truth. Diffraction uses it to:
...
Diffraction keeps short in-process caches for:
...
- Set `DIFFRACTION_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `DIFFRACTION_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `DIFFRACTION_PLUGIN_DISCOVERY_CACHE_MS` and
  `DIFFRACTION_PLUGIN_MANIFEST_CACHE_MS`.
...
Diffraction still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Diffraction calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Diffraction tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on Diffraction's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Diffraction's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Diffraction adds those
...
- If Diffraction adds a new capability such as video generation later, define the
...
- Diffraction only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Diffraction plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `diffraction/plugin-sdk` import when
authoring plugins:

- `diffraction/plugin-sdk/plugin-entry` for plugin registration primitives.
- `diffraction/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `diffraction/plugin-sdk/channel-setup`,
  `diffraction/plugin-sdk/channel-pairing`,
  `diffraction/plugin-sdk/channel-contract`,
  `diffraction/plugin-sdk/channel-feedback`,
  `diffraction/plugin-sdk/channel-inbound`,
  `diffraction/plugin-sdk/channel-lifecycle`,
  `diffraction/plugin-sdk/channel-reply-pipeline`,
  `diffraction/plugin-sdk/command-auth`,
  `diffraction/plugin-sdk/secret-input`, and
  `diffraction/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `diffraction/plugin-sdk/channel-config-helpers`,
  `diffraction/plugin-sdk/allow-from`,
  `diffraction/plugin-sdk/channel-config-schema`,
  `diffraction/plugin-sdk/channel-policy`,
  `diffraction/plugin-sdk/config-runtime`,
  `diffraction/plugin-sdk/infra-runtime`,
  `diffraction/plugin-sdk/agent-runtime`,
  `diffraction/plugin-sdk/lazy-runtime`,
  `diffraction/plugin-sdk/reply-history`,
  `diffraction/plugin-sdk/routing`,
  `diffraction/plugin-sdk/status-helpers`,
  `diffraction/plugin-sdk/runtime-store`, and
  `diffraction/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `diffraction/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `diffraction/plugin-sdk/*` subpaths. Diffraction core/test code may use the repo
...
- Avoid the root `diffraction/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `diffraction/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `diffraction/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `diffraction/plugin-sdk/<extension>`.
...
`diffraction/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Diffraction writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Diffraction's
...
- if both `catalog` and `discovery` are registered, Diffraction uses `catalog`
...
- Read-only command paths such as `diffraction status`, `diffraction status --all`,
  `diffraction channels status`, `diffraction channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `diffraction.extensions`:
...
Security guardrail: every `diffraction.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `diffraction plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `diffraction.setupEntry` can point at a lightweight setup-only module.
When Diffraction needs setup surfaces for a disabled channel plugin, or
...
Optional: `diffraction.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Diffraction load the
...
Channel plugins can advertise setup/discovery metadata via `diffraction.channel` and
install hints via `diffraction.install`. This keeps the core catalog data-free.
...
Diffraction can also merge **external channel catalogs** (for example, an MPM
...
Or point `DIFFRACTION_PLUGIN_CATALOG_PATHS` (or `DIFFRACTION_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "diffraction": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "diffraction/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `DiffractionPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Diffraction stays opinionated without becoming hardcoded to one
