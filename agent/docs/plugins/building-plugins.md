...
summary: "Create your first Diffraction plugin in minutes"
read_when:
  - You want to create a new Diffraction plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to Diffraction
---

# Building Plugins

Plugins extend Diffraction with new capabilities: channels, model providers, speech,
image generation, web search, agent tools, or any combination.

You do not need to add your plugin to the Diffraction repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`diffraction plugins install <package-name>`. Diffraction tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Diffraction to a messaging platform (Discord, IRC, etc.)
  </Card>
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/diffraction-my-plugin",
      "version": "1.0.0",
      "type": "module",
      "diffraction": {
        "extensions": ["./index.ts"]
    ...
    ```json diffraction.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to Diffraction",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** publish to [ClawHub](/tools/clawhub) or npm, then install:
    ...
    Diffraction checks ClawHub first, then falls back to npm.
...
Always import from focused `diffraction/plugin-sdk/<subpath>` paths:
...
import { ... } from "diffraction/plugin-sdk";
```
...
<Check>**package.json** has correct `diffraction` metadata</Check>
<Check>**diffraction.plugin.json** manifest is present and valid</Check>
