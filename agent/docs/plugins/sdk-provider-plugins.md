...
summary: "Step-by-step guide to building a model provider plugin for Diffraction"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Diffraction
...
(LLM) to Diffraction. By the end you will have a provider with a model catalog,
API key auth, and dynamic model resolution.

<Info>
  If you have not built any Diffraction plugin before, read
...
<Steps>
  <Step title="Package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/diffraction-acme-ai",
      "version": "1.0.0",
      "type": "module",
      "diffraction": {
        "extensions": ["./index.ts"],
    ...
    The manifest declares `providerAuthEnvVars` so Diffraction can detect
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "diffraction/plugin-sdk/provider-auth";
    ...
    `diffraction onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "diffraction/plugin-sdk/provider-entry";
    ...
    `diffraction/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      Diffraction calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Diffraction classifies this as a **hybrid-capability** plugin. This is the
...
```
extensions/acme-ai/
├── package.json              # diffraction.providers metadata
├── diffraction.plugin.json      # Manifest with providerAuthEnvVars
