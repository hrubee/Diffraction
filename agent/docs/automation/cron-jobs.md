...
- Jobs persist under `~/.diffraction/cron/` so restarts don’t lose schedules.
...
- For upgrades, `diffraction doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
diffraction cron list
diffraction cron run <job-id>
diffraction cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.diffraction/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `diffraction cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Diffraction applies a
...
If `delivery` is omitted for isolated jobs, Diffraction defaults to `announce`.
...
- CLI equivalents: `diffraction cron add --light-context ...` and `diffraction cron edit --light-context`.
...
- Job store: `~/.diffraction/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.diffraction/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Diffraction classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
```json5
{
  cron: {
    ...
    store: "~/.diffraction/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, Diffraction archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `diffraction cron runs` and adjust retention before logs become large
...
diffraction cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
diffraction cron edit <jobId> --agent ops
diffraction cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
diffraction cron run <jobId>
diffraction cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `diffraction cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
diffraction cron edit <jobId> \
...
```bash
diffraction cron edit <jobId> --exact
```
...
```bash
diffraction system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`diffraction system event`](/cli/system).

## Troubleshooting

### "Nothing runs"

- Check cron is enabled: `cron.enabled` and `DIFFRACTION_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Diffraction applies exponential retry backoff for recurring jobs after consecutive errors:
