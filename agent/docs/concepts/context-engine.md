...
read_when:
  - You want to understand how Diffraction assembles model context
...
A **context engine** controls how Diffraction builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Diffraction ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.diffraction/diffraction.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Diffraction plugin. Install
...
diffraction plugins install -l ./my-context-engine
```
...
Every time Diffraction runs a model prompt, the context engine participates at
...
Diffraction currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Diffraction
...
The built-in `legacy` engine preserves Diffraction's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Diffraction uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Diffraction disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Diffraction automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `diffraction/plugin-sdk/core` to use
  Diffraction's built-in compaction behavior.
...
Diffraction resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Diffraction's built-in summarization. Plugin engines can implement
...
- Use `diffraction doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Diffraction
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `diffraction plugins install -l ./my-engine` to link a
