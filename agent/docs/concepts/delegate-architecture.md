---
summary: "Delegate architecture: running Diffraction as a named agent on behalf of an organization"
...
Goal: run Diffraction as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Diffraction agent that:
...
Diffraction's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Diffraction's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.diffraction/workspace-delegate",
  tools: {
...
- Cron run history: `~/.diffraction/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.diffraction/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Diffraction's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
diffraction agents add delegate
```

This creates:

- Workspace: `~/.diffraction/workspace-delegate`
- State: `~/.diffraction/agents/delegate/agent`
- Sessions: `~/.diffraction/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.diffraction/workspace" },
      {
        id: "delegate",
        workspace: "~/.diffraction/workspace-delegate",
        tools: {
...
~/.diffraction/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.diffraction/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.diffraction/workspace-org",
        agentDir: "~/.diffraction/agents/org-assistant/agent",
