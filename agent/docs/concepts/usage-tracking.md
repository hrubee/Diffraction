...
- `/usage cost` in chats: local cost summary aggregated from Diffraction session logs.
- CLI: `diffraction status --usage` prints a full per-provider breakdown.
- CLI: `diffraction channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
