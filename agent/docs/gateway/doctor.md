...
`diffraction doctor` is the repair + migration tool for Diffraction. It fixes stale
...
```bash
diffraction doctor --yes
```

Accept defaults without prompting (including restart/service/sandbox repair steps when applicable).

```bash
diffraction doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
diffraction doctor --repair --force
```
...
```bash
cat ~/.diffraction/diffraction.json
```
...
- Extra workspace dir detection (`~/diffraction`).
...
you to run `diffraction doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.diffraction/diffraction.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.diffraction/sessions/` to `~/.diffraction/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.diffraction/agent/` to `~/.diffraction/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.diffraction/credentials/*.json` (except `oauth.json`)
  - to `~/.diffraction/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `diffraction doctor`.

### 3b) Legacy cron store migrations

Doctor also checks the cron job store (`~/.diffraction/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.diffraction` folders exist across
  home directories or when `DIFFRACTION_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.diffraction/diffraction.json` is
...
offers to remove them and install the Diffraction service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Diffraction gateway services are considered first-class and are not
flagged as "extra."
...
- `diffraction doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `diffraction doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `diffraction doctor` prompts before rewriting supervisor config.
- `diffraction doctor --yes` accepts the default repair prompts.
- `diffraction doctor --repair` applies recommended fixes without prompts.
- `diffraction doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `diffraction gateway install --force`.
