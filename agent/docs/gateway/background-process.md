...
Diffraction runs shell commands through the `exec` tool and keeps long‑running tasks in memory. The `process` tool manages those background sessions.
...
- Spawned exec commands receive `DIFFRACTION_SHELL=exec` for context-aware shell/profile rules.
...
- `DIFFRACTION_BASH_PENDING_MAX_OUTPUT_CHARS`: pending stdout/stderr cap per stream (chars)
