---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Diffraction agents"
read_when:
...
OpenShell is a managed sandbox backend for Diffraction. Instead of running Docker
containers locally, Diffraction delegates sandbox lifecycle to the `openshell` CLI,
...
- Diffraction Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Diffraction creates an OpenShell
...
- You edit files locally outside Diffraction and want those changes visible in the
...
- When the sandbox is first created, Diffraction seeds the remote workspace from
...
- Diffraction does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Diffraction after the initial seed,
the remote sandbox does **not** see those changes. Use
`diffraction sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"diffraction"`  | Sandbox source for first-time create                  |
...
1. Diffraction calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Diffraction calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `diffraction sandbox` commands
