...
read_when:
  - Running Diffraction behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Diffraction
...
- You run Diffraction behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Diffraction checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Diffraction extracts the user identity from the configured header
...
- Diffraction can stay on loopback HTTP behind the proxy.
...
If Diffraction itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://diffraction.example.com
    to: http://diffraction-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
diffraction.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy diffraction:18789 {
...
location / {
    ...
    proxy_pass http://diffraction:18789;
...
`diffraction security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Diffraction config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `diffraction security audit` and review findings
