...
- `diffraction gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `diffraction doctor` reports no blocking config/service issues.
- `diffraction channels status --probe` shows connected/ready channels.
...
diffraction config get agents.defaults.models
```
...
diffraction status
diffraction channels status --probe
diffraction pairing list --channel <channel> [--account <id>]
diffraction config get channels
diffraction logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `diffraction config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `diffraction devices list` then `diffraction devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `diffraction configure`). If you are running Diffraction via Podman using the dedicated `diffraction` user, the config lives at `~diffraction/.diffraction/diffraction.json`.
...
```bash
diffraction cron status
diffraction cron list
diffraction cron runs --id <jobId> --limit 20
diffraction system heartbeat last
diffraction logs --follow
```
...
diffraction approvals get --node <idOrNameOrIp>
diffraction logs --follow
diffraction status
```
...
diffraction browser start --browser-profile diffraction
...
```bash
diffraction gateway status
diffraction config get gateway.mode
diffraction config get gateway.remote.url
diffraction config get gateway.auth.mode
```
...
```bash
diffraction config get gateway.bind
