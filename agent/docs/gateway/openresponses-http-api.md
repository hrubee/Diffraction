...
Diffraction’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`diffraction agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "diffraction:<agentId>"`, `model: "agent:<agentId>"`, or `x-diffraction-agent-id`
- use `x-diffraction-session-key` for explicit session routing
