...
Diffraction supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `diffraction gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.diffraction/.env` so the daemon can read it:

```bash
cat >> ~/.diffraction/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `diffraction onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.diffraction/.env`, systemd/launchd).
...
Then paste it into Diffraction:
...
```bash
diffraction models auth paste-token --provider anthropic
diffraction models auth paste-token --provider openrouter
```
...
```bash
diffraction models status --check
```
...
- Priority order:
  - `DIFFRACTION_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Diffraction retries with the next key only for rate-limit errors (for example
...
Run `diffraction models status` to confirm which profile is expiring. If the profile
