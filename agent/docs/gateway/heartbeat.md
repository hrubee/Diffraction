...
- During heartbeat runs, Diffraction treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `diffraction sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Diffraction skips the heartbeat run entirely (no model call).
...
```bash
diffraction system event --text "Check for urgent follow-ups" --mode now
```
