...
read_when:
  - Setting up Diffraction for the first time
...
Diffraction reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.diffraction/diffraction.json`.

If the file is missing, Diffraction uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `diffraction onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    diffraction onboard       # full onboarding flow
    diffraction configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    diffraction config get agents.defaults.workspace
    diffraction config set agents.defaults.heartbeat.every "2h"
    diffraction config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.diffraction/diffraction.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Diffraction only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`diffraction doctor`, `diffraction logs`, `diffraction health`, `diffraction status`)
- Run `diffraction doctor` to see exact issues
- Run `diffraction doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@diffraction", "diffraction"],
            },
          },
        ],
      },
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `diffraction.json`.
    ...
    ```bash
    diffraction config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `DIFFRACTION_APNS_RELAY_BASE_URL` and `DIFFRACTION_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `DIFFRACTION_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.diffraction/diffraction.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `diffraction config set` for single keys.
    </Warning>
    ...
    ```bash
    diffraction gateway call config.get --params '{}'  # capture payload.hash
    diffraction gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.diffraction/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    diffraction gateway call config.patch --params '{
...
Diffraction reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.diffraction/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Diffraction runs your login shell and imports only the missing keys:
...
Env var equivalent: `DIFFRACTION_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${DIFFRACTION_GATEWAY_TOKEN}" } },
