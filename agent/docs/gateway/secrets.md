...
Diffraction supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `DIFFRACTION_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Diffraction runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Diffraction validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Diffraction QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/diffraction"],
...
- Diffraction resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `diffraction.json` refs)
...
When reload-time activation fails after a healthy state, Diffraction enters degraded secrets state.
...
- Strict command paths (for example `diffraction memory` remote-memory paths and `diffraction qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `diffraction status`, `diffraction status --all`, `diffraction channels status`, `diffraction channels resolve`, `diffraction security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `diffraction secrets reload`.
...
```bash
diffraction secrets audit --check
diffraction secrets configure
diffraction secrets audit --check
```
...
- plaintext values at rest (`diffraction.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `diffraction.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `diffraction secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `diffraction.json` plus `auth-profiles.json` for one agent scope
...
- `diffraction secrets configure --providers-only`
- `diffraction secrets configure --skip-provider-setup`
- `diffraction secrets configure --agent <id>`
...
```bash
diffraction secrets apply --from /tmp/diffraction-secrets-plan.json
diffraction secrets apply --from /tmp/diffraction-secrets-plan.json --allow-exec
diffraction secrets apply --from /tmp/diffraction-secrets-plan.json --dry-run
diffraction secrets apply --from /tmp/diffraction-secrets-plan.json --dry-run --allow-exec
```
...
Diffraction intentionally does not write rollback backups containing historical plaintext secret values.
