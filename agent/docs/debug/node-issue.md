...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Diffraction
...
Running Diffraction via Node with `tsx` fails at startup with:

```
[diffraction] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `diffraction status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch diffraction.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node diffraction.mjs status` works on Node 25.
