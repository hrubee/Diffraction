...
Diffraction can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Diffraction can send audio; Telegram gets a round voice-note bubble.
...
Diffraction defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `diffraction.json`.
...
- If `provider` is **unset**, Diffraction prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.diffraction/settings/tts.json`, override with `DIFFRACTION_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Diffraction retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, Diffraction:
...
Discord note: `/tts` is a built-in Discord command, so Diffraction registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from Diffraction
```
...
reply delivery. When the result is Telegram-compatible, Diffraction marks it for
