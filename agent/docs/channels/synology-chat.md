---
summary: "Synology Chat webhook setup and Diffraction config"
read_when:
  - Setting up Synology Chat with Diffraction
...
```bash
diffraction plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `diffraction onboard` now shows Synology Chat in the same channel setup list as `diffraction channels add`.
   - Non-interactive setup: `diffraction channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Diffraction gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Diffraction.
   - Guided: `diffraction onboard`
   - Direct: `diffraction channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `diffraction pairing list synology-chat`
  - `diffraction pairing approve synology-chat <CODE>`
...
```bash
diffraction message send --channel synology-chat --target 123456 --text "Hello from Diffraction"
diffraction message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Diffraction now rejects duplicate exact paths
