...
Diffraction routes replies **back to the channel where a message came from**. The
...
Diffraction infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Diffraction still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Diffraction would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.diffraction/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.diffraction`):

- `~/.diffraction/agents/<agentId>/sessions/sessions.json`
