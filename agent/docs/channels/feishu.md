...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Diffraction to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Diffraction releases, so no separate plugin install
is required.
...
```bash
diffraction plugins install @diffraction/feishu
```
...
If you just installed Diffraction, run onboarding:
...
2. Configuring app credentials in Diffraction
...
1. You already ran `diffraction channels add` for Feishu
2. The gateway is running (`diffraction gateway status`)
...
Edit `~/.diffraction/diffraction.json`:
...
```bash
diffraction pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  diffraction pairing list feishu
  diffraction pairing approve feishu <CODE>
  ```
...
2. Run `diffraction logs --follow` and look for `chat_id`
...
2. Run `diffraction logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `diffraction gateway status`  | Show gateway status           |
| `diffraction gateway install` | Install/start gateway service |
| `diffraction gateway stop`    | Stop gateway service          |
| `diffraction gateway restart` | Restart gateway service       |
| `diffraction logs --follow`   | Tail gateway logs             |
...
4. Check logs: `diffraction logs --follow`
...
5. Ensure the gateway is running: `diffraction gateway status`
6. Check logs: `diffraction logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.diffraction/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.diffraction/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
