...
read_when:
  - You want Diffraction to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Diffraction to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`diffraction onboard`) and `diffraction channels add` list optional channel plugins.
...
```bash
diffraction plugins install @diffraction/nostr
```

Use a local checkout (dev workflows):

```bash
diffraction plugins install --link <path-to-diffraction>/extensions/nostr
```
...
diffraction channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "diffraction@example.com",
        lud16: "diffraction@example.com",
      },
    },
  },
}
