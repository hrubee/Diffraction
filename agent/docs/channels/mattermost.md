---
summary: "Mattermost bot setup and Diffraction config"
read_when:
...
```bash
diffraction plugins install @diffraction/mattermost
```

Local checkout (when running from a git repo):

```bash
diffraction plugins install ./extensions/mattermost
```
...
4. Configure Diffraction and start the gateway.
...
Native slash commands are opt-in. When enabled, Diffraction registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Diffraction derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Diffraction.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Diffraction.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Diffraction, not `404`.
...
- Approve via:
  - `diffraction pairing list mattermost`
  - `diffraction pairing approve mattermost <CODE>`
...
Use these target formats with `diffraction message send` or cron/webhooks:
...
Diffraction resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Diffraction sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Diffraction sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Diffraction derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Diffraction run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="diffraction-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"diffraction-mattermost-interactions",
...
`diffraction message send` and cron/webhook deliveries.
