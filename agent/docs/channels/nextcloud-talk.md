...
```bash
diffraction plugins install @diffraction/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
diffraction plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Diffraction" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `diffraction pairing list nextcloud-talk`
  - `diffraction pairing approve nextcloud-talk <CODE>`
