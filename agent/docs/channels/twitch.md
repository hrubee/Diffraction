...
read_when:
  - Setting up Twitch chat integration for Diffraction
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Diffraction connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
diffraction plugins install @diffraction/twitch
```

Local checkout (when running from a git repo):

```bash
diffraction plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `DIFFRACTION_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "diffraction", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use DIFFRACTION_TWITCH_ACCESS_TOKEN env var)
...
```bash
DIFFRACTION_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
