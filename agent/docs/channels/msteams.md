...
```bash
diffraction plugins install @diffraction/msteams
```

Local checkout (when running from a git repo):

```bash
diffraction plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
Diffraction will offer the local install path automatically.
...
3. Configure Diffraction with those credentials.
...
- Talk to Diffraction via Teams DMs, group chats, or channels.
...
- On startup, Diffraction resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.diffraction/diffraction.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Diffraction, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `diffraction-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `diffraction plugins install @diffraction/msteams`
   - From a local checkout: `diffraction plugins install ./extensions/msteams`
...
```json5
{
  ...
  description: { short: "Diffraction in Teams", full: "Diffraction in Teams" },
...
Diffraction handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Diffraction only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/DiffractionShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Diffraction sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `diffraction message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.diffraction/msteams-polls.json`.
...
```bash
diffraction message send --channel msteams \
...
diffraction message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
diffraction message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
diffraction message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
diffraction message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
