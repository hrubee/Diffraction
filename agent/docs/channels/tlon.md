...
Tlon is a decentralized messenger built on Urbit. Diffraction connects to your Urbit ship and can
...
```bash
diffraction plugins install @diffraction/tlon
```

Local checkout (when running from a git repo):

```bash
diffraction plugins install ./extensions/tlon
```
...
By default, Diffraction blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `diffraction message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Diffraction replies in-thread.
