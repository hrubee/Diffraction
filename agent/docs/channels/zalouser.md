...
read_when:
  - Setting up Zalo Personal for Diffraction
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Diffraction.
...
- Install via CLI: `diffraction plugins install @diffraction/zalouser`
- Or from a source checkout: `diffraction plugins install ./extensions/zalouser`
...
- `diffraction pairing list zalouser`
- `diffraction pairing approve zalouser <code>`
...
- On startup, Diffraction resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Diffraction stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Diffraction state. Example:
...
- Diffraction sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Diffraction sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `diffraction channels logout --channel zalouser && diffraction channels login --channel zalouser`
...
- The channel now runs fully in Diffraction without external CLI binaries.
