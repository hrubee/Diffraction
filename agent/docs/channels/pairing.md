...
read_when:
  ...
  - Reviewing Diffraction security posture
title: "Pairing"
---

# Pairing

“Pairing” is Diffraction’s explicit **owner approval** step.
...
Stored under `~/.diffraction/credentials/`:
...
3. On your phone, open the Diffraction iOS app → Settings → Gateway.
...
Stored under `~/.diffraction/devices/`:
...
- The legacy `node.pair.*` API (CLI: `diffraction nodes pending/approve`) is a
