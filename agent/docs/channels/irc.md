...
read_when:
  - You want to connect Diffraction to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

# IRC

Use IRC when you want Diffraction in classic channels (`#room`) and direct messages.
IRC ships as an extension plugin, but it is configured in the main config under `channels.irc`.

## Quick start

1. Enable IRC config in `~/.diffraction/diffraction.json`.
2. Set at least:
...
```json5
{
  channels: {
    irc: {
      ...
      port: 6697,
      tls: true,
      nick: "diffraction-bot",
      channels: ["#diffraction"],
    },
  },
}
```
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Diffraction defaults to **mention-gating** in group contexts.
