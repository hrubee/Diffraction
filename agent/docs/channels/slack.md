...
<Tabs>
  <Tab title="Socket Mode (default)">
    <Steps>
      ...
      <Step title="Configure Diffraction">
...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Diffraction HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `diffraction pairing approve slack <code>`.
...
- The interactive callback values are Diffraction-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Diffraction falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Diffraction is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Diffraction",
    "description": "Slack connector for Diffraction"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/diffraction",
        "description": "Send a message to Diffraction",
        "should_escape": false
      }
    ]
  },
...
```bash
diffraction pairing list slack
```
...
Diffraction supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Diffraction falls back to normal delivery for remaining payloads.
