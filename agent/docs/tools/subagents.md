...
- For manual spawns, delivery is resilient:
  - Diffraction tries direct `agent` delivery first with a stable idempotency key.
...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, Diffraction uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout).
...
2. Diffraction creates or binds a thread to that session target in the active channel.
...
- Otherwise delivery depends on requester depth:
  ...
  - if a nested requester subagent session is gone, Diffraction falls back to that session's requester when available
