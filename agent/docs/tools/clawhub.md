---
summary: "ClawHub guide: public registry, native Diffraction install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Diffraction skills and plugins**.

- Use native `diffraction` commands to search/install/update skills and install
...
```bash
diffraction plugins install clawhub:<package>
...
Native `diffraction` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for Diffraction skills.
...
4. Users browse, download, and install skills in Diffraction.
...
If you want to add new capabilities to your Diffraction agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `diffraction skills install <skill-slug>`
3. Start a new Diffraction session so it picks up the new skill.
...
## How it fits into Diffraction

Native `diffraction skills install` installs into the active workspace `skills/`
directory. `diffraction plugins install clawhub:...` records a normal managed
...
current working directory. If an Diffraction workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Diffraction loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.diffraction/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Diffraction how to perform a
...
Interested in becoming a moderator? Ask in the Diffraction Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Diffraction workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/diffraction/skills` and `~/.diffraction/skills`). This is designed to find older skill installs without extra flags.
