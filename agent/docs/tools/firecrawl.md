...
Diffraction can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `diffraction configure --section web` enables the bundled Firecrawl plugin automatically.
...
Diffraction always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
