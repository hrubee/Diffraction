...
read_when:
  - Running Diffraction Gateway in WSL2 while Chrome lives on Windows
...
- Diffraction Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/diffraction/diffraction/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Diffraction and Chrome are on the same machine
...
- Diffraction points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Diffraction is not the problem yet.
...
Fix that before touching Diffraction config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Diffraction at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Diffraction to succeed
...
```bash
diffraction browser open https://example.com --browser-profile remote
diffraction browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `diffraction browser tabs` returns the target
...
3. Diffraction config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Diffraction config or Control UI auth
