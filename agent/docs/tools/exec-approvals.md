...
- **gateway host** → `diffraction` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Diffraction also tries to bind
...
`~/.diffraction/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.diffraction/exec-approvals.sock",
...
When `tools.exec.strictInlineEval=true`, Diffraction treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.diffraction/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `diffraction approvals allowlist ...`).
- `diffraction security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `diffraction doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Diffraction still rejects the `env` builtin in safe-bin
...
`~/.diffraction/exec-approvals.json` directly.

CLI: `diffraction approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Diffraction does not treat
...
topics, Diffraction preserves the topic for the approval prompt and the post-approval follow-up.
