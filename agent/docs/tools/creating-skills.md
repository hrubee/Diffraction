...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.diffraction/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Diffraction picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    diffraction agent --message "give me a greeting"
    ```
...
| `metadata.diffraction.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.diffraction.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.diffraction.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `diffraction agent --message "..."` to test before sharing
...
| `~/.diffraction/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Diffraction) | Lowest     | Global                |
