---
summary: "Diffraction tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools Diffraction provides
...
Diffraction has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). Diffraction ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    Some plugins are **core** (shipped with Diffraction), others are **external**
...
These tools ship with Diffraction and are available without installing any plugins:
...
| `group:nodes`      | nodes                                                                          |
| `group:diffraction`   | All built-in Diffraction tools (excludes plugin tools)                            |
