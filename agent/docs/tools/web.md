...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    diffraction configure --section web
    ```
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Diffraction plugin SDK" });
    ```
...
If no `provider` is set, Diffraction checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  Diffraction resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
<Tabs>
  <Tab title="Config file">
    Run `diffraction configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.diffraction/.env`.
