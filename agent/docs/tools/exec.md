...
- `gateway`/`node` approvals are controlled by `~/.diffraction/exec-approvals.json`.
...
- Diffraction sets `DIFFRACTION_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Diffraction prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`diffraction security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `diffraction doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`diffraction security audit` and `diffraction doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
