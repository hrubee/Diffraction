...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `diffraction sandbox explain`.
...
Legacy `agent.*` configs are migrated by `diffraction doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=diffraction-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${DIFFRACTION_STATE_DIR:-$HOME/.diffraction}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
