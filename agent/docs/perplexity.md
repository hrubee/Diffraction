...
Diffraction supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Diffraction also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `diffraction configure --section web`. It stores the key in
`~/.diffraction/diffraction.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.diffraction/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
