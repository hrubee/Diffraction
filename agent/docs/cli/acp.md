...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Diffraction Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`diffraction acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Diffraction gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
diffraction acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
diffraction acp --url wss://gateway-host:18789 --token-file ~/.diffraction/gateway.token

# Attach to an existing session key
diffraction acp --session agent:main:main

# Attach by label (must already exist)
diffraction acp --session-label "support inbox"

# Reset the session key before the first prompt
diffraction acp --session agent:main:main --reset-session
```
...
```bash
diffraction acp client

# Point the spawned bridge at a remote Gateway
diffraction acp client --server-args --url wss://gateway-host:18789 --token-file ~/.diffraction/gateway.token

# Override the server command (default: diffraction)
diffraction acp client --server "node" --server-args diffraction.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an Diffraction Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `diffraction acp` over stdio.

Example config (persisted):

```bash
diffraction config set gateway.remote.url wss://gateway-host:18789
diffraction config set gateway.remote.token <token>
```
...
```bash
diffraction acp --session agent:main:main
diffraction acp --session agent:design:main
diffraction acp --session agent:qa:bug-123
```
...
Diffraction bot over ACP, use `acpx` with its built-in `diffraction` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx diffraction` at `diffraction acp`.
3. Target the Diffraction session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Diffraction ACP session
acpx diffraction exec "Summarize the active Diffraction session state."

# Persistent named session for follow-up turns
acpx diffraction sessions ensure --name codex-bridge
acpx diffraction -s codex-bridge --cwd /path/to/repo \
  "Ask my Diffraction work agent for recent context relevant to this repo."
```

If you want `acpx diffraction` to target a specific Gateway and session key every
time, override the `diffraction` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "diffraction": {
      "command": "env DIFFRACTION_HIDE_BANNER=1 DIFFRACTION_SUPPRESS_NOTES=1 diffraction acp --url ws://127.0.0.1:18789 --token-file ~/.diffraction/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Diffraction checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env DIFFRACTION_HIDE_BANNER=1 DIFFRACTION_SUPPRESS_NOTES=1 node diffraction.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Diffraction agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Diffraction ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`DIFFRACTION_GATEWAY_TOKEN`, `DIFFRACTION_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`DIFFRACTION_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `DIFFRACTION_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `diffraction acp client` sets `DIFFRACTION_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `diffraction`).
