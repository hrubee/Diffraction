---
summary: "CLI reference for `diffraction configure` (interactive configuration prompts)"
read_when:
...
Tip: `diffraction config` without a subcommand opens the same wizard. Use
`diffraction config get|set|unset` for non-interactive edits.
...
```bash
diffraction configure
diffraction configure --section model --section channels
```
