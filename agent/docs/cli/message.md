---
summary: "CLI reference for `diffraction message` (send + channel actions)"
read_when:
...
```
diffraction message <subcommand> [flags]
```
...
- On cache miss, Diffraction will attempt a live directory lookup when the provider supports it.
...
- `diffraction message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Diffraction does not force a `default` account SecretRef scope
...
```
diffraction message send --channel discord \
...
```
diffraction message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
diffraction message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
diffraction message poll --channel msteams \
...
```
diffraction message react --channel slack \
...
```
diffraction message react --channel signal \
...
```
diffraction message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
diffraction message send --channel telegram --target @mychat \
