---
summary: "CLI reference for `diffraction qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
diffraction qr
diffraction qr --setup-code-only
diffraction qr --json
diffraction qr --remote
diffraction qr --url wss://gateway.example/ws
```
