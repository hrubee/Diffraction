---
summary: "CLI reference for `diffraction onboard` (interactive onboarding)"
read_when:
...
```bash
diffraction onboard
diffraction onboard --flow quickstart
diffraction onboard --flow manual
diffraction onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`DIFFRACTION_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `diffraction gateway run`.
...
- Fastest first chat: `diffraction dashboard` (Control UI, no channel setup).
