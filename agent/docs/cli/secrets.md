---
summary: "CLI reference for `diffraction secrets` (reload, audit, configure, apply)"
read_when:
...
Use `diffraction secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
diffraction secrets reload
diffraction secrets reload --json
```
...
Scan Diffraction state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `diffraction.json` refs)
...
```bash
diffraction secrets audit
diffraction secrets audit --check
diffraction secrets audit --json
diffraction secrets audit --allow-exec
```
...
```bash
diffraction secrets configure
diffraction secrets configure --plan-out /tmp/diffraction-secrets-plan.json
diffraction secrets configure --apply --yes
diffraction secrets configure --providers-only
diffraction secrets configure --skip-provider-setup
diffraction secrets configure --agent ops
diffraction secrets configure --json
```
...
- `configure` targets secret-bearing fields in `diffraction.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Diffraction fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
diffraction secrets apply --from /tmp/diffraction-secrets-plan.json --json
```
...
- `diffraction.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.diffraction/.env` known secret keys whose values were migrated
