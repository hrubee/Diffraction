...
- With Anthropic API-key auth profiles, Diffraction seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Diffraction injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Diffraction exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.diffraction/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$DIFFRACTION_STATE_DIR/logs/cache-trace.jsonl`
...
- `DIFFRACTION_CACHE_TRACE=1` enables cache tracing.
- `DIFFRACTION_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `DIFFRACTION_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `DIFFRACTION_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `DIFFRACTION_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
