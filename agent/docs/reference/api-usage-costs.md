...
Diffraction features that can generate provider usage or paid API calls.
...
- `diffraction status --usage` and `diffraction channels list` show provider **usage windows**
...
Diffraction can pick up credentials from:
...
- `diffraction status --usage`
- `diffraction models status --json`
...
`diffraction models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
