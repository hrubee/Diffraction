...
- Gateway integration: opt-in via `DIFFRACTION_TEST_INCLUDE_GATEWAY=1 pnpm test` or `pnpm test:gateway`.
- `pnpm test:e2e`: Runs gateway end-to-end smoke tests (multi-instance WS/HTTP/node pairing). Defaults to `forks` + adaptive workers in `vitest.e2e.config.ts`; tune with `DIFFRACTION_E2E_WORKERS=<n>` and set `DIFFRACTION_E2E_VERBOSE=1` for verbose logs.
...
- `DIFFRACTION_TEST_PROFILE=low DIFFRACTION_TEST_SERIAL_GATEWAY=1 pnpm test`
- `DIFFRACTION_VITEST_FS_MODULE_CACHE=0 pnpm test:changed`

## Model latency bench (local keys)

Script: [`scripts/bench-model.ts`](https://github.com/diffraction/diffraction/blob/main/scripts/bench-model.ts)
...
Script: [`scripts/bench-cli-startup.ts`](https://github.com/diffraction/diffraction/blob/main/scripts/bench-cli-startup.ts)
...
This script drives the interactive wizard via a pseudo-tty, verifies config/workspace/session files, then starts the gateway and runs `diffraction health`.
