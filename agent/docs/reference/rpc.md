...
Diffraction integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Diffraction owns lifecycle when `channels.signal.autoStart=true`.
...
- Diffraction spawns `imsg rpc` as a child process (legacy iMessage integration).
