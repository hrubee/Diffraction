...
This is the full reference for `diffraction onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.diffraction/diffraction.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `diffraction doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.diffraction/credentials/oauth.json`; auth profiles live in `~/.diffraction/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.diffraction/credentials/oauth.json` (or `$DIFFRACTION_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.diffraction/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `diffraction pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `diffraction configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `diffraction health`.
    - Tip: `diffraction status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export DIFFRACTION_GATEWAY_TOKEN="your-token"
diffraction onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env DIFFRACTION_GATEWAY_TOKEN
```
...
- Stores it under `~/.diffraction/tools/signal-cli/<version>/`.
...
Typical fields in `~/.diffraction/diffraction.json`:
...
`diffraction agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.diffraction/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.diffraction/agents/<agentId>/sessions/`.
