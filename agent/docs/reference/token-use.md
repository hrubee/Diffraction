---
summary: "How Diffraction builds prompt context and reports token usage + costs"
read_when:
...
Diffraction tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Diffraction assembles its own system prompt on every run. It includes:
...
For images, Diffraction downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Diffraction session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `diffraction status --usage` and `diffraction channels list` show
...
`cacheWrite`. If pricing is missing, Diffraction shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Diffraction can
...
Anthropic's 1M context window is currently beta-gated. Diffraction can inject the
...
Diffraction skips the `context-1m-*` beta header because Anthropic currently
