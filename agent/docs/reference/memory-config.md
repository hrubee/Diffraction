...
summary: "Full configuration reference for Diffraction memory search, embedding providers, QMD backend, hybrid search, and multimodal memory"
read_when:
...
This page covers the full configuration surface for Diffraction memory search. For
...
- Configure memory search under `agents.defaults.memorySearch` (not top-level
  `memorySearch`).
- Uses remote embeddings by default. If `memorySearch.provider` is not set, Diffraction auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. Diffraction
...
BM25 + vectors + reranking. Markdown stays the source of truth; Diffraction shells
...
- The gateway runs QMD in a self-contained XDG home under
  `~/.diffraction/agents/<agentId>/qmd/` by setting `XDG_CONFIG_HOME` and
  `XDG_CACHE_HOME`.
...
- The gateway writes a self-contained QMD home under
  `~/.diffraction/agents/<agentId>/qmd/` (config + cache + sqlite DB).
...
- Searches run via `memory.qmd.searchMode` (default `qmd search --json`; also
  ...
  QMD build, Diffraction retries with `qmd query`. If QMD fails or the binary is
  missing, Diffraction automatically falls back to the builtin SQLite manager so
  memory tools keep working.
- Diffraction does not expose QMD embed batch-size tuning today; batch behavior is
  controlled by QMD itself.
- **First search may be slow**: QMD may download local GGUF models (reranker/query
  expansion) on the first `qmd query` run.
  - Diffraction sets `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` automatically when it runs QMD.
  - If you want to pre-download models manually (and warm the same index Diffraction
    uses), run a one-off query with the agent's XDG dirs.

    Diffraction's QMD state lives under your **state dir** (defaults to `~/.diffraction`).
    You can point `qmd` at the exact same index by exporting the same XDG vars
    Diffraction uses:

    ```bash
    # Pick the same state dir Diffraction uses
    STATE_DIR="${DIFFRACTION_STATE_DIR:-$HOME/.diffraction}"
...
- When `scope` denies a search, Diffraction logs a warning with the derived
  `channel`/`chatType` so empty results are easier to debug.
- Snippets sourced outside the workspace show up as
  `qmd/<collection>/<relative-path>` in `memory_search` results; `memory_get`
  understands that prefix and reads from the configured QMD collection root.
- When `memory.qmd.sessions.enabled = true`, Diffraction exports sanitized session
  transcripts (User/Assistant turns) into a dedicated QMD collection under
  `~/.diffraction/agents/<id>/qmd/sessions/`, so `memory_search` can recall recent
...
- If `memorySearch.multimodal.enabled = true`, Diffraction also indexes supported image/audio files under `extraPaths` only. Default memory roots (`MEMORY.md`, `memory.md`, `memory/**/*.md`) stay Markdown-only.
- Symlinks are ignored (files or directories).

## Multimodal memory files (Gemini image + audio)

Diffraction can index image and audio files from `memorySearch.extraPaths` when using Gemini embedding 2:
...
> Diffraction will automatically reindex when it detects a model or dimension change.
...
- Index storage: per-agent SQLite at `~/.diffraction/memory/<agentId>.sqlite` (configurable via `agents.defaults.memorySearch.store.path`, supports `{agentId}` token).
- Freshness: watcher on `MEMORY.md` + `memory/` marks the index dirty (debounce 1.5s). Sync is scheduled on session start, on search, or on an interval and runs asynchronously. Session transcripts use delta thresholds to trigger background sync.
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, Diffraction automatically resets and reindexes the entire store.

## Hybrid search (BM25 + vector)

When enabled, Diffraction combines:

- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, Diffraction falls back to vector-only search.
...
Diffraction can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
- Session logs live on disk (`~/.diffraction/agents/<agentId>/sessions/*.jsonl`). Any process/user with filesystem access can read them, so treat disk access as the trust boundary. For stricter isolation, run agents under separate OS users or hosts.
...
When the sqlite-vec extension is available, Diffraction stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Diffraction logs the
