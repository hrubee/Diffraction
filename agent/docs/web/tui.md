...
```bash
diffraction tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `diffraction tui --deliver`
...
- Local shell commands receive `DIFFRACTION_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `DIFFRACTION_THEME=light` before launching `diffraction tui`.
- To force the original dark palette instead, set `DIFFRACTION_THEME=dark`.
...
- Check the Gateway logs: `diffraction logs --follow`.
- Confirm the agent can run: `diffraction status` and `diffraction models status`.
...
- No agents in picker: check `diffraction agents list` and your routing config.
