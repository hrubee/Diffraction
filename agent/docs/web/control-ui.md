...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/diffraction`)
...
If the page fails to load, start the Gateway first: `diffraction gateway`.
...
created. Re-run `diffraction devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `diffraction devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.diffraction/diffraction.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop diffraction`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Diffraction
...
Diffraction **blocks** Control UI connections without device identity.
