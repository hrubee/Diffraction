...
- Same path: run the app from a fixed location (for Diffraction, `dist/Diffraction.app`).
...
```bash
sudo tccutil reset Accessibility ai.diffraction.mac
sudo tccutil reset ScreenCapture ai.diffraction.mac
...
Workaround: move files into the Diffraction workspace (`~/.diffraction/workspace`) if you want to avoid per-folder grants.
