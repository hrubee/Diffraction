...
service via the external `diffraction` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.diffraction.gateway`
  (or `ai.diffraction.<profile>` when using `--profile`/`DIFFRACTION_PROFILE`; legacy `com.diffraction.*` is supported).
