---
summary: "Diffraction on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Diffraction on DigitalOcean
  - Looking for cheap VPS hosting for Diffraction
title: "DigitalOcean (Platform)"
---

# Diffraction on DigitalOcean

## Goal

Run a persistent Diffraction Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status diffraction-gateway.service

# View logs
journalctl --user -u diffraction-gateway.service -f
```
...
```bash
diffraction config set gateway.bind tailnet
diffraction gateway restart
```
...
```bash
diffraction channels login whatsapp
# Scan QR code
```
...
- `~/.diffraction/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u diffraction --no-pager -n 50
```
