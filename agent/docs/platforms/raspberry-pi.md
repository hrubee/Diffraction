---
summary: "Diffraction on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Diffraction on a Raspberry Pi
  - Running Diffraction on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# Diffraction on Raspberry Pi

## Goal

Run a persistent, always-on Diffraction Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u diffraction -f
```

## 9) Access the Diffraction Dashboard
...
```bash
ssh user@gateway-host 'diffraction dashboard --no-open'
```
...
(or `DIFFRACTION_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/diffraction-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/diffraction-compile-cache
mkdir -p /var/tmp/diffraction-compile-cache
export DIFFRACTION_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `DIFFRACTION_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Diffraction, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit diffraction
```

```ini
[Service]
Environment=DIFFRACTION_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/diffraction-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Diffraction state/cache on SSD-backed storage to avoid SD-card
...
Most Diffraction features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled diffraction

# Enable if not
sudo systemctl enable diffraction

# Start on boot
sudo systemctl start diffraction
```
...
```bash
# Check logs
journalctl -u diffraction --no-pager -n 100

# Common fix: rebuild
cd ~/diffraction  # if using hackable install
npm run build
sudo systemctl restart diffraction
```
