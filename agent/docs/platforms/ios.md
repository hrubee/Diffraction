...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `diffraction.internal.`), **or**
...
Run `diffraction devices list` again before approval.

4. Verify connection:

```bash
diffraction nodes status
diffraction gateway call node.list --params "{}"
```
...
- `DIFFRACTION_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine Diffraction iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Diffraction builds.
...
```bash
export DIFFRACTION_APNS_TEAM_ID="TEAMID"
export DIFFRACTION_APNS_KEY_ID="KEYID"
export DIFFRACTION_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_diffraction-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `diffraction.internal.`) and Tailscale split DNS.
...
```bash
diffraction nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__diffraction__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__diffraction__/canvas/` and `/__diffraction__/a2ui/`.
...
```bash
diffraction nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__diffraction; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
diffraction nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `diffraction devices list` and approve manually.
