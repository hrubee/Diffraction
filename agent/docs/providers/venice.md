---
summary: "Use Venice AI privacy-focused models in Diffraction"
read_when:
  - You want privacy-focused inference in Diffraction
...
```bash
diffraction onboard --auth-choice venice-api-key
```
...
```bash
diffraction agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Diffraction shows all available Venice models. Pick based on your needs:
...
```bash
diffraction models set venice/kimi-k2-5
diffraction models set venice/claude-opus-4-6
```

List all available models:

```bash
diffraction models list | grep venice
```

## Configure via `diffraction configure`

1. Run `diffraction configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Diffraction automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
diffraction agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
diffraction agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
diffraction agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
diffraction agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
diffraction agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `diffraction models list` to see currently available models. Some models may be temporarily offline.
