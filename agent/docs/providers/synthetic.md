---
summary: "Use Synthetic's Anthropic-compatible API in Diffraction"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Diffraction registers it as the
...
```bash
diffraction onboard --auth-choice synthetic-api-key
```
...
Note: Diffraction's Anthropic client appends `/v1` to the base URL, so use
