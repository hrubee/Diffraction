---
summary: "Run Diffraction with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Diffraction against a local SGLang server
...
Diffraction can connect to SGLang using the `openai-completions` API.

Diffraction can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Diffraction will query:
