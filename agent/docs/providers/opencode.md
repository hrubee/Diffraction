---
summary: "Use OpenCode Zen and Go catalogs with Diffraction"
read_when:
...
OpenCode exposes two hosted catalogs in Diffraction:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Diffraction keeps the runtime provider ids
...
diffraction onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
diffraction onboard --auth-choice opencode-go
diffraction onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
