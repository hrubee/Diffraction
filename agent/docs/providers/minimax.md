---
summary: "Use MiniMax models in Diffraction"
read_when:
  - You want MiniMax models in Diffraction
...
Diffraction's MiniMax provider defaults to **MiniMax M2.7** and keeps
...
```bash
diffraction plugins enable minimax  # skip if already loaded.
diffraction gateway restart  # restart if gateway is already running
diffraction onboard --auth-choice minimax-portal
```
...
See [MiniMax plugin README](https://github.com/diffraction/diffraction/tree/main/extensions/minimax) for details.
...
Configure manually via `diffraction.json`:
...
- Use `diffraction models list` and `diffraction models set minimax/MiniMax-M2.7` to switch.
...
- Running `diffraction configure` and selecting a **MiniMax** auth option, or
