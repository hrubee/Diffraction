...
read_when:
  - You want to use Groq with Diffraction
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Diffraction connects
...
media-understanding provider, Diffraction uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.diffraction/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `diffraction models list | grep groq`
