...
This guide summarizes a sane workflow for working on the pi integration in Diffraction.
...
```bash
DIFFRACTION_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm diffraction agent --message "Hello" --thinking low`
...
State lives under the Diffraction state directory. Default is `~/.diffraction`. If `DIFFRACTION_STATE_DIR` is set, use that directory instead.

To reset everything:

- `diffraction.json` for config
