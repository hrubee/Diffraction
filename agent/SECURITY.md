...
If you believe you've found a security issue in Diffraction, please report it privately.

## Reporting

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [diffraction/diffraction](https://github.com/diffraction/diffraction)
- **macOS desktop app** — [diffraction/diffraction](https://github.com/diffraction/diffraction) (apps/macos)
- **iOS app** — [diffraction/diffraction](https://github.com/diffraction/diffraction) (apps/ios)
- **Android app** — [diffraction/diffraction](https://github.com/diffraction/diffraction) (apps/android)
- **ClawHub** — [diffraction/clawhub](https://github.com/diffraction/clawhub)
- **Trust and threat model** — [diffraction/trust](https://github.com/diffraction/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@diffraction.ai](mailto:security@diffraction.ai)** and we'll route it.

For full reporting instructions see our [Trust page](https://trust.diffraction.ai).
...
- Tested version details (Diffraction version and/or commit SHA).
...
- Demonstrated impact tied to Diffraction's documented trust boundaries.
- For exposed-secret reports: proof the credential is Diffraction-owned (or grants access to Diffraction-operated infrastructure/services).
...
**Jamieson O'Reilly** ([@theonejvo](https://twitter.com/theonejvo)) is Security & Trust at Diffraction. Jamieson is the founder of [Dvuln](https://dvuln.com) and brings extensive experience in offensive security, penetration testing, and security program development.

## Bug Bounties

Diffraction is a labor of love. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly.
...
Diffraction does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Diffraction can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need Diffraction, use one VPS (or host/OS user boundary) per user.
...
- This is expected in Diffraction's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

## Trusted Plugin Concept (Core)

Plugins/extensions are part of Diffraction's trusted computing base for a gateway.
...
- Using Diffraction in ways that the docs recommend not to
...
- Reports that require write access to trusted local state (`~/.diffraction`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not Diffraction-owned and not granting access to Diffraction-operated infrastructure/services) without demonstrated Diffraction impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.

## Deployment Assumptions

Diffraction security guidance assumes:

- The host where Diffraction runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.diffraction` state/config (including `diffraction.json`) is effectively a trusted operator.
...
Diffraction's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
Diffraction separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when Diffraction can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the Diffraction process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

## Temp Folder Boundary (Media/Sandbox)

Diffraction uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/diffraction` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/diffraction` (or `diffraction-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the Diffraction-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use Diffraction temp helpers (`resolvePreferredDiffractionTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-diffraction-dir.ts`
...
For threat model + hardening guidance (including `diffraction security audit --deep` and `--fix`), see:

- `https://docs.diffraction.ai/gateway/security`
...
Diffraction's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `diffraction gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - Diffraction keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `diffraction security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__diffraction__/canvas/`, `/__diffraction__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__diffraction__/canvas/`, `/__diffraction__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

Diffraction requires **Node.js 22.12.0 or later** (LTS). This version includes important security patches:
...
When running Diffraction in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v diffraction-data:/app/data \
  diffraction/diffraction:latest
```
