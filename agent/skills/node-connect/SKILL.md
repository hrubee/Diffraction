---
name: node-connect
description: Diagnose Diffraction node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Diffraction is advertising that route, then fix pairing/auth.
...
- whether `diffraction devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `diffraction qr --json`. It uses the same setup-code payload Android scans.

```bash
diffraction config get gateway.mode
diffraction config get gateway.bind
diffraction config get gateway.tailscale.mode
diffraction config get gateway.remote.url
diffraction config get gateway.auth.mode
diffraction config get gateway.auth.allowTailscale
diffraction config get plugins.entries.device-pair.config.publicUrl
diffraction qr --json
diffraction devices list
diffraction nodes status
```

If this Diffraction instance is pointed at a remote gateway, also run:

```bash
diffraction qr --remote --json
```
...
`diffraction qr --json` success means:
...
If `diffraction qr --json` says `Gateway is only bound to loopback`:
...
- `diffraction devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run diffraction qr again, rescan, then approve the pending device pairing.`
