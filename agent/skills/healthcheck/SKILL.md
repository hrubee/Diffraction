---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Diffraction deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Diffraction cron scheduling for periodic checks, or version status checks on a machine running Diffraction (laptop, workstation, Pi, VPS).
---

# Diffraction Host Hardening

## Overview

Assess and harden the host running Diffraction, then align it to a user-defined risk tolerance without breaking access. Use Diffraction security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Diffraction changes the host firewall, SSH, or OS updates; it does not.
...
5. Diffraction gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Diffraction can access sensitive data.
...
### 2) Run Diffraction security audits (read-only)

As part of the default read-only checks, run `diffraction security audit --deep`. Only offer alternatives if the user requests them:

1. `diffraction security audit` (faster, non-probing)
2. `diffraction security audit --json` (structured output)

Offer to apply Diffraction safe defaults (numbered):

1. `diffraction security audit --fix`

Be explicit that `--fix` only tightens Diffraction defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Diffraction version/update status (read-only)

As part of the default read-only checks, run `diffraction update status`.
...
- Credential hygiene notes (location of Diffraction creds, prefer disk encryption)
...
- Diffraction security audit (re-run)
...
After Diffraction install or first hardening pass, run at least one baseline audit and version check:

- `diffraction security audit`
- `diffraction security audit --deep`
- `diffraction update status`

Ongoing monitoring is recommended. Use the Diffraction cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `diffraction cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `diffraction update status`
...
Before creating, `diffraction cron list` and match on exact `name`. If found, `diffraction cron edit <id> ...`.
If not found, `diffraction cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `diffraction update status` (preferred for source checkouts and channels)
2. `npm view diffraction version` (published npm version)

## Diffraction command accuracy

Use only supported commands and flags:

- `diffraction security audit [--deep] [--fix] [--json]`
- `diffraction status` / `diffraction status --deep`
- `diffraction health --json`
- `diffraction update status`
- `diffraction cron add|list|runs|run`

Do not invent CLI flags or imply Diffraction enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Diffraction compaction:
