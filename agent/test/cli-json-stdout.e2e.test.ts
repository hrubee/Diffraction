  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.DIFFRACTION_HOME;
        delete env.DIFFRACTION_STATE_DIR;
        delete env.DIFFRACTION_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "diffraction.mjs");
