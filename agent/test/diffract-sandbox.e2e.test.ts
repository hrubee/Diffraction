...
import { createDiffractSandboxBackendFactory } from "../extensions/diffract/src/backend.js";
import { resolveDiffractPluginConfig } from "../extensions/diffract/src/config.js";
import { createSandboxTestContext } from "../src/agents/sandbox/test-fixtures.js";

const DIFFRACTION_OPENSHELL_E2E = process.env.DIFFRACTION_E2E_OPENSHELL === "1";
const DIFFRACTION_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const DIFFRACTION_OPENSHELL_COMMAND =
  process.env.DIFFRACTION_E2E_OPENSHELL_COMMAND?.trim() || "diffract";
...
RUN echo "diffraction-diffract-e2e" > /opt/diffract-e2e-marker.txt
...
function diffractEnv(rootDir: string): NodeJS.ProcessEnv {
...
function buildDiffractPolicyYaml(params: { port: number; binaryPath: string }): string {
  const networkPolicies = `  host_echo:
    name: host-echo
    endpoints:
      - host: host.diffract.internal
        port: ${params.port}
        allowed_ips:
          - "0.0.0.0/0"
    binaries:
...
async function runBackendExec(params: {
  backend: Awaited<ReturnType<ReturnType<typeof createDiffractSandboxBackendFactory>>>;
  command: string;
  allowFailure?: boolean;
  timeoutMs?: number;
}): Promise<ExecResult> {
...
describe("diffract sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && DIFFRACTION_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through Diffract and executes over SSH",
    { timeout: DIFFRACTION_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(DIFFRACTION_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-diffract-e2e-"));
      const env = diffractEnv(rootDir);
      ...
      const gatewayName = `diffraction-e2e-${scopeSuffix}`;
      const scopeKey = `session:diffract-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `diffraction-policy-allow-${scopeSuffix}`;
      const gatewayPort = await allocatePort();
      let hostPolicyServer: HostPolicyServer | null = null;
      const sandboxCfg = {
        mode: "all" as const,
        backend: "diffract" as const,
        ...
        browser: {
          enabled: false,
          image: "diffraction-browser",
          containerPrefix: "diffraction-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
      ...
      const pluginConfig = resolveDiffractPluginConfig({
        command: DIFFRACTION_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      const backendFactory = createDiffractSandboxBackendFactory({ pluginConfig });
      ...
      try {
        ...
        const execResult = await runBackendExec({
          backend,
          command: "pwd && cat /opt/diffract-e2e-marker.txt && cat seed.txt",
          timeoutMs: 2 * 60_000,
        });
        ...
        expect(stdout).toContain("diffraction-diffract-e2e");
        ...
        if (!bridge) {
          throw new Error("diffract backend did not create a filesystem bridge");
        }
        ...
        const blockedGetResult = await runBackendExec({
          backend,
          command: `curl --fail --silent --show-error --max-time 15 "http://host.diffract.internal:${hostPolicyServer.port}/policy-test"`,
          allowFailure: true,
          timeoutMs: 60_000,
        });
        ...
        const allowedGetResult = await runCommand({
          ...
          args: [
            ...
            "--max-time",
            "15",
            `http://host.diffract.internal:${hostPolicyServer.port}/policy-test`,
          ],
          env,
          timeoutMs: 60_000,
        });
