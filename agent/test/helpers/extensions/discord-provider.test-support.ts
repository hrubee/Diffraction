import type { RuntimeEnv } from "diffraction/plugin-sdk/runtime-env";
import type { Mock } from "vitest";
import { expect, vi } from "vitest";
import type { DiffractionConfig } from "../../../extensions/discord/src/runtime-api.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: DiffractionConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): DiffractionConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("diffraction/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("diffraction/plugin-sdk/acp-runtime")>(
    "diffraction/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("diffraction/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("diffraction/plugin-sdk/command-auth")>(
...
vi.mock("diffraction/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("diffraction/plugin-sdk/reply-runtime")>(
    "diffraction/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("diffraction/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("diffraction/plugin-sdk/config-runtime")>(
...
vi.mock("diffraction/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("diffraction/plugin-sdk/runtime-env")>(
    "diffraction/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("diffraction/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("diffraction/plugin-sdk/infra-runtime")>(
