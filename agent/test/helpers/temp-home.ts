...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked DIFFRACTION_HOME.
  delete process.env.DIFFRACTION_HOME;
  process.env.DIFFRACTION_STATE_DIR = path.join(base, ".diffraction");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "diffraction-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".diffraction", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
