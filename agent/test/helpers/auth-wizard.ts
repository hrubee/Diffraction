...
export function requireDiffractionAgentDir(): string {
  const agentDir = process.env.DIFFRACTION_AGENT_DIR;
  if (!agentDir) {
    throw new Error("DIFFRACTION_AGENT_DIR not set");
  }
  return agentDir;
}
