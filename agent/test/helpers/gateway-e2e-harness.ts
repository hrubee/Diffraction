...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `diffraction-e2e-${name}-`));
  const configDir = path.join(homeDir, ".diffraction");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          DIFFRACTION_SKIP_PROVIDERS: "1",
          DIFFRACTION_SKIP_GMAIL_WATCHER: "1",
          DIFFRACTION_SKIP_CRON: "1",
          DIFFRACTION_SKIP_BROWSER_CONTROL_SERVER: "1",
          DIFFRACTION_SKIP_CANVAS_HOST: "1",
          DIFFRACTION_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
