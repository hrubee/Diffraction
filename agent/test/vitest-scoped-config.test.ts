...
describe("resolveVitestIsolation", () => {
  ...
  it("restores isolate mode when explicitly requested", () => {
    expect(resolveVitestIsolation({ DIFFRACTION_TEST_ISOLATE: "1" })).toBe(true);
    expect(resolveVitestIsolation({ DIFFRACTION_TEST_NO_ISOLATE: "0" })).toBe(true);
    expect(resolveVitestIsolation({ DIFFRACTION_TEST_NO_ISOLATE: "false" })).toBe(true);
  });
});
