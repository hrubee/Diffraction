...
process.env.DIFFRACTION_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
// Set HOME/state isolation before importing any runtime Diffraction modules.
const testEnv = withIsolatedTestHome();
afterAll(() => testEnv.cleanup());

installProcessWarningFilter();

const REGISTRY_STATE = Symbol.for("diffraction.pluginRegistryState");
...
const createStubPlugin = (params: {
  ...
  config: {
    ...
    resolveAccount: (cfg: DiffractionConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: DiffractionConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
