...
describe("loadVitestExperimentalConfig", () => {
  ...
  it("enables the filesystem module cache explicitly", () => {
    expect(
      loadVitestExperimentalConfig({
        DIFFRACTION_VITEST_FS_MODULE_CACHE: "1",
      }),
    ).toEqual({
      experimental: {
        fsModuleCache: true,
  ...
  it("allows disabling the filesystem module cache explicitly", () => {
    expect(
      loadVitestExperimentalConfig({
        DIFFRACTION_VITEST_FS_MODULE_CACHE: "0",
      }),
    ).toEqual({});
  });
  ...
  it("enables import timing output and import breakdown reporting", () => {
    expect(
      loadVitestExperimentalConfig({
        DIFFRACTION_VITEST_IMPORT_DURATIONS: "true",
        DIFFRACTION_VITEST_PRINT_IMPORT_BREAKDOWN: "1",
      }),
    ).toEqual({
      experimental: {
        fsModuleCache: true,
