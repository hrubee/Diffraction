...
description: Triage GitHub security advisories for Diffraction with high-confidence close/keep decisions, exact tag and commit verification, trust-model checks, optional hardening notes, and a final reply ready to post and copy to clipboard.
---

# Security Triage

Use when reviewing Diffraction security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/diffraction/diffraction/security-advisories/<GHSA>`.
...
```bash
gh api /repos/diffraction/diffraction/security-advisories/<GHSA>
gh api /repos/diffraction/diffraction/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view diffraction version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo diffraction/diffraction --match title,body,comments -- "<terms>"
gh search prs --repo diffraction/diffraction --match title,body,comments -- "<terms>"
```
