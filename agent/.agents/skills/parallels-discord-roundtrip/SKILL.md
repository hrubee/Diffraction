...
```bash
export DIFFRACTION_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.diffraction/diffraction.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env DIFFRACTION_PARALLELS_DISCORD_TOKEN \
...
- Use the `diffraction` wrapper for guest `message send/read`; `node diffraction.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/diffraction-parallels-smoke.*`
...
- host inbound nonce appears in `diffraction message read` output
