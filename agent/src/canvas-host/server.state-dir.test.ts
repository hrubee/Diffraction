...
describe("canvas host state dir defaults", () => {
  ...
  it("uses DIFFRACTION_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("diffraction-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Diffraction Canvas");
      } finally {
        await handler.close();
      }
    });
