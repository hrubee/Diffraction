...
<title>Diffraction Canvas</title>
<style>
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.diffractionCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.diffractionCanvasA2UIAction &&
        typeof window.diffractionCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.diffractionSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("diffraction:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Diffraction node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.diffractionSendUserAction === "function"
        ? window.diffractionSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.DIFFRACTION_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.DIFFRACTION_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Diffraction Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
