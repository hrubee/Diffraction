...
import { ensureDiffractionExecMarkerOnProcess } from "./infra/diffraction-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "diffraction.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "diffraction.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.DIFFRACTION_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureCliRespawnReady(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        "[diffraction] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `Diffraction ${VERSION} (${commit})` : `Diffraction ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[diffraction] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureCliRespawnReady()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[diffraction] ${parsed.error}`);
      process.exit(2);
    }
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[diffraction] Failed to display help:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
...
function runMainOrRootHelp(argv: string[]): void {
  ...
  import("./cli/run-main.js")
    ...
    .catch((error) => {
      console.error(
        "[diffraction] Failed to start CLI:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
