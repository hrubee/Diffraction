...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): DiffractionConfig {
    const defaults: NonNullable<NonNullable<DiffractionConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: DiffractionConfig) {
