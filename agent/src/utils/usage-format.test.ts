...
describe("usage-format", () => {
  const originalAgentDir = process.env.DIFFRACTION_AGENT_DIR;
  let agentDir: string;

  beforeEach(async () => {
    agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-usage-format-"));
    process.env.DIFFRACTION_AGENT_DIR = agentDir;
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  });
  ...
  afterEach(async () => {
    if (originalAgentDir === undefined) {
      ...
      process.env.DIFFRACTION_AGENT_DIR = originalAgentDir;
    }
    __resetUsageFormatCachesForTest();
    __resetGatewayModelPricingCacheForTest();
  ...
  it("prefers models.json pricing over diffraction config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          openai: {
  ...
  it("falls back to diffraction config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
          anthropic: {
