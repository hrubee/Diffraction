...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".diffraction");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.DIFFRACTION_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$DIFFRACTION_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via DIFFRACTION_STATE_DIR.
