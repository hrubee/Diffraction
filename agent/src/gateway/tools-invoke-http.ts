...
import { createDiffractionTools } from "../agents/diffraction-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-diffraction-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-diffraction-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-diffraction-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-diffraction-thread-id")?.trim() || undefined;
  ...
  const allTools = createDiffractionTools({
