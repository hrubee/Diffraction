...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: DiffractionConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: DiffractionConfig) {
  ...
  async function expectNoTokenGeneration(cfg: DiffractionConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  function createMissingGatewayTokenSecretRefConfig(): DiffractionConfig {
    return {
      gateway: {
        auth: {
          mode: "token",
    ...
    await expectResolvedToken({
      cfg: {
        gateway: {
          auth: {
            mode: "token",
            token: "${DIFFRACTION_GATEWAY_TOKEN}",
          },
        },
      },
      env: {
        DIFFRACTION_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${DIFFRACTION_GATEWAY_TOKEN}",
    });
  });

  it("uses DIFFRACTION_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
      cfg: createMissingGatewayTokenSecretRefConfig(),
      env: {
        DIFFRACTION_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
  ...
  it("uses DIFFRACTION_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await ensureGatewayStartupAuth({
      cfg: {
        gateway: {
          auth: {
      ...
      env: {
        DIFFRACTION_GATEWAY_PASSWORD: "password-from-env", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      persist: true,
    });
