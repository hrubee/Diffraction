...
export function attachDiffractionTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__diffraction && typeof record.__diffraction === "object" && !Array.isArray(record.__diffraction)
      ? (record.__diffraction as Record<string, unknown>)
      : {};
  return {
    ...record,
    __diffraction: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachDiffractionTranscriptMeta(parsed.message, {
...
export function resolveSessionTranscriptCandidates(
  ...
  const legacyDir = path.join(home, ".diffraction", "sessions");
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "diffraction" && model === "delivery-mirror";
