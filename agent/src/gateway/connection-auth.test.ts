...
function cfg(input: Partial<DiffractionConfig>): DiffractionConfig {
  return input as DiffractionConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: DiffractionConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  DIFFRACTION_GATEWAY_TOKEN: "env-token",
  DIFFRACTION_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when DIFFRACTION env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when DIFFRACTION env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when DIFFRACTION env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
