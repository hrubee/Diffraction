...
const ensureDiffractionModelsJsonMock = vi.fn<
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveDiffractionAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureDiffractionModelsJson: (config: unknown, agentDir: unknown) =>
    ensureDiffractionModelsJsonMock(config, agentDir),
}));
...
describe("gateway startup primary model warmup", () => {
  beforeEach(() => {
    ensureDiffractionModelsJsonMock.mockClear();
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureDiffractionModelsJsonMock).toHaveBeenCalledWith(cfg, "/tmp/agent");
  ...
  it("skips warmup when no explicit primary model is configured", async () => {
    ...
    expect(ensureDiffractionModelsJsonMock).not.toHaveBeenCalled();
