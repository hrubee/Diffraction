...
import { ensureDiffractionCliOnPath } from "../infra/path-env.js";
import {
...
import {
  attachDiffractionTranscriptMeta,
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("diffraction doctor")}" to repair, then retry.`
    : "";
...
async function prepareGatewayStartupConfig(params: {
  ...
  ) => Promise<{ config: DiffractionConfig }>;
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.DIFFRACTION_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.DIFFRACTION_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "DIFFRACTION_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "DIFFRACTION_RAW_STREAM_PATH",
  ...
  let cfgAtStart: DiffractionConfig;
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `diffraction config set gateway.auth.mode token` and `diffraction config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  const transcriptUnsub = minimalTestGateway
    ...
    : onSessionTranscriptUpdate((update) => {
        ...
        const sessionSnapshot = sessionRow
          ...
          : {};
        const message = attachDiffractionTranscriptMeta(update.message, {
