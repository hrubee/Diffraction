...
type GatewaySecretDefaults = NonNullable<DiffractionConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${DIFFRACTION_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.DIFFRACTION_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.DIFFRACTION_GATEWAY_PASSWORD);
}
