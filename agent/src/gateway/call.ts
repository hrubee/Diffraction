...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.DIFFRACTION_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.DIFFRACTION_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set DIFFRACTION_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: diffraction doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__DIFFRACTION_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
