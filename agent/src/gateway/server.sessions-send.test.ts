...
const { createDiffractionTools } = await import("../agents/diffraction-tools.js");
...
type SessionSendTool = ReturnType<typeof createDiffractionTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createDiffractionTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["DIFFRACTION_GATEWAY_PORT", "DIFFRACTION_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.DIFFRACTION_GATEWAY_PORT = String(gatewayPort);
  process.env.DIFFRACTION_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("DIFFRACTION_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createDiffractionTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
