...
import { type DiffractionConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: DiffractionConfig): string[] {
...
export function listAgentsForGateway(cfg: DiffractionConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: DiffractionConfig): string {
...
function resolveSessionStoreAgentId(cfg: DiffractionConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: DiffractionConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: DiffractionConfig): GatewaySessionsDefaults {
