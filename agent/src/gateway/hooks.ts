...
export function resolveHooksConfig(cfg: DiffractionConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: DiffractionConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-diffraction-token"] === "string"
      ? req.headers["x-diffraction-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-diffraction-idempotency-key"]) ||
