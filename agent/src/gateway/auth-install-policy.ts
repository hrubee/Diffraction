...
type GatewayInstallAuthMode = NonNullable<NonNullable<DiffractionConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: DiffractionConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.DIFFRACTION_GATEWAY_PASSWORD?.trim() ||
