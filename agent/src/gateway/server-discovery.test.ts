...
describe("resolveTailnetDnsHint", () => {
  ...
  beforeEach(() => {
    prevTailnetDns.value = process.env.DIFFRACTION_TAILNET_DNS;
    delete process.env.DIFFRACTION_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.DIFFRACTION_TAILNET_DNS = prevTailnetDns.value;
    }
  });

  test("returns env hint when disabled", async () => {
    process.env.DIFFRACTION_TAILNET_DNS = "studio.tailnet.ts.net.";
