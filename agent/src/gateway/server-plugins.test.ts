...
const loadDiffractionPlugins = vi.hoisted(() => vi.fn());
...
async function createSubagentRuntime(
  ...
  loadDiffractionPlugins.mockReturnValue(createRegistry([]));
  ...
  const call = loadDiffractionPlugins.mock.calls.at(-1)?.[0] as
...
beforeEach(() => {
  loadDiffractionPlugins.mockReset();
...
describe("loadGatewayPlugins", () => {
  test("logs plugin errors with details", async () => {
    ...
    loadDiffractionPlugins.mockReturnValue(createRegistry(diagnostics));
  ...
  test("includes docs guidance when a plugin fallback override is not trusted", async () => {
    ...
    await expect(
      ...
      'plugin "voice-call" is not trusted for fallback provider/model override requests. See https://docs.diffraction.ai/tools/plugin#runtime-helpers and search for: plugins.entries.<id>.subagent.allowModelOverride',
    );
  });
