...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (/diffraction/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (Diffraction)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.DIFFRACTION_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "diffraction");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.DIFFRACTION_TAILNET_DNS?.trim();
