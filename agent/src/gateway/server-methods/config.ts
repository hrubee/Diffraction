...
import type { ConfigValidationIssue, DiffractionConfig } from "../../config/types.diffraction.js";
import {
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: DiffractionConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadDiffractionPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    runtimeOptions: {
  ...
  // our cache. However, both loadConfig() and loadDiffractionPlugins() already cache
