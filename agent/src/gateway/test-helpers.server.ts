...
const GATEWAY_TEST_ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "DIFFRACTION_STATE_DIR",
  "DIFFRACTION_CONFIG_PATH",
  "DIFFRACTION_SKIP_BROWSER_CONTROL_SERVER",
  "DIFFRACTION_SKIP_GMAIL_WATCHER",
  "DIFFRACTION_SKIP_CANVAS_HOST",
  "DIFFRACTION_BUNDLED_PLUGINS_DIR",
  "DIFFRACTION_SKIP_CHANNELS",
  "DIFFRACTION_SKIP_PROVIDERS",
  "DIFFRACTION_SKIP_CRON",
  "DIFFRACTION_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionStorePath(storePath: string): Promise<void> {
  ...
  if (process.env.DIFFRACTION_CONFIG_PATH) {
    configPaths.add(process.env.DIFFRACTION_CONFIG_PATH);
  }
  if (process.env.DIFFRACTION_STATE_DIR) {
    configPaths.add(path.join(process.env.DIFFRACTION_STATE_DIR, "diffraction.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.DIFFRACTION_STATE_DIR = path.join(tempHome, ".diffraction");
  delete process.env.DIFFRACTION_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.DIFFRACTION_SKIP_BROWSER_CONTROL_SERVER = "1";
  ...
  process.env.DIFFRACTION_SKIP_PROVIDERS = "1";
  process.env.DIFFRACTION_SKIP_CRON = "1";
  process.env.DIFFRACTION_TEST_MINIMAL_GATEWAY = "1";
  process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "diffraction-test-no-bundled-extensions")
    : "diffraction-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  applyGatewaySkipEnv();
  const stateDir = process.env.DIFFRACTION_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".diffraction-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".diffraction-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
...
const CONNECT_CHALLENGE_NONCE_KEY = "__diffractionTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__diffractionTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["DIFFRACTION_GATEWAY_TOKEN"]);
  const prev = process.env.DIFFRACTION_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.DIFFRACTION_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.DIFFRACTION_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
