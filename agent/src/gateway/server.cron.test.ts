...
async function getCronSuiteTempRoot(): Promise<string> {
  if (!cronSuiteTempRootPromise) {
    cronSuiteTempRootPromise = fs.mkdtemp(path.join(os.tmpdir(), "diffraction-gw-cron-suite-"));
  }
...
async function cleanupCronTestRun(params: {
  ...
  process.env.DIFFRACTION_SKIP_CRON = params.prevSkipCron;
}
...
async function setupCronTestRun(params: {
  ...
  const prevSkipCron = process.env.DIFFRACTION_SKIP_CRON;
  process.env.DIFFRACTION_SKIP_CRON = "0";
...
describe("gateway server cron", () => {
  ...
  test("handles cron CRUD, normalization, and patch semantics", { timeout: 20_000 }, async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "diffraction-gw-cron-",
  ...
  test("writes cron run history and auto-runs due jobs", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "diffraction-gw-cron-log-",
    });
  ...
  test("returns from cron.run immediately while isolated work continues in background", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "diffraction-gw-cron-run-detached-",
    });
  ...
  test("returns already-running without starting background work", async () => {
    ...
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "diffraction-gw-cron-run-busy-",
      jobs: [
        {
          id: "busy-job",
          name: "busy job",
  ...
  test("returns not-due without starting background work", async () => {
    const now = Date.now();
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "diffraction-gw-cron-run-not-due-",
      jobs: [
        {
          id: "future-job",
          name: "future job",
  ...
  test("posts webhooks for delivery mode and legacy notify fallback only when summary exists", async () => {
    ...
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "diffraction-gw-cron-webhook-",
      cronEnabled: false,
  ...
  test("ignores non-string cron.webhookToken values without crashing webhook delivery", async () => {
    const { prevSkipCron } = await setupCronTestRun({
      tempPrefix: "diffraction-gw-cron-webhook-secretinput-",
      cronEnabled: false,
    });
