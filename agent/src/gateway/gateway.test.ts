...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "diffraction");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".diffraction");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "diffraction.json");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-wizard-home-"));
      process.env.HOME = tempHome;
