...
async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-gateway-ui-hardlink-"));
  try {
    const packageRoot = path.join(tmp, "pnpm-global", "5", "node_modules", "diffraction");
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global Diffraction package control-ui", async () => {
