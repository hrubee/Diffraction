...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("diffraction doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with DIFFRACTION_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with DIFFRACTION_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.DIFFRACTION_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://diffraction-gateway.ai:18789",
      onConnectError,
    });
