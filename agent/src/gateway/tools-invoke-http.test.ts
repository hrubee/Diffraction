...
let lastCreateDiffractionToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/diffraction-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateDiffractionToolsContext?.agentTo,
          agentThreadId: lastCreateDiffractionToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createDiffractionTools: (ctx: Record<string, unknown>) => {
      lastCreateDiffractionToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateDiffractionToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateDiffractionToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateDiffractionToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-diffraction-message-to": "channel:24514",
        "x-diffraction-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
