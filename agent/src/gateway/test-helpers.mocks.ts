...
const GATEWAY_TEST_PLUGIN_REGISTRY_STATE_KEY = Symbol.for(
  "diffraction.gatewayTestHelpers.pluginRegistryState",
);
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("diffraction.gatewayTestHelpers.configRoot");

const hoisted = vi.hoisted(() => {
  const key = Symbol.for("diffraction.gatewayTestHelpers.hoisted");
...
const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `diffraction-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.DIFFRACTION_CONFIG_PATH = path.join(root, "diffraction.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "diffraction.json");
  ...
  const composeTestConfig = (baseConfig: Record<string, unknown>) => {
    ...
    const defaults = {
      ...
      workspace: path.join(os.tmpdir(), "diffraction-gateway-test"),
      ...fileDefaults,
  ...
  return {
    ...
    applyConfigOverrides: (cfg: DiffractionConfig) =>
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadDiffractionPlugins: () => pluginRegistryState.registry,
  };
});
