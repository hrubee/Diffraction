...
export function resolveTaskScriptPath(env: GatewayServiceEnv): string {
  const override = env.DIFFRACTION_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.DIFFRACTION_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
function resolveConfiguredGatewayPort(env: GatewayServiceEnv): number | null {
  const raw = env.DIFFRACTION_GATEWAY_PORT?.trim();
  if (!raw) {
    return null;
  }
...
async function resolveScheduledTaskPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  return (
    ...
    parsePositivePort(command?.environment?.DIFFRACTION_GATEWAY_PORT) ??
