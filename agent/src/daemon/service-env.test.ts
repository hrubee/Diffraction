...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.DIFFRACTION_GATEWAY_PORT).toBe("18789");
    expect(env.DIFFRACTION_GATEWAY_TOKEN).toBeUndefined();
    expect(env.DIFFRACTION_SERVICE_MARKER).toBe("diffraction");
    expect(env.DIFFRACTION_SERVICE_KIND).toBe("gateway");
    expect(typeof env.DIFFRACTION_SERVICE_VERSION).toBe("string");
    expect(env.DIFFRACTION_SYSTEMD_UNIT).toBe("diffraction-gateway.service");
    expect(env.DIFFRACTION_WINDOWS_TASK_NAME).toBe("Diffraction Gateway");
    if (process.platform === "darwin") {
      expect(env.DIFFRACTION_LAUNCHD_LABEL).toBe("ai.diffraction.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", DIFFRACTION_PROFILE: "work" },
      port: 18789,
    });
    expect(env.DIFFRACTION_SYSTEMD_UNIT).toBe("diffraction-gateway-work.service");
    expect(env.DIFFRACTION_WINDOWS_TASK_NAME).toBe("Diffraction Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.DIFFRACTION_LAUNCHD_LABEL).toBe("ai.diffraction.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through DIFFRACTION_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", DIFFRACTION_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.DIFFRACTION_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits DIFFRACTION_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".diffraction"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", DIFFRACTION_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".diffraction-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", DIFFRACTION_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".diffraction"));
  });

  it("uses DIFFRACTION_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", DIFFRACTION_STATE_DIR: "/var/lib/diffraction" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/diffraction"));
  });

  it("expands ~ in DIFFRACTION_STATE_DIR", () => {
    const env = { HOME: "/Users/test", DIFFRACTION_STATE_DIR: "~/diffraction-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/diffraction-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { DIFFRACTION_STATE_DIR: "C:\\State\\diffraction" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\diffraction");
  });
});
