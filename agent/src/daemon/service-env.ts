...
export function buildServiceEnvironment(params: {
  ...
  return {
    ...
    DIFFRACTION_PROFILE: profile,
    DIFFRACTION_GATEWAY_PORT: String(port),
    DIFFRACTION_LAUNCHD_LABEL: resolvedLaunchdLabel,
    DIFFRACTION_SYSTEMD_UNIT: systemdUnit,
    DIFFRACTION_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    DIFFRACTION_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    DIFFRACTION_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = env.DIFFRACTION_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    DIFFRACTION_GATEWAY_TOKEN: gatewayToken,
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    DIFFRACTION_STATE_DIR: sharedEnv.stateDir,
    DIFFRACTION_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.DIFFRACTION_STATE_DIR;
  const configPath = env.DIFFRACTION_CONFIG_PATH;
