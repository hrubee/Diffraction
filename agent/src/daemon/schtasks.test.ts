...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when DIFFRACTION_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".diffraction", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when DIFFRACTION_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", DIFFRACTION_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".diffraction-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers DIFFRACTION_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        DIFFRACTION_PROFILE: "rescue",
        DIFFRACTION_STATE_DIR: "C:\\State\\diffraction",
      },
      expected: path.join("C:\\State\\diffraction", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", DIFFRACTION_PROFILE: "default" },
      expected: path.join("/home/test", ".diffraction", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Diffraction Gateway",
          "cd /d C:\\Projects\\diffraction",
          "set NODE_ENV=production",
          "set DIFFRACTION_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\diffraction",
          environment: {
            NODE_ENV: "production",
            DIFFRACTION_PORT: "18789",
          },
          sourcePath: resolveTaskScriptPath(env),
        });
      },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\diffraction\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\diffraction\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Diffraction Share\\node.exe" "\\\\fileserver\\Diffraction Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Diffraction Share\\node.exe",
            "\\\\fileserver\\Diffraction Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from DIFFRACTION_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ DIFFRACTION_STATE_DIR: path.join(tmpDir, "custom-state") }),
