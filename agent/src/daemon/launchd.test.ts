...
function expectLaunchctlEnableBootstrapOrder(env: Record<string, string | undefined>) {
  ...
  const label = "ai.diffraction.gateway";
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.diffraction.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", DIFFRACTION_PROFILE: "default" },
    });
...
describe("launchd install", () => {
  ...
  it("uses the configured gateway port for stale cleanup", async () => {
    const env = {
      ...createDefaultLaunchdEnv(),
      DIFFRACTION_GATEWAY_PORT: "19001",
    };
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.diffraction.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when DIFFRACTION_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.diffraction.gateway.plist",
    },
    {
      name: "uses profile-specific label when DIFFRACTION_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", DIFFRACTION_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.diffraction.jbphoenix.plist",
    },
    {
      name: "prefers DIFFRACTION_LAUNCHD_LABEL over DIFFRACTION_PROFILE",
      env: {
        HOME: "/Users/test",
        DIFFRACTION_PROFILE: "jbphoenix",
        DIFFRACTION_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from DIFFRACTION_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        DIFFRACTION_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty DIFFRACTION_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        DIFFRACTION_PROFILE: "myprofile",
        DIFFRACTION_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.diffraction.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
