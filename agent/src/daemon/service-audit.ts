...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds DIFFRACTION_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `diffraction gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service DIFFRACTION_GATEWAY_TOKEN does not match gateway.auth.token in diffraction.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.DIFFRACTION_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.DIFFRACTION_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `diffraction gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
