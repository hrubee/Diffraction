...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `diffraction-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      DIFFRACTION_LAUNCHD_LABEL: `ai.diffraction.launchd-int-${testId}`,
      DIFFRACTION_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
