...
const TEST_MANAGED_HOME = "/tmp/diffraction-test-home";
const GATEWAY_SERVICE = "diffraction-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/diffraction gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/diffraction", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/diffraction-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "diffraction-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "diffraction-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/diffraction-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled diffraction-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled diffraction-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when DIFFRACTION_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/diffraction-gateway.service",
    },
    {
      name: "uses profile-specific service name when DIFFRACTION_PROFILE is set to a custom value",
      env: { HOME: "/home/test", DIFFRACTION_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/diffraction-gateway-jbphoenix.service",
    },
    {
      name: "prefers DIFFRACTION_SYSTEMD_UNIT over DIFFRACTION_PROFILE",
      env: {
        HOME: "/home/test",
        DIFFRACTION_PROFILE: "jbphoenix",
        DIFFRACTION_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles DIFFRACTION_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        DIFFRACTION_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from DIFFRACTION_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        DIFFRACTION_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/diffraction gateway start --name "My Bot"')).toEqual([
      "/usr/bin/diffraction",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('diffraction --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["diffraction", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('diffraction --path "C:\\\\Program Files\\\\Diffraction"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["diffraction", "--path", "C:\\\\Program Files\\\\Diffraction"]);

    expect(
      splitArgsPreservingQuotes('diffraction --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["diffraction", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/diffraction gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads DIFFRACTION_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/diffraction gateway run", "EnvironmentFile=%h/.diffraction/.env"],
      { [`${TEST_SERVICE_HOME}/.diffraction/.env`]: "DIFFRACTION_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.DIFFRACTION_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/diffraction gateway run",
        "EnvironmentFile=%h/.diffraction/.env",
        'Environment="DIFFRACTION_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.diffraction/.env`]: "DIFFRACTION_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.DIFFRACTION_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.DIFFRACTION_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.diffraction/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/diffraction-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/diffraction gateway run",
          'EnvironmentFile=%h/.diffraction/first.env "%h/.diffraction/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.diffraction/first.env") {
        return "DIFFRACTION_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.diffraction/second env.env") {
        return 'DIFFRACTION_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      DIFFRACTION_GATEWAY_TOKEN: "first-token",
      DIFFRACTION_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "DIFFRACTION_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "DIFFRACTION_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "DIFFRACTION_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      DIFFRACTION_GATEWAY_TOKEN: "override-token",
      DIFFRACTION_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/diffraction-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/diffraction gateway run",
          "EnvironmentFile=%h/.diffraction/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.diffraction/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'DIFFRACTION_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "DIFFRACTION_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      DIFFRACTION_GATEWAY_TOKEN: "quoted token",
      DIFFRACTION_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      DIFFRACTION_GATEWAY_TOKEN: "file",
      DIFFRACTION_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "diffraction-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ DIFFRACTION_PROFILE: "work" });
  });
