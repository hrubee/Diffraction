...
declare const __DIFFRACTION_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "diffraction";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["DIFFRACTION_VERSION"],
      runtimeVersion,
      env["DIFFRACTION_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Diffraction version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __DIFFRACTION_VERSION__ === "string" ? __DIFFRACTION_VERSION__ : undefined,
  bundledVersion: process.env.DIFFRACTION_BUNDLED_VERSION,
});
