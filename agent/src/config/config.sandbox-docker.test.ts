...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("diffraction-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "diffraction-sandbox-browser-global" },
      agentBrowser: { network: "diffraction-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("diffraction-sandbox-browser-agent");
  });
