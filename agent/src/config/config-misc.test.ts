...
import { buildWebSearchProviderConfig, withTempHome, writeDiffractionConfig } from "./test-helpers.js";
import { DiffractionSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = DiffractionSchema.safeParse({
      $schema: "https://diffraction.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://diffraction.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = DiffractionSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = DiffractionSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeDiffractionConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeDiffractionConfig(home, {
        gateway: { bind: "${DIFFRACTION_BIND}" },
      });

      const prev = process.env.DIFFRACTION_BIND;
      process.env.DIFFRACTION_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.DIFFRACTION_BIND;
        } else {
          process.env.DIFFRACTION_BIND = prev;
        }
      }
    });
  });
