...
import type { ConfigFileSnapshot } from "./types.diffraction.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.diffraction/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/diffraction/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/diffraction/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/diffraction/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/diffraction/nickserv-password.txt");
