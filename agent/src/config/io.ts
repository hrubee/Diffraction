...
import type { DiffractionConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareDiffractionVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  diffraction config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  diffraction config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: DiffractionConfig,
  pathSegments: string[],
): { changed: boolean; next: DiffractionConfig } {
...
function coerceConfig(value: unknown): DiffractionConfig {
...
function stampConfigVersion(cfg: DiffractionConfig): DiffractionConfig {
...
function warnIfConfigFromFuture(cfg: DiffractionConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareDiffractionVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Diffraction (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as DiffractionConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): DiffractionConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as DiffractionConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: DiffractionConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as DiffractionConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.DIFFRACTION_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.DIFFRACTION_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.DIFFRACTION_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.DIFFRACTION_WATCH_SESSION === "string" &&
        deps.env.DIFFRACTION_WATCH_SESSION.trim().length > 0
          ? deps.env.DIFFRACTION_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.DIFFRACTION_WATCH_COMMAND === "string" &&
        deps.env.DIFFRACTION_WATCH_COMMAND.trim().length > 0
          ? deps.env.DIFFRACTION_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `DIFFRACTION_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: DiffractionConfig | null = null;
let runtimeConfigSourceSnapshot: DiffractionConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.DIFFRACTION_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.DIFFRACTION_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): DiffractionConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): DiffractionConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: DiffractionConfig;
  candidate: DiffractionConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: DiffractionConfig): DiffractionConfig {
...
export function loadConfig(): DiffractionConfig {
...
export async function readBestEffortConfig(): Promise<DiffractionConfig> {
...
export async function writeConfigFile(
  ...
  // edits to diffraction.json remain visible (no stale snapshot).
}
