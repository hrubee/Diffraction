...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "diffraction-config-" });
}

export async function writeDiffractionConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.DIFFRACTION_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected DIFFRACTION_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeDiffractionConfig(home, config);
