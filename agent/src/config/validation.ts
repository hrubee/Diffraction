...
import type { DiffractionConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: DiffractionConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: DiffractionConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: DiffractionConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = DiffractionSchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as DiffractionConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as DiffractionConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as DiffractionConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as DiffractionConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; Diffraction mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: DiffractionConfig | null | undefined;

  const ensureCompatConfig = (): DiffractionConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Diffraction config schema.
