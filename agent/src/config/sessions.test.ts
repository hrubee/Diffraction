...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ DIFFRACTION_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-diffraction",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-diffraction#general");
  });
  ...
  it("derives session transcripts dir from DIFFRACTION_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { DIFFRACTION_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when DIFFRACTION_STATE_DIR differs from stored paths", () => {
