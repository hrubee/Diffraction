...
export function computeBaseConfigSchemaResponse(params?: {
  ...
  const schema = DiffractionSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "DiffractionConfig";
  const hints = applyDerivedTags(mapSensitivePaths(DiffractionSchema, "", buildBaseHints()));
  return {
