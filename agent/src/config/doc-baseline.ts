...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.DIFFRACTION_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveDiffractionPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    DIFFRACTION_STATE_DIR: path.join(os.tmpdir(), "diffraction-config-doc-baseline-state"),
    DIFFRACTION_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
  ...
  const channelPlugins =
    process.env.DIFFRACTION_CONFIG_DOC_BASELINE_DEBUG === "1"
