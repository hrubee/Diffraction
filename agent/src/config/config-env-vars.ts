...
function collectConfigEnvVarsByTarget(cfg?: DiffractionConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: DiffractionConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: DiffractionConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: DiffractionConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.DIFFRACTION_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
