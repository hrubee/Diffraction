...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): DiffractionConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: DiffractionConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        DIFFRACTION_STATE_DIR: envStateDir,
      };
      const cfg: DiffractionConfig = {};
