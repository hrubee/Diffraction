...
import type { DiffractionConfig } from "./config.js";
...
const ENV_CATALOG_PATHS = ["DIFFRACTION_PLUGIN_CATALOG_PATHS", "DIFFRACTION_MPM_CATALOG_PATHS"];

function collectModelRefs(cfg: DiffractionConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: DiffractionConfig, providerId: string): boolean {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.diffraction) || !isRecord(entry.diffraction.channel)) {
      continue;
    }
    const channel = entry.diffraction.channel;
...
function collectCandidateChannelIds(cfg: DiffractionConfig): string[] {
...
function configMayNeedPluginManifestRegistry(cfg: DiffractionConfig): boolean {
  const configuredChannels = cfg.channels as Record<string, unknown> | undefined;
  if (!configuredChannels || typeof configuredChannels !== "object") {
    return false;
  }
...
function isPluginExplicitlyDisabled(cfg: DiffractionConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: DiffractionConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: DiffractionConfig, pluginId: string): DiffractionConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
