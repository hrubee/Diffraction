...
const { makeStorePath } = createCronStoreHarness({ prefix: "diffraction-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses DIFFRACTION_HOME for tilde expansion", () => {
    vi.stubEnv("DIFFRACTION_HOME", "/srv/diffraction-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/diffraction-home", "cron", "jobs.json"));
  });
});
