...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "diffraction-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".diffraction", "sessions");
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<DiffractionConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<DiffractionConfig>,
