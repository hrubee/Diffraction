...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "DIFFRACTION_ENV_TEST_A";
    const keyB = "DIFFRACTION_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "DIFFRACTION_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "DIFFRACTION_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "DIFFRACTION_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "DIFFRACTION_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "DIFFRACTION_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "DIFFRACTION_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "DIFFRACTION_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "diffraction-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousDiffractionHome = process.env.DIFFRACTION_HOME;
    const previousStateDir = process.env.DIFFRACTION_STATE_DIR;
    const previousBundledDir = process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR;
    process.env.DIFFRACTION_HOME = "/srv/diffraction-home";
    process.env.DIFFRACTION_STATE_DIR = "/srv/diffraction-state";
    process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR = "/srv/diffraction-bundled";
    ...
    try {
      ...
      expect(env.DIFFRACTION_HOME).toBeUndefined();
      expect(env.DIFFRACTION_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.DIFFRACTION_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("DIFFRACTION_HOME", previousDiffractionHome);
      restoreEnvKey("DIFFRACTION_STATE_DIR", previousStateDir);
      restoreEnvKey("DIFFRACTION_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { DIFFRACTION_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processDiffractionHome: process.env.DIFFRACTION_HOME,
          processBundledDir: process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.DIFFRACTION_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processDiffractionHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.DIFFRACTION_HOME).toBe("/srv/diffraction-home");
    } finally {
      restoreEnvKey("DIFFRACTION_HOME", previousDiffractionHome);
    }
  });
