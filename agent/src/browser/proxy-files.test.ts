...
describe("persistBrowserProxyFiles", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("diffraction-browser-proxy-files-");
  });
  ...
  it("persists browser proxy files under the shared media store", async () => {
    ...
    expect(path.normalize(savedPath ?? "")).toContain(
      `${path.sep}.diffraction${path.sep}media${path.sep}browser${path.sep}`,
    );
  ...
  it("rejects browser proxy files that exceed the shared media size limit", async () => {
    ...
    await expect(
      fs.stat(path.join(tempHome.home, ".diffraction", "media", "browser")),
    ).rejects.toThrow();
  });
});
