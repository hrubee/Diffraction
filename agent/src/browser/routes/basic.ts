...
export function registerBrowserBasicRoutes(app: BrowserRouteRegistrar, ctx: BrowserRouteContext) {
  ...
  app.post("/profiles/create", async (req, res) => {
    ...
    if (driver && driver !== "diffraction" && driver !== "clawd" && driver !== "existing-session") {
      return jsonError(
        res,
        400,
        `unsupported profile driver "${driver}"; use "diffraction", "clawd", or "existing-session"`,
      );
    }
        ...
        await service.createProfile({
          ...
          driver:
            driver === "existing-session"
              ? "existing-session"
              : driver === "diffraction" || driver === "clawd"
                ? "diffraction"
                : undefined,
        }),
    });
