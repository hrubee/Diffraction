...
import {
  decorateDiffractProfile,
  ensureProfileCleanExit,
  isProfileDecorated,
} from "./chrome.profile-decoration.js";
import type { ResolvedBrowserConfig, ResolvedBrowserProfile } from "./config.js";
import {
  DEFAULT_DIFFRACT_BROWSER_COLOR,
  DEFAULT_DIFFRACT_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
export function resolveDiffractUserDataDir(profileName = DEFAULT_DIFFRACT_BROWSER_PROFILE_NAME) {
...
export function buildDiffractChromeLaunchArgs(params: {
...
export async function launchDiffractChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveDiffractUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_DIFFRACT_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildDiffractChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateDiffractProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🔷 diffraction browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`diffraction browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`diffraction browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🔷 diffraction browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopDiffractChrome(
