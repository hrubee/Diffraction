...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".diffraction-profile-decorated");
}
 ...
 */
export function decorateDiffractionProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_DIFFRACTION_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_DIFFRACTION_BROWSER_COLOR).toUpperCase();
