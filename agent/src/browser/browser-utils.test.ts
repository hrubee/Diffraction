...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    ...
    if (!diffraction) {
      throw new Error("expected diffraction profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...diffraction, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
    ...
    expect(listKnownProfileNames(state).toSorted()).toEqual(["diffraction", "stale-removed", "user"]);
  });
});
