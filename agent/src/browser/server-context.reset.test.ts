...
function localDiffractionProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "diffraction",
...
function createLocalDiffractionResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localDiffractionProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveDiffractionUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-reset-"));
    const profileDir = path.join(tempRoot, "diffraction");
    ...
    const ops = createLocalDiffractionResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveDiffractionUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-reset-no-own-"));
    const profileDir = path.join(tempRoot, "diffraction");
    fs.mkdirSync(profileDir, { recursive: true });
