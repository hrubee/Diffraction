...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "diffraction" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.DIFFRACTION_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "diffraction",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/diffraction" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    diffraction: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchDiffractionChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "diffraction";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.DIFFRACTION_GATEWAY_PORT;
  process.env.DIFFRACTION_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.DIFFRACTION_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.DIFFRACTION_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.DIFFRACTION_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.DIFFRACTION_GATEWAY_PASSWORD;
  } else {
    process.env.DIFFRACTION_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
