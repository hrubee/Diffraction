...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredDiffractionTmpDir: vi.fn(() => "/tmp/diffraction"),
}));
vi.mock("../infra/tmp-diffraction-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredDiffractionTmpDir.mockReturnValue("/tmp/diffraction");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".diffraction-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredDiffractionTmpDir.mockReturnValue("/tmp/diffraction-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "diffraction-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "diffraction-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredDiffractionTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "diffraction-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "diffraction-preferred", "downloads")}${path.sep}`),
    );
  });
