...
import {
  isChromeCdpReady,
  isChromeReachable,
  launchDiffractionChrome,
  stopDiffractionChrome,
} from "./chrome.js";
...
export function createProfileAvailability({
  ...
  const reconcileProfileRuntime = async (): Promise<void> => {
    ...
    if (profileState.running) {
      await stopDiffractionChrome(profileState.running).catch(() => {});
      setProfileRunning(null);
    }
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchDiffractionChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      const launched = await launchDiffractionChrome(current.resolved, profile);
      attachRunning(launched);
      try {
        await waitForCdpReadyAfterLaunch();
      } catch (err) {
        await stopDiffractionChrome(launched).catch(() => {});
        setProfileRunning(null);
        throw err;
      }
      return;
    ...
    if (!profileState.running) {
      throw new BrowserProfileUnavailableError(
        `Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by diffraction. ` +
          `Run action=reset-profile profile=${profile.name} to kill the process.`,
      );
    }

    await stopDiffractionChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchDiffractionChrome(current.resolved, profile);
