  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          diffraction: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.DIFFRACTION_GATEWAY_PORT;
    process.env.DIFFRACTION_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.DIFFRACTION_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.DIFFRACTION_GATEWAY_PASSWORD;
