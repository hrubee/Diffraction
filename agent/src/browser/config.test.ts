...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("diffraction");
    expect(profile?.driver).toBe("diffraction");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const diffraction = resolveProfile(resolved, "diffraction");
    expect(diffraction?.driver).toBe("diffraction");
    expect(diffraction?.cdpPort).toBe(18800);
    expect(diffraction?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from DIFFRACTION_GATEWAY_PORT when unset", () => {
    withEnv({ DIFFRACTION_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const diffraction = resolveProfile(resolved, "diffraction");
      expect(diffraction?.cdpPort).toBe(19012);
      expect(diffraction?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ DIFFRACTION_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const diffraction = resolveProfile(resolved, "diffraction");
      expect(diffraction?.cdpPort).toBe(19022);
      expect(diffraction?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(diffraction?.cdpPort).toBe(19000);
    expect(diffraction?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "diffraction");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "diffraction")!;
  ...
  describe("default profile preference", () => {
    it("defaults to diffraction profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("diffraction");
    });

    it("keeps diffraction default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("diffraction");
    ...
    it("keeps diffraction default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("diffraction");
    ...
    it("keeps diffraction default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
