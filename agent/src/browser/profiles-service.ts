...
import type { BrowserProfileConfig, DiffractionConfig } from "../config/config.js";
...
export function createBrowserProfilesService(ctx: BrowserRouteContext) {
  ...
  const createProfile = async (params: CreateProfileParams): Promise<CreateProfileResult> => {
    ...
    const nextConfig: DiffractionConfig = {
      ...cfg,
      browser: {
        ...cfg.browser,
        profiles: {
  ...
  const deleteProfile = async (nameRaw: string): Promise<DeleteProfileResult> => {
    ...
    if (resolved?.cdpIsLoopback && resolved.driver === "diffraction") {
      try {
      ...
      const userDataDir = resolveDiffractionUserDataDir(name);
