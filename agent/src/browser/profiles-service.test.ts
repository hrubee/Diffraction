...
vi.mock("./chrome.js", () => ({
  resolveDiffractionUserDataDir: vi.fn(() => "/tmp/diffraction-test/diffraction/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveDiffractionUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "diffraction-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveDiffractionUserDataDir).mockReturnValue(userDataDir);
