...
vi.mock("./paths.js", () => {
  return {
    DEFAULT_UPLOAD_DIR: "/tmp/diffraction/uploads",
...
describe("setInputFilesViaPlaywright", () => {
  ...
  beforeEach(() => {
    ...
    resolveStrictExistingPathsWithinRoot.mockResolvedValue({
      ok: true,
      paths: ["/private/tmp/diffraction/uploads/ok.txt"],
    });
  });
  ...
  it("revalidates upload paths and uses resolved canonical paths for inputRef", async () => {
    ...
    await setInputFilesViaPlaywright({
      ...
      targetId: "T1",
      inputRef: "e7",
      paths: ["/tmp/diffraction/uploads/ok.txt"],
    });

    expect(resolveStrictExistingPathsWithinRoot).toHaveBeenCalledWith({
      rootDir: "/tmp/diffraction/uploads",
      requestedPaths: ["/tmp/diffraction/uploads/ok.txt"],
      scopeLabel: "uploads directory (/tmp/diffraction/uploads)",
    });
    expect(refLocator).toHaveBeenCalledWith(page, "e7");
    expect(setInputFiles).toHaveBeenCalledWith(["/private/tmp/diffraction/uploads/ok.txt"]);
  });
  ...
  it("throws and skips setInputFiles when use-time validation fails", async () => {
    ...
    await expect(
      setInputFilesViaPlaywright({
        ...
        paths: ["/tmp/diffraction/uploads/missing.txt"],
      }),
