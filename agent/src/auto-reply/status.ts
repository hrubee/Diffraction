...
import type { DiffractConfig } from "../config/config.js";
import {
...
type AgentDefaults = NonNullable<NonNullable<DiffractConfig["agents"]>["defaults"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.diffraction/sessions/<SessionId>.jsonl)
  if (!sessionId) {
    return undefined;
  }
  let logPath: string;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🔷 Diffract ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: DiffractConfig): string {
  const lines = ["ℹ️ Help", ""];
