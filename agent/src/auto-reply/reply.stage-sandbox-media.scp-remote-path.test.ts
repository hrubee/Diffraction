...
function createRemoteStageParams(home: string): {
  ...
  return {
    ...
    workspaceDir: join(home, "diffraction"),
    sessionKey,
    remoteCacheDir: join(home, ".diffraction", "media", "remote-cache", sessionKey),
  };
}
...
describe("stageSandboxMedia scp remote paths", () => {
  it("rejects remote attachment filenames with shell metacharacters before spawning scp", async () => {
    await withSandboxMediaTempHome("diffraction-triggers-", async (home) => {
