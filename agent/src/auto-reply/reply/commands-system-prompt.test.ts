...
const { createDiffractionCodingToolsMock } = vi.hoisted(() => ({
  createDiffractionCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    createDiffractionCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createDiffractionCodingToolsMock).toHaveBeenCalledWith(
