...
const workspaceHarness = createCommandWorkspaceHarness("diffraction-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".diffraction", "extensions", params.pluginId);
...
function buildCfg(): DiffractionConfig {
  return {
    commands: {
      text: true,
      plugins: true,
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and inspects plugin details", async () => {
    await withTempHome("diffraction-command-plugins-home-", async () => {
