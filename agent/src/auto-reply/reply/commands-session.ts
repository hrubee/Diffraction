...
import { scheduleGatewaySigusr1Restart, triggerDiffractionRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Diffraction in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerDiffractionRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Diffraction via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
