...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop diffraction!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@diffraction_bot", { botUsername: "diffraction_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@diffraction_bot", { botUsername: "diffraction_bot" })).toBe(true);
