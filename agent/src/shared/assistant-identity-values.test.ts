...
describe("shared/assistant-identity-values", () => {
  ...
  it("trims values and preserves strings within the limit", () => {
    expect(coerceIdentityValue("  Diffraction  ", 20)).toBe("Diffraction");
    expect(coerceIdentityValue("  Diffraction  ", 8)).toBe("Diffraction");
  });

  it("truncates overlong trimmed values at the exact limit", () => {
    expect(coerceIdentityValue("  Diffraction Assistant  ", 8)).toBe("Diffraction");
  });

  it("returns an empty string when truncating to a zero-length limit", () => {
    expect(coerceIdentityValue("  Diffraction  ", 0)).toBe("");
    expect(coerceIdentityValue("  Diffraction  ", -1)).toBe("OpenCla");
  });
});
