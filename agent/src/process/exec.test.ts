...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        DIFFRACTION_BASE_ENV: "base",
        DIFFRACTION_TO_REMOVE: undefined,
      },
      env: {
        DIFFRACTION_TEST_ENV: "ok",
      },
    });

    expect(resolved.DIFFRACTION_BASE_ENV).toBe("base");
    expect(resolved.DIFFRACTION_TEST_ENV).toBe("ok");
    expect(resolved.DIFFRACTION_TO_REMOVE).toBeUndefined();
    expect(resolved.DIFFRACTION_CLI).toBe(DIFFRACTION_CLI_ENV_VALUE);
  });
