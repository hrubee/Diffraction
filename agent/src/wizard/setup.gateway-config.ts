...
import type { GatewayBindMode, GatewayTailscaleMode, DiffractionConfig } from "../config/config.js";
...
export async function configureGatewayForSetup(
  ...
  if (authMode === "token") {
    ...
    if (tokenMode === "ref") {
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.DIFFRACTION_GATEWAY_TOKEN)) ||
        randomToken();
      ...
      const tokenInput = await prompter.text({
        ...
        initialValue:
          ...
          normalizeGatewayTokenInput(process.env.DIFFRACTION_GATEWAY_TOKEN) ??
          "",
      });
