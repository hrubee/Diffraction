...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("diffraction-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("diffraction-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("diffraction-lid-a-", async (first) => {
      await withTempDir("diffraction-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.diffraction when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "diffraction-config-dir-"));
    try {
  ...
  it("expands DIFFRACTION_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/diffraction-home",
      DIFFRACTION_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/diffraction-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers DIFFRACTION_HOME over HOME", () => {
    vi.stubEnv("DIFFRACTION_HOME", "/srv/diffraction-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/diffraction-home"));
...
describe("shortenHomePath", () => {
  it("uses $DIFFRACTION_HOME prefix when DIFFRACTION_HOME is set", () => {
    vi.stubEnv("DIFFRACTION_HOME", "/srv/diffraction-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/diffraction-home")}/.diffraction/diffraction.json`)).toBe(
      "$DIFFRACTION_HOME/.diffraction/diffraction.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $DIFFRACTION_HOME replacement when DIFFRACTION_HOME is set", () => {
    vi.stubEnv("DIFFRACTION_HOME", "/srv/diffraction-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/diffraction-home")}/.diffraction/diffraction.json`),
    ).toBe("config: $DIFFRACTION_HOME/.diffraction/diffraction.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/diffraction", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "diffraction"),
    );
  });
  ...
  it("prefers DIFFRACTION_HOME for tilde expansion", () => {
    vi.stubEnv("DIFFRACTION_HOME", "/srv/diffraction-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/diffraction")).toBe(path.resolve("/srv/diffraction-home", "diffraction"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/diffraction", env)).toBe(path.resolve("/srv/diffraction-home", "diffraction"));
  });
