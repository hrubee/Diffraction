...
import { readConfigFileSnapshot, writeConfigFile, type DiffractionConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("diffraction config set gateway.auth.mode token")} or ${formatCliCommand("diffraction config set gateway.auth.mode password")}.`,
  ].join(" ");
}
