...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.DIFFRACTION_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.DIFFRACTION_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from diffraction.json",
        },
      ],
      ...
      if (!needsCredential) {
        ...
        note(
          [
            ...
            "Diffraction enabled the plugin and selected it as your web_search provider.",
            `Docs: ${entry.docsUrl ?? "https://docs.diffraction.ai/tools/web"}`,
          ].join("\n"),
          "Web search",
        );
      } else {
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "Diffraction update wizard" : "Diffraction configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: DiffractionConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("diffraction doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.DIFFRACTION_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.DIFFRACTION_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });
