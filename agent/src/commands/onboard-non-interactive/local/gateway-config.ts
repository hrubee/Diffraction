...
export function applyNonInteractiveGatewayConfig(params: {
  ...
  const envGatewayToken = normalizeGatewayTokenInput(process.env.DIFFRACTION_GATEWAY_TOKEN);
  ...
  if (authMode === "token") {
    if (gatewayTokenRefEnv) {
      if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) {
        runtime.error(
          "Invalid --gateway-token-ref-env (use env var name like DIFFRACTION_GATEWAY_TOKEN).",
        );
        runtime.exit(1);
        return null;
      }
