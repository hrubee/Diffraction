...
function resolveAgentName(cfg: DiffractionConfig, agentId: string) {
...
function resolveAgentModel(cfg: DiffractionConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: DiffractionConfig): AgentSummary[] {
