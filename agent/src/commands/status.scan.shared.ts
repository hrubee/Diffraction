...
export function hasExplicitMemorySearchConfig(cfg: DiffractionConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: DiffractionConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: DiffractionConfig, agentId: string) => { store: { path: string } } | null;
