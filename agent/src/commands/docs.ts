...
const SEARCH_TOOL = "https://docs.diffraction.ai/mcp.SearchDiffraction";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.diffraction.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('diffraction docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.diffraction.ai/");
      runtime.log(`Search: ${formatCliCommand('diffraction docs "your query"')}`);
    }
    return;
  }
