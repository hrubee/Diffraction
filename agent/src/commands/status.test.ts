...
beforeAll(() => {
  envSnapshot = captureEnv(["DIFFRACTION_PROFILE"]);
  process.env.DIFFRACTION_PROFILE = "isolated";
});
...
vi.mock("../infra/diffraction-root.js", () => ({
  resolveDiffractionPackageRoot: vi.fn().mockResolvedValue("/tmp/diffraction"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/diffraction/pnpm-lock.yaml",
      markerPath: "/tmp/diffraction/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.diffraction.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.diffraction.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("diffraction status --all") ||
          line.includes("diffraction --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("DIFFRACTION_GATEWAY_TOKEN", "abcd1234", async () => {
