...
type PromptDefaultModelResult = { model?: string; config?: DiffractionConfig };
...
function resolveConfiguredModelRaw(cfg: DiffractionConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: DiffractionConfig): string[] {
...
export function applyModelAllowlist(cfg: DiffractionConfig, models: string[]): DiffractionConfig {
