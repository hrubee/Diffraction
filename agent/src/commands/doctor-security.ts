...
import type { DiffractionConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: DiffractionConfig): string[] {
...
export async function noteSecurityWarnings(cfg: DiffractionConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("diffraction security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.diffraction/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("diffraction approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.diffraction.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("diffraction configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("diffraction config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("diffraction doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("diffraction config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('diffraction config set session.dmScope "per-channel-peer"') +
