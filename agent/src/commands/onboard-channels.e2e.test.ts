...
function createTelegramCfg(botToken: string, enabled?: boolean): DiffractionConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function patchTelegramAdapter(overrides: Parameters<typeof patchChannelSetupWizardAdapter>[1]) {
  return patchChannelSetupWizardAdapter("telegram", {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: DiffractionConfig }) => ({
        channel: "telegram",
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as DiffractionConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("./channel-setup/plugin-install.js", async (importOriginal) => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: DiffractionConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await runSetupChannels({} as DiffractionConfig, prompter, {
      quickstartDefaults: true,
  ...
  it("renders the QuickStart channel picker without requiring the LINE runtime", async () => {
    ...
    await expect(
      runSetupChannels({} as DiffractionConfig, prompter, {
        quickstartDefaults: true,
      }),
    ).resolves.toEqual({} as DiffractionConfig);
  ...
  it("keeps configured external plugin channels visible when the active registry starts empty", async () => {
    ...
    catalogMocks.listChannelPluginCatalogEntries.mockReturnValue([
      {
        id: "msteams",
        pluginId: "@diffraction/msteams-plugin",
        meta: {
          id: "msteams",
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@diffraction/msteams-plugin": { enabled: true },
          },
        },
      } as DiffractionConfig,
      prompter,
        ...
        if (channel === "msteams") {
          registry.channels.push({
            ...
            plugin: {
              ...
              capabilities: { chatTypes: ["direct"] },
              config: {
                listAccountIds: (cfg: DiffractionConfig) =>
                  Object.keys(
                    (cfg.channels?.msteams as { accounts?: Record<string, unknown> } | undefined)
                      ?.accounts ?? {},
                  ),
                resolveAccount: (cfg: DiffractionConfig, accountId: string) =>
                  (
                    cfg.channels?.msteams as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
              ...
              setupWizard: {
                ...
                status: {
                  ...
                  resolveConfigured: ({ cfg }: { cfg: DiffractionConfig }) =>
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: DiffractionConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: DiffractionConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
