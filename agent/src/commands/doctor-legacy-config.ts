...
export function normalizeCompatibilityConfigValues(cfg: DiffractionConfig): {
  ...
  let next: DiffractionConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as DiffractionConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as DiffractionConfig["browser"],
    };
    changes.push(
  ...
  const normalizeLegacyNanoBananaSkill = () => {
    type ModelProviderEntry = Partial<
      NonNullable<NonNullable<DiffractionConfig["models"]>["providers"]>[string]
    >;
    type ModelsConfigPatch = Partial<NonNullable<DiffractionConfig["models"]>>;
    ...
    if (!hasGoogleApiKey && legacyApiKey) {
      ...
      rawModels.providers = rawProviders as NonNullable<DiffractionConfig["models"]>["providers"];
      next = {
        ...next,
        models: rawModels as DiffractionConfig["models"],
      };
      changes.push(
  ...
  const normalizeLegacyTalkConfig = () => {
    ...
    const normalizedTalk = normalizeTalkSection(rawTalk as DiffractionConfig["talk"]);
    if (!normalizedTalk) {
      return;
    }
  ...
  const normalizeLegacyMediaProviderOptions = () => {
    ...
    next = {
      ...
      tools: {
        ...next.tools,
        media: nextMedia as NonNullable<DiffractionConfig["tools"]>["media"],
      },
    };
  };
