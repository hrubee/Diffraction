...
import { resolveDiffractionPackageRoot } from "../infra/diffraction-root.js";
...
function resolveMode(cfg: DiffractionConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Diffraction doctor");

  const root = await resolveDiffractionPackageRoot({
  ...
  let cfg: DiffractionConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("diffraction configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("diffraction config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("diffraction setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("diffraction config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("diffraction config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("diffraction doctor --fix")}" to apply changes.`);
  }
