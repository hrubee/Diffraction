...
describe("writeOAuthCredentials", () => {
  const lifecycle = createAuthTestLifecycle([
    ...
    "DIFFRACTION_AGENT_DIR",
    "PI_CODING_AGENT_DIR",
    "DIFFRACTION_OAUTH_DIR",
  ]);
  ...
  it("writes auth-profiles.json under DIFFRACTION_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("diffraction-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-oauth-sync-"));
    ...
    process.env.DIFFRACTION_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-oauth-nosync-"));
    process.env.DIFFRACTION_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside DIFFRACTION_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-oauth-external-"));
    process.env.DIFFRACTION_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* DIFFRACTION_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to DIFFRACTION_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("diffraction-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
