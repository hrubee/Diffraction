...
describe("backupCreateCommand atomic archive write", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("diffraction-backup-atomic-test-");
  ...
  it("does not leave a partial final archive behind when tar creation fails", async () => {
    ...
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-backup-failure-"));
    try {
  ...
  it("does not overwrite an archive created after readiness checks complete", async () => {
    const stateDir = path.join(tempHome.home, ".diffraction");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-backup-race-"));
  ...
  it("falls back to exclusive copy when hard-link publication is unsupported", async () => {
    const stateDir = path.join(tempHome.home, ".diffraction");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-backup-copy-fallback-"));
    const linkSpy = vi.spyOn(fs, "link");
    try {
      await fs.writeFile(path.join(stateDir, "diffraction.json"), JSON.stringify({}), "utf8");
      await fs.writeFile(path.join(stateDir, "state.txt"), "state\n", "utf8");
