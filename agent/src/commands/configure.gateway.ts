...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "DIFFRACTION_GATEWAY_TOKEN",
          placeholder: "DIFFRACTION_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like DIFFRACTION_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Diffraction will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Diffraction trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.diffraction.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
