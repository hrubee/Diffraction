...
type EnvSnapshot = {
  HOME?: string;
  DIFFRACTION_HOME?: string;
  DIFFRACTION_STATE_DIR?: string;
  DIFFRACTION_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    DIFFRACTION_HOME: process.env.DIFFRACTION_HOME,
    DIFFRACTION_STATE_DIR: process.env.DIFFRACTION_STATE_DIR,
    DIFFRACTION_OAUTH_DIR: process.env.DIFFRACTION_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: DiffractionConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: DiffractionConfig) {
...
async function runStateIntegrityText(cfg: DiffractionConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.DIFFRACTION_HOME = tempHome;
    process.env.DIFFRACTION_STATE_DIR = path.join(tempHome, ".diffraction");
    delete process.env.DIFFRACTION_OAUTH_DIR;
    fs.mkdirSync(process.env.DIFFRACTION_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when DIFFRACTION_OAUTH_DIR is explicitly configured", async () => {
    process.env.DIFFRACTION_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: DiffractionConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints diffraction-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: DiffractionConfig = {};
    ...
    expect(text).toMatch(/diffraction sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/diffraction sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /diffraction sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
