...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: DiffractionConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: DiffractionConfig) => { changed: boolean; next: DiffractionConfig };
  defaultModel: string;
  overrideConfig: DiffractionConfig;
  alreadyDefaultConfig: DiffractionConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: DiffractionConfig) => config,
      applyDefaultConfig: (config: DiffractionConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: DiffractionConfig = { agents: { defaults: {} } };
