...
const ensureDiffractionModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveDiffractionAgentDir = vi.fn().mockReturnValue("/tmp/diffraction-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/diffraction-state",
  loadConfig,
...
beforeEach(() => {
  ...
  ensureDiffractionModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureDiffractionModelsJson).toHaveBeenCalled();
    expect(ensureDiffractionModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
