...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByDiffraction ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByDiffraction ? "installed · " : "";
  ...
  runtime.log(theme.heading("Diffraction status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`diffraction devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("diffraction devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("diffraction devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("diffraction security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("diffraction security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.diffraction.ai/faq");
  runtime.log("Troubleshooting: https://docs.diffraction.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("diffraction status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("diffraction logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("diffraction status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("diffraction gateway probe")}`);
  }
}
