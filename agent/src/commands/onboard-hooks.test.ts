...
describe("onboard-hooks", () => {
  ...
  const createMockHook = (
    ...
    ...params,
    source: "diffraction-bundled" as const,
    pluginId: undefined,
    homepage: undefined,
    always: false,
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.diffraction/hooks",
    hooks: [
      createMockHook(
        {
          name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("should show informative notes to user", async () => {
      ...
      expect(noteCalls[1][0]).toMatch(/(?:diffraction|diffraction)( --profile isolated)? hooks list/);
    });
  });
});
