...
import { DiffractionSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: DiffractionConfig): {
  config: DiffractionConfig;
  removed: string[];
} {
  const parsed = DiffractionSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: DiffractionConfig): void {
