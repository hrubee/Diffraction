...
export const resolveDiffractionPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadDiffractionPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/diffraction.json",
    createConfigIO,
...
beforeEach(() => {
  ...
  resolveDiffractionPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.DIFFRACTION_STATE_DIR;
  originalUpdateInProgress = process.env.DIFFRACTION_UPDATE_IN_PROGRESS;
  process.env.DIFFRACTION_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.DIFFRACTION_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
