...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<DiffractionConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<DiffractionConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: DiffractionConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: DiffractionConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

/** Build an env-backed SecretRef for a search provider. */
function buildSearchEnvRef(config: DiffractionConfig, provider: SearchProvider): SecretRef {
  const entry =
...
export function applySearchKey(
  ...
  const nextBase: DiffractionConfig = {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: DiffractionConfig, result: DiffractionConfig): DiffractionConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: DiffractionConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as DiffractionConfig["plugins"],
  };
}
...
export async function setupSearch(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with diffraction configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Diffraction will enable the plugin and use it as your web_search provider.",
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Diffraction will store a reference instead of the API key.",
