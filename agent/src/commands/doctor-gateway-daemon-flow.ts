...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.DIFFRACTION_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("diffraction gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
