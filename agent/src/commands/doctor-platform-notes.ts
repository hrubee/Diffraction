...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".diffraction", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: DiffractionConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["DIFFRACTION_GATEWAY_TOKEN", await getenv("DIFFRACTION_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["DIFFRACTION_GATEWAY_PASSWORD", await getenv("DIFFRACTION_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "DIFFRACTION_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.DIFFRACTION_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- DIFFRACTION_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
