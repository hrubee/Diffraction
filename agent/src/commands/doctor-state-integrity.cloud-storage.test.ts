...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Diffraction", ".diffraction");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/diffraction-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".diffraction");
  ...
  it("anchors cloud detection to OS homedir when DIFFRACTION_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".diffraction");
    const originalDiffractionHome = process.env.DIFFRACTION_HOME;
    process.env.DIFFRACTION_HOME = "/tmp/diffraction-home-override";
    ...
    try {
      ...
      if (originalDiffractionHome === undefined) {
