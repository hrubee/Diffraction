...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "diffraction-agent-acp-" });
}

function createAcpEnabledConfig(home: string, storePath: string): DiffractionConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function mockConfigWithAcpOverrides(
  home: string,
  storePath: string,
  acpOverrides: Partial<NonNullable<DiffractionConfig["acp"]>>,
) {
  ...
  it.each([
    {
      ...
      acpOverrides: { enabled: false } satisfies Partial<NonNullable<DiffractionConfig["acp"]>>,
    },
    {
      name: "blocks ACP turns when ACP dispatch is disabled by policy",
      acpOverrides: {
        dispatch: { enabled: false },
      } satisfies Partial<NonNullable<DiffractionConfig["acp"]>>,
    },
