...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.diffraction/diffraction.json",
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.diffraction/workspace",
  applyWizardMetadata: (cfg: DiffractionConfig) => cfg,
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.applySearchProviderSelection.mockImplementation((cfg: DiffractionConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by applySearchKey", async () => {
    ...
    mocks.applySearchKey.mockImplementation(
      (cfg: DiffractionConfig, provider: string, key: string) => ({
        ...cfg,
        tools: {
          ...cfg.tools,
          web: {
  ...
  it("applies provider selection side effects when a key already exists via secret ref or env", async () => {
    ...
    mocks.applySearchProviderSelection.mockImplementation(
      (cfg: DiffractionConfig, provider: string) => ({
        ...cfg,
        tools: {
          ...cfg.tools,
          web: {
    ...
    mocks.resolveSearchProviderOptions.mockReturnValue([
      {
        ...
        docsUrl: "https://docs.diffraction.ai/tools/web",
        credentialPath: "",
      },
    ]);
    mocks.applySearchProviderSelection.mockImplementation(
