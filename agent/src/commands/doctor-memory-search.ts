 ...
 * Runs as part of `diffraction doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
        ...
        if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
          ...
          note(
            [
              ...
              "",
              `Verify: ${formatCliCommand("diffraction memory status --deep")}`,
            ]
              .filter(Boolean)
              .join("\n"),
            "Memory search",
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("diffraction config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("diffraction memory status --deep")}`,
        ].join("\n"),
        "Memory search",
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("diffraction configure --section model")}`,
        `- To disable: ${formatCliCommand("diffraction config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("diffraction memory status --deep")}`,
      ].join("\n"),
      "Memory search",
