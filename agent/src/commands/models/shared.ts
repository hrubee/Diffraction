...
export async function loadValidConfigOrThrow(): Promise<DiffractionConfig> {
...
export async function updateConfig(
  mutator: (cfg: DiffractionConfig) => DiffractionConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: DiffractionConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: DiffractionConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("diffraction agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
