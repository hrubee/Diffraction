...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveDiffractionAgentDir: vi.fn().mockReturnValue("/tmp/diffraction-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/diffraction-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/diffraction-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/diffraction-dev/diffraction.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveDiffractionAgentDir: mocks.resolveDiffractionAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/diffraction-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveDiffractionAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/diffraction-dev/diffraction.json");
    expect(payload.auth.storePath).toBe("/tmp/diffraction-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/diffraction-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/diffraction-agent-custom");
