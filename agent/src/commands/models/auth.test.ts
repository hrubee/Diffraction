...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: DiffractionConfig;
  let lastUpdatedConfig: DiffractionConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/diffraction/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/diffraction/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/diffraction/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: DiffractionConfig) => DiffractionConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/diffraction/agents/main",
    });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/diffraction/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `diffraction plugins list --json`.',
    );
  });
