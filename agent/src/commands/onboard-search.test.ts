...
function createPerplexityConfig(apiKey: string, enabled?: boolean): DiffractionConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: DiffractionConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): DiffractionConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: DiffractionConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: DiffractionConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
