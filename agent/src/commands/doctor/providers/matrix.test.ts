...
describe("doctor matrix provider helpers", () => {
  ...
  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "diffraction-matrix-missing-provider-test");
    ...
    expect(warnings[1]).toContain("diffraction plugins install @diffraction/matrix");
    expect(warnings[2]).toContain("diffraction plugins install ./extensions/matrix");
  });
