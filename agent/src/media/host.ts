...
export async function ensureMediaHosted(
  ...
  if (needsServerStart && !opts.startServer) {
    ...
    throw new Error(
      `Media hosting requires the webhook/Funnel server. Start \`${formatCliCommand("diffraction webhook")}\`/\`${formatCliCommand("diffraction up")}\` or re-run with --serve-media.`,
    );
  }
  if (needsServerStart && opts.startServer) {
    if (!mediaServer) {
