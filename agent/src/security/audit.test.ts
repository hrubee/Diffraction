...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: DiffractionConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: DiffractionConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: DiffractionConfig) => string[];
  isConfigured?: (account: unknown, cfg: DiffractionConfig) => boolean;
  isEnabled?: (account: unknown, cfg: DiffractionConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
async function runInstallMetadataAudit(
  ...
  return runSecurityAudit({
    ...
    includeChannelSecurity: false,
    stateDir,
    configPath: path.join(stateDir, "diffraction.json"),
...
describe("security audit", () => {
  ...
  let previousDiffractionHome: string | undefined;
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "diffraction.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ DIFFRACTION_STATE_DIR: sharedChannelSecurityStateDir }, () =>
      fn(sharedChannelSecurityStateDir),
    );
  };

  const runSharedExtensionsAudit = async (config: DiffractionConfig) => {
    return runSecurityAudit({
      config,
      includeFilesystem: true,
      includeChannelSecurity: false,
      stateDir: sharedExtensionsStateDir,
      configPath: path.join(sharedExtensionsStateDir, "diffraction.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  };
  ...
  const createSharedCodeSafetyFixture = async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        diffraction: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-security-audit-"));
    previousDiffractionHome = process.env.DIFFRACTION_HOME;
    process.env.DIFFRACTION_HOME = path.join(fixtureRoot, "home");
    await fs.mkdir(process.env.DIFFRACTION_HOME, { recursive: true, mode: 0o700 });
    ...
    const cases = [
          ...
          withEnvAsync(
            {
              ...
              DIFFRACTION_GATEWAY_PASSWORD: undefined,
            },
            async () =>
              audit({
                gateway: {
              ...
              gateway: {
                ...
                auth: {
                  password: {
                    source: "env",
                    provider: "default",
                    id: "DIFFRACTION_GATEWAY_PASSWORD",
                  },
                },
              },
            },
      ...
      },
      {
        ...
        run: async () => {
          ...
          const resolvedConfig: DiffractionConfig = {
            gateway: {
              bind: "lan",
              auth: {},
            },
  ...
  it("evaluates safeBinTrustedDirs risk findings", async () => {
    const riskyGlobalTrustedDirs =
      process.platform === "win32"
        ...
        : ["/usr/local/bin", "/tmp/diffraction-safe-bins"];
    const cases = [
      {
    ...
    const cases = [
      {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from(
                    "diffraction-sbx-browser-old\ndiffraction-sbx-browser-missing-hash\n",
                  ),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "diffraction-sbx-browser-old") {
                return {
              ...
              if (args[0] === "inspect" && args.at(-1) === "diffraction-sbx-browser-missing-hash") {
                return {
        ...
        assert: (res: SecurityAuditReport) => {
          ...
          expect(staleEpoch?.detail).toContain("diffraction-sbx-browser-old");
        },
      },
      {
        ...
        run: async () => {
          ...
          return runSecurityAudit({
            ...
            execDockerRawFn: (async (args: string[]) => {
              if (args[0] === "ps") {
                return {
                  stdout: Buffer.from("diffraction-sbx-browser-exposed\n"),
                  stderr: Buffer.alloc(0),
                  code: 0,
                };
              }
              if (args[0] === "inspect" && args.at(-1) === "diffraction-sbx-browser-exposed") {
                return {
              ...
              if (args[0] === "port" && args.at(-1) === "diffraction-sbx-browser-exposed") {
                return {
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-diffraction.json");
    ...
    await Promise.all(
      cases
        ...
        .map(async (testCase) => {
          ...
          const configPath = path.join(fixture.stateDir, "diffraction.json");
          await fs.writeFile(configPath, "{}\n", "utf-8");
          if (!isWindows) {
            await fs.chmod(configPath, 0o600);
          }
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): DiffractionConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    const cases = [
      ...
      },
      {
        ...
        plugin: (sourceConfig: DiffractionConfig) =>
          stubChannelPlugin({
            id: "slack",
            label: "Slack",
            inspectAccount: (cfg) => {
  ...
  it.each([
    {
      ...
      detailIncludes: [
        ...
        "~/.diffraction/credentials/discord-allowFrom.json:team.owner",
      ],
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: DiffractionConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("evaluates hooks ingress auth and routing findings", async () => {
    ...
    } satisfies NonNullable<DiffractionConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        env: {
          DIFFRACTION_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
        },
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies DiffractionConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
        name: "scores unrestricted hooks.allowedAgentIds by remote exposure",
        cfg: { gateway: { bind: "lan" }, hooks: unrestrictedBaseHooks } satisfies DiffractionConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "critical" as const,
      },
      {
        name: "scores hooks request sessionKey override by local exposure",
        cfg: { hooks: requestSessionKeyHooks } satisfies DiffractionConfig,
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.diffraction",
      configPath: "/Users/test/Dropbox/.diffraction/diffraction.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: DiffractionConfig = { logging: { redactSensitive: "off" } };
    ...
    const cases = [
          ...
          runInstallMetadataAudit(
            {
              plugins: {
                installs: {
                  "voice-call": {
                    source: "npm",
                    spec: "@diffraction/voice-call@1.2.3",
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@diffraction/test-hooks@1.2.3",
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            path.join(pluginDir, "package.json"),
            JSON.stringify({ name: "@diffraction/voice-call", version: "9.9.9" }),
            "utf-8",
          );
          await fs.writeFile(
            path.join(hookDir, "package.json"),
            JSON.stringify({ name: "@diffraction/test-hooks", version: "8.8.8" }),
            "utf-8",
          );
    ...
    const cases = [
      {
        ...
        cfg: {} satisfies DiffractionConfig,
        assert: (res: SecurityAuditReport) => {
          expect(res.findings).toEqual(
            expect.arrayContaining([
              expect.objectContaining({
      ...
      },
      {
        ...
        run: async () => {
          ...
          await fs.writeFile(
            ...
            JSON.stringify({
              name: "escape-plugin",
              diffraction: { extensions: ["../outside.js"] },
            }),
          );
          ...
          try {
            ...
            await fs.writeFile(
              ...
              JSON.stringify({
                ...
                diffraction: { extensions: ["index.js"] },
              }),
            );
            await fs.writeFile(path.join(pluginDir, "index.js"), "export {};");
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.DIFFRACTION_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.DIFFRACTION_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
