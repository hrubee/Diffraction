...
import type { ConfigFileSnapshot, DiffractionConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: DiffractionConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Diffraction state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Diffraction state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.DIFFRACTION_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.DIFFRACTION_GATEWAY_PASSWORD);
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("diffraction security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.DIFFRACTION_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(env.DIFFRACTION_GATEWAY_PASSWORD) ||
...
function collectLoggingFindings(cfg: DiffractionConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: DiffractionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: DiffractionConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: DiffractionConfig): string[] {
  const channels = asRecord(cfg.channels);
  if (!channels) {
    return [];
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("diffraction status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("diffraction security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set DIFFRACTION_GATEWAY_TOKEN/DIFFRACTION_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("diffraction security audit --deep")}".`,
    });
  }
