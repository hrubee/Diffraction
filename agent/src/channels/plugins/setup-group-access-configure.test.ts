...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: DiffractionConfig, policy: ChannelAccessPolicy) => DiffractionConfig;
  resolveAllowlist: (params: { cfg: DiffractionConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: DiffractionConfig; resolved: TResolved }) => DiffractionConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: DiffractionConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: DiffractionConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: DiffractionConfig, policy: ChannelAccessPolicy): DiffractionConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: DiffractionConfig, policy: ChannelAccessPolicy): DiffractionConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: DiffractionConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: DiffractionConfig; resolved: string[] }) => {
      calls.push("apply");
