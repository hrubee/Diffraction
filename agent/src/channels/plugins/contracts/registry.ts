...
type SetupContractEntry = {
  ...
  cases: Array<{
    ...
    beforeTest?: () => void;
    assertPatchedConfig?: (cfg: DiffractionConfig) => void;
    assertResolvedAccount?: (account: unknown, cfg: DiffractionConfig) => void;
  }>;
};
...
bundledChannelRuntimeSetters.setLineRuntime({
  channel: {
    line: {
      listLineAccountIds,
      resolveDefaultLineAccountId,
      resolveLineAccount: ({ cfg, accountId }: { cfg: DiffractionConfig; accountId?: string }) =>
...
const matrixSessionBindingStateDir = fs.mkdtempSync(
  path.join(os.tmpdir(), "diffraction-matrix-session-binding-contract-"),
);
