...
function telegramCfg(): DiffractionConfig {
  return { channels: { telegram: { botToken: "tok" } } } as DiffractionConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: DiffractionConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as DiffractionConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as DiffractionConfig;
...
function createSignalAccountOverrideCfg(): DiffractionConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("derives discord action listings from channel and moderation gates", () => {
    const cases = [
      {
        name: "defaults",
        cfg: { channels: { discord: { token: "d0" } } } as DiffractionConfig,
        expectUploads: true,
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as DiffractionConfig,
        accountId: undefined,
        params: {
