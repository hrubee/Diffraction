...
import type { DiffractionConfig } from "../../config/types.diffraction.js";
import {
...
describe("group policy warning builders", () => {
  ...
  it("projects cfg-only warning collector inputs", () => {
    const collect = projectConfigWarningCollector<{ cfg: DiffractionConfig; accountId: string }>(
      ({ cfg }) => [cfg.channels ? "configured" : "none"],
    );

    expect(
      collect({
        cfg: { channels: { slack: {} } } as DiffractionConfig,
        accountId: "acct-1",
      }),
    ).toEqual(["configured"]);
  });
  ...
  it("projects account+cfg warning collector inputs", () => {
    const collect = projectAccountConfigWarningCollector<
      ...
      { account: { accountId: string }; cfg: DiffractionConfig }
    >(
      (cfg: DiffractionConfig) => cfg.channels ?? {},
