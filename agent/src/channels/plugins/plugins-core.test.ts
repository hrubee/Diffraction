...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@diffraction/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-catalog-"));
        ...
        entries: [
          {
            ...
            diffraction: {
              ...
              install: {
                npmSpec: "@diffraction/demo-channel",
              },
            },
          },
        ],
  ...
  it("preserves plugin ids when they differ from channel ids", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-channel-catalog-state-"));
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-catalog-home-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@diffraction/env-demo-channel",
            diffraction: {
              channel: {
                id: "env-demo-channel",
              ...
              install: {
                npmSpec: "@diffraction/env-demo-channel",
              },
            },
          },
        ],
    ...
    const ids = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        DIFFRACTION_PLUGIN_CATALOG_PATHS: "~/catalog.json",
        HOME: home,
      },
    }).map((entry) => entry.id);
  ...
  it("uses the provided env for default catalog paths", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-catalog-state-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@diffraction/default-env-demo",
            diffraction: {
              channel: {
                id: "default-env-demo",
              ...
              install: {
                npmSpec: "@diffraction/default-env-demo",
              },
            },
          },
        ],
  ...
  it("includes bundled metadata-only channel entries even when the runtime entrypoint is omitted", () => {
    const packageRoot = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-bundled-catalog-"));
    const bundledDir = path.join(packageRoot, "dist", "extensions", "whatsapp");
    fs.mkdirSync(bundledDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "diffraction" }),
      "utf8",
    );
    fs.writeFileSync(
      path.join(bundledDir, "package.json"),
      JSON.stringify({
        name: "@diffraction/whatsapp",
        diffraction: {
          extensions: ["./index.js"],
          channel: {
            id: "whatsapp",
    ...
    fs.writeFileSync(
      path.join(bundledDir, "diffraction.plugin.json"),
    ...
    const entry = listChannelPluginCatalogEntries({
      env: {
        ...process.env,
        DIFFRACTION_BUNDLED_PLUGINS_DIR: path.join(packageRoot, "dist", "extensions"),
      },
    }).find((item) => item.id === "whatsapp");

    expect(entry?.install.npmSpec).toBe("@diffraction/whatsapp");
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: DiffractionConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
