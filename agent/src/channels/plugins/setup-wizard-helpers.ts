...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: DiffractionConfig) => Array<string | number> | undefined;
}): DiffractionConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: DiffractionConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: DiffractionConfig) => Array<string | number> | undefined;
}): (cfg: DiffractionConfig, dmPolicy: DmPolicy) => DiffractionConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: DiffractionConfig, allowFrom: string[]) => DiffractionConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: DiffractionConfig, groupPolicy: "open" | "allowlist" | "disabled") => DiffractionConfig {
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: DiffractionConfig) => DiffractionConfig | Promise<DiffractionConfig>;
  applySet?: (
    cfg: DiffractionConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => DiffractionConfig | Promise<DiffractionConfig>;
}): Promise<{
  cfg: DiffractionConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in Diffraction config",
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "DIFFRACTION_SECRET",
      envVarFormatError:
        'Use an env var name like "DIFFRACTION_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends DiffractionConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export function createPromptParsedAllowFromForAccount<TConfig extends DiffractionConfig>(params: {
...
export function createTopLevelChannelParsedAllowFromPrompt(params: {
  ...
  placeholder: string;
  parseEntries: (raw: string) => ParsedAllowFromResult;
  getExistingAllowFrom?: (cfg: DiffractionConfig) => Array<string | number>;
  mergeEntries?: (params: { existing: Array<string | number>; parsed: string[] }) => string[];
}): NonNullable<ChannelSetupDmPolicy["promptAllowFrom"]> {
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: DiffractionConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: DiffractionConfig) => Array<string | number>;
