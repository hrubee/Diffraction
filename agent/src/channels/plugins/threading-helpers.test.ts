...
describe("createStaticReplyToModeResolver", () => {
  it("always returns the configured mode", () => {
    expect(createStaticReplyToModeResolver("off")({ cfg: {} as DiffractionConfig })).toBe("off");
    expect(createStaticReplyToModeResolver("all")({ cfg: {} as DiffractionConfig })).toBe("all");
  });
});
...
describe("createTopLevelChannelReplyToModeResolver", () => {
  it("reads the top-level channel config", () => {
    ...
    expect(
      resolver({
        cfg: { channels: { discord: { replyToMode: "first" } } } as DiffractionConfig,
      }),
    ).toBe("first");
  });
  ...
  it("falls back to off", () => {
    const resolver = createTopLevelChannelReplyToModeResolver("discord");
    expect(resolver({ cfg: {} as DiffractionConfig })).toBe("off");
  });
});
...
describe("createScopedAccountReplyToModeResolver", () => {
  ...
  it("passes chatType through", () => {
    ...
    expect(resolver({ cfg: {} as DiffractionConfig, chatType: "group" })).toBe("first");
