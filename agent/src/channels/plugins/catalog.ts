...
import { discoverDiffractionPlugins } from "../../plugins/discovery.js";
import { loadPluginManifest } from "../../plugins/manifest.js";
import type { DiffractionPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, DiffractionPackageManifest>>;
...
function toChannelMeta(params: {
  channel: NonNullable<DiffractionPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: DiffractionPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function loadBundledMetadataCatalogEntries(options: CatalogOptions): ChannelPluginCatalogEntry[] {
  ...
  for (const dirent of fs.readdirSync(bundledDir, { withFileTypes: true })) {
    ...
    const entry = buildCatalogEntry({
      ...
      packageManifest: packageJson.diffraction,
    });
    if (entry) {
      entries.push(entry);
    }
