...
type ConfigGroupPolicyWarningCollector<Params extends { cfg: DiffractionConfig }> = (
  params: Params,
) => string[];
...
export function projectConfigWarningCollector<Params extends { cfg: DiffractionConfig }>(
  collector: WarningCollector<{ cfg: DiffractionConfig }>,
): WarningCollector<Params> {
  return projectWarningCollector((params) => ({ cfg: params.cfg }), collector);
}

export function projectConfigAccountIdWarningCollector<
  Params extends { cfg: DiffractionConfig; accountId?: string | null },
>(
  collector: WarningCollector<{ cfg: DiffractionConfig; accountId?: string | null }>,
): WarningCollector<Params> {
...
export function projectAccountConfigWarningCollector<
  ResolvedAccount,
  ProjectedCfg,
  Params extends { account: ResolvedAccount; cfg: DiffractionConfig },
>(
  projectCfg: (cfg: DiffractionConfig) => ProjectedCfg,
...
export function createAllowlistProviderRestrictSendersWarningCollector<ResolvedAccount>(
  params: {
    providerConfigPresent: (cfg: DiffractionConfig) => boolean;
  ...
  return createAllowlistProviderGroupPolicyWarningCollector({
    providerConfigPresent: params.providerConfigPresent,
    resolveGroupPolicy: ({ account }: { account: ResolvedAccount; cfg: DiffractionConfig }) =>
...
export function createAllowlistProviderGroupPolicyWarningCollector<
  Params extends { cfg: DiffractionConfig },
>(params: {
  providerConfigPresent: (cfg: DiffractionConfig) => boolean;
