...
const SANDBOX_EXAMPLES = {
  main: [
    ["diffraction sandbox list", "List all sandbox containers."],
    ["diffraction sandbox list --browser", "List only browser containers."],
    ["diffraction sandbox recreate --all", "Recreate all containers."],
    ["diffraction sandbox recreate --session main", "Recreate a specific session."],
    ["diffraction sandbox recreate --agent mybot", "Recreate agent containers."],
    ["diffraction sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["diffraction sandbox list", "List all sandbox containers."],
    ["diffraction sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["diffraction sandbox recreate --all", "Recreate all containers."],
    ["diffraction sandbox recreate --session main", "Recreate a specific session."],
    ["diffraction sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["diffraction sandbox recreate --browser --all", "Recreate only browser containers."],
    ["diffraction sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["diffraction sandbox explain", "Show effective sandbox config."],
    ["diffraction sandbox explain --session agent:main:main", "Explain a specific session."],
    ["diffraction sandbox explain --agent work", "Explain an agent sandbox."],
    ["diffraction sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.diffraction.ai/cli/sandbox")}\n`,
    )
    .action(() => {
