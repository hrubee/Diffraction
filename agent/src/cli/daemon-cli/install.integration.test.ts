...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("diffraction-daemon-install-int-");
    configPath = path.join(tempHome, "diffraction.json");
    process.env.HOME = tempHome;
    process.env.DIFFRACTION_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.DIFFRACTION_GATEWAY_TOKEN).toBeUndefined();
  });
});
