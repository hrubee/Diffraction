...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("diffraction gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.DIFFRACTION_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.DIFFRACTION_PROFILE),
      }),
    );
  }
  return hints;
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  return buildPlatformServiceStartHints({
    installCommand: formatCliCommand("diffraction gateway install", env),
    startCommand: formatCliCommand("diffraction gateway", env),
