...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    DIFFRACTION_STATE_DIR: "/tmp/diffraction-daemon",
    DIFFRACTION_CONFIG_PATH: "/tmp/diffraction-daemon/diffraction.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.DIFFRACTION_STATE_DIR ?? "/tmp/diffraction-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.DIFFRACTION_CONFIG_PATH ?? `${stateDir}/diffraction.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/diffraction-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.DIFFRACTION_STATE_DIR = "/tmp/diffraction-cli";
    process.env.DIFFRACTION_CONFIG_PATH = "/tmp/diffraction-cli/diffraction.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.DIFFRACTION_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.DIFFRACTION_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.DIFFRACTION_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "diffraction-gateway" }],
        hints: [],
      },
      healthy: false,
