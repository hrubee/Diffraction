...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-bundled-skills-test-"));
    process.env.DIFFRACTION_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
