...
describe("acp cli option collisions", () => {
  ...
  it.each([
    ...
    await withTempSecretFiles("diffraction-acp-cli-", files, async ({ tokenFile, passwordFile }) => {
  ...
  it("trims token file path before reading", async () => {
    await withTempSecretFiles("diffraction-acp-cli-", { token: "tok_file\n" }, async (files) => {
  ...
  it("reports missing token-file read errors", async () => {
    await parseAcp(["--token-file", "/tmp/diffraction-acp-missing-token.txt"]);
