...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Diffraction and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["diffraction update", "Update a source checkout (git)"],
        ["diffraction update --channel beta", "Switch to beta channel (git + npm)"],
        ["diffraction update --channel dev", "Switch to dev channel (git + npm)"],
        ["diffraction update --tag beta", "One-off update to a dist-tag or version"],
        ["diffraction update --tag main", "One-off package install from GitHub main"],
        ["diffraction update --dry-run", "Preview actions without changing anything"],
        ["diffraction update --no-restart", "Update without restarting the service"],
        ["diffraction update --json", "Output result as JSON"],
        ["diffraction update --yes", "Non-interactive (accept downgrade prompts)"],
        ["diffraction update wizard", "Interactive update wizard"],
        ["diffraction --update", "Shorthand for diffraction update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run diffraction update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.diffraction.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["diffraction update status", "Show channel + version status."],
          ["diffraction update status --json", "JSON output."],
          ["diffraction update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.diffraction.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
