...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Diffraction Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(diffraction completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# Diffraction Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(diffraction completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.diffraction.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $DIFFRACTION_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "diffraction") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
