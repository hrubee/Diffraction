    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["diffraction nodes status", "List known nodes with live status."],
          ["diffraction nodes pairing pending", "Show pending node pairing requests."],
          ['diffraction nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["diffraction nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.diffraction.ai/cli/nodes")}\n`,
    );
