...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["diffraction security audit", "Run a local security audit."],
          ["diffraction security audit --deep", "Include best-effort live Gateway probe checks."],
          ["diffraction security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "diffraction security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["diffraction security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["diffraction security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.diffraction.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Diffraction security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("diffraction security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("diffraction security audit --fix")}`));
        if (!fixResult) {
