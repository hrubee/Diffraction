...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@diffraction/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@diffraction/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@diffraction/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@diffraction/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @diffraction/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@diffraction/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@diffraction/plugin-alpha@1.2.3",
        installPath: "/tmp/diffraction/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@diffraction/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @diffraction/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@diffraction/plugin-alpha@latest",
      true,
      "/tmp/diffraction/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@diffraction/plugin-alpha@latest",
      installPath: "/tmp/diffraction/extensions/alpha",
      version: "1.2.3",
