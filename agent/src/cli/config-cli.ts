...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "diffraction config set secrets.providers.vault --provider-source file --provider-path /etc/diffraction/secrets.json --provider-mode json",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("diffraction doctor")}\` ${message}`;
}
...
function collectDryRunSchemaErrors(config: DiffractionConfig): ConfigSetDryRunError[] {
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as DiffractionConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "diffraction.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.diffraction.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing diffraction.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
