    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["diffraction memory status", "Show index and provider status."],
          ["diffraction memory status --deep", "Probe embedding provider readiness."],
          ["diffraction memory index --force", "Force a full reindex."],
          ['diffraction memory search "meeting notes"', "Quick search using positional query."],
          [
            'diffraction memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["diffraction memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.diffraction.ai/cli/memory")}\n`,
    );
