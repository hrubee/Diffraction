...
beforeEach(() => {
  ...
  originalHideBanner = process.env.DIFFRACTION_HIDE_BANNER;
  originalForceStderr = loggingState.forceConsoleToStderr;
  loggingState.forceConsoleToStderr = false;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.DIFFRACTION_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.DIFFRACTION_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("diffraction");
    program
      .command("status")
      .option("--json")
      .action(() => {});
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "diffraction", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("diffraction-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "diffraction", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "diffraction", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "diffraction", "channels", "add"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "diffraction", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.DIFFRACTION_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "diffraction", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "diffraction", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "diffraction", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "diffraction", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "diffraction", "agents", "list", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "diffraction", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
    ...
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "diffraction", "agents", "list"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "diffraction", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "diffraction", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "diffraction", "backup", "create", "--json"],
    });
