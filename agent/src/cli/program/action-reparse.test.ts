...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "diffraction", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "diffraction", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "diffraction", "status"]);
  });
