...
${formatHelpExamples([
  ['diffraction agent --to +15555550123 --message "status update"', "Start a new session."],
  ['diffraction agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'diffraction agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'diffraction agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['diffraction agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'diffraction agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.diffraction.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.diffraction.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['diffraction agents set-identity --agent main --name "Diffract" --emoji "🔷"', "Set name + emoji."],
  ["diffraction agents set-identity --agent main --avatar avatars/diffraction.png", "Set avatar path."],
  [
    "diffraction agents set-identity --workspace ~/.diffraction/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "diffraction agents set-identity --identity-file ~/.diffraction/workspace/IDENTITY.md --agent main",
