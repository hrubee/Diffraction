...
function formatPluginLine(plugin: PluginRecord, verbose = false): string {
  ...
  const format = plugin.format ?? "diffraction";
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage Diffraction plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.diffraction.ai/cli/plugins")}\n`,
    );
      ...
      if (!opts.verbose) {
        ...
        const rows = list.map((plugin) => {
          ...
          return {
            ...
            Format: plugin.format ?? "diffraction",
            Status:
              plugin.status === "loaded"
                ? theme.success("loaded")
                : plugin.status === "disabled"
  ...
  plugins
    ...
    .action((id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "diffraction"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: DiffractionConfig = enableResult.config;
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.diffraction.ai/plugin");
      lines.push("");
