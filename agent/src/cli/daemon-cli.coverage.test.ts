...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      DIFFRACTION_GATEWAY_PORT: String(params.port),
      ...(params.token ? { DIFFRACTION_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.DIFFRACTION_STATE_DIR = "/tmp/diffraction-cli-state";
    process.env.DIFFRACTION_CONFIG_PATH = "/tmp/diffraction-cli-state/diffraction.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        DIFFRACTION_STATE_DIR: "/tmp/diffraction-daemon-state",
        DIFFRACTION_CONFIG_PATH: "/tmp/diffraction-daemon-state/diffraction.json",
        DIFFRACTION_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.diffraction.gateway.plist",
    });
