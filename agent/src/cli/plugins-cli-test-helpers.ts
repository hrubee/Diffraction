...
export const loadConfig = vi.fn<() => DiffractionConfig>(() => ({}) as DiffractionConfig);
export const writeConfigFile = vi.fn<(config: DiffractionConfig) => Promise<void>>(
  async () => undefined,
);
export const resolveStateDir = vi.fn(() => "/tmp/diffraction-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: DiffractionConfig) => writeConfigFile(config),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as DiffractionConfig);
  writeConfigFile.mockResolvedValue(undefined);
  resolveStateDir.mockReturnValue("/tmp/diffraction-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation((cfg: DiffractionConfig) => ({ config: cfg }));
  recordPluginInstall.mockImplementation((cfg: DiffractionConfig) => cfg);
  buildPluginStatusReport.mockReturnValue({
    plugins: [],
    diagnostics: [],
  });
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: DiffractionConfig }) => ({
    config,
    warnings: [],
  }));
  uninstallPlugin.mockResolvedValue({
    ok: true,
    config: {} as DiffractionConfig,
    warnings: [],
    actions: {
      entry: false,
      install: false,
  ...
  recordHookInstall.mockImplementation((cfg: DiffractionConfig) => cfg);
}
