    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "diffraction node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["diffraction node status", "Check node host service status."],
          ["diffraction node install", "Install the node host service."],
          ["diffraction node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.diffraction.ai/cli/node")}\n`,
    );
