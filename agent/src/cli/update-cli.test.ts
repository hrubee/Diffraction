...
import type { DiffractionConfig, ConfigFileSnapshot } from "../config/types.diffraction.js";
...
vi.mock("../infra/diffraction-root.js", () => ({
  resolveDiffractionPackageRoot: vi.fn(),
}));
...
const { resolveDiffractionPackageRoot } = await import("../infra/diffraction-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/diffraction-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as DiffractionConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/diffraction-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveDiffractionPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("diffraction-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("diffraction-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveDiffractionPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("diffraction");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/diffraction-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "diffraction-gateway" }],
      hints: [],
    });
    ...
    const cases = [
      {
        ...
        assert: () => {
          ...
          expect(logs.join("\n")).toContain("Diffraction update status");
        },
      },
      {
        name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "diffraction", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as DiffractionConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "diffraction@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("diffraction-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "diffraction@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:diffraction/diffraction#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("diffraction-update"));
          await updateCommand({ yes: true, tag: "github:diffraction/diffraction#main" });
        },
        expectedSpec: "github:diffraction/diffraction#main",
      },
      {
        name: "DIFFRACTION_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("diffraction-update"));
          await withEnvAsync(
        ...
        expectedSpec: "http://10.211.55.2:8138/diffraction-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("diffraction");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("diffraction-update");
    const localAppData = createCaseDir("diffraction-localappdata");
  ...
  it.each([
    ...
    },
    {
      ...
      invoke: async () => {
        await withEnvAsync(
          {
            DIFFRACTION_STATE_DIR: "./state",
            DIFFRACTION_CONFIG_PATH: "./config/diffraction.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            DIFFRACTION_STATE_DIR: path.resolve("./state"),
            DIFFRACTION_CONFIG_PATH: path.resolve("./config/diffraction.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            DIFFRACTION_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ DIFFRACTION_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.DIFFRACTION_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("diffraction-update-wizard");
    await withEnvAsync({ DIFFRACTION_GIT_DIR: tempDir }, async () => {
      setTty(true);
