...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-install-sh-"));
  const cliPath = path.join(root, "diffraction");
...
DIFFRACTION_BIN="$FAKE_DIFFRACTION_BIN"
resolve_diffraction_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_DIFFRACTION_BIN: cliPath,
        DIFFRACTION_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Diffraction 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Diffraction dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("Diffraction dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-install-stdin-"));
