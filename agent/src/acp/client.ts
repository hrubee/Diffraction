...
export function resolveAcpClientSpawnEnv(
  ...
  env.DIFFRACTION_SHELL = "acp-client";
  return env;
}
...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "diffraction";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "diffraction-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("Diffraction ACP client");
