...
type ConfiguredBinding = NonNullable<DiffractionConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<DiffractionConfig>,
): DiffractionConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: DiffractionConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/diffraction" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
