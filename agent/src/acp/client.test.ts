...
const createTempDir = () => tempDirs.make("diffraction-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets DIFFRACTION_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "diffraction",
    });

    expect(env.DIFFRACTION_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("diffraction");
  });

  it("overrides pre-existing DIFFRACTION_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      DIFFRACTION_SHELL: "wrong",
    });
    expect(env.DIFFRACTION_SHELL).toBe("acp-client");
  });
  ...
  it("preserves DIFFRACTION_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        DIFFRACTION_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["DIFFRACTION_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Diffraction bridge", () => {
    ...
    expect(env.DIFFRACTION_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.DIFFRACTION_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "diffraction",
        serverArgs: ["acp"],
        defaultServerCommand: "diffraction",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("DIFFRACTION_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "diffraction", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "diffraction", "dist", "entry.js");
    const shimPath = path.join(dir, "diffraction.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\diffraction\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/diffraction-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/diffraction-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/diffraction-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
