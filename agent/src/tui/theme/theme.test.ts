...
describe("light background detection", () => {
  ...
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      DIFFRACTION_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when DIFFRACTION_THEME=light", async () => {
    const mod = await importThemeWithEnv({ DIFFRACTION_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when DIFFRACTION_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ DIFFRACTION_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats DIFFRACTION_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ DIFFRACTION_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("DIFFRACTION_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      DIFFRACTION_THEME: "dark",
      COLORFGBG: "0;15",
    });
    expect(mod.lightMode).toBe(false);
  });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ DIFFRACTION_THEME: "light" });
    const darkMod = await importThemeWithEnv({ DIFFRACTION_THEME: "dark" });
...
describe("light palette accessibility", () => {
  it("keeps light theme text colors at WCAG AA contrast or better", async () => {
    vi.resetModules();
    process.env.DIFFRACTION_THEME = "light";
