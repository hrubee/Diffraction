...
describe("resolveInitialTuiAgentId", () => {
  const cfg: DiffractionConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/diffraction" },
        { id: "ops", workspace: "/tmp/diffraction/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/diffraction/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run diffraction devices list");
    expect(state.pairingHint).toContain("diffraction devices list");
  });
