...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set DIFFRACTION_GATEWAY_TOKEN/DIFFRACTION_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "diffraction-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.DIFFRACTION_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.DIFFRACTION_GATEWAY_PASSWORD);
