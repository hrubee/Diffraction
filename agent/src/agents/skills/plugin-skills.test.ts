    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "diffraction.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "diffraction.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "diffraction.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("diffraction-");
  const acpxRoot = await tempDirs.make("diffraction-acpx-plugin-");
  const helperRoot = await tempDirs.make("diffraction-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("diffraction-");
  const pluginRoot = await tempDirs.make("diffraction-plugin-");
  const outsideDir = await tempDirs.make("diffraction-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("diffraction-");
    const pluginRoot = await tempDirs.make("diffraction-claude-bundle-");
