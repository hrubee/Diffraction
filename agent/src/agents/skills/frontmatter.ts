...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseDiffractionManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyDiffractionManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveDiffractionMetadata(
  frontmatter: ParsedSkillFrontmatter,
): DiffractionSkillMetadata | undefined {
  const metadataObj = resolveDiffractionManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveDiffractionManifestRequires(metadataObj);
  const install = resolveDiffractionManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveDiffractionManifestOs(metadataObj);
  return {
