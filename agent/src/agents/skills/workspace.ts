...
function resolveSkillsLimits(config?: DiffractionConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "diffraction-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "diffraction-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveDiffractionMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: DiffractionConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`diffraction skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`diffraction skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
