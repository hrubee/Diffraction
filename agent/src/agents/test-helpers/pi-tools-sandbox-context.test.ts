...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("diffraction-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("diffraction-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("diffraction-sbx-");
