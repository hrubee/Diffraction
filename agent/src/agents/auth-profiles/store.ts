 ...
 * A common mistake — caused by the similarity with the `diffraction.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (diffraction.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.DIFFRACTION_AUTH_STORE_READONLY === "1";
