...
function redactSessionsSpawnAttachmentsArgs(value: unknown): unknown {
  ...
  const next = raw.map((item) => {
    ...
    return { ...rest, content: "__DIFFRACTION_REDACTED__" };
  });
...
function makeMissingToolResult(params: {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: "[diffraction] missing tool result in session history; inserted synthetic error result for transcript repair.",
      },
    ],
    isError: true,
    timestamp: Date.now(),
