...
function findTool(name: string, config: DiffractionConfig) {
  const allTools = createDiffractionTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: DiffractionConfig; env?: NodeJS.ProcessEnv }) {
...
describe("diffraction tools runtime web metadata wiring", () => {
