...
type CoreToolDefinition = {
  ...
  includeInDiffractionGroup?: boolean;
};
...
const CORE_TOOL_DEFINITIONS: CoreToolDefinition[] = [
  ...
  },
  {
    ...
    sectionId: "web",
    profiles: ["coding"],
    includeInDiffractionGroup: true,
  },
  {
    id: "web_fetch",
    label: "web_fetch",
...
function buildCoreToolGroupMap() {
  ...
  const diffractionTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInDiffractionGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:diffraction": diffractionTools,
