...
import { markDiffractionExecEnv } from "../../infra/diffraction-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "diffraction.sandbox=1");
  args.push("--label", `diffraction.sessionKey=${params.scopeKey}`);
  args.push("--label", `diffraction.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `diffraction.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markDiffractionExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "diffraction.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`diffraction sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`diffraction sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("diffraction sandbox recreate --all");
}
