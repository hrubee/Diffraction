  ...
  return {
    ...
    spawn: (command: string, args: string[]) => {
      ...
      if (command !== "docker") {
        ...
        args[0] === "inspect" &&
        args[1] === "-f" &&
        args[2]?.includes('index .Config.Labels "diffraction.configHash"')
      ) {
...
function createSandboxConfig(
  ...
  return {
    ...
    docker: {
      image: "diffraction-sandbox:test",
    ...
    browser: {
      enabled: false,
      image: "diffraction-browser:test",
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall?.args).toContain(`diffraction.configHash=${newHash}`);
  ...
  it("applies custom binds after workspace mounts so overlapping binds can override", async () => {
    ...
    expect(createCall).toBeDefined();
    expect(createCall?.args).toContain(`diffraction.configHash=${expectedHash}`);
