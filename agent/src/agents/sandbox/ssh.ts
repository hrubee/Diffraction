...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "diffraction-sandbox-ssh-"));
...
export async function createSshSandboxSessionFromSettings(
  ...
  try {
    ...
    const hostAlias = "diffraction-sandbox";
    const configPath = path.join(configDir, "config");
    const lines = [
      `Host ${hostAlias}`,
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredDiffractionTmpDir() ?? os.tmpdir());
}
