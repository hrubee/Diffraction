      ...
      defaults: {
        sandbox: {
          ...
          ssh: {
            ...
            command: "ssh",
            workspaceRoot: "/remote/diffraction",
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "diffraction-test-ssh-config"),
    host: "diffraction-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "diffraction-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "diffraction-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/diffraction/diffraction-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/diffraction/diffraction-ssh-agent-worker");
