...
import { resolvePreferredDiffractionTmpDir } from "../infra/tmp-diffraction-dir.js";
...
async function withOutsideHardlinkInDiffractionTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  const openClawTmpDir = resolvePreferredDiffractionTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred Diffraction tmp root",
      media: path.join(openClawTmpDir, "image.png"),
      expected: path.join(openClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred Diffraction tmp root",
      media: pathToFileURL(path.join(openClawTmpDir, "photo.png")).href,
      expected: path.join(openClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred Diffraction tmp root",
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside diffraction tmp root",
      media: path.join(os.tmpdir(), "outside-diffraction", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Diffraction tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Diffraction tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDiffractionTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Diffraction tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInDiffractionTmp(
