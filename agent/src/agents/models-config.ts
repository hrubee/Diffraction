...
import { planDiffractionModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  ...
  sourceConfigForSecrets: DiffractionConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: DiffractionConfig): {
  config: DiffractionConfig;
  sourceConfigForSecrets: DiffractionConfig;
} {
...
export async function ensureDiffractionModelsJson(
  config?: DiffractionConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveDiffractionAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planDiffractionModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
