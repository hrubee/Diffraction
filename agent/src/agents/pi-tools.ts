...
function applyModelProviderToolPolicy(
  ...
  // Models with a native web_search tool cannot receive Diffraction's
...
function resolveExecConfig(params: { cfg?: DiffractionConfig; agentId?: string }) {
...
export function createDiffractionCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createDiffractionReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
