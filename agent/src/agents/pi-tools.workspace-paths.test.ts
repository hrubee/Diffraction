...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("diffraction-ws-", async (workspaceDir) => {
      await withTempDir("diffraction-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createDiffractionCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("diffraction-ws-", async (workspaceDir) => {
      await withTempDir("diffraction-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("diffraction-ws-", async (workspaceDir) => {
      const cfg: DiffractionConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createDiffractionCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-diffraction.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("diffraction-sandbox-", async (sandboxDir) => {
      await withTempDir("diffraction-workspace-", async (workspaceDir) => {
        ...
        const tools = createDiffractionCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
