...
describe("createDiffractionCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-read-"));
    try {
      ...
      const contents = "Hello from diffraction read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createDiffractionCodingTools({ sandbox });
