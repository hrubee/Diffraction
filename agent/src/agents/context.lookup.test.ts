...
function mockContextDeps(params: {
  ...
  const ensureDiffractionModelsJson = vi.fn(async () => {});
  vi.doMock("../config/config.js", () => ({
    loadConfig: params.loadConfig,
  }));
  vi.doMock("./models-config.js", () => ({
    ensureDiffractionModelsJson,
  }));
  vi.doMock("./agent-paths.js", () => ({
    resolveDiffractionAgentDir: () => "/tmp/diffraction-agent",
  }));
  ...
  return { ensureDiffractionModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("can skip async warmup for read-only callers", async () => {
    const { ensureDiffractionModelsJson } = mockContextModuleDeps(() => ({
      models: {
        providers: {
          openrouter: {
            models: [{ id: "openrouter/claude-sonnet", contextWindow: 321_000 }],
    ...
    await flushAsyncWarmup();
    expect(ensureDiffractionModelsJson).not.toHaveBeenCalled();
  });

  it("only warms eagerly for real diffraction startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "diffraction", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "diffraction", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "diffraction", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureDiffractionModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureDiffractionModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
