...
describe("sanitizeToolCallInputs redacts sessions_spawn attachments", () => {
  it("replaces attachments[].content with __DIFFRACTION_REDACTED__", () => {
    ...
    expect(tool?.arguments?.attachments?.[0]?.content).toBe("__DIFFRACTION_REDACTED__");
