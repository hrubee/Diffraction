...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("diffraction-bundled-");
  const workspaceDir = await createTempDir("diffraction-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: DiffractionConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
