...
import { resolveDiffractionDocsPath } from "../../docs-path.js";
...
import { createDiffractionCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
const SESSIONS_YIELD_INTERRUPT_CUSTOM_TYPE = "diffraction.sessions_yield_interrupt";
const SESSIONS_YIELD_CONTEXT_CUSTOM_TYPE = "diffraction.sessions_yield";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveDiffractionAgentDir();
    ...
    const docsPath = await resolveDiffractionDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      try {
        ...
        // double-compaction. See: https://github.com/diffraction/diffraction/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("diffraction:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/diffraction/diffraction/issues/8643
