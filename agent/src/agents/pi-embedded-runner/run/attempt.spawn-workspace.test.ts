...
vi.mock("../../docs-path.js", () => ({
  resolveDiffractionDocsPath: async () => undefined,
}));
...
vi.mock("../../pi-tools.js", () => ({
  createDiffractionCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) => [
    {
      name: "sessions_spawn",
      execute: async (
        _callId: string,
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("diffraction.cache-ttl", data),
...
describe("runEmbeddedAttempt sessions_spawn workspace inheritance", () => {
  ...
  it("passes the real workspace to sessions_spawn when workspaceAccess is ro", async () => {
    const realWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-real-workspace-"));
    const sandboxWorkspace = await fs.mkdtemp(
      path.join(os.tmpdir(), "diffraction-sandbox-workspace-"),
    );
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-agent-dir-"));
...
describe("runEmbeddedAttempt bootstrap warning prompt assembly", () => {
  ...
  it("keeps bootstrap warnings in the sent prompt after hook prepend context", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-warning-workspace-"));
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-warning-agent-dir-"));
...
describe("runEmbeddedAttempt cache-ttl tracking after compaction", () => {
  ...
  async function runAttemptWithCacheTtl(compactionCount: number) {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-cache-ttl-workspace-"));
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-cache-ttl-agent-"));
    const sessionFile = path.join(workspaceDir, "session.jsonl");
    tempPaths.push(workspaceDir, agentDir);
    await fs.writeFile(sessionFile, "", "utf8");
...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  async function runAttemptWithContextEngine(contextEngine: {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-ctx-engine-workspace-"));
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-ctx-engine-agent-"));
    const sessionFile = path.join(workspaceDir, "session.jsonl");
    tempPaths.push(workspaceDir, agentDir);
    await fs.writeFile(sessionFile, "", "utf8");
