...
function resolveAttachmentLimits(config: DiffractionConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".diffraction", "attachments");
  const relDir = path.posix.join(".diffraction", "attachments", attachmentId);
