...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a merged --mcp-config overlay for claude-cli", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("diffraction-cli-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("diffraction-cli-bundle-mcp-workspace-");
