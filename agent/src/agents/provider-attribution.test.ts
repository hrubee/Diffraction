...
describe("provider attribution", () => {
  it("resolves the canonical Diffraction product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      DIFFRACTION_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in Diffraction runtime wrapper.",
      product: "Diffraction",
      version: "2026.3.22",
      headers: {
        "HTTP-Referer": "https://diffraction.ai",
        "X-OpenRouter-Title": "Diffraction",
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { DIFFRACTION_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      version: "2026.3.22",
      headers: {
        originator: "diffraction",
        "User-Agent": "diffraction/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { DIFFRACTION_VERSION: "2026.3.22" })).toEqual({
      originator: "diffraction",
      "User-Agent": "diffraction/2026.3.22",
    });
  });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { DIFFRACTION_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ DIFFRACTION_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
