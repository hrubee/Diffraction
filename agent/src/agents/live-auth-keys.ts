...
const GOOGLE_LIVE_SINGLE_KEY = "DIFFRACTION_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "DIFFRACTION_LIVE_ANTHROPIC_KEY",
    listVar: "DIFFRACTION_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "DIFFRACTION_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `DIFFRACTION_LIVE_${base}_KEY`;
