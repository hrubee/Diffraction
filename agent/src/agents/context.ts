...
let configuredConfig: DiffractionConfig | undefined;
...
function isLikelyDiffractionCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = path
    .basename(argv[1] ?? "")
    .trim()
    .toLowerCase();
  return (
    entryBasename === "diffraction" ||
    entryBasename === "diffraction.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Diffraction CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureDiffractionModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyDiffractionCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): DiffractionConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureDiffractionModelsJson(cfg);
    } catch {
...
function resolveConfiguredModelParams(
  cfg: DiffractionConfig | undefined,
  provider: string,
  model: string,
