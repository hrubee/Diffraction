...
import { resolveDiffractionAgentDir } from "./agent-paths.js";
import {
...
import { ensureDiffractionModelsJson } from "./models-config.js";
...
const DIRECT_ENABLED = Boolean(process.env.DIFFRACTION_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.DIFFRACTION_LIVE_REQUIRE_PROFILE_KEYS);
const LIVE_HEARTBEAT_MS = Math.max(1_000, toInt(process.env.DIFFRACTION_LIVE_HEARTBEAT_MS, 30_000));
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set DIFFRACTION_LIVE_MODELS=modern|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const rawModels = process.env.DIFFRACTION_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.DIFFRACTION_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.DIFFRACTION_LIVE_MODEL_TIMEOUT_MS, 30_000);
      const maxModels = toInt(process.env.DIFFRACTION_LIVE_MAX_MODELS, 0);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via DIFFRACTION_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
