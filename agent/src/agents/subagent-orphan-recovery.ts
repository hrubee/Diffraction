 ...
 *
 * @see https://github.com/diffraction/diffraction/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /diffraction\.json|diffraction gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify diffraction.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
