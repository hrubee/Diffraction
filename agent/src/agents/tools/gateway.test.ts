...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    diffraction: process.env.DIFFRACTION_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.diffraction === undefined) {
      delete process.env.DIFFRACTION_GATEWAY_TOKEN;
    } else {
      process.env.DIFFRACTION_GATEWAY_TOKEN = envSnapshot.diffraction;
    }
  });
  ...
  it("uses DIFFRACTION_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.DIFFRACTION_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
