...
async function resolveContext(config: DiffractionConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): DiffractionConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
    ...
    const cfg: DiffractionConfig = {
      agents: {
        ...
        list: [
          {
            id: "isolated",
            workspace: "~/diffraction-isolated",
            sandbox: {
              mode: "all",
              scope: "agent",
