...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: DiffractionConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/diffraction" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: DiffractionConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: DiffractionConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: DiffractionConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: DiffractionConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: DiffractionConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: DiffractionConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: DiffractionConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: DiffractionConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/diffraction-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/diffraction");
  });

  it("uses DIFFRACTION_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "diffraction-home");
    vi.stubEnv("DIFFRACTION_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as DiffractionConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".diffraction", "workspace"));
  });

  it("uses DIFFRACTION_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "diffraction-home");
    vi.stubEnv("DIFFRACTION_HOME", home);
    // Clear state dir so it falls back to DIFFRACTION_HOME
    vi.stubEnv("DIFFRACTION_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as DiffractionConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".diffraction", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/diffraction-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/diffraction-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-agent-scope-"));
