...
type AgentEntry = NonNullable<NonNullable<DiffractionConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: DiffractionConfig): AgentEntry[] {
...
export function listAgentIds(cfg: DiffractionConfig): string[] {
...
export function resolveDefaultAgentId(cfg: DiffractionConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: DiffractionConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: DiffractionConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: DiffractionConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
