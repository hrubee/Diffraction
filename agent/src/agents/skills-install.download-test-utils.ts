...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("diffraction-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-skills-install-"));
  try {
