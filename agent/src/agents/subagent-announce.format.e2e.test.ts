...
let configOverride: DiffractionConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: DiffractionConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/diffraction/diffraction/issues/31298
    previousFastTestEnv = process.env.DIFFRACTION_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.DIFFRACTION_TEST_FAST;
      return;
    }
    process.env.DIFFRACTION_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // DIFFRACTION_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Diffraction runtime context (internal):");
