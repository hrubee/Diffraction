...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["DIFFRACTION_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-subagent-"));
  ...
  it("uses isolated temp state when DIFFRACTION_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "diffraction-test-state"));
  });
});
