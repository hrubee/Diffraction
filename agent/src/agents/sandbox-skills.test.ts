...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "diffraction-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.DIFFRACTION_BUNDLED_SKILLS_DIR = bundledDir;
