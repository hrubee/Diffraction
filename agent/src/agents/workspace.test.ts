...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses DIFFRACTION_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      DIFFRACTION_HOME: "/srv/diffraction-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/diffraction-home"), ".diffraction", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".diffraction", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("diffraction-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("diffraction-workspace-");
    await fs.mkdir(path.join(tempDir, ".diffraction"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-workspace-hardlink-"));
    try {
