...
describe("normalizeProviders", () => {
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    ...
    try {
      const providers: NonNullable<NonNullable<DiffractionConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<DiffractionConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<DiffractionConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
