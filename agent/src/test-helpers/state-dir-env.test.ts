...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    diffraction: process.env.DIFFRACTION_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.DIFFRACTION_STATE_DIR).toBe(snapshot.diffraction);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips DIFFRACTION_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/diffraction-state-dir-test");
    expect(process.env.DIFFRACTION_STATE_DIR).toBe("/tmp/diffraction-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("diffraction-state-dir-env-", async ({ tempRoot, stateDir }) => {
      capturedTempRoot = tempRoot;
      capturedStateDir = stateDir;
      expect(process.env.DIFFRACTION_STATE_DIR).toBe(stateDir);
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("diffraction-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
