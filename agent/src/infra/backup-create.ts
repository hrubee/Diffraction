...
export async function createBackupArchive(
  ...
  if (plan.included.length === 0) {
    throw new Error(
      onlyConfig
        ? "No Diffraction config file was found to back up."
        : "No local Diffraction state was found to back up.",
    );
  }
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-backup-"));
