...
import { DIFFRACTION_CLI_ENV_VALUE } from "./diffraction-exec-env.js";
...
describe("sanitizeHostExecEnv", () => {
  ...
  it("blocks PATH and dangerous override values", () => {
    ...
    expect(env.DIFFRACTION_CLI).toBe(DIFFRACTION_CLI_ENV_VALUE);
...
describe("shell wrapper exploit regression", () => {
  it("blocks SHELLOPTS/PS4 chain after sanitization", async () => {
    ...
    const marker = path.join(os.tmpdir(), `diffraction-ps4-marker-${process.pid}-${Date.now()}`);
    try {
      fs.unlinkSync(marker);
    } catch {
      // no-op
...
describe("git env exploit regression", () => {
  it("blocks inherited GIT_EXEC_PATH so git cannot execute helper payloads", async () => {
    ...
    const helperDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `diffraction-git-exec-path-${process.pid}-${Date.now()}-`),
    );
    const helperPath = path.join(helperDir, "git-remote-https");
    const marker = path.join(
      os.tmpdir(),
      `diffraction-git-exec-path-marker-${process.pid}-${Date.now()}`,
    );
    try {
      clearMarker(marker);
  ...
  it("blocks GIT_SSH_COMMAND override so git cannot execute helper payloads", async () => {
    ...
    const marker = path.join(os.tmpdir(), `diffraction-git-ssh-command-${process.pid}-${Date.now()}`);
    clearMarker(marker);
