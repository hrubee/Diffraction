...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/diffraction-state", "devices")).toEqual({
      dir: path.join("/tmp/diffraction-state", "devices"),
      pendingPath: path.join("/tmp/diffraction-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/diffraction-state", "devices", "paired.json"),
    });
  });
