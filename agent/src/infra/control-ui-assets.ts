...
import { resolveDiffractionPackageRoot, resolveDiffractionPackageRootSync } from "./diffraction-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveDiffractionPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "diffraction" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveDiffractionPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // diffraction.mjs or dist/<bundle>.js
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/diffraction -> .../dist/index.js)
