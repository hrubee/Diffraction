 ...
 * returning control to the caller (typically `triggerDiffractionRestart` →
...
/**
 * Parse diffraction gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("diffraction")) {
 ...
 * Returns only PIDs that belong to diffraction gateway processes (not the current process).
 */
