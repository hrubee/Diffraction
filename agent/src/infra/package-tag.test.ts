...
describe("normalizePackageTagInput", () => {
  const packageNames = ["diffraction", "@diffraction/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("diffraction@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@diffraction/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("diffraction@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("diffraction", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @diffraction/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("diffractioner@beta", packageNames)).toBe("diffractioner@beta");
  });
});
