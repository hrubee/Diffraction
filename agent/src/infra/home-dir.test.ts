...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers DIFFRACTION_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for DIFFRACTION_HOME", () => {
    const result = resolveRequiredHomeDir(
      { DIFFRACTION_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when DIFFRACTION_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ DIFFRACTION_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("resolveOsHomeDir", () => {
  it("ignores DIFFRACTION_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          DIFFRACTION_HOME: "/srv/diffraction-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { DIFFRACTION_HOME: "/srv/diffraction-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/diffraction-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/diffraction-home " },
      expected: "/srv/diffraction-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/diffraction-home/docs"));
  });
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of DIFFRACTION_HOME", () => {
    expect(
