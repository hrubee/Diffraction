...
import { triggerDiffractionRestart } from "./restart.js";
 ...
 * - DIFFRACTION_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.DIFFRACTION_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerDiffractionRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
