...
type EnsureDiffractionPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureDiffractionPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `diffraction` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "diffraction");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.DIFFRACTION_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "diffraction"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `diffraction` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureDiffractionCliOnPath(opts: EnsureDiffractionPathOpts = {}) {
  if (isTruthyEnvValue(process.env.DIFFRACTION_PATH_BOOTSTRAPPED)) {
    return;
  }
