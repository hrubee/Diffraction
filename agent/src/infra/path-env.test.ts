...
let ensureDiffractionCliOnPath: typeof import("./path-env.js").ensureDiffractionCliOnPath;

describe("ensureDiffractionCliOnPath", () => {
  const envKeys = [
    "PATH",
    "DIFFRACTION_PATH_BOOTSTRAPPED",
    "DIFFRACTION_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureDiffractionCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/diffraction-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "diffraction");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureDiffractionCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling diffraction exists", () => {
    const { tmp, appBinDir, appCli } = setupAppCliRoot("case-bundled");
    process.env.PATH = "/usr/bin";
    delete process.env.DIFFRACTION_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.DIFFRACTION_PATH_BOOTSTRAPPED = "1";
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "diffraction");
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.DIFFRACTION_PATH_BOOTSTRAPPED;
      delete process.env.DIFFRACTION_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.DIFFRACTION_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/diffraction-path/case-linuxbrew");
