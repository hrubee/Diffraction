...
function createDiffractionBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "diffraction-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createDiffractionBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses diffraction-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "diffraction-gateway" },
          { pid: process.pid, cmd: "diffraction-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'diffraction'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple diffraction pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "diffraction-gateway" },
          { pid: pid2, cmd: "diffraction-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-diffraction pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks diffraction processes — non-diffraction listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty diffraction stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty diffraction stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + diffraction pid in stdout — container-restricted lsof reports partial results
          return createDiffractionBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "diffraction-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'diffraction'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("diffraction")
      // inside the mid-loop flush: a non-diffraction cmd between two entries must not
      // be pushed, but the following diffraction entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-diffraction entry first, then diffraction entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid diffraction entry must still be found.
      const stdout = `p0\ncdiffraction-gateway\np${stalePid}\ncdiffraction-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero diffraction pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-diffraction stdout (line 145)", () => {
    it("treats status 1 + non-diffraction stdout as port-free (not an diffraction process)", () => {
      // status 1 + non-empty stdout where no diffraction pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill diffraction pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-diffraction output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no diffraction pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
