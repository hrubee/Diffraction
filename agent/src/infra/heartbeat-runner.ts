...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: DiffractionConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: DiffractionConfig) {
...
function resolveHeartbeatAgents(cfg: DiffractionConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: DiffractionConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: DiffractionConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: DiffractionConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
