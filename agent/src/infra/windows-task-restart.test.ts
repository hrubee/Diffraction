...
const resolvePreferredDiffractionTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./tmp-diffraction-dir.js", () => ({
  resolvePreferredDiffractionTmpDir: () => resolvePreferredDiffractionTmpDirMock(),
}));
...
afterEach(() => {
  envSnapshot.restore();
  spawnMock.mockReset();
  resolvePreferredDiffractionTmpDirMock.mockReset();
  resolvePreferredDiffractionTmpDirMock.mockReturnValue(os.tmpdir());
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ DIFFRACTION_PROFILE: "work" });

    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "Diffraction Gateway (work)"']),
    });
    ...
    expect(script).toContain('schtasks /Run /TN "Diffraction Gateway (work)" >nul 2>&1');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers DIFFRACTION_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    relaunchGatewayScheduledTask({
      DIFFRACTION_PROFILE: "work",
      DIFFRACTION_WINDOWS_TASK_NAME: "Diffraction Gateway (custom)",
    });

    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Diffraction Gateway (custom)" >nul 2>&1');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredDiffractionTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ DIFFRACTION_PROFILE: "work" });
