...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.DIFFRACTION_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Diffraction";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Diffraction\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw = process.env.DIFFRACTION_MDNS_HOSTNAME?.trim() || "diffraction";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
