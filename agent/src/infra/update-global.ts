...
const PRIMARY_PACKAGE_NAME = "diffraction";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const DIFFRACTION_MAIN_PACKAGE_SPEC = "github:diffraction/diffraction#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "Diffraction", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.DIFFRACTION_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.DIFFRACTION_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return DIFFRACTION_MAIN_PACKAGE_SPEC;
  }
