...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Diffraction tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideDiffraction = path.join(os.tmpdir(), "outside-diffraction", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
            params: {
              channel: "slack",
              target: "#C12345678",
              media: hostTmpOutsideDiffraction,
              message: "",
            },
            sandboxRoot: sandboxDir,
            dryRun: true,
