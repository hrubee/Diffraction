...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: DiffractionConfig | null = null;
  ...
  get(key: string, cfg: DiffractionConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: DiffractionConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: DiffractionConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: DiffractionConfig): void {
