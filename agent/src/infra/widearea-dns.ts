...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.DIFFRACTION_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*diffraction-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "diffraction";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "diffraction");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "diffraction-gw");
  ...
  records.push(`_diffraction-gw._tcp IN PTR ${instanceLabel}._diffraction-gw._tcp`);
  records.push(`${instanceLabel}._diffraction-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._diffraction-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; diffraction-content-hash: ${contentHash}\n${contentBody}`;
}
