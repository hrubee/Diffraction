...
export const POSIX_DIFFRACTION_TMP_DIR = "/tmp/diffraction";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredDiffractionTmpDirOptions = {
...
export function resolvePreferredDiffractionTmpDir(
  options: ResolvePreferredDiffractionTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "diffraction" : `diffraction-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[diffraction] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Diffraction temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback Diffraction temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_DIFFRACTION_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_DIFFRACTION_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_DIFFRACTION_TMP_DIR)) {
      return POSIX_DIFFRACTION_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_DIFFRACTION_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_DIFFRACTION_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_DIFFRACTION_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_DIFFRACTION_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_DIFFRACTION_TMP_DIR;
