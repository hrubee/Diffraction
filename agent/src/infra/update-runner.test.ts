...
describe("runGatewayUpdate", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-update-"));
  });
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "diffraction.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "diffraction.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "diffraction", version: "1.0.0" };
  ...
  async function seedGlobalPackageRoot(pkgRoot: string, version = "1.0.0") {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "diffraction", version }),
      "utf-8",
    );
  }
  ...
  async function createGlobalPackageFixture(rootDir: string) {
    ...
    const pkgRoot = path.join(nodeModules, "diffraction");
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = "npm i -g diffraction@latest --no-fund --no-audit --loglevel=error";
    const omitOptionalInstallKey =
      "npm i -g diffraction@latest --omit=optional --no-fund --no-audit --loglevel=error";
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "diffraction.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("falls back to npm when pnpm is unavailable for git installs", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "diffraction", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    const uiIndexPath = path.join(tempDir, "dist", "control-ui", "index.html");
    await fs.mkdir(path.dirname(uiIndexPath), { recursive: true });
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (
        key === `${process.execPath} ${path.join(tempDir, "diffraction.mjs")} doctor --non-interactive`
      ) {
        return { stdout: "", stderr: "", code: 0 };
      }
      return { stdout: "", stderr: "", code: 0 };
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "diffraction", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "diffraction", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g diffraction@latest --no-fund --no-audit --loglevel=error",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g diffraction@beta --no-fund --no-audit --loglevel=error",
  ...
  it("updates global npm installs from the GitHub main package spec", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand:
        "npm i -g github:diffraction/diffraction#main --no-fund --no-audit --loglevel=error",
      tag: "main",
    });
  ...
  it("falls back to global npm update when git is missing from PATH", async () => {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: "npm i -g diffraction@latest --no-fund --no-audit --loglevel=error",
      gitRootMode: "missing",
    ...
    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain("npm i -g diffraction@latest --no-fund --no-audit --loglevel=error");
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "diffraction");
    const staleDir = path.join(nodeModules, ".diffraction-stale");
  ...
  it("uses DIFFRACTION_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand =
      "npm i -g http://10.211.55.2:8138/diffraction-next.tgz --no-fund --no-audit --loglevel=error";
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
    ...
    await withEnvAsync(
      { DIFFRACTION_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/diffraction-next.tgz" },
      async () => {
        const result = await runWithCommand(runCommand, { cwd: pkgRoot });
        expect(result.status).toBe("ok");
      },
  ...
  it("updates global bun installs when detected", async () => {
    ...
    await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
      ...
      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g diffraction@latest",
        onInstall: async () => {
          await writeGlobalPackageVersion(pkgRoot);
        },
      });
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g diffraction@latest")).toBe(true);
    });
  });

  it("rejects git roots that are not a diffraction checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-diffraction-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when diffraction.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "diffraction.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("diffraction doctor entry");
  });
