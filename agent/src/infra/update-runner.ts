...
const DEFAULT_PACKAGE_NAME = "diffraction";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["diffraction", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "diffraction.mjs");
    ...
    const doctorStep = await runStep(
      step("diffraction doctor", doctorArgv, gitRoot, { DIFFRACTION_UPDATE_IN_PROGRESS: "1" }),
    );
