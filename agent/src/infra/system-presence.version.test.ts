...
describe("system-presence version fallback", () => {
  ...
  it("uses runtime VERSION when DIFFRACTION_VERSION is not set", async () => {
  ...
  it("prefers DIFFRACTION_VERSION over runtime VERSION", async () => {
    await expectSelfVersion(
      {
  ...
  it("still prefers runtime VERSION over DIFFRACTION_SERVICE_VERSION when DIFFRACTION_VERSION is blank", async () => {
    await expectSelfVersion(
      {
        DIFFRACTION_VERSION: " ",
        DIFFRACTION_SERVICE_VERSION: "2.4.6-service",
  ...
  it("uses runtime VERSION when DIFFRACTION_VERSION and DIFFRACTION_SERVICE_VERSION are blank", async () => {
    await expectSelfVersion(
      {
        DIFFRACTION_VERSION: " ",
        DIFFRACTION_SERVICE_VERSION: "\t",
