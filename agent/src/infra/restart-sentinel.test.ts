...
describe("restart sentinel", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["DIFFRACTION_STATE_DIR"]);
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-sentinel-"));
  ...
  it("formats summary, distinct reason, and doctor hint together", () => {
    const payload = {
      ...
      doctorHint: "Run diffraction doctor",
