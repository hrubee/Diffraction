...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["DIFFRACTION_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.DIFFRACTION_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveDiffractionPackageRoot } = await import("./diffraction-root.js"));
    ...
    vi.mocked(resolveDiffractionPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveDiffractionPackageRoot).mockResolvedValue("/opt/diffraction");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/diffraction",
      installKind: "package",
      packageManager: "npm",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/diffraction/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
