...
export function triggerDiffractionRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.DIFFRACTION_SYSTEMD_UNIT,
      process.env.DIFFRACTION_PROFILE,
    );
  ...
  const label =
    process.env.DIFFRACTION_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.DIFFRACTION_PROFILE);
