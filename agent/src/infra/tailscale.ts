...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.DIFFRACTION_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Diffraction. You can keep running the web gateway without it: \`${formatCliCommand("diffraction gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
