...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevDiffractionHome = process.env.DIFFRACTION_HOME;
    const prevHome = process.env.HOME;
    process.env.DIFFRACTION_HOME = "/srv/diffraction-home";
    process.env.HOME = "/home/other";
    const openClawHome = path.join(path.resolve("/srv/diffraction-home"), "bin", "tool");
    ...
    try {
      ...
      if (prevDiffractionHome === undefined) {
        delete process.env.DIFFRACTION_HOME;
      } else {
        process.env.DIFFRACTION_HOME = prevDiffractionHome;
      }
