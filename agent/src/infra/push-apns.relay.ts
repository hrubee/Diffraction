...
const GATEWAY_DEVICE_ID_HEADER = "x-diffraction-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-diffraction-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-diffraction-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.DIFFRACTION_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "DIFFRACTION_APNS_RELAY_BASE_URL"
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.DIFFRACTION_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require DIFFRACTION_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.DIFFRACTION_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
