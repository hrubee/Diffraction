...
export function resolveTtsConfig(cfg: DiffractionConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.DIFFRACTION_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: DiffractionConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: DiffractionConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredDiffractionTmpDir();
