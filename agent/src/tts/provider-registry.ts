...
function resolveSpeechProviderPluginEntries(cfg?: DiffractionConfig): SpeechProviderPlugin[] {
  const active = getActivePluginRegistry();
  const registry =
    (active?.speechProviders?.length ?? 0) > 0 || !cfg
      ? active
      : loadDiffractionPlugins({ config: cfg });
  return registry?.speechProviders?.map((entry) => entry.provider) ?? [];
}

function buildProviderMaps(cfg?: DiffractionConfig): {
...
export function listSpeechProviders(cfg?: DiffractionConfig): SpeechProviderPlugin[] {
