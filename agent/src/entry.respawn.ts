...
export const DIFFRACTION_NODE_OPTIONS_READY = "DIFFRACTION_NODE_OPTIONS_READY";
export const DIFFRACTION_NODE_EXTRA_CA_CERTS_READY = "DIFFRACTION_NODE_EXTRA_CA_CERTS_READY";
...
export function buildCliRespawnPlan(
  ...
  if (shouldSkipRespawnForArgv(argv) || isTruthyEnvValue(env.DIFFRACTION_NO_RESPAWN)) {
    return null;
  }
  ...
  if (
    ...
    !isTruthyEnvValue(env[DIFFRACTION_NODE_EXTRA_CA_CERTS_READY]) &&
    !env.NODE_EXTRA_CA_CERTS
  ) {
    childEnv.NODE_EXTRA_CA_CERTS = autoNodeExtraCaCerts;
    childEnv[DIFFRACTION_NODE_EXTRA_CA_CERTS_READY] = "1";
    needsRespawn = true;
  }

  if (
    !isTruthyEnvValue(env[DIFFRACTION_NODE_OPTIONS_READY]) &&
    !hasExperimentalWarningSuppressed({ env, execArgv })
  ) {
    childEnv[DIFFRACTION_NODE_OPTIONS_READY] = "1";
