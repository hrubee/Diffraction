...
let originalDiffractionLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.DIFFRACTION_TEST_FILE_LOG;
  originalDiffractionLogLevel = process.env.DIFFRACTION_LOG_LEVEL;
  delete process.env.DIFFRACTION_TEST_FILE_LOG;
  delete process.env.DIFFRACTION_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.DIFFRACTION_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalDiffractionLogLevel === undefined) {
    delete process.env.DIFFRACTION_LOG_LEVEL;
  } else {
    process.env.DIFFRACTION_LOG_LEVEL = originalDiffractionLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
