...
function createOpenAiAudioCfg(extra?: Partial<DiffractionConfig>): DiffractionConfig {
  return {
    models: {
      providers: {
        openai: {
...
async function runAutoAudioCase(params: {
  transcribeAudio: (req: { model?: string }) => Promise<{ text: string; model: string }>;
  cfgExtra?: Partial<DiffractionConfig>;
}) {
  let runResult: Awaited<ReturnType<typeof runCapability>> | undefined;
  await withAudioFixture("diffraction-auto-audio", async ({ ctx, media, cache }) => {
...
describe("runCapability auto audio entries", () => {
  ...
  it("uses mistral when only mistral key is configured", async () => {
    const isolatedAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-audio-agent-"));
    let runResult: Awaited<ReturnType<typeof runCapability>> | undefined;
    try {
      await withEnvAsync(
        {
        ...
        async () => {
          await withAudioFixture("diffraction-auto-audio-mistral", async ({ ctx, media, cache }) => {
