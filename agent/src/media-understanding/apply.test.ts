...
const TEMP_MEDIA_PREFIX = "diffraction-media-";
...
function createGroqAudioConfig(): DiffractionConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): DiffractionConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): DiffractionConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: DiffractionConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredDiffractionTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        DIFFRACTION_AGENT_DIR: isolatedAgentDir,
