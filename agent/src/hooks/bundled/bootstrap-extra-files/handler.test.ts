...
function createBootstrapExtraConfig(paths: string[]): DiffractionConfig {
  return {
    hooks: {
      internal: {
        entries: {
...
describe("bootstrap-extra-files hook", () => {
  it("appends extra bootstrap files from configured patterns", async () => {
    const tempDir = await makeTempWorkspace("diffraction-bootstrap-extra-");
  ...
  it("re-applies subagent bootstrap allowlist after extras are added", async () => {
    const tempDir = await makeTempWorkspace("diffraction-bootstrap-extra-subagent-");
