...
homepage: https://docs.diffraction.ai/automation/hooks#command-logger
metadata:
  {
    "diffraction":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.diffraction/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.diffraction/logs/commands.log ~/.diffraction/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/diffraction`:
   ```
   /home/username/.diffraction/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
