...
This directory contains hooks that ship with Diffraction. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.diffraction/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.diffraction/diffraction.json`:
...
2. Restart gateway: `pkill -9 -f 'diffraction.*gateway' && pnpm diffraction gateway`
3. Enable the hook: `diffraction hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.diffraction.ai/automation/hooks
