...
const fixtureRoot = path.join(process.cwd(), ".tmp", `diffraction-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@diffraction/test-hooks",
      version: "0.0.1",
      diffraction: { hooks: params.hooks },
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "diffraction.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "diffraction.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@diffraction/test-hooks@0.0.1",
              name: "@diffraction/test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@diffraction/test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@diffraction/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@diffraction/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@diffraction/test-hooks@0.0.2-beta.1",
      name: "@diffraction/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@diffraction/test-hooks@beta"');
    }
  });
});
