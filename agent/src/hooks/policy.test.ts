...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "diffraction-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "diffraction-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "diffraction-bundled");
      const plugin = makeHookEntry("shared", "diffraction-plugin");
      const managed = makeHookEntry("shared", "diffraction-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("diffraction-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "diffraction-managed");
      const workspace = makeHookEntry("shared", "diffraction-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "diffraction-workspace");
      const second = makeHookEntry("shared", "diffraction-workspace");
      second.hook.handlerPath = "/tmp/diffraction-workspace/shared/handler-2.js";
