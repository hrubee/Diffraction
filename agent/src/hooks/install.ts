...
async function ensureDiffractionHooks(manifest: HookPackageManifest) {
  const hooks = manifest[MANIFEST_KEY]?.hooks;
  if (!Array.isArray(hooks)) {
    throw new Error("package.json missing diffraction.hooks");
  }
  const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json diffraction.hooks is empty");
  }
  return list;
}
...
async function installHookPackageFromDir(
  ...
  try {
    hookEntries = await ensureDiffractionHooks(manifest);
  } catch (err) {
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `diffraction.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `diffraction.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "diffraction-hook-",
    timeoutMs,
    logger,
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "diffraction-hook-pack-",
    spec,
    timeoutMs,
