...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "diffraction-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.DIFFRACTION_BUNDLED_HOOKS_DIR;
    process.env.DIFFRACTION_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.DIFFRACTION_BUNDLED_HOOKS_DIR;
    } else {
      process.env.DIFFRACTION_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"diffraction":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): DiffractionConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("diffraction-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Diffraction hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".diffraction", "extensions", "claude-bundle");
