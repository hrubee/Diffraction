import type { DiffractionConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "diffraction-bundled": {
    ...
    canOverride: ["diffraction-bundled"],
    canBeOverriddenBy: ["diffraction-managed", "diffraction-plugin"],
  },
  "diffraction-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["diffraction-bundled", "diffraction-plugin"],
    canBeOverriddenBy: ["diffraction-managed"],
  },
  "diffraction-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["diffraction-bundled", "diffraction-managed", "diffraction-plugin"],
    canBeOverriddenBy: ["diffraction-managed"],
  },
  "diffraction-workspace": {
    ...
    canOverride: ["diffraction-workspace"],
    canBeOverriddenBy: ["diffraction-workspace"],
  },
};
...
export function resolveHookConfig(
  config: DiffractionConfig | undefined,
  hookKey: string,
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "diffraction-plugin") {
    return { enabled: true };
  }
