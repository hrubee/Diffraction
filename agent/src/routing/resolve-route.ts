...
function listAgents(cfg: DiffractionConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: DiffractionConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<DiffractionConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: DiffractionConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: DiffractionConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: DiffractionConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<DiffractionConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  DiffractionConfig,
  {
    bindingsRef: DiffractionConfig["bindings"];
    agentsRef: DiffractionConfig["agents"];
    sessionRef: DiffractionConfig["session"];
...
function resolveRouteCacheForConfig(cfg: DiffractionConfig): Map<string, ResolvedAgentRoute> {
