...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: DiffractionConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: DiffractionConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/diffraction-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<DiffractionConfig["bindings"]>[number];
