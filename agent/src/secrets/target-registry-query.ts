...
const DIFFRACTION_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of DIFFRACTION_COMPILED_SECRET_TARGETS) {
...
const DIFFRACTION_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: DIFFRACTION_COMPILED_SECRET_TARGETS,
    entriesById: DIFFRACTION_TARGETS_BY_ID,
  });
