...
function getSecretProviders(config: DiffractionConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: DiffractionConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: DiffractionConfig): Array<{ value: SecretRefSource; label: string }> {
...
function configureCandidateKey(candidate: {
  configFile: "diffraction.json" | "auth-profiles.json";
  path: string;
  agentId?: string;
}): string {
  ...
  return `diffraction:${candidate.path}`;
}
...
function resolveConfigureAgentId(config: DiffractionConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: DiffractionConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredDiffractionConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "diffraction.json",
