...
export function buildConfigureCandidates(config: DiffractionConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  ...
  authoredDiffractionConfig?: DiffractionConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredDiffractionConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const diffractionCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "diffraction.json" as const,
  ...
  return [...diffractionCandidates, ...authCandidates].toSorted((a, b) =>
...
export function collectConfigureProviderChanges(params: {
  original: DiffractionConfig;
  next: DiffractionConfig;
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "diffraction secrets configure",
