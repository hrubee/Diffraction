...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("diffraction-bundle-commands-home-");
      const workspaceDir = await tempHarness.createTempDir("diffraction-bundle-commands-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.DIFFRACTION_HOME;
      delete process.env.DIFFRACTION_STATE_DIR;

      const pluginRoot = path.join(homeDir, ".diffraction", "extensions", "compound-bundle");
