...
import type { DiffractionPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("diffraction-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "diffraction.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  format?: "diffraction" | "bundle";
  bundleFormat?: "codex" | "claude" | "cursor";
  packageManifest?: DiffractionPackageManifest;
  packageDir?: string;
}): PluginCandidate {
  return {
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "diffraction.plugin.json");
  const linkedManifest = path.join(rootDir, "diffraction.plugin.json");
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("skips plugins whose minHostVersion is newer than the current host", () => {
    ...
    const registry = loadPluginManifestRegistry({
      cache: false,
      env: { DIFFRACTION_VERSION: "2026.3.21" },
      candidates: [
        createPluginCandidate({
          ...
          packageManifest: {
            install: {
              npmSpec: "@diffraction/synology-chat",
    ...
    expect(
      registry.diagnostics.some((diag) =>
        diag.message.includes("plugin requires Diffraction >=2026.3.22, but this host is 2026.3.21"),
      ),
    ).toBe(true);
  });
  ...
  it("rejects invalid minHostVersion metadata", () => {
    ...
    expect(
      registry.diagnostics.some((diag) =>
        diag.message.includes("plugin manifest invalid | diffraction.install.minHostVersion must use"),
      ),
    ).toBe(true);
  });
  ...
  it("warns distinctly when host version cannot be determined", () => {
    ...
    const registry = loadPluginManifestRegistry({
      cache: false,
      env: { DIFFRACTION_VERSION: "unknown" },
      candidates: [
        createPluginCandidate({
          idHint: "synology-chat",
          rootDir: dir,
  ...
  it("accepts sandbox-style id hints without warning", () => {
    const dir = makeTempDir();
    writeManifest(dir, { id: "diffract", configSchema: { type: "object" } });

    const registry = loadRegistry([
      createPluginCandidate({
        idHint: "diffract-sandbox",
        rootDir: dir,
        origin: "bundled",
      }),
    ]);
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: {
        ...
        DIFFRACTION_STATE_DIR: path.join(homeA, ".state"),
      },
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: {
        ...
        HOME: homeB,
        DIFFRACTION_HOME: undefined,
        DIFFRACTION_STATE_DIR: path.join(homeB, ".state"),
      },
    });
