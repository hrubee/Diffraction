...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      diffraction: { extensions: ["./dist/index.js"] },
...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(bundleTempRoot, "diffraction-plugin-install-"));
  return suiteTempRoot;
}
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@diffraction/cognee-diffraction",
      version: "0.0.1",
      diffraction: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "diffraction.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@diffraction/voice-call" });
  ...
  it("installs from a zip archive", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@diffraction/zipper" });
  });

  it("installs flat-root plugin archives from ClawHub-style downloads", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: {
        name: "@diffraction/rootless",
        version: "0.0.1",
        diffraction: { extensions: ["./dist/index.js"] },
      },
  ...
  it("rejects packages without diffraction.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@diffraction/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("diffraction.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_DIFFRACTION_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when diffraction.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@diffraction/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing diffraction.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_DIFFRACTION_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without diffraction.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.diffraction).toBeUndefined();
  ...
  it("rejects plugins whose minHostVersion is newer than the current host", async () => {
    vi.stubEnv("DIFFRACTION_VERSION", "2026.3.21");
    const { pluginDir, extensionsDir } = setupInstallPluginFromDirFixture();
    const packageJsonPath = path.join(pluginDir, "package.json");
    const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
      diffraction?: { install?: Record<string, unknown> };
    };
    manifest.diffraction = {
      ...manifest.diffraction,
      install: {
        ...manifest.diffraction?.install,
    ...
    expect(result.error).toContain("requires Diffraction >=2026.3.22, but this host is 2026.3.21");
  ...
  it("rejects plugins with invalid minHostVersion metadata", async () => {
    ...
    expect(result.error).toContain("invalid package.json diffraction.install.minHostVersion");
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });

  it("reports unknown host versions distinctly for minHostVersion-gated plugins", async () => {
    vi.stubEnv("DIFFRACTION_VERSION", "unknown");
    const { pluginDir, extensionsDir } = setupInstallPluginFromDirFixture();
    const packageJsonPath = path.join(pluginDir, "package.json");
    const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
      diffraction?: { install?: Record<string, unknown> };
  ...
  it("uses diffraction.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@diffraction/cognee-diffraction"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it("keeps scoped install ids aligned across manifest and package-name cases", async () => {
    const scenarios = [
      ...
      },
      {
        ...
        expectedPluginId: "@diffraction/test-plugin",
        install: (pluginDir: string, extensionsDir: string) =>
          installPluginFromDir({
            dirPath: pluginDir,
            extensionsDir,
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@diffraction/voice-call@0.0.1",
              name: "@diffraction/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@diffraction/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@diffraction/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@diffraction/voice-call@0.0.1",
    });
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@diffraction/voice-call@0.0.2-beta.1",
      name: "@diffraction/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@diffraction/voice-call@beta"');
      }
    }
...

    {
      ...
      const result = await installPluginFromNpmSpec({
        spec: "@diffraction/voice-call@beta",
        extensionsDir,
        logger: { info: () => {}, warn: () => {} },
      });
      expect(result.ok).toBe(true);
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@diffraction/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls,
        expectedSpec: "@diffraction/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
