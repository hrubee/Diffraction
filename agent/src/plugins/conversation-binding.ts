...
const APPROVALS_PATH = "~/.diffraction/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "diffraction-app-server:thread:",
  "diffraction-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("diffraction.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("diffraction.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Diffraction instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
