...
const PLUGIN_INTERACTIVE_STATE_KEY = Symbol.for("diffraction.pluginInteractiveState");
...
const state = resolveGlobalSingleton<InteractiveState>(PLUGIN_INTERACTIVE_STATE_KEY, () => ({
  ...
  callbackDedupe: resolveGlobalDedupeCache(Symbol.for("diffraction.pluginInteractiveCallbackDedupe"), {
    ttlMs: 5 * 60_000,
    maxSize: 4096,
  }),
}));
