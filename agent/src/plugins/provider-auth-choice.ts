...
function restoreConfiguredPrimaryModel(
  ...
  originalConfig: DiffractionConfig,
): DiffractionConfig {
...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: DiffractionConfig; defaultModel?: string }> {
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveDiffractionAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
