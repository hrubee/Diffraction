...
describe("bundled plugin metadata", () => {
  ...
  it("prefers built generated paths when present and falls back to source paths", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-bundled-plugin-metadata-"));
  ...
  it("supports check mode for stale generated artifacts", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-bundled-plugin-generated-"));
    tempDirs.push(tempRoot);

    writeJson(path.join(tempRoot, "extensions", "alpha", "package.json"), {
      name: "@diffraction/alpha",
      version: "0.0.1",
      diffraction: {
        extensions: ["./index.ts"],
      },
    });
    writeJson(path.join(tempRoot, "extensions", "alpha", "diffraction.plugin.json"), {
      id: "alpha",
