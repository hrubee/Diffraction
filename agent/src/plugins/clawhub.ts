...
export const DIFFRACTION_PLUGIN_API_VERSION = "1.2.0";
...
function validateClawHubPluginPackage(params: {
  ...
  if (pkg.family === "skill") {
    return buildClawHubInstallFailure(
      `"${pkg.name}" is a skill. Use "diffraction skills install ${pkg.name}" instead.`,
  ...
  if (
    ...
    !satisfiesPluginApiRange(DIFFRACTION_PLUGIN_API_VERSION, compatibility.pluginApiRange)
  ) {
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires plugin API ${compatibility.pluginApiRange}, but this Diffraction runtime exposes ${DIFFRACTION_PLUGIN_API_VERSION}.`,
  ...
  if (
    ...
    return buildClawHubInstallFailure(
      `Plugin "${pkg.name}" requires Diffraction >=${compatibility.minGatewayVersion}, but this host is ${runtimeVersion}.`,
  ...
  try {
    ...
    return {
      ...
      clawhub: {
        ...
        clawhubUrl:
          params.baseUrl?.trim() ||
          process.env.DIFFRACTION_CLAWHUB_URL?.trim() ||
          "https://clawhub.ai",
