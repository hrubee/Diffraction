...
function readBundledPluginRecords(): BundledPluginRecord[] {
  ...
  for (const dirName of fs.readdirSync(EXTENSIONS_ROOT).toSorted()) {
    ...
    const manifestPath = path.join(rootDir, "diffraction.plugin.json");
    if (!fs.existsSync(packagePath) || !fs.existsSync(manifestPath)) {
      continue;
    }

    const manifest = readJsonFile<PluginManifestShape>(manifestPath);
    const pkg = readJsonFile<DiffractionPackageShape>(packagePath);
    ...
    records.push({
      ...
      packageName,
      manifestId,
      installNpmSpec: normalizeText(pkg.diffraction?.install?.npmSpec),
      channelId: normalizeText(pkg.diffraction?.channel?.id),
    });
  }
  return records;
}
...
function resolveAllowedPackageNamesForId(pluginId: string): string[] {
  return ALLOWED_PACKAGE_SUFFIXES.map((suffix) => `@diffraction/${pluginId}${suffix}`);
}
...
describe("bundled plugin naming guardrails", () => {
  it("keeps bundled workspace package names anchored to the plugin id", () => {
    ...
    expect(
      mismatches,
      `Bundled extension package names must stay anchored to the manifest id via @diffraction/<id> or an approved suffix (${ALLOWED_PACKAGE_SUFFIXES.join(", ")}). Update the plugin naming docs and this invariant before adding a new naming form.\nFound: ${mismatches.join(", ") || "<none>"}`,
    ).toEqual([]);
  });
  ...
  it("keeps bundled workspace directories aligned with the plugin id unless explicitly allowlisted", () => {
    ...
    expect(
      mismatches,
      `Bundled extension directory names should match diffraction.plugin.json:id. If a legacy exception is unavoidable, add it to DIR_ID_EXCEPTIONS with a comment.\nFound: ${mismatches.join(", ") || "<none>"}`,
    ).toEqual([]);
  });

  it("keeps bundled diffraction.install.npmSpec aligned with the package name", () => {
    const mismatches = readBundledPluginRecords()
      .filter(
    ...
    expect(
      mismatches,
      `Bundled diffraction.install.npmSpec values must match the package name so install/update paths stay deterministic.\nFound: ${mismatches.join(", ") || "<none>"}`,
    ).toEqual([]);
  });
  ...
  it("keeps bundled channel ids aligned with the canonical plugin id", () => {
    ...
    expect(
      mismatches,
      `Bundled diffraction.channel.id values must match diffraction.plugin.json:id for the owning plugin.\nFound: ${mismatches.join(", ") || "<none>"}`,
    ).toEqual([]);
  });
});
