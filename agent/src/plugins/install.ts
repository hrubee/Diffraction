...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing diffraction.extensions; update the plugin package to include diffraction.extensions (for example ["./dist/index.js"]). See https://docs.diffraction.ai/help/troubleshooting#plugin-install-fails-with-missing-diffraction-extensions';
const PLUGIN_ARCHIVE_ROOT_MARKERS = [
  "package.json",
  "diffraction.plugin.json",
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_DIFFRACTION_EXTENSIONS: "missing_diffraction_extensions",
  EMPTY_DIFFRACTION_EXTENSIONS: "empty_diffraction_extensions",
...
function ensureDiffractionExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_DIFFRACTION_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json diffraction.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_DIFFRACTION_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "diffraction security audit --deep" after install.`,
    );
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureDiffractionExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureDiffractionExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from diffraction.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-diffraction"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json diffraction.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Diffraction >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set DIFFRACTION_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Diffraction >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Diffraction and retry.`,
  ...
  try {
    ...
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "diffraction security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "diffraction-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "diffraction-npm-pack-",
    spec,
    timeoutMs,
