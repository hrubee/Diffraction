...
import { __testing, clearPluginLoaderCache, loadDiffractionPlugins } from "./loader.js";
...
type PluginLoadConfig = NonNullable<Parameters<typeof loadDiffractionPlugins>[0]>["config"];
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "diffraction-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "diffraction.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadDiffractionPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          diffraction: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadDiffractionPlugins>) {
...
function useNoBundledPlugins() {
  process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadDiffractionPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadDiffractionPlugins>[0], "cache" | "config">,
) {
  return loadDiffractionPlugins({
    cache: false,
    ...options,
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "diffraction.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".diffraction", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ DIFFRACTION_STATE_DIR: stateDir, ...params.env }, () =>
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadDiffractionPlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadDiffractionPlugins>;
  loadSecond: () => ReturnType<typeof loadDiffractionPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadDiffractionPlugins>;
  loadVariant: () => ReturnType<typeof loadDiffractionPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const diffractionHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(diffractionHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  ...
  const env = {
    ...process.env,
    DIFFRACTION_HOME: diffractionHome,
    HOME: ignoredHome,
    DIFFRACTION_STATE_DIR: stateDir,
    DIFFRACTION_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.DIFFRACTION_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".diffraction", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ DIFFRACTION_STATE_DIR: stateDir }, () =>
      loadDiffractionPlugins({
        workspaceDir,
...
describe("loadDiffractionPlugins", () => {
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadDiffractionPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
      ...
      },
      {
        ...
        run: () => {
          ...
          const full = loadDiffractionPlugins(options);
          const scoped = loadDiffractionPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
          const scopedAgain = loadDiffractionPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadDiffractionPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadDiffractionPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadDiffractionPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadDiffractionPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadDiffractionPlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadDiffractionPlugins({
              ...
              env: {
                ...process.env,
                DIFFRACTION_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadDiffractionPlugins({
              ...
              env: {
                ...process.env,
                DIFFRACTION_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadDiffractionPlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                DIFFRACTION_HOME: undefined,
                DIFFRACTION_STATE_DIR: stateDir,
                DIFFRACTION_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadDiffractionPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(diffractionHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadDiffractionPlugins(options),
          loadVariant: () =>
            loadDiffractionPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadDiffractionPlugins({
      env: {
        ...
        HOME: homeDir,
        DIFFRACTION_HOME: undefined,
        DIFFRACTION_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers DIFFRACTION_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const diffractionHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "diffraction-home-demo",
      dir: path.join(diffractionHome, "plugins", "diffraction-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "diffraction-home-demo", register() {} };`,
    });
    ...
    const registry = loadDiffractionPlugins({
      ...
      config: {
        plugins: {
          allow: ["diffraction-home-demo"],
          entries: {
            "diffraction-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/diffraction-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "diffraction-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadDiffractionPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadDiffractionPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "diffraction.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadDiffractionPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@diffraction/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@diffraction/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@diffraction/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@diffraction/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "diffraction.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "diffraction.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const stateDir = makeTempDir();
          return withEnv({ DIFFRACTION_STATE_DIR: stateDir }, () => {
  ...
  it("warns about open allowlists for discoverable plugins once per plugin set", () => {
    ...
    for (const scenario of scenarios) {
      ...
      for (let index = 0; index < scenario.loads; index += 1) {
        loadDiffractionPlugins(options);
      }
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const workspaceDir = makeTempDir();
          const workspaceExtDir = path.join(workspaceDir, ".diffraction", "extensions", "shadowed");
          mkdirSafe(workspaceExtDir);
          writePlugin({
            id: "shadowed",
            body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("diffraction/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ DIFFRACTION_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadDiffractionPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__diffractionLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("diffraction/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("diffraction/plugin-sdk");
