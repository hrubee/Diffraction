...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-diffraction",
      targetDir: "/tmp/opik-diffraction",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-diffraction": {
              source: "npm",
              spec: "@opik/opik-diffraction",
              integrity: "sha512-old",
              installPath: "/tmp/opik-diffraction",
            },
          },
        },
      },
      pluginIds: ["opik-diffraction"],
      dryRun: true,
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-diffraction": {
              source: "npm",
              spec: "@opik/opik-diffraction@0.2.5",
              integrity: "sha512-old",
              installPath: "/tmp/opik-diffraction",
            },
          },
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @diffraction/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @diffraction/missing.",
      },
    ]);
  });
  ...
  it("reuses a recorded npm dist-tag spec for id-based updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "diffraction-codex-app-server",
      targetDir: "/tmp/diffraction-codex-app-server",
    ...
    const result = await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "diffraction-codex-app-server": {
              source: "npm",
              spec: "diffraction-codex-app-server@beta",
              installPath: "/tmp/diffraction-codex-app-server",
              resolvedName: "diffraction-codex-app-server",
              resolvedSpec: "diffraction-codex-app-server@0.2.0-beta.3",
            },
          },
        },
      },
      pluginIds: ["diffraction-codex-app-server"],
    });

    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "diffraction-codex-app-server@beta",
        expectedPluginId: "diffraction-codex-app-server",
      }),
    );
    expect(result.config.plugins?.installs?.["diffraction-codex-app-server"]).toMatchObject({
      source: "npm",
      spec: "diffraction-codex-app-server@beta",
      installPath: "/tmp/diffraction-codex-app-server",
      version: "0.2.0-beta.4",
  ...
  it("uses and persists an explicit npm spec override during updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      extensions: ["index.ts"],
      npmResolution: {
        name: "diffraction-codex-app-server",
        version: "0.2.0-beta.4",
        resolvedSpec: "diffraction-codex-app-server@0.2.0-beta.4",
      },
    });
    ...
    const result = await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "diffraction-codex-app-server": {
              source: "npm",
              spec: "diffraction-codex-app-server",
              installPath: "/tmp/diffraction-codex-app-server",
            },
          },
        },
      ...
      pluginIds: ["diffraction-codex-app-server"],
      specOverrides: {
        "diffraction-codex-app-server": "diffraction-codex-app-server@beta",
      },
    });
  ...
  it("skips recorded integrity checks when an explicit npm version override changes the spec", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "diffraction-codex-app-server": {
              source: "npm",
              spec: "diffraction-codex-app-server@0.2.0-beta.3",
              integrity: "sha512-old",
              installPath: "/tmp/diffraction-codex-app-server",
            },
          },
      ...
      pluginIds: ["diffraction-codex-app-server"],
      specOverrides: {
        "diffraction-codex-app-server": "diffraction-codex-app-server@0.2.0-beta.4",
      },
    });

    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "diffraction-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      }),
    );
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@diffraction/voice-call",
      targetDir: "/tmp/diffraction-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@diffraction/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@diffraction/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@diffraction/voice-call");
    expect(result.config.plugins?.entries?.["@diffraction/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@diffraction/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@diffraction/voice-call",
      installPath: "/tmp/diffraction-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/diffraction-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
