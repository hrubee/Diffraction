...
const { loadDiffractionPluginsMock } = vi.hoisted(() => ({
  loadDiffractionPluginsMock: vi.fn((params?: { config?: { plugins?: Record<string, unknown> } }) => {
...
vi.mock("./loader.js", () => ({
  loadDiffractionPlugins: loadDiffractionPluginsMock,
}));

describe("resolvePluginWebSearchProviders", () => {
  beforeEach(() => {
    loadDiffractionPluginsMock.mockClear();
  ...
  it("loads bundled providers through the plugin loader in auto-detect order", () => {
    ...
    expect(loadDiffractionPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("memoizes snapshot provider resolution for the same config and env", () => {
    ...
    const env = { DIFFRACTION_HOME: "/tmp/diffraction-home" } as NodeJS.ProcessEnv;
  ...
  it("invalidates the snapshot cache when config or env contents change in place", () => {
    ...
    const env = {
      DIFFRACTION_HOME: "/tmp/diffraction-home-a",
    ...
    env.DIFFRACTION_HOME = "/tmp/diffraction-home-b";
    resolvePluginWebSearchProviders({
      config,
      env,
      bundledAllowlistCompat: true,
    ...
    expect(loadDiffractionPluginsMock).toHaveBeenCalledTimes(2);
  });
  ...
  it("skips web-search snapshot memoization when plugin cache opt-outs are set", () => {
    ...
    const env = {
      DIFFRACTION_HOME: "/tmp/diffraction-home",
      DIFFRACTION_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
    } as NodeJS.ProcessEnv;
  ...
  it("skips web-search snapshot memoization when discovery cache ttl is zero", () => {
    ...
    const env = {
      DIFFRACTION_HOME: "/tmp/diffraction-home",
      DIFFRACTION_PLUGIN_DISCOVERY_CACHE_MS: "0",
    } as NodeJS.ProcessEnv;
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const env = {
      DIFFRACTION_HOME: "/tmp/diffraction-home",
      DIFFRACTION_PLUGIN_DISCOVERY_CACHE_MS: "5",
      DIFFRACTION_PLUGIN_MANIFEST_CACHE_MS: "20",
    } as NodeJS.ProcessEnv;
  ...
  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    ...
    const env = {
      DIFFRACTION_HOME: "/tmp/diffraction-home",
      DIFFRACTION_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    } as NodeJS.ProcessEnv;
    ...
    env.DIFFRACTION_PLUGIN_DISCOVERY_CACHE_MS = "5";
  ...
  it("prefers the active plugin registry for runtime resolution", () => {
    ...
    expect(loadDiffractionPluginsMock).not.toHaveBeenCalled();
  });
});
