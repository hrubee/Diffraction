...
> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-marketplace-"));
...
> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-marketplace-download-"));
