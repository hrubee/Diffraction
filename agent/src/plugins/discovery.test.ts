...
import { clearPluginDiscoveryCache, discoverDiffractionPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("diffraction-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverDiffractionPlugins>[0],
) {
  return discoverDiffractionPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      diffraction: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverDiffractionPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".diffraction", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".diffraction", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverDiffractionPlugins({
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@diffraction/voice-call",
  ...
  it("strips provider suffixes from package-derived ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@diffraction/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@diffraction/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@diffraction/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@diffraction/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("diffraction");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@diffraction/escape-pack",
  ...
  it("skips missing package extension entries without escape diagnostics", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@diffraction/missing-entry-pack",
  ...
  it("ignores package manifests that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      outsideManifest,
      JSON.stringify({
        name: "@diffraction/pack",
        diffraction: { extensions: ["./entry.ts"] },
      }),
      "utf-8",
    );
    try {
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverDiffractionPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        DIFFRACTION_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverDiffractionPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        DIFFRACTION_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverDiffractionPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        DIFFRACTION_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
