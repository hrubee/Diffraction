...
import type {
  ...
  DiffractionPluginChannelRegistration,
  DiffractionPluginCliRegistrar,
  DiffractionPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  DiffractionPluginHttpRouteAuth,
  DiffractionPluginHttpRouteMatch,
  DiffractionPluginHttpRouteHandler,
  DiffractionPluginHttpRouteParams,
  DiffractionPluginHookOptions,
  MediaUnderstandingProviderPlugin,
  ProviderPlugin,
  DiffractionPluginService,
  DiffractionPluginToolContext,
  DiffractionPluginToolFactory,
  PluginConfigUiHint,
  PluginDiagnostic,
  PluginBundleFormat,
  PluginFormat,
...
export type PluginToolRegistration = {
  pluginId: string;
  pluginName?: string;
  factory: DiffractionPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
  rootDir?: string;
...
export type PluginCliRegistration = {
  pluginId: string;
  pluginName?: string;
  register: DiffractionPluginCliRegistrar;
  commands: string[];
  source: string;
  rootDir?: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  pluginName?: string;
  service: DiffractionPluginService;
  source: string;
  rootDir?: string;
};
...
export type PluginCommandRegistration = {
  pluginId: string;
  pluginName?: string;
  command: DiffractionPluginCommandDefinition;
  source: string;
  rootDir?: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: DiffractionPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: DiffractionPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: DiffractionPluginHookOptions | undefined,
    config: DiffractionPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: DiffractionPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: DiffractionPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as DiffractionPluginChannelRegistration).plugin === "object"
        ? (registration as DiffractionPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: DiffractionPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: DiffractionPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: DiffractionPluginCommandDefinition) => {
