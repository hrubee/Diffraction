...
function createPluginSourceRoots() {
  ...
  const globalRoot = path.resolve(path.sep, "Users", "x", ".diffraction", "extensions");
  const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".diffraction", "extensions");
  return {
    stock: stockRoot,
    global: globalRoot,
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "diffraction-home");
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".diffraction", "extensions"),
    });
  });
});
