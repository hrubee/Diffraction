...
import { discoverDiffractionPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: DiffractionPluginDefinition;
  register?: DiffractionPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as DiffractionPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as DiffractionPluginDefinition;
...
function createPluginRecord(params: {
  ...
  return {
    ...
    format: params.format ?? "diffraction",
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.DIFFRACTION_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverDiffractionPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverDiffractionPlugins>["candidates"][number];
  right: ReturnType<typeof discoverDiffractionPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadDiffractionPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadDiffractionPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  const discovery = discoverDiffractionPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into Diffraction yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: DiffractionPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as DiffractionPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
