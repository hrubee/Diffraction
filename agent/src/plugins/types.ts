 ...
 */
export type DiffractionPluginConfigSchema = {
...
export type DiffractionPluginToolContext = {
...
export type DiffractionPluginToolFactory = (
  ctx: DiffractionPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type DiffractionPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type DiffractionPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<DiffractionConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Diffraction uses this to expose
  ...
  ) => Promise<DiffractionConfig | null>;
};
 ...
 *
 * Runs after Diffraction resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Diffraction auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Diffraction wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Diffraction builds its generic
...
/**
 * Provider-owned stream wrapper hook after Diffraction applies its generic
 ...
 *
 * Return `true` or `false` to override Diffraction's built-in provider cache TTL
 ...
 *
 * Runs only after Diffraction exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after Diffraction loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * Diffraction calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic Diffraction wrappers.
   *
   ...
   *
   * Called after Diffraction resolves the raw configured credential but before the
   ...
   * hint than Diffraction's generic auth-store guidance.
   */
   ...
   * `errorMessage` when Diffraction should surface a provider-specific hint for
   ...
   * merging. Diffraction deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Diffraction uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `diffraction doctor`.
   *
   ...
   *
   * Diffraction calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Diffraction's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
  ...
  getConfiguredCredentialValue?: (config?: DiffractionConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: DiffractionConfig, value: unknown) => void;
  applySelectionConfig?: (config: DiffractionConfig) => DiffractionConfig;
...
export type DiffractionPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current Diffraction configuration */
  config: DiffractionConfig;
 ...
 */
export type DiffractionPluginCommandDefinition = {
...
export type DiffractionPluginHttpRouteAuth = "gateway" | "plugin";
export type DiffractionPluginHttpRouteMatch = "exact" | "prefix";

export type DiffractionPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type DiffractionPluginHttpRouteParams = {
  path: string;
  handler: DiffractionPluginHttpRouteHandler;
  auth: DiffractionPluginHttpRouteAuth;
  match?: DiffractionPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type DiffractionPluginCliContext = {
  program: Command;
  config: DiffractionConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type DiffractionPluginCliRegistrar = (ctx: DiffractionPluginCliContext) => void | Promise<void>;

/** Context passed to long-lived plugin services. */
export type DiffractionPluginServiceContext = {
  config: DiffractionConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type DiffractionPluginService = {
  id: string;
  start: (ctx: DiffractionPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: DiffractionPluginServiceContext) => void | Promise<void>;
};

export type DiffractionPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type DiffractionPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  kind?: PluginKind;
  configSchema?: DiffractionPluginConfigSchema;
  register?: (api: DiffractionPluginApi) => void | Promise<void>;
  activate?: (api: DiffractionPluginApi) => void | Promise<void>;
};

export type DiffractionPluginModule =
  | DiffractionPluginDefinition
  | ((api: DiffractionPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

/** Main registration API injected into native plugin entry files. */
export type DiffractionPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | DiffractionPluginToolFactory,
    opts?: DiffractionPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: DiffractionPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: DiffractionPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: DiffractionPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: DiffractionPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: DiffractionPluginService) => void;
   ...
   */
  registerCommand: (command: DiffractionPluginCommandDefinition) => void;
...
export type PluginFormat = "diffraction" | "bundle";
