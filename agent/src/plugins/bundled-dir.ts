...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.DIFFRACTION_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
  ...
  try {
    const packageRoots = [
      resolveDiffractionPackageRootSync({ cwd: process.cwd() }),
      resolveDiffractionPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
