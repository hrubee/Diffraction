...
import { resolveDiffractionPackageRootSync } from "../infra/diffraction-root.js";
...
function hasTrustedDiffractionRootIndicator(params: {
  packageRoot: string;
  ...
  const hasDiffractionBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("diffraction")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.diffraction === "string");
  const hasDiffractionEntrypoint = fs.existsSync(path.join(params.packageRoot, "diffraction.mjs"));
  return hasCliEntryExport || hasDiffractionBin || hasDiffractionEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedDiffractionRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveDiffractionPackageRootSync({ cwd });
  const fromExplicitHints =
    params.argv1 || params.moduleUrl
      ? resolveDiffractionPackageRootSync({
          cwd,
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({ modulePath })) {
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`diffraction/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(modulePath: string): Record<string, string> {
  ...
  return {
    ...(extensionApiAlias ? { "diffraction/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "diffraction/plugin-sdk": pluginSdkAlias } : {}),
