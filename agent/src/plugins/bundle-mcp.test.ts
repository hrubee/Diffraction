...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled Claude bundle MCP config and absolutizes relative args", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "DIFFRACTION_HOME", "DIFFRACTION_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("diffraction-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("diffraction-bundle-mcp-workspace-");
  ...
  it("merges inline bundle MCP servers and skips disabled bundles", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "DIFFRACTION_HOME", "DIFFRACTION_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("diffraction-bundle-inline-home-");
      const workspaceDir = await tempHarness.createTempDir("diffraction-bundle-inline-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.DIFFRACTION_HOME;
      delete process.env.DIFFRACTION_STATE_DIR;

      const enabledRoot = path.join(homeDir, ".diffraction", "extensions", "inline-enabled");
      const disabledRoot = path.join(homeDir, ".diffraction", "extensions", "inline-disabled");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "DIFFRACTION_HOME", "DIFFRACTION_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("diffraction-bundle-inline-placeholder-home-");
      const workspaceDir = await tempHarness.createTempDir(
        "diffraction-bundle-inline-placeholder-workspace-",
      );
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.DIFFRACTION_HOME;
      ...
      const pluginRoot = path.join(homeDir, ".diffraction", "extensions", "inline-claude");
