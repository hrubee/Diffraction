...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `diffraction: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["diffraction: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:diffraction/diffraction#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g diffraction@latest`.");
  return lines.join("\n");
};
