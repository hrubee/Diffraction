...
export function loadVitestExperimentalConfig(env: EnvMap = process.env): {
  ...
  if (!isDisabled(env.DIFFRACTION_VITEST_FS_MODULE_CACHE)) {
    experimental.fsModuleCache = true;
  }
  if (isEnabled(env.DIFFRACTION_VITEST_IMPORT_DURATIONS)) {
    experimental.importDurations = { print: true };
  }
  if (isEnabled(env.DIFFRACTION_VITEST_PRINT_IMPORT_BREAKDOWN)) {
