...
export function loadIncludePatternsFromEnv(
  ...
  const includeFile = env.DIFFRACTION_VITEST_INCLUDE_FILE?.trim();
  if (!includeFile) {
    return null;
  }
  return loadPatternListFile(includeFile, "DIFFRACTION_VITEST_INCLUDE_FILE");
}

export function loadExtraExcludePatternsFromEnv(
  env: Record<string, string | undefined> = process.env,
): string[] {
  const extraExcludeFile = env.DIFFRACTION_VITEST_EXTRA_EXCLUDE_FILE?.trim();
  if (!extraExcludeFile) {
    return [];
  }
  return loadPatternListFile(extraExcludeFile, "DIFFRACTION_VITEST_EXTRA_EXCLUDE_FILE");
}
