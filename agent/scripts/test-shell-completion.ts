 ...
 * `diffraction update`. Use it to verify the completion installation flow
...
\`diffraction update\`, \`diffraction doctor\`, and \`diffraction onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Diffraction Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`diffraction completion --install\` later to enable.`));
    return;
  }
