...
# Build and bundle Diffraction into a minimal .app we can open.
# Outputs to dist/Diffraction.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Diffraction.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Diffraction"
BUNDLE_ID="${BUNDLE_ID:-ai.diffraction.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/diffraction/diffraction/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Diffraction/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :DiffractionBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :DiffractionGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Diffraction"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Diffraction"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Diffraction"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Diffraction" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Diffraction/Resources/Diffraction.icns" "$APP_ROOT/Contents/Resources/Diffraction.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Diffraction/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying DiffractionKit resources"
DIFFRACTIONKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/DiffractionKit_DiffractionKit.bundle"
if [ -d "$DIFFRACTIONKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/DiffractionKit_DiffractionKit.bundle"
  cp -R "$DIFFRACTIONKIT_BUNDLE" "$APP_ROOT/Contents/Resources/DiffractionKit_DiffractionKit.bundle"
else
  echo "WARN: DiffractionKit resource bundle not found at $DIFFRACTIONKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Diffraction"
killall -q Diffraction 2>/dev/null || true
