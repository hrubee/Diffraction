...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.diffraction?.bundle?.stageRuntimeDependencies === true;
}
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.diffraction) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies.diffraction;
  ...
  if (packageJson.peerDependenciesMeta?.diffraction) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta.diffraction;
  ...
  if (packageJson.devDependencies?.diffraction) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies.diffraction;
