...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "diffraction.plugin.json");
    ...
    if (packageJson.diffraction && "extensions" in packageJson.diffraction) {
      packageJson.diffraction = {
        ...packageJson.diffraction,
        extensions: rewritePackageExtensions(packageJson.diffraction.extensions),
        ...(typeof packageJson.diffraction.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.diffraction.setupEntry) }
          : {}),
      };
    }
