...
INSTALL_URL="https://diffraction.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_DIFFRACTION_BIN="/opt/homebrew/bin/diffraction"
GUEST_DIFFRACTION_ENTRY="/opt/homebrew/lib/node_modules/diffraction/diffraction.mjs"
...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/diffraction-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/diffraction-parallels-build.lock"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://diffraction.ai/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: diffraction@2026.3.13-beta.1
...
spawn {*}$cmd
send -- "printf '__DIFFRACTION_READY__\\n'\r"
expect "__DIFFRACTION_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/diffraction-prl.sh <<'__DIFFRACTION_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__DIFFRACTION_SCRIPT__\r"
send -- "/bin/bash /tmp/diffraction-prl.sh; rc=\$?; rm -f /tmp/diffraction-prl.sh; printf '__DIFFRACTION_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__DIFFRACTION_RC__:(-?[0-9]+)} {
...
resolve_latest_version() {
  ...
  npm view diffraction version --userconfig "$(mktemp)"
}
...
export DIFFRACTION_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/diffraction-install.sh
bash /tmp/diffraction-install.sh${version_arg_q}
$GUEST_DIFFRACTION_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_DIFFRACTION_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/diffraction-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/diffraction-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/diffraction"; check_path "\$root/diffraction/extensions"; if [ -d "\$root/diffraction/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/diffraction/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
run_ref_onboard() {
  guest_current_user_exec \
    ...
    "$GUEST_NODE_BIN" "$GUEST_DIFFRACTION_ENTRY" onboard \
    --non-interactive \
    --mode local \
...
verify_gateway() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_DIFFRACTION_ENTRY" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_DIFFRACTION_ENTRY" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_DIFFRACTION_ENTRY" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_DIFFRACTION_ENTRY" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_DIFFRACTION_ENTRY" agent --agent main --message ping --json
}
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/diffraction-discord-token <<'__DIFFRACTION_TOKEN__'
$DISCORD_TOKEN_VALUE
__DIFFRACTION_TOKEN__
cat >/tmp/diffraction-discord-guilds.json <<'__DIFFRACTION_GUILDS__'
$guilds_json
__DIFFRACTION_GUILDS__
token="\$(tr -d '\n' </tmp/diffraction-discord-token)"
guilds_json="\$(cat /tmp/diffraction-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_DIFFRACTION_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_DIFFRACTION_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_DIFFRACTION_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_DIFFRACTION_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_DIFFRACTION_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_DIFFRACTION_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_DIFFRACTION_ENTRY channels status --probe --json
rm -f /tmp/diffraction-discord-token /tmp/diffraction-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
matches = re.findall(r"Diffraction [^\r\n]+ \([0-9a-f]{7,}\)", text)
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "diffraction-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "diffraction-main-upgrade.tgz"
