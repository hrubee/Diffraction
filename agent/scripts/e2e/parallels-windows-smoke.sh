...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-diffraction-native-e2e-2026-03-12"
MODE="both"
OPENAI_API_KEY_ENV="OPENAI_API_KEY"
INSTALL_URL="https://diffraction.ai/install.ps1"
HOST_PORT="18426"
...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/diffraction-parallels-windows.XXXXXX)"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-diffraction-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --openai-api-key-env <var> Host env var name for OpenAI API key.
                             Default: OPENAI_API_KEY
  --install-url <url>        Installer URL for latest release. Default: https://diffraction.ai/install.ps1
...
guest_run_diffraction() {
  ...
  stdout_name="diffraction-stdout-$RANDOM-$RANDOM.log"
  stderr_name="diffraction-stderr-$RANDOM-$RANDOM.log"
...
try {
  ...
  \$proc = Start-Process -FilePath (Join-Path \$env:APPDATA 'npm\diffraction.cmd') -ArgumentList $args_literal -NoNewWindow -PassThru -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr
  \$proc.WaitForExit()
  if (Test-Path \$stdout) {
    Get-Content \$stdout
  }
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "diffraction-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\Diffraction\\deps\" mkdir \"%LOCALAPPDATA%\\Diffraction\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/diffraction-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\diffraction.cmd') --version
EOF
)"
}
...
install_main_tgz() {
  ...
  guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Diffraction\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\diffraction.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_diffraction "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="diffraction-onboard-$RANDOM-$RANDOM.ps1"
  log_name="diffraction-onboard-$RANDOM-$RANDOM.log"
  done_name="diffraction-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$diffraction = Join-Path \$env:APPDATA 'npm\diffraction.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$diffraction, \$log)
...
verify_gateway() {
  guest_run_diffraction "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_diffraction "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_diffraction "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_diffraction "" "" gateway status --deep
}

verify_turn() {
  guest_run_diffraction "" "" agent --agent main --message "Reply with exact ASCII text OK only." --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "diffraction-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "diffraction-main-upgrade.tgz" || return $?
