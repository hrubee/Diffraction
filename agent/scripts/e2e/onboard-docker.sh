...
IMAGE_NAME="diffraction-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/diffraction-bin:$PATH"
  mkdir -p /tmp/diffraction-bin
  cat > /tmp/diffraction-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/diffraction-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$DIFFRACTION_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_diffraction_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/diffraction-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/diffraction-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$DIFFRACTION_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/diffraction-e2e-$1.XXXXXX"
  }

  set_isolated_diffraction_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export DIFFRACTION_HOME="$home_dir"
    export DIFFRACTION_STATE_DIR="$home_dir/.diffraction"
    export DIFFRACTION_CONFIG_PATH="$DIFFRACTION_STATE_DIR/diffraction.json"
    mkdir -p "$DIFFRACTION_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    set_isolated_diffraction_env "$home_dir"
    node "$DIFFRACTION_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$DIFFRACTION_STATE_DIR/workspace"
    config_path="$DIFFRACTION_CONFIG_PATH"
    sessions_dir="$DIFFRACTION_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_diffraction_env "$home_dir"
	    ...
	    node "$DIFFRACTION_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$DIFFRACTION_CONFIG_PATH" <<'"'"'JSON'"'"'
{
  "meta": {},
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$DIFFRACTION_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$DIFFRACTION_ENTRY\" configure --section skills" send_skills_flow
