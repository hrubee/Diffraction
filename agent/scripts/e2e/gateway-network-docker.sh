...
IMAGE_NAME="diffraction-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="diffraction-net-e2e-$$"
GW_NAME="diffraction-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "DIFFRACTION_GATEWAY_TOKEN=$TOKEN" \
  -e "DIFFRACTION_SKIP_CHANNELS=1" \
  -e "DIFFRACTION_SKIP_GMAIL_WATCHER=1" \
  -e "DIFFRACTION_SKIP_CRON=1" \
  -e "DIFFRACTION_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
