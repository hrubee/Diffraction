...
IMAGE_NAME="diffraction-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  DIFFRACTION_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  DIFFRACTION_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export DIFFRACTION_ENTRY

home_dir=$(mktemp -d "/tmp/diffraction-plugins-e2e.XXXXXX")
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$DIFFRACTION_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$DIFFRACTION_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$DIFFRACTION_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/diffraction.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/diffraction.plugin.json" "$id"
const fs = require("node:fs");
...
mkdir -p "$HOME/.diffraction/extensions/demo-plugin"

cat > "$HOME/.diffraction/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.diffraction/extensions/demo-plugin/diffraction.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$DIFFRACTION_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/diffraction-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@diffraction/demo-plugin-tgz",
  "version": "0.0.1",
  "diffraction": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/diffraction.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$DIFFRACTION_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$DIFFRACTION_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/diffraction-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@diffraction/demo-plugin-dir",
  "version": "0.0.1",
  "diffraction": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/diffraction.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$DIFFRACTION_ENTRY" plugins install "$dir_plugin"
node "$DIFFRACTION_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/diffraction-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@diffraction/demo-plugin-file",
  "version": "0.0.1",
  "diffraction": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/diffraction.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$DIFFRACTION_ENTRY" plugins install "file:$file_pack_dir/package"
node "$DIFFRACTION_ENTRY" plugins list --json > /tmp/plugins4.json
...
bundle_root="$HOME/.diffraction/extensions/claude-bundle-e2e"
...
const configPath = path.join(process.env.HOME, ".diffraction", "diffraction.json");
...
gateway_log="/tmp/diffraction-plugin-command-e2e.log"
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "diffraction/fixture-marketplace"
    }
  }
}
JSON

node "$DIFFRACTION_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$DIFFRACTION_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$DIFFRACTION_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$DIFFRACTION_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$DIFFRACTION_ENTRY" plugins update marketplace-shortcut --dry-run
node "$DIFFRACTION_ENTRY" plugins update marketplace-shortcut
node "$DIFFRACTION_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
