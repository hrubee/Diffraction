...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("diffraction/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("diffraction/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced diffraction/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced diffraction/plugin-sdk/<subpath> must exist in the public package exports.",
  );
