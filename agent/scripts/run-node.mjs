...
const extensionRestartMetadataFiles = new Set(["diffraction.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.DIFFRACTION_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.DIFFRACTION_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[diffraction] ${message}\n`);
};

const runDiffraction = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["diffraction.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    ...
    return await runDiffraction(deps);
  }
