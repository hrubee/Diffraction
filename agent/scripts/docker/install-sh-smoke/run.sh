...
SMOKE_PREVIOUS_VERSION="${DIFFRACTION_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${DIFFRACTION_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="diffraction"
PACKAGE_NAME="${DIFFRACTION_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (DIFFRACTION_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${DIFFRACTION_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${DIFFRACTION_INSTALL_LATEST_OUT:-}"
fi
