...
elif [[ -n "${DIFFRACTION_LIVE_PROVIDERS:-}" && -n "${DIFFRACTION_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      diffraction_live_collect_auth_dirs_from_csv "${DIFFRACTION_LIVE_PROVIDERS:-}"
      diffraction_live_collect_auth_dirs_from_csv "${DIFFRACTION_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
docker run --rm -t \
  ...
  -e DIFFRACTION_LIVE_MODELS="${DIFFRACTION_LIVE_MODELS:-modern}" \
  -e DIFFRACTION_LIVE_PROVIDERS="${DIFFRACTION_LIVE_PROVIDERS:-}" \
  -e DIFFRACTION_LIVE_MAX_MODELS="${DIFFRACTION_LIVE_MAX_MODELS:-48}" \
  -e DIFFRACTION_LIVE_MODEL_TIMEOUT_MS="${DIFFRACTION_LIVE_MODEL_TIMEOUT_MS:-}" \
  -e DIFFRACTION_LIVE_REQUIRE_PROFILE_KEYS="${DIFFRACTION_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e DIFFRACTION_LIVE_GATEWAY_MODELS="${DIFFRACTION_LIVE_GATEWAY_MODELS:-}" \
  -e DIFFRACTION_LIVE_GATEWAY_PROVIDERS="${DIFFRACTION_LIVE_GATEWAY_PROVIDERS:-}" \
  -e DIFFRACTION_LIVE_GATEWAY_MAX_MODELS="${DIFFRACTION_LIVE_GATEWAY_MAX_MODELS:-}" \
  -v "$ROOT_DIR":/src:ro \
