...
# Diffraction Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://diffraction.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Diffraction."
...
GUM_VERSION="${DIFFRACTION_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Diffraction Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://diffraction.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$DIFFRACTION_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.diffraction.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_diffraction_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.diffraction-* "$npm_root"/diffraction 2>/dev/null || true
}

extract_diffraction_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_diffraction_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/diffraction" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/diffraction"|"/usr/local/bin/diffraction")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/diffraction/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale diffraction symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing diffraction binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Diffraction package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_diffraction_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Diffraction npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*diffraction" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_diffraction_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_diffraction_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Diffraction npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an diffraction binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${DIFFRACTION_TAGLINE_INDEX:-}" ]]; then
        if [[ "${DIFFRACTION_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((DIFFRACTION_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${DIFFRACTION_NO_ONBOARD:-0}
NO_PROMPT=${DIFFRACTION_NO_PROMPT:-0}
DRY_RUN=${DIFFRACTION_DRY_RUN:-0}
INSTALL_METHOD=${DIFFRACTION_INSTALL_METHOD:-}
DIFFRACTION_VERSION=${DIFFRACTION_VERSION:-latest}
USE_BETA=${DIFFRACTION_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/diffraction"
GIT_DIR=${DIFFRACTION_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${DIFFRACTION_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${DIFFRACTION_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${DIFFRACTION_VERBOSE:-0}"
VERIFY_INSTALL="${DIFFRACTION_VERIFY_INSTALL:-0}"
DIFFRACTION_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Diffraction installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://diffraction.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/diffraction)
...
Environment variables:
  DIFFRACTION_INSTALL_METHOD=git|npm
  DIFFRACTION_VERSION=latest|next|main|<semver>|<spec>
  DIFFRACTION_BETA=0|1
  DIFFRACTION_GIT_DIR=...
  DIFFRACTION_GIT_UPDATE=0|1
  DIFFRACTION_NO_PROMPT=1
  DIFFRACTION_VERIFY_INSTALL=1
  DIFFRACTION_DRY_RUN=1
  DIFFRACTION_NO_ONBOARD=1
  DIFFRACTION_VERBOSE=1
  DIFFRACTION_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://diffraction.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://diffraction.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://diffraction.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://diffraction.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://diffraction.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Diffraction checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Diffraction source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_diffraction_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"diffraction"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://diffraction.ai/install.sh | bash"
}
...
ensure_diffraction_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/diffraction" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/diffraction" ]]; then
        ln -sf "$npm_root/diffraction/dist/entry.js" "${npm_bin}/diffraction"
        ui_info "Created diffraction bin link at ${npm_bin}/diffraction"
    fi
    return 0
}

# Check for existing Diffraction installation
check_existing_diffraction() {
    if [[ -n "$(type -P diffraction 2>/dev/null || true)" ]]; then
        ui_info "Existing Diffraction installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make diffraction show as \"command not found\" in new terminals."
...
warn_diffraction_not_found() {
    ui_warn "Installed, but diffraction is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t diffraction 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named diffraction; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_diffraction_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P diffraction 2>/dev/null || true)"
...
install_diffraction_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/diffraction/diffraction.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Diffraction from git checkout: ${repo_dir}"
    else
        ui_info "Installing Diffraction from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Diffraction" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Diffraction" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/diffraction" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/diffraction"
    ui_success "Diffraction wrapper installed to \$HOME/.local/bin/diffraction"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view diffraction dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:diffraction/diffraction#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_diffraction() {
    local package_name="diffraction"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            DIFFRACTION_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="diffraction"
        else
            DIFFRACTION_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${DIFFRACTION_VERSION}" ]]; then
        DIFFRACTION_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${DIFFRACTION_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${DIFFRACTION_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Diffraction v${resolved_version}"
    else
        ui_info "Installing Diffraction (${DIFFRACTION_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${DIFFRACTION_VERSION}")"

    if ! install_diffraction_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_diffraction_paths
        install_diffraction_npm "${install_spec}"
    fi

    if [[ "${DIFFRACTION_VERSION}" == "latest" && "${package_name}" == "diffraction" ]]; then
        if ! resolve_diffraction_bin &> /dev/null; then
            ui_warn "npm install diffraction@latest failed; retrying diffraction@next"
            cleanup_npm_diffraction_paths
            install_diffraction_npm "diffraction@next"
        fi
    fi

    ensure_diffraction_bin_link || true

    ui_success "Diffraction installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${DIFFRACTION_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_diffraction_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (diffraction not on PATH yet)"
        warn_diffraction_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${DIFFRACTION_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.diffraction/workspace-${profile}"
    else
        echo "${HOME}/.diffraction/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${DIFFRACTION_CONFIG_PATH:-$HOME/.diffraction/diffraction.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run diffraction onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but diffraction not on PATH; skipping onboarding"
        warn_diffraction_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run diffraction onboard to retry"
        return
    }
}
...
resolve_diffraction_version() {
    local version=""
    local raw_version_output=""
    local claw="${DIFFRACTION_BIN:-}"
    if [[ -z "$claw" ]] && command -v diffraction &> /dev/null; then
        claw="$(command -v diffraction)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_diffraction_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/diffraction/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/diffraction/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: diffraction not on PATH yet"
        warn_diffraction_not_found
        return 1
    fi

    run_quiet_step "Checking Diffraction version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: diffraction gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_diffraction_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Diffraction checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set DIFFRACTION_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_diffraction; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Diffraction"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g diffraction &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g diffraction 2>/dev/null || true
        ...
        install_diffraction_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/diffraction" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/diffraction"
    ...
    DIFFRACTION_BIN="$(resolve_diffraction_bin || true)"
    ...
    installed_version=$(resolve_diffraction_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Diffraction installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Diffraction installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/diffraction"
        ui_kv "Update command" "diffraction update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://diffraction.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running diffraction doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                DIFFRACTION_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                DIFFRACTION_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                DIFFRACTION_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run diffraction doctor and diffraction plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run diffraction onboard later"
        else
            local config_path="${DIFFRACTION_CONFIG_PATH:-$HOME/.diffraction/diffraction.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (diffraction not on PATH yet)"
                    warn_diffraction_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run diffraction onboard to finish setup"
            return 0
        fi
    fi

    if command -v diffraction &> /dev/null; then
        local claw="${DIFFRACTION_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_diffraction_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (diffraction daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if DIFFRACTION_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: diffraction daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${DIFFRACTION_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
