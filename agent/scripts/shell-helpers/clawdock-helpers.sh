#!/usr/bin/env bash
# ClawDock - Docker helpers for Diffraction
# Inspired by Simon Willison's "Running Diffraction in Docker"
# https://til.simonwillison.net/llms/diffraction-docker
#
# Installation:
...
# Common paths to check for Diffraction
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/diffraction"
  "${HOME}/workspace/diffraction"
  "${HOME}/projects/diffraction"
  "${HOME}/dev/diffraction"
  "${HOME}/code/diffraction"
  "${HOME}/src/diffraction"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Diffraction at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/diffraction"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Diffraction not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/diffraction/diffraction.git ~/diffraction"
    echo "  cd ~/diffraction && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^DIFFRACTION_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d diffraction-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart diffraction-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f diffraction-gateway
}
...
clawdock-workspace() {
  cd ~/.diffraction/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec diffraction-gateway \
    bash -c 'echo "alias diffraction=\"./diffraction.mjs\"" > /tmp/.bashrc_diffraction && bash --rcfile /tmp/.bashrc_diffraction'
}
...
clawdock-cli() {
  _clawdock_compose run --rm diffraction-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build diffraction-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "DIFFRACTION_GATEWAY_TOKEN=$token" diffraction-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" diffraction-gateway \
    bash -c './diffraction.mjs config set gateway.remote.token "$TOKEN" && ./diffraction.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec diffraction-gateway \
    bash -c "./diffraction.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart diffraction-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm diffraction-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec diffraction-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'diffraction config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Diffraction${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (diffraction alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to diffraction project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.diffraction)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'diffraction channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'diffraction status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.diffraction.ai${_CLR_RESET}"
  echo ""
}
