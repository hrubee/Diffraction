...
const warningFilterKey = Symbol.for("diffraction.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadDiffractionPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadDiffractionPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@diffraction/build-smoke-plugin",
      type: "module",
      diffraction: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'diffraction/plugin-sdk';",
...
const registry = loadDiffractionPlugins({
  ...
  env: {
    ...process.env,
    DIFFRACTION_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
