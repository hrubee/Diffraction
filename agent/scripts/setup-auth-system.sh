#!/bin/bash
# Setup Diffraction Auth Management System
...
echo "=== Diffraction Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'diffraction-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Diffraction message: Send warning via Diffraction itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/diffraction-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/diffraction-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/diffraction-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now diffraction-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status diffraction-auth-monitor.timer"
echo ""
echo "Quick commands:"
