...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "diffraction-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "diffraction.json");
  ...
  const sessionId = process.env.DIFFRACTION_ZAI_FALLBACK_SESSION_ID ?? randomUUID();
  ...
  const run1 = await runCommand(
    "run1",
    ["diffraction", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["diffraction", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
