...
type DriverMode = "token" | "webhook" | "diffraction";
...
function resolveStateDir(): string {
  const override = process.env.DIFFRACTION_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.DIFFRACTION_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".diffraction");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver diffraction] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Diffraction spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|diffraction> Driver transport mode (default: token)\n" +
    ...
    "  --diffraction-bin <path>        Diffraction CLI binary for driver=diffraction (default: diffraction)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  DIFFRACTION_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  DIFFRACTION_DISCORD_SMOKE_DRIVER\n" +
    "  DIFFRACTION_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  DIFFRACTION_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  DIFFRACTION_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  DIFFRACTION_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  DIFFRACTION_DISCORD_SMOKE_AGENT\n" +
    "  DIFFRACTION_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  DIFFRACTION_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  DIFFRACTION_DISCORD_SMOKE_POLL_MS\n" +
    "  DIFFRACTION_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  DIFFRACTION_DISCORD_SMOKE_DIFFRACTION_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.DIFFRACTION_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.DIFFRACTION_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "diffraction"
        ? "diffraction"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.DIFFRACTION_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.DIFFRACTION_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.DIFFRACTION_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.DIFFRACTION_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.DIFFRACTION_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.DIFFRACTION_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.DIFFRACTION_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.DIFFRACTION_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.DIFFRACTION_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.DIFFRACTION_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const diffractionBin =
    resolveArg("--diffraction-bin") || process.env.DIFFRACTION_DISCORD_SMOKE_DIFFRACTION_BIN || "diffraction";
...
async function diffractionCliJson<T>(params: { diffractionBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.diffractionBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`diffraction ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await diffractionCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    diffractionBin: params.diffractionBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "diffraction") {
    return await readMessagesWithOpenclaw({
      diffractionBin: params.args.diffractionBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `diffraction-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await diffractionCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        diffractionBin: args.diffractionBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("diffraction message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Diffraction.`,
        diagnostics: {
          bindingCandidates: [
            {
