...
const EXPECTED_REPOSITORY_URL = "https://github.com/diffraction/diffraction";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "diffraction") {
    errors.push(`package.json name must be "diffraction"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.diffraction !== "diffraction.mjs") {
    errors.push(
      `package.json bin.diffraction must be "diffraction.mjs"; found "${pkg.bin?.diffraction ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`diffraction-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `diffraction-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
