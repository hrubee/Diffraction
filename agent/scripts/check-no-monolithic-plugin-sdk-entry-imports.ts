...
import { discoverDiffractionPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']diffraction\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']diffraction\/plugin-sdk\/compat["']/;
...
function main() {
  ...
  if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic diffraction/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy diffraction/plugin-sdk/compat.",
      );
    ...
    if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
      console.error(
        "Use diffraction/plugin-sdk/<domain> or diffraction/plugin-sdk/<channel> subpaths for bundled plugins; root and compat are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
