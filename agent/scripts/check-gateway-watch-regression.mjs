...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir) {
  const shellSource = [
    'echo "$$" > "$DIFFRACTION_WATCH_PID_FILE"',
    "exec node scripts/watch-node.mjs gateway --force --allow-unconfigured",
  ].join("\n");
  const env = {
    DIFFRACTION_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    DIFFRACTION_HOME: isolatedHomeDir,
  };
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "diffraction-gateway-watch-"));
