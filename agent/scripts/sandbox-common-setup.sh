...
BASE_IMAGE="${BASE_IMAGE:-diffraction-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-diffraction-sandbox-common:bookworm-slim}"
...
DIFFRACTION_DOCKER_BUILD_USE_BUILDX="${DIFFRACTION_DOCKER_BUILD_USE_BUILDX:-0}"
DIFFRACTION_DOCKER_BUILD_CACHE_FROM="${DIFFRACTION_DOCKER_BUILD_CACHE_FROM:-}"
DIFFRACTION_DOCKER_BUILD_CACHE_TO="${DIFFRACTION_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${DIFFRACTION_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${DIFFRACTION_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${DIFFRACTION_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${DIFFRACTION_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${DIFFRACTION_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=diffraction.sandbox=1)
NOTE
