...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/diffraction/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Diffraction Re-Auth" -c "After getting key, run: ssh $SERVER '~/diffraction/scripts/mobile-reauth.sh'" --id diffraction-auth
        ;;
    *)
