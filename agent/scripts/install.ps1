# Diffraction Installer for Windows (PowerShell)
# Usage: iwr -useb https://diffraction.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://diffraction.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\diffraction",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 Diffraction Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Diffraction.$NC" -Level info
    Write-Host ""
}
...
function Install-DiffractionGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Diffraction from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/diffraction/diffraction.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\diffraction\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\diffraction.cmd" -Encoding ASCII -Force
...
function Resolve-PackageInstallSpec {
    ...
    if ([string]::IsNullOrWhiteSpace($trimmed)) {
        return "diffraction@latest"
    }
    if ($trimmed.ToLowerInvariant() -eq "main") {
        return "github:diffraction/diffraction#main"
    }
    if (Test-ExplicitPackageInstallSpec -Target $trimmed) {
        return $trimmed
    }
    return "diffraction@$trimmed"
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Diffraction from git to $GitDir" -Level info
        } else {
            Install-DiffractionGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Diffraction via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-DiffractionNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'diffraction onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 Diffraction installed successfully!" -Level success
}
