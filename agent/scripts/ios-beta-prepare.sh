...
- regenerates apps/ios/Diffraction.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${DIFFRACTION_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid DIFFRACTION_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid DIFFRACTION_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid DIFFRACTION_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid DIFFRACTION_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing DIFFRACTION_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(DIFFRACTION_URL_SLASH)$(DIFFRACTION_URL_SLASH)#g'
)"
...
DIFFRACTION_CODE_SIGN_STYLE = Automatic
DIFFRACTION_DEVELOPMENT_TEAM = ${TEAM_ID}
DIFFRACTION_IOS_SELECTED_TEAM = ${TEAM_ID}
DIFFRACTION_APP_BUNDLE_ID = ai.diffraction.client
DIFFRACTION_SHARE_BUNDLE_ID = ai.diffraction.client.share
DIFFRACTION_ACTIVITY_WIDGET_BUNDLE_ID = ai.diffraction.client.activitywidget
DIFFRACTION_WATCH_APP_BUNDLE_ID = ai.diffraction.client.watchkitapp
DIFFRACTION_WATCH_EXTENSION_BUNDLE_ID = ai.diffraction.client.watchkitapp.extension
DIFFRACTION_APP_PROFILE =
DIFFRACTION_SHARE_PROFILE =
DIFFRACTION_PUSH_TRANSPORT = relay
DIFFRACTION_PUSH_DISTRIBUTION = official
DIFFRACTION_URL_SLASH = /
DIFFRACTION_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
DIFFRACTION_PUSH_APNS_ENVIRONMENT = production
EOF
