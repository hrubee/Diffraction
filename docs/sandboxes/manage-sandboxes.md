...
Every sandbox requires a gateway. If you run `diffract sandbox create` without a gateway, the CLI auto-bootstraps a local gateway.
...
```console
$ diffract sandbox create --gpu -- claude
```
...
$ diffract sandbox create --from ./my-sandbox-dir
$ diffract sandbox create --from my-registry.example.com/my-image:latest
```

The CLI resolves community names against the [Diffract Community](https://github.com/NVIDIA/Diffract-Community) catalog, pulls the bundled Dockerfile and policy, builds the image locally, and creates the sandbox. For the full catalog and how to contribute your own, refer to {doc}`community-sandboxes`.
...
```console
$ diffract sandbox create --editor vscode --name my-sandbox
$ diffract sandbox connect my-sandbox --editor cursor
```

When `--editor` is used, Diffract keeps the sandbox alive and installs an
Diffract-managed SSH include file instead of cluttering your main
...
```console
$ diffract sandbox get my-sandbox
```
...
|---|---|---|
| `--tail` | Stream logs in real time | `diffract logs my-sandbox --tail` |
| `--source` | Filter by log source | `--source sandbox` |
| `--level` | Filter by severity | `--level warn` |
| `--since` | Show logs from a time window | `--since 5m` |

Diffract Terminal combines sandbox status and live logs in a single real-time dashboard:
...
```console
$ diffract forward start 8000 my-sandbox
$ diffract forward start 8000 my-sandbox -d    # run in background
```
...
$ diffract forward stop 8000 my-sandbox
```
...
```console
$ diffract sandbox create --forward 8000 -- claude
```
:::
...
```console
$ diffract sandbox ssh-config my-sandbox
```
...
```console
$ diffract sandbox upload my-sandbox ./src /sandbox/src
```

Download files from the sandbox to your host:

```console
$ diffract sandbox download my-sandbox /sandbox/output ./local
```
