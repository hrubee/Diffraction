...
The gateway is the control plane for Diffract. All control-plane traffic between the CLI and running sandboxes flows through the gateway.
...
:::{tip}
You do not need to deploy a gateway manually. If you run `diffract sandbox create` without a gateway, the CLI auto-bootstraps a local gateway for you.
:::

To use a different port or name:

```console
$ diffract gateway start --port 9090
$ diffract gateway start --name dev-local
```
...
```console
$ diffract gateway start --remote user@hostname
```
...
```console
$ diffract gateway start --remote user@hostname --ssh-key ~/.ssh/my_key
```
...
Use `diffract gateway add` to register a gateway that is already running.
...
```console
$ diffract gateway add https://gateway.example.com
```
...
```console
$ diffract gateway add https://gateway.example.com --name production
```

If the token expires later, re-authenticate with:

```console
$ diffract gateway login
```
...
```console
$ diffract gateway add https://remote-host:8080 --remote user@remote-host
```

Or use the `ssh://` scheme to combine the SSH destination and gateway port:

```console
$ diffract gateway add ssh://user@remote-host:8080
```
...
```console
$ diffract gateway add https://127.0.0.1:8080 --local
```
...
```console
$ diffract gateway select
```

Switch the active gateway:

```console
$ diffract gateway select my-remote-cluster
```

Override the active gateway for a single command with `-g`:

```console
$ diffract status -g my-other-cluster
```
...
$ diffract gateway info --name my-remote-cluster
```
...
```console
$ diffract gateway stop --name my-gateway
$ diffract gateway destroy --name my-gateway
```
...
```console
$ diffract doctor logs
$ diffract doctor logs --tail              # stream live
$ diffract doctor logs --lines 50          # last 50 lines
```
...
```console
$ diffract gateway start --recreate
```
