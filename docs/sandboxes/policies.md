...
title:
  ...
  nav: Policies
description: Apply, iterate, and debug sandbox network policies with hot-reload on running Diffract sandboxes.
topics:
- Generative AI
- Cybersecurity
tags:
...
Dynamic sections can be updated on a running sandbox with `diffract policy set` and take effect without restarting.
...
```console
$ diffract sandbox create --policy ./my-policy.yaml -- claude
```

`diffract sandbox create` keeps the sandbox running after the initial command exits, which is useful when you plan to iterate on the policy. Add `--no-keep` if you want the sandbox deleted automatically instead.
...
2. Monitor denials. Each log entry shows host, port, binary, and reason. Alternatively, use `diffract term` for a live dashboard.

   ```console
   $ diffract logs <name> --tail --source sandbox
   ```

3. Pull the current policy. Strip the metadata header (Version, Hash, Status) before reusing the file.

   ```console
   $ diffract policy get <name> --full > current-policy.yaml
   ```
...
5. Push the updated policy. Exit codes: 0 = loaded, 1 = validation failed, 124 = timeout.
   ...
   ```console
   $ diffract policy set <name> --policy current-policy.yaml --wait
   ```
...
```console
$ diffract policy set --global --policy ./global-policy.yaml
```
...
```console
$ diffract settings get <name>
```

## Debug Denied Requests

Check `diffract logs <name> --tail --source sandbox` for the denied host, path, and binary.
...
Add these blocks to the `network_policies` section of your sandbox policy. Apply with `diffract policy set <name> --policy <file> --wait`.
