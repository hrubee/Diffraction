...
This tutorial describes how to configure Diffract to route inference requests to a local LM Studio server.
...
- Expose a local inference server to Diffract sandboxes.
- Verify end-to-end inference from inside a sandbox.

## Prerequisites

First, complete Diffract installation and follow the {doc}`/get-started/quickstart`.
...
LM Studio will listen to `127.0.0.1:1234` by default. For use with Diffract, you'll need to configure LM Studio to listen on all interfaces (`0.0.0.0`).
...
Add LM Studio as an OpenAI-compatible provider through `host.diffract.internal`:

```console
$ diffract provider create \
    --name lmstudio \
    --type openai \
    --credential OPENAI_API_KEY=lmstudio \
    --config OPENAI_BASE_URL=http://host.diffract.internal:1234/v1
```
...
$ diffract provider create \
    ...
    --config ANTHROPIC_BASE_URL=http://host.diffract.internal:1234
```
...
```console
$ diffract inference set --provider lmstudio --model qwen/qwen3.5-2b
```

If the command succeeds, Diffract has verified that the upstream is reachable and accepts the expected OpenAI-compatible request shape.
...
```console
$ diffract inference set --provider lmstudio-anthropic --model qwen/qwen3.5-2b
```

If the command succeeds, Diffract has verified that the upstream is reachable and accepts the expected Anthropic-compatible request shape.

::::

:::::

The active `inference.local` route is gateway-scoped, so only one provider and model pair is active at a time. Re-run `diffract inference set` whenever you want to switch between OpenAI-compatible and Anthropic-compatible clients.
...
- `OPENAI_BASE_URL` uses `http://host.diffract.internal:1234/v1` when you use an `openai` provider
- `ANTHROPIC_BASE_URL` uses `http://host.diffract.internal:1234` when you use an `anthropic` provider
...
$ diffract status
$ diffract inference get
$ diffract provider get lmstudio
$ diffract provider get lmstudio-anthropic
```
