...
The CLI loads three PEM files from `~/.config/diffract/gateways/<name>/mtls/`:
...
**Registration flow** (`diffract gateway add https://gateway.example.com`):
...
**Re-authentication**: If the token expires, run `diffract gateway login` to open the browser flow again and update the stored token.
...
All gateway credentials and metadata are stored under `~/.config/diffract/`:
