...
```sh
openshell sandbox create --forward 18789 -- diffraction
```

`diffraction` is a helper script pre-installed in the sandbox that runs the
...
- **Health check:** `diffraction health`
...
```sh
diffraction onboard
nohup diffraction gateway run > /tmp/gateway.log 2>&1 &
exit
```
