...
|-------|---------|----------|
| **Community sandbox name** | `--from diffraction` | Resolves to `ghcr.io/nvidia/openshell-community/sandboxes/diffraction:latest` |
...
```bash
openshell sandbox create --from diffraction
```
...
| Community name resolution | Bare names like `diffraction` expand to the GHCR community registry, making the common case simple |
