...
/**
 * Detect whether the plugin is running inside an Diffract sandbox.
 * Inside sandboxes the root filesystem is mounted at /sandbox and diffract
 * host commands are not available, so querying `diffract sandbox status`
 * would always fail — producing false-negative "not running" reports.
 */
function isInsideSandbox(): boolean {
  return existsSync("/sandbox/.diffract") || existsSync("/sandbox/.diffract");
}
...
export async function cliStatus(opts: StatusOptions): Promise<void> {
  ...
  const sandboxName = state.sandboxName ?? "diffract";
  ...
  logger.info("Diffract Status");
  logger.info("===============");
  logger.info("");

  if (insideSandbox) {
    logger.info("Context: running inside an active Diffract sandbox");
    logger.info("  Host sandbox state is not inspectable from inside the sandbox.");
    logger.info("  Run 'diffract sandbox status' on the host for full details.");
    logger.info("");
  }
  ...
  if (inference.configured) {
    ...
    logger.info("  Note:    Run 'diffract inference get' on the host to check.");
  } else {
  ...
  if (state.migrationSnapshot) {
    ...
    logger.info("  Run 'diffract diffract eject' to restore host installation.");
  }
}
...
async function getSandboxStatus(sandboxName: string, insideSandbox: boolean): Promise<SandboxStatus> {
  ...
  try {
    const { stdout } = await execAsync(`diffract sandbox status ${sandboxName} --json`, {
      timeout: 5000,
    });
...
async function getInferenceStatus(insideSandbox: boolean): Promise<InferenceStatus> {
  ...
  try {
    const { stdout } = await execAsync("diffract inference get --json", {
      timeout: 5000,
    });
