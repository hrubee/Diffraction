...
export async function cliEject(opts: EjectOptions): Promise<void> {
  ...
  if (!state.lastAction) {
    logger.error("No Diffract deployment found. Nothing to eject from.");
    return;
  }
  ...
  const snapshotDiffractDir = join(snapshotPath, "diffract");

  if (!existsSync(snapshotDiffractDir)) {
    logger.error(`Snapshot directory not found: ${snapshotDiffractDir}`);
    return;
  }

  if (!confirm) {
    logger.info("Eject will:");
    logger.info("  1. Stop the Diffract sandbox");
    logger.info("  2. Rollback blueprint state");
    logger.info(`  3. Restore ~/.diffract from snapshot: ${snapshotPath}`);
    logger.info("  4. Clear Diffract state");
    logger.info("");
  ...
  if (state.lastRunId && state.blueprintVersion) {
    const blueprintPath = join(HOME, ".diffract", "blueprints", state.blueprintVersion);
  ...
  if (!restored) {
    logger.info(`Manual restore available at: ${snapshotDiffractDir}`);
    return;
  }

  // Step 3: Clear Diffract state
  clearState();

  logger.info("");
  logger.info("Eject complete. Host Diffract installation has been restored.");
  logger.info("You can now run 'diffract' directly on your host.");
}
