...
import type { DiffractState } from "../blueprint/state.js";
...
// Mock node:child_process — controls diffract command results
...
function blankState(): DiffractState {
  return {
    lastRunId: null,
    lastAction: null,
...
function populatedState(): DiffractState {
  return {
    lastRunId: "run-a1b2c3d4",
    lastAction: "migrate",
    blueprintVersion: "0.1.0",
    sandboxName: "diffract",
    migrationSnapshot: "/root/.diffract/snapshots/pre-migrate.tar.gz",
    hostBackupPath: "/root/.diffract/backups/host-backup",
    createdAt: "2026-03-15T10:30:00.000Z",
    updatedAt: "2026-03-15T10:32:45.000Z",
  };
}

const defaultConfig: DiffractConfig = {
  blueprintVersion: "latest",
  blueprintRegistry: "ghcr.io/nvidia/diffract-blueprint",
  sandboxName: "diffract",
...
describe("cliStatus", () => {
  // =========================================================================
  // Scenario 1: Host — no diffract, blank state
  // =========================================================================
  describe("host — no diffract, blank state", () => {
  ...
  describe("host — sandbox running, inference configured", () => {
    ...
    it("shows running sandbox with uptime in text output", async () => {
      ...
      expect(output).toContain("Name:    diffract");
      expect(output).not.toContain("inside sandbox");
    });
  ...
  describe("inside sandbox — core bug fix", () => {
    beforeEach(() => {
      vi.mocked(existsSync).mockImplementation((p: string | URL | Buffer) => {
        const path = String(p);
        return path === "/sandbox/.diffract" || path === "/sandbox/.diffract";
      });
    });
    ...
    it("shows sandbox context banner", async () => {
      ...
      const output = lines.join("\n");
      expect(output).toContain("Context: running inside an active Diffract sandbox");
    ...
    it("does not call diffract commands", async () => {
  ...
  describe("inside sandbox — with prior plugin state", () => {
    ...
    it("shows rollback section when migrationSnapshot exists", async () => {
      ...
      expect(output).toContain("Snapshot:  /root/.diffract/snapshots/pre-migrate.tar.gz");
      expect(output).toContain("diffract diffract eject");
    });

    it("JSON includes full diffract state alongside insideSandbox: true", async () => {
      const { lines, logger } = captureLogger();
      ...
      const data = JSON.parse(lines.join(""));
      expect(data.insideSandbox).toBe(true);
      expect(data.diffract.lastAction).toBe("migrate");
      expect(data.diffract.blueprintVersion).toBe("0.1.0");
      expect(data.diffract.lastRunId).toBe("run-a1b2c3d4");
      expect(data.diffract.migrationSnapshot).toBe(
        "/root/.diffract/snapshots/pre-migrate.tar.gz",
      );
    });
  });
  ...
  describe("edge cases", () => {
    ...
    it("defaults sandbox name to 'diffract' when state has none", async () => {
      mockExec({
      ...
      expect(exec).toHaveBeenCalledWith(
        expect.stringContaining("diffract"),
        expect.anything(),
        expect.anything(),
      );
    });

    it("only detects sandbox via /sandbox/.diffract", async () => {
      vi.mocked(existsSync).mockImplementation((p: string | URL | Buffer) => {
        return String(p) === "/sandbox/.diffract";
      });
