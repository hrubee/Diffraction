...
function isExperimentalEnabled(): boolean {
  return process.env.DIFFRACTION_EXPERIMENTAL === "1";
}
...
function showConfig(config: DiffractOnboardConfig, logger: PluginLogger): void {
...
function execDiffract(args: string[]): string {
  return execFileSync("diffract", args, {
    encoding: "utf-8",
...
export async function cliOnboard(opts: OnboardOptions): Promise<void> {
  ...
  logger.info("Diffract Onboarding");
  logger.info("-------------------");
  ...
  const summaryConfig: DiffractOnboardConfig = {
    endpointType,
    endpointUrl,
    ncpPartner,
    model,
  ...
  try {
    execDiffract(["inference", "set", "--provider", providerName, "--model", model]);
  ...
  logger.info("  diffract diffract launch     # Bootstrap sandbox");
  logger.info("  diffract diffract status     # Check configuration");
}
