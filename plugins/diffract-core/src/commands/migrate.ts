...
export { detectHostDiffract, type HostDiffractState } from "./migration-state.js";

const SANDBOX_ARCHIVE_DIR = "/sandbox/.diffract/migration/archives";
...
export async function cliMigrate(opts: MigrateOptions): Promise<void> {
  ...
  logger.info("Diffract migrate: moving host Diffract into Diffract sandbox");
  ...
  if (!hostState.exists || !hostState.stateDir) {
    ...
    logger.info("Use 'diffract diffract launch' for a fresh install.");
    return;
  }
  ...
  logger.info("  diffract diffract status     # Verify everything is healthy");
  ...
  if (skipBackup) {
    ...
    logger.info("  diffract diffract eject");
  }
}
...
function syncArchive(sandboxName: string, archiveName: string, archivePath: string, destinationDir: string): void {
  ...
  execFileSync("diffract", ["sandbox", "cp", archivePath, `${sandboxName}:${sandboxArchivePath}`], {
    encoding: "utf-8",
...
function execSandboxCommand(sandboxName: string, args: string[]): void {
  try {
    execFileSync("diffract", ["sandbox", "connect", sandboxName, "--", ...args], {
      encoding: "utf-8",
      stdio: ["ignore", "pipe", "pipe"],
    });
  } catch (err: unknown) {
