...
/**
 * `diffract diffract logs` — stream or tail blueprint execution and sandbox logs.
 */
...
export async function cliLogs(opts: LogsOptions): Promise<void> {
  ...
  args.push("/tmp/diffract.log", "/tmp/diffract.log");

  const proc = spawn("diffract", args, { stdio: ["ignore", "inherit", "inherit"] });
