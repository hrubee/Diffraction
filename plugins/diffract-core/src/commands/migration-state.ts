...
const SANDBOX_MIGRATION_DIR = "/sandbox/.diffract/migration";
...
export interface HostDiffractState {
  exists: boolean;
  homeDir: string | null;
...
type DiffractConfigDocument = Record<string, unknown>;
...
function resolveStateDir(env: NodeJS.ProcessEnv = process.env): string {
  ...
  return path.join(resolveHostHome(env), ".diffract");
}
...
function resolveConfigPath(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  ...
  return path.join(stateDir, "diffract.json");
}

function loadConfigDocument(configPath: string): DiffractConfigDocument | null {
  ...
  return parsed as DiffractConfigDocument;
}
...
function defaultWorkspacePath(env: NodeJS.ProcessEnv = process.env): string {
  ...
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".diffract", `workspace-${profile}`);
  }
  return path.join(home, ".diffract", "workspace");
}

function collectExternalRoots(
  config: DiffractConfigDocument | null,
  stateDir: string,
...
export function detectHostDiffract(env: NodeJS.ProcessEnv = process.env): HostDiffractState {
  ...
  const errors: string[] = [];
  const warnings: string[] = [];
  let config: DiffractConfigDocument | null = null;

  if (!stateExists) {
    errors.push(`Resolved Diffract state directory does not exist: ${stateDir}`);
  }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    errors.push(`Failed to parse Diffract config at ${configPath}: ${msg}`);
  }
...
function resolveConfigSourcePath(manifest: SnapshotManifest, snapshotDir: string): string {
  if (manifest.hasExternalConfig) {
    return path.join(snapshotDir, "config", "diffract.json");
  }
  return path.join(snapshotDir, "diffract", "diffract.json");
}
...
function prepareSandboxState(snapshotDir: string, manifest: SnapshotManifest): string {
  const preparedStateDir = path.join(snapshotDir, "sandbox-bundle", "diffract");
  rmSync(preparedStateDir, { recursive: true, force: true });
  mkdirSync(path.dirname(preparedStateDir), { recursive: true });
  copyDirectory(path.join(snapshotDir, "diffract"), preparedStateDir);
  ...
  writeFileSync(path.join(preparedStateDir, "diffract.json"), JSON.stringify(config, null, 2));
  return preparedStateDir;
}

export function createSnapshotBundle(
  hostState: HostDiffractState,
  logger: PluginLogger,
  options: { persist: boolean },
): SnapshotBundle | null {
  if (!hostState.stateDir || !hostState.homeDir) {
    logger.error("Cannot snapshot host Diffract state: no state directory was resolved.");
    return null;
  }
  ...
  try {
    ...
    const snapshotStateDir = path.join(parentDir, "diffract");
    ...
    if (hostState.configPath && hostState.hasExternalConfig) {
      ...
      copyFileSync(hostState.configPath, path.join(configSnapshotDir, "diffract.json"));
    }
...
export function restoreSnapshotToHost(snapshotDir: string, logger: PluginLogger): boolean {
  ...
  const snapshotStateDir = path.join(snapshotDir, "diffract");
  ...
  try {
    if (existsSync(manifest.stateDir)) {
      const archiveName = `${manifest.stateDir}.diffract-archived-${String(Date.now())}`;
    ...
    if (manifest.hasExternalConfig && manifest.configPath) {
      const configSnapshotPath = path.join(snapshotDir, "config", "diffract.json");
    ...
    logger.info("Host Diffract state restored.");
    return true;
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
