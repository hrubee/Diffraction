...
/**
 * Diffract — Diffract Plugin for Diffract
 *
...
// ---------------------------------------------------------------------------
// Diffract Plugin SDK compatible types (mirrors diffract/plugin-sdk)
// ---------------------------------------------------------------------------
...
export interface PluginService {
  id: string;
  start: (ctx: { config: DiffractConfig; logger: PluginLogger }) => void | Promise<void>;
  stop?: (ctx: { config: DiffractConfig; logger: PluginLogger }) => void | Promise<void>;
}
...
// ---------------------------------------------------------------------------
// Plugin-specific config (read from pluginConfig in diffract.plugin.json)
// ---------------------------------------------------------------------------
...
function registeredProviderForConfig(
  ...
  return {
    ...
    aliases: ["inference-local", "diffract"],
...
const DEFAULT_PLUGIN_CONFIG: DiffractConfig = {
...
export function getPluginConfig(api: DiffractPluginApi): DiffractConfig {
...
export default function register(api: DiffractPluginApi): void {
  // 1. Register /diffract slash command (chat interface)
  api.registerCommand({
    name: "diffract",
    description: "Diffract sandbox management (status, eject).",
    acceptsArgs: true,
    handler: (ctx) => handleSlashCommand(ctx, api),
  });

  // 2. Register `diffract diffract` CLI subcommands (commander.js)
  api.registerCli(
    (cliCtx) => {
  ...
  api.logger.info("");
  api.logger.info("  ┌─────────────────────────────────────────────────────┐");
  api.logger.info("  │  Diffract registered                                │");
  api.logger.info("  │                                                     │");
  ...
  api.logger.info("  │  Commands:  diffract diffract <command>             │");
  api.logger.info("  └─────────────────────────────────────────────────────┘");
  api.logger.info("");
}
