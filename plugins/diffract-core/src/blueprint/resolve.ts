...
import type { DiffractConfig } from "../index.js";
...
const CACHE_DIR = join(process.env.HOME ?? "/tmp", ".diffract", "blueprints");
...
function parseManifestHeader(raw: string): BlueprintManifest {
  ...
  return {
    version: get("version"),
    minDiffractVersion: get("min_diffract_version"),
    minDiffractVersion: get("min_diffract_version"),
...
export async function resolveBlueprint(config: DiffractConfig): Promise<ResolvedBlueprint> {
