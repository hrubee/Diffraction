...
const CONFIG_DIR = join(process.env.HOME ?? "/tmp", ".diffract");

export type EndpointType = "build" | "ncp" | "nim-local" | "vllm" | "ollama" | "custom";

export interface DiffractOnboardConfig {
...
export function describeOnboardEndpoint(config: DiffractOnboardConfig): string {
...
export function describeOnboardProvider(config: DiffractOnboardConfig): string {
...
export function loadOnboardConfig(): DiffractOnboardConfig | null {
  ensureConfigDir();
  const path = configPath();
  if (!existsSync(path)) {
    return null;
  }
  return JSON.parse(readFileSync(path, "utf-8")) as DiffractOnboardConfig;
}

export function saveOnboardConfig(config: DiffractOnboardConfig): void {
  ensureConfigDir();
