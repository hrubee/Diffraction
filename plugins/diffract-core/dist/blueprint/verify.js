...
function checkCompatibility(manifest, diffractVersion, diffractVersion) {
    const errors = [];
    if (manifest.minDiffractVersion &&
        !satisfiesMinVersion(diffractVersion, manifest.minDiffractVersion)) {
        errors.push(`Diffract version ${diffractVersion} < required ${manifest.minDiffractVersion}`);
    }
    if (manifest.minDiffractVersion &&
        !satisfiesMinVersion(diffractVersion, manifest.minDiffractVersion)) {
        errors.push(`Diffract version ${diffractVersion} < required ${manifest.minDiffractVersion}`);
