...
/**
 * `diffract nemoclaw logs` — stream or tail blueprint execution and sandbox logs.
 */
...
async function cliLogs(opts) {
    ...
    // Stream sandbox logs via diffract
    ...
    args.push("/tmp/nemoclaw.log", "/tmp/diffract.log");
    const proc = (0, node_child_process_1.spawn)("diffract", args, { stdio: ["ignore", "inherit", "inherit"] });
...
async function isSandboxRunning(sandboxName) {
    try {
        const { stdout } = await execAsync(`diffract sandbox get ${sandboxName} --json`, {
            timeout: 5000,
        });
