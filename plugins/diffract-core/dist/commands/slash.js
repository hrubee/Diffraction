...
function slashHelp() {
    return {
        text: [
            ...
            "  `diffract nemoclaw status`",
            "  `diffract nemoclaw migrate`",
            "  `diffract nemoclaw launch`",
            "  `diffract nemoclaw connect`",
            "  `diffract nemoclaw eject --confirm`",
        ].join("\n"),
    };
}
function slashStatus() {
    ...
    if (!state.lastAction) {
        return {
            text: "**NemoClaw**: No operations performed yet. Run `diffract nemoclaw launch` or `diffract nemoclaw migrate` to get started.",
        };
    }
    const lines = [
        "**NemoClaw Status**",
...
function slashOnboard() {
    ...
    if (config) {
        return {
            text: [
                ...
                "",
                "To reconfigure, run: `diffract nemoclaw onboard`",
            ]
                .filter(Boolean)
                .join("\n"),
        };
    ...
    return {
        text: [
            ...
            "",
            "```",
            "diffract nemoclaw onboard",
            "```",
            "",
            "Or non-interactively:",
            "```",
            'diffract nemoclaw onboard --api-key "$NVIDIA_API_KEY" --endpoint build --model nvidia/nemotron-3-super-120b-a12b',
            "```",
        ].join("\n"),
    };
}
