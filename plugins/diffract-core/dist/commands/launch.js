...
async function cliLaunch(opts) {
    ...
    logger.info("NemoClaw launch: setting up Diffract inside Diffract");
    // Check if there's an existing host Diffract installation
    const hostState = (0, migrate_js_1.detectHostDiffract)();
    if (!hostState.exists && !force) {
        ...
        logger.info("");
        logger.info("This avoids installing Diffract on the host only to redeploy it inside Diffract.");
        logger.info("");
        logger.info("To proceed with NemoClaw-driven bootstrap anyway, use --force.");
        return;
    }
    if (hostState.exists && !force) {
        logger.info("Existing Diffract installation detected. Consider using 'diffract nemoclaw migrate' instead.");
    ...
    const compat = (0, verify_js_1.checkCompatibility)(blueprint.manifest, diffractVersion, diffractVersion);
    if (compat.length > 0) {
    ...
    logger.info("  diffract nemoclaw status     # Check health");
    logger.info("  diffract term               # Monitor network egress");
}
function getOpenshellVersion() {
    try {
        return (0, node_child_process_1.execSync)("diffract --version", { encoding: "utf-8" }).trim();
    }
    catch {
        return "0.0.0";
    }
