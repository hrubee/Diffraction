...
exports.detectHostDiffract = void 0;
...
Object.defineProperty(exports, "detectHostDiffract", { enumerable: true, get: function () { return migration_state_js_2.detectHostDiffract; } });
...
async function cliMigrate(opts) {
    ...
    logger.info("NemoClaw migrate: moving host Diffract into Diffract sandbox");
    logger.info("Detecting host Diffract installation...");
    const hostState = (0, migration_state_js_1.detectHostDiffract)();
    if (!hostState.exists || !hostState.stateDir) {
        logger.error("No Diffract installation found for the current host environment.");
        logger.info("Use 'diffract nemoclaw launch' for a fresh install.");
        return;
    }
    ...
    if (hostState.errors.length > 0) {
        ...
        logger.error("Refusing to migrate until all external Diffract roots can be resolved.");
        return;
    }
    if (dryRun) {
        logger.info("");
        ...
        if (hostState.externalRoots.length > 0) {
            logger.info("  3. Capture external Diffract roots and rewrite config paths for the sandbox:");
            for (const root of hostState.externalRoots) {
            ...
            logger.info("  5. Copy archives into the Diffract sandbox and verify the migrated paths");
        }
        else {
            logger.info("  3. Package state dir as a tar archive to preserve symlinks");
            logger.info("  4. Copy the state archive into the Diffract sandbox and verify the config");
        }
    ...
    logger.info("Provisioning Diffract sandbox...");
    ...
    try {
        logger.info("Packaging Diffract state for sandbox import...");
    ...
    logger.info("");
    logger.info("Migration complete. Diffract is now running inside Diffract.");
    logger.info(`Sandbox: ${pluginConfig.sandboxName}`);
    logger.info("");
    logger.info("Next steps:");
    logger.info("  diffract nemoclaw connect    # Enter the sandbox");
    logger.info("  diffract nemoclaw status     # Verify everything is healthy");
    logger.info("  diffract term               # Monitor sandbox activity");
    logger.info("");
    ...
    if (skipBackup) {
        ...
        logger.info("  diffract nemoclaw eject");
    }
}
...
function syncSnapshotBundleIntoSandbox(bundle, sandboxName) {
    ...
    syncArchive(sandboxName, "state.tar", stateArchivePath(bundle), "/sandbox/.diffract");
    for (const root of bundle.manifest.externalRoots) {
...
function syncArchive(sandboxName, archiveName, archivePath, destinationDir) {
    ...
    (0, node_child_process_1.execFileSync)("diffract", ["sandbox", "cp", archivePath, `${sandboxName}:${sandboxArchivePath}`], {
        encoding: "utf-8",
...
function verifySandboxMigration(bundle, sandboxName) {
    ...
    const verification = {
        stateDir: "/sandbox/.diffract",
        configPath: "/sandbox/.diffract/diffract.json",
...
function execSandboxCommand(sandboxName, args) {
    try {
        (0, node_child_process_1.execFileSync)("diffract", ["sandbox", "connect", sandboxName, "--", ...args], {
            encoding: "utf-8",
            stdio: ["ignore", "pipe", "pipe"],
        });
    }
