...
exports.detectHostDiffract = detectHostDiffract;
...
function resolveStateDir(env = process.env) {
    ...
    return node_path_1.default.join(resolveHostHome(env), ".diffract");
}
function resolveConfigPath(stateDir, env = process.env) {
    ...
    return node_path_1.default.join(stateDir, "diffract.json");
}
...
function defaultWorkspacePath(env = process.env) {
    ...
    if (profile && profile.toLowerCase() !== "default") {
        return node_path_1.default.join(home, ".diffract", `workspace-${profile}`);
    }
    return node_path_1.default.join(home, ".diffract", "workspace");
}
...
function detectHostDiffract(env = process.env) {
...
function resolveConfigSourcePath(manifest, snapshotDir) {
    if (manifest.hasExternalConfig) {
        return node_path_1.default.join(snapshotDir, "config", "diffract.json");
    }
    return node_path_1.default.join(snapshotDir, "diffract", "diffract.json");
}
...
function prepareSandboxState(snapshotDir, manifest) {
    const preparedStateDir = node_path_1.default.join(snapshotDir, "sandbox-bundle", "diffract");
    ...
    copyDirectory(node_path_1.default.join(snapshotDir, "diffract"), preparedStateDir);
    ...
    (0, node_fs_1.writeFileSync)(node_path_1.default.join(preparedStateDir, "diffract.json"), JSON.stringify(config, null, 2));
...
function createSnapshotBundle(hostState, logger, options) {
    ...
    try {
        ...
        const snapshotStateDir = node_path_1.default.join(parentDir, "diffract");
        ...
        if (hostState.configPath && hostState.hasExternalConfig) {
            ...
            (0, node_fs_1.copyFileSync)(hostState.configPath, node_path_1.default.join(configSnapshotDir, "diffract.json"));
        }
        const externalRoots = [];
...
function restoreSnapshotToHost(snapshotDir, logger) {
    ...
    const snapshotStateDir = node_path_1.default.join(snapshotDir, "diffract");
    ...
    try {
        ...
        if (manifest.hasExternalConfig && manifest.configPath) {
            const configSnapshotPath = node_path_1.default.join(snapshotDir, "config", "diffract.json");
